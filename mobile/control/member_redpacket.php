...
class member_redpacket extends mobileMemberControl {
	...
	public function getpackOp() {
		...
		$member = $member_model->getMemberInfo ( array (
		...
		// 检查红包状态
		$packet = $model->table ( 'red_packet' )->where ( array (
				'id' => $id 
		) )->find ();
		if ($packet ['state'] == 2) {
		...
		// 判断该用户是否已经领取
		$rec = $model->table ( 'red_packet_rec' )->where ( array (
				'packet_id' => $id,
		...
		if (empty ( $rec )) {
			...
			if ($rand <= $packet ['win_rate']) {
				 ...
				 * 中奖操作
				 */
				$win_rec = $model->table ( 'red_packet_list' )->where ( array (
						'packet_id' => $packet ['id'] 
				) )->find ();
				...
				$member_update = array (
						'available_predeposit' => array (
								'exp',
								'available_predeposit+' . $win_rec ['packet_price'] 
						) 
				);
				...
				// 增加领取红包数量
				$model->table ( 'red_packet' )->where ( array (
						'id' => $packet ['id'] 
				) )->update ( array (
						'packet_numbered' => array (
								'exp',
								'packet_numbered+1' 
						) 
				) );
				...
				// 删除小红包记录
				$model->table ( 'red_packet_list' )->where ( array (
						'id' => $win_rec ['id'] 
				) )->delete ();
