...
class goods_class extends mobileHomeControl
{
    ...
    private function _get_root_class()
    {
        ...
        foreach ($class_list as $key => $value) {
            ...
            $class_list[$key]['text'] = '';
            $child_class_string = isset($goods_class_array[$value['gc_id']]['child']) ? $goods_class_array[$value['gc_id']]['child'] : '';
            $child_class_array = explode(',', $child_class_string);
            foreach ($child_class_array as $child_class) {
                $class_list[$key]['text'] .= (isset($goods_class_array[$child_class]['gc_name']) ? $goods_class_array[$child_class]['gc_name'] : '') . '/';
            }
    ...
    private function _get_class_list($gc_id)
    {
        ...
        if (empty($goods_class['child'])) {
            ...
            foreach ($child_class_array as $child_class) {
                $class_item = array();
				if(!isset($class_item['gc_id'])){
					$class_item['gc_id'] = '';
				}
                $class_item['gc_id'] .= $goods_class_array[$child_class]['gc_id'];
				if(!isset($class_item['gc_name'])){
					$class_item['gc_name'] = '';
				}
