...
class distributor_chat extends mobileMemberControl
{
    ...
    {
        $output_data = array('node_chat' => core\config::get('node_chat'), 'node_site_url' => NODE_SITE_URL, 'resource_site_url' => RESOURCE_SITE_URL);
        ...
        $goods_id = isset($_GET['chat_goods_id']) ? intval($_GET['chat_goods_id']) : 0;
        if ($goods_id > 0) {
    ...
    public function get_user_listOp()
    {
        ...
        $member_info = $model_chat->getMember($member_id);
        $node_info = array();
        $node_info['node_chat'] = core\config::get('node_chat');
