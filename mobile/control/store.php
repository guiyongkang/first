...
class store extends mobileHomeControl
{
    ...
    public function store_infoOp()
    {
        ...
        // 店铺头像
        $store_info['store_avatar'] = $store_online_info['store_avatar'] ? UPLOAD_SITE_URL . '/' . ATTACH_STORE . '/' . $store_online_info['store_avatar'] : UPLOAD_SITE_URL . '/' . ATTACH_COMMON . DS . core\config::get('default_store_avatar');
        // 商品数
        ...
        output_data(array('store_info' => $store_info, 'rec_goods_list_count' => count($goods_list), 'rec_goods_list' => $goods_list));
    }
    /**
     * 店铺商品分类
     */
    public function store_goods_classOp()
    {
        ...
        output_data(array('store_info' => $store_info, 'store_goods_class' => $store_goods_class));
    }
    /**
     * 店铺商品
     */
    public function store_goodsOp()
    {
        ...
        $stc_id = isset($param['stc_id']) ? intval($param['stc_id']) : 0;
        ...
        $price_from = preg_match('/^[\\d.]{1,20}$/', isset($param['price_from']) ? $param['price_from'] : '') ? $param['price_from'] : null;
        $price_to = preg_match('/^[\\d.]{1,20}$/', isset($param['price_to']) ? $param['price_to'] : '') ? $param['price_to'] : null;
		...
		$param['key'] = isset($param['key']) ? trim($param['key']) : 0;
        switch ($param['key']) {
            case '1':
        ...
        $goods_list = $this->_goods_list_extend($goods_list);
        output_data(array('goods_list_count' => count($goods_list), 'goods_list' => $goods_list), mobile_page($page_count));
    }
    private function getGoodsFields()
    {
        return implode(',', array('goods_id', 'goods_commonid', 'store_id', 'store_name', 'goods_name', 'goods_price', 'goods_promotion_price', 'goods_promotion_type', 'goods_marketprice', 'goods_image', 'goods_salenum', 'evaluation_good_star', 'evaluation_count', 'is_virtual', 'is_presell', 'is_fcode', 'have_gift', 'goods_addtime'));
    }
    /**
     * 处理商品列表(团购、限时折扣、商品图片)
     */
    ...
    public function store_new_goodsOp()
    {
        ...
        $show_day = ($t = isset($_REQUEST['show_day']) ? intval($_REQUEST['show_day']) : '') > 0 ? $t : 30;
        $where = array();
        $where['store_id'] = $store_id;
        //$where['is_book'] = 0;//默认不显示预订商品
    ...
    public function store_introOp()
    {
        ...
        $store_info['sc_name'] = isset($store_class['sc_name']) ? $store_class['sc_name'] : '';
        //如果已登录 判断该店铺是否已被收藏
