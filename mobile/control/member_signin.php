...
class member_signin extends mobileMemberControl
{
    ...
    public function checksigninOp()
    {
        ...
        if (!empty($log_array)) {
            ...
            $points_signin = intval(core\config::get('points_signin'));
            output_data(array('points_signin' => $points_signin));
        }
    }
    /**
     * by 33 hao.co m 签到 array('pl_memberid'=>'会员编号','pl_membername'=>'会员名称','pl_adminid'=>'管理员编号','pl_adminname'=>'管理员名称','pl_points'=>'积分','pl_desc'=>'描述','orderprice'=>'订单金额','order_sn'=>'订单编号','order_id'=>'订单序号','point_ordersn'=>'积分兑换订单编号');
     */
