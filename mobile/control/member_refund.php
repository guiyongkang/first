...
class member_refund extends mobileMemberControl
{
    ...
    public function refund_all_formOp()
    {
        ...
        if (!empty($order_info) && is_array($order_info)) {
            ...
            $order['order_type'] = isset($order_info['order_type']) ? $order_info['order_type'] : 0;
            ...
            $order['allow_refund_amount'] = ncPriceFormat($order_info['order_amount']);
            //可退款金额
            ...
            foreach ($order_goods_list as $key => $value) {
                ...
                $goods['goods_spec'] = isset($value['goods_spec']) ? $value['goods_spec'] : '';
    ...
    public function refund_all_postOp()
    {
        ...
        if (empty($order_info) || $payment_code == 'offline' || (isset($refund['refund_id']) && $refund['refund_id'] > 0)) {
            //检查数据,防止页面刷新不及时造成数据错误
            output_error('参数错误');
        } else {
            //$book_amount = logic('order_book')->getDepositAmount($order_info);
            //订金金额
            //$allow_refund_amount = ncPriceFormat($order_info['order_amount'] - $book_amount);
			$allow_refund_amount = ncPriceFormat($order_info['order_amount']);
            //可退款金额
            $refund_array = array();
            ...
            $pic_array = array();
            $pic_array['buyer'] = $_POST['refund_pic'];
            //上传凭证
    ...
    public function refund_formOp()
    {
        $model_refund = model('refund_return');
        $condition = array();
        $reason_list = $model_refund->getReasonList($condition, '', '', 'reason_id,reason_info');
        //退款退货原因
        $member_id = $this->member_info['member_id'];
        $order_id = intval($_GET['order_id']);
        $goods_id = intval($_GET['order_goods_id']);
        //订单商品表编号
        $model_order = model('order');
        $condition = array();
        $condition['buyer_id'] = $member_id;
        ...
        $refund_state = $model_refund->getRefundState($order_info);
        //根据订单状态判断是否可以退款退货
        if ($refund_state == 1 && $goods_id > 0) {
            $order = array();
            $order['order_id'] = $order_info['order_id'];
            $order['order_type'] = isset($order_info['order_type']) ? $order_info['order_type'] : '';
            $order['order_amount'] = ncPriceFormat($order_info['order_amount']);
            $order['order_sn'] = $order_info['order_sn'];
            $order['store_name'] = $order_info['store_name'];
            $order['store_id'] = $order_info['store_id'];
            ...
            $goods['goods_spec'] = isset($goods_info['goods_spec']) ? $goods_info['goods_spec'] : array();
            $goods['goods_num'] = $goods_info['goods_num'];
            $goods_pay_price = $goods_info['goods_pay_price'];
            //商品实际成交价
            $order_amount = $order_info['order_amount'];
            //订单金额
            $order_refund_amount = $order_info['refund_amount'];
            //订单退款金额
    ...
    public function refund_postOp()
    {
        $member_id = $this->member_info['member_id'];
        $order_id = intval($_POST['order_id']);
        $goods_id = intval($_POST['order_goods_id']);
        //订单商品表编号
        $model_order = model('order');
        $model_refund = model('refund_return');
        $condition = array();
        ...
        if ($refund_state == 1 && $goods_id > 0 && empty($refund)) {
            ...
            $goods_num = isset($_POST['goods_num']) ? intval($_POST['goods_num']) : 0;
            //退货数量
            if ($goods_num <= 0 || $goods_num > $goods_info['goods_num']) {
                $goods_num = 1;
            }
            $reason_list = $model_refund->getReasonList(array(), '', '', 'reason_id,reason_info');
            //退款退货原因
            ...
            $refund_array['refund_type'] = $_POST['refund_type'];
            //类型:1为退款,2为退货
    ...
    public function get_refund_listOp()
    {
        ...
        if (isset($_GET['k']) && trim($_GET['k']) != '' && in_array($_GET['type'], $keyword_type)) {
            $type = $_GET['type'];
            $condition[$type] = array('like', '%' . $_GET['k'] . '%');
        }
        if ((isset($_GET['add_time_from']) && trim($_GET['add_time_from']) != '') || (isset($_GET['add_time_to']) && trim($_GET['add_time_to']) != '')) {
        ...
        if (!empty($list) && is_array($list)) {
            ...
            foreach ($list as $k => $v) {
                ...
                if ($v['goods_id'] > 0) {
                    ...
                    $order_goods_list = $model_order->getOrderGoodsList($condition);
                    $goods['goods_spec'] = isset($order_goods_list[0]['goods_spec']) ? $order_goods_list[0]['goods_spec'] : '';
