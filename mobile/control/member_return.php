...
class member_return extends mobileMemberControl
{
    ...
    public function ship_formOp()
    {
        ...
        //发货默认5天后才能选择没收到
        $return_confirm = $model_trade->getMaxDay('return_confirm');
        //商家不处理收货时按同意并弃货处理
    ...
    public function get_return_listOp()
    {
        ...
        if (!empty($list) && is_array($list)) {
            ...
            foreach ($goods_list as $k => $v) {
                $order_goods_id = isset($v['order_goods_id']) ? $v['order_goods_id'] : 0;
            ...
            foreach ($list as $k => $v) {
                ...
                $val['goods_state_v'] = $v['goods_state'];
