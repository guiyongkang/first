...
class member_voucher extends mobileMemberControl
{
    ...
    public function voucher_listOp()
    {
        if ($this->member_info['member_id']) {
            ...
            $page = new lib\page();
            $voucher_list = $model_voucher->getMemberVoucherList($this->member_info['member_id'], isset($_POST['voucher_state']) ? $_POST['voucher_state'] : 0, $page);
    ...
    public function voucher_pwexOp()
    {
        if ($this->member_info['member_id']) {
            ...
            $obj_validate->validateparam = array(array('input' => $_POST['pwd_code'], 'require' => 'true', 'message' => '请输入代金券卡密'));
            ...
            if (intval(core\session::get('store_id')) == $voucher_info['voucher_store_id']) {
