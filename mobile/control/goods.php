...
class goods extends mobileHomeControl
{
    private $PI = 3.141592653589793;
    private $x_pi = 0;
    public function __construct()
    {
        $this->x_pi = 3.141592653589793 * 3000.0 / 180.0;
        parent::__construct();
    }
    /**
     * 商品列表
    ...
    public function goods_listOp()
    {
        ...
        // ==== 暂时不显示定金预售商品，手机端未做。 ====
        // $condition['is_book'] = 0;
        if (!empty($_GET['gc_id']) && intval($_GET['gc_id']) > 0) {//分类id
            ...
            setcookie('hisSearch', implode('~', $his_sh_list), time() + 2592000, '/', SUBDOMAIN_SUFFIX ?: '', false);
        ...
        $price_from = preg_match('/^[\\d.]{1,20}$/', isset($_GET['price_from']) ? $_GET['price_from'] : '') ? $_GET['price_from'] : null;
        $price_to = preg_match('/^[\\d.]{1,20}$/', isset($_GET['price_to']) ? $_GET['price_to'] : '') ? $_GET['price_to'] : null;
        // 所需字段
        $fieldstr = 'goods_id,goods_commonid,goods_name,goods_jingle,goods_price,goods_promotion_price,goods_promotion_type,goods_marketprice,goods_image,goods_salenum,evaluation_good_star,evaluation_count';
        $fieldstr .= ',is_virtual,is_presell,is_fcode,have_gift,store_id,store_name,is_own_shop';
        // 排序方式
        $order = $this->_goods_list_order(isset($_GET['key']) ? $_GET['key'] : 0, isset($_GET['order']) ? $_GET['order'] : 0);
        // 全文搜索搜索参数
        $indexer_searcharr = $_GET;
        // 搜索消费者保障服务
        $search_ci_arr = array();
        $_GET['ci'] = isset($_GET['ci']) ? trim($_GET['ci'], '_') : '';
        if (!empty($_GET['ci']) && is_string($_GET['ci'])) {
            // 处理参数
            $search_ci = $_GET['ci'];
            $search_ci_arr = explode('_', $search_ci);
            $indexer_searcharr['search_ci_arr'] = $search_ci_arr;
        }
        if (isset($_GET['own_shop']) && $_GET['own_shop'] == 1) {
        ...
        if (!is_null($goods_list)) {
            ...
            if (isset($_GET['gift']) && $_GET['gift'] == 1) {
            ...
            // 团购和限时折扣搜索
            $_tmp = array();
            if (isset($_GET['groupbuy']) && $_GET['groupbuy'] == 1) {
                $_tmp[] = 1;
            }
            if (isset($_GET['xianshi']) && $_GET['xianshi'] == 1) {
                $_tmp[] = 2;
            }
            if ($_tmp) {
            ...
            unset($_tmp);
            // 虚拟商品
            if (isset($_GET['virtual']) && $_GET['virtual'] == 1) {
     ...
     * 商品列表排序方式
     */
    private function _goods_list_order($key = 0, $order = 0)
    {
    ...
    public function goods_detailOp()
    {
        ...
        if ($memberId = $this->getMemberIdIfExists()) {
            ...
            // QueueClient::push('addViewedGoods', array('goods_id'=>$goods_id,'member_id'=>$memberId));
        ...
        if ($goods_detail) {
            $model_goods_browse = model('goods_browse')->addViewedGoods($goods_id, $memberId);
            // 加入浏览历史数据库
    ...
    private function _goods_detail_extend($goods_detail)
    {
        ...
        // 商品链接
        $goods_detail['goods_info']['goods_url'] = 'javascript:;';
        // 整理数据
    ...
    public function goods_bodyOp()
    {
		...
		$mobile_body = '';
		if (!empty($goods_common_info['mobile_body'])) {
            $mobile_body_arr = unserialize($goods_common_info['mobile_body']);
            if (is_array($mobile_body_arr)) {
				foreach ($mobile_body_arr as $val) {
					if ($val['type'] == 'text') {
						$mobile_body .= '<p>' . $val['value'] . '</p>';
					}
					if ($val['type'] == 'image') {
    ...
    public function goods_evaluateOp()
    {
        ...
        $goodsevallist = $this->_get_comments($goods_id, isset($_GET['type']) ? $_GET['type'] : '', $this->page);
    ...
    public function _calc($area_id, $goods_id)
    {
        ...
        if ($area_id <= 0) {
            ...
            $area_id = isset($store_info['deliver_region_ids'][1]) ? intval($store_info['deliver_region_ids'][1]) : 0;
            $area_name = isset($store_info['deliver_region'][1]) ? $store_info['deliver_region'][1] : 0;
        }
        if ($goods_info['transport_id'] && $area_id > 0) {
            ...
            if ($freight_total > 0) {
                if ($store_info['store_free_price'] > 0) {
                    if ($freight_total >= $store_info['store_free_price']) {
                        ...
                        $freight_total = '运费：' . $freight_total . ' 积分，店铺满 ' . $store_info['store_free_price'] . ' 积分 免运费';
                    }
                } else {
                    $freight_total = '运费：' . $freight_total . ' 积分';
                }
            } else {
            ...
            $freight_total = $goods_info['goods_freight'] > 0 ? '运费：' . $goods_info['goods_freight'] . ' 积分' : '免运费';
        }
        return array('content' => $freight_total, 'if_store_cn' => (isset($if_store) && $if_store === false) ? '无货' : ($goods_info['goods_storage'] > 0 ? '有货' : '无货'), 'if_store' => (isset($if_store) && $if_store === false) ? false : true, 'area_name' => isset($area_name) ? $area_name : '全国');
    }
    /**
     * 取得店铺分店地址
     */
    ...
    public function bd_decrypt($bdLat, $bdLon)
    {
        $x = $bdLon - 0.0065;
        $y = $bdLat - 0.006;
        $z = sqrt($x * $x + $y * $y) - 2.0E-5 * sin($y * $this->x_pi);
        $theta = atan2($y, $x) - 3.0E-6 * cos($x * $this->x_pi);
        $gcjLon = $z * cos($theta);
        $gcjLat = $z * sin($theta);
