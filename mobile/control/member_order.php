...
class member_order extends mobileMemberControl
{
    ...
    public function order_cancelOp()
    {
        ...
        if (isset($order_info['api_pay_time']) && TIMESTAMP - 86400 < $order_info['api_pay_time']) {
    ...
    public function order_infoOp()
    {
        ...
        if (!empty($order_info['add_time'])) {
            $order_info['add_time'] = date('Y-m-d H:i:s', $order_info['add_time']);
        } else {
            $order_info['add_time'] = '';
        }
        if (!empty($order_info['extend_order_common']['order_message'])) {
            $order_info['order_message'] = $order_info['extend_order_common']['order_message'];
        }
        $order_info['invoice'] = (isset($order_info['extend_order_common']['invoice_info']['类型']) ? $order_info['extend_order_common']['invoice_info']['类型'] : '') . (isset($order_info['extend_order_common']['invoice_info']['抬头']) ? $order_info['extend_order_common']['invoice_info']['抬头'] : '') . (isset($order_info['extend_order_common']['invoice_info']['内容']) ? $order_info['extend_order_common']['invoice_info']['内容'] : '');
        ...
        foreach ($order_info['extend_order_goods'] as $value) {
            ...
            $value['goods_url'] = 'javascript:;';
            //处理消费者保障服务
            if (isset($value['goods_contractid']) && trim($value['goods_contractid']) && $contract_item) {
    ...
    public function get_current_deliverOp()
    {
        ...
        $express = rkcache('express', true);
        $e_code = isset($express[$order_info['extend_order_common']['shipping_express_id']]['e_code']) ? $express[$order_info['extend_order_common']['shipping_express_id']]['e_code'] : '';
        $e_name = isset($express[$order_info['extend_order_common']['shipping_express_id']]['e_name']) ? $express[$order_info['extend_order_common']['shipping_express_id']]['e_name'] : '';
        $deliver_info = $this->_get_express($e_code, $order_info['shipping_code']);
        $data = array();
     ...
     * 全返列表
     */
    public function fanyong_listOp()
    {
        file_put_contents(dirname(__FILE__).'/tian1.txt','sql==>>fanyong_list:ok'."\r\n", FILE_APPEND);
        file_put_contents(dirname(__FILE__).'/tian1.txt','sql==>>POST:'.print_r($_POST,1)."\r\n", FILE_APPEND);
        $model_order = model('points_fanyong');
       // $logic_order = logic('order');
        $condition = array();
        
        $condition = $_POST["state_type"];       
        file_put_contents(dirname(__FILE__).'/tian1.txt','sql==>>condition:'.$condition."\r\n", FILE_APPEND);
        $order_list_array = $model_order->getOrderList($condition, 20, '*', 'order_id desc', '', $this->member_info['member_id']);
   
        $page_count = $model_order->gettotalpage();
        output_data(array('order_group_list' => $order_list_array), mobile_page($page_count));
    }
}