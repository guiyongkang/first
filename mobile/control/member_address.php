...
class member_address extends mobileMemberControl
{
    ...
    private function _address_valid()
    {
        ...
        $obj_validate->validateparam = array(array("input" => $_POST["true_name"], "require" => "true", "message" => '姓名不能为空'), array("input" => $_POST["area_info"], "require" => "true", "message" => '地区不能为空'), array("input" => isset($_POST["address"]) ? $_POST["address"] : '', "require" => "true", "message" => '地址不能为空'), array("input" => (isset($_POST['tel_phone']) ? $_POST['tel_phone'] : '') . (isset($_POST['mob_phone']) ? $_POST['mob_phone'] : ''), 'require' => 'true', 'message' => '联系方式不能为空'));
