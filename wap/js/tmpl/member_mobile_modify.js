$(function() {
	...
	$.ajax({
		...
		success: function(e) {
			if (e.code == 200) {
				...
				if(!e.datas.mobile_is_open){
					$('#send').remove();
					$('#auth_code').parents('li.form-item').remove();
					$('#nextform').parent('.form-btn').addClass('ok');
				}
			}
		}
	});
	...
	$('#send').click(function() {
		if ($.sValid()) {
			...
			$.ajax({
				...
				dataType: 'json',
				success: function(e) {
					if (e.code == 200 && typeof e.datas.error == 'undefined') {
						$('#send').hide();
	...
	$('#nextform').click(function() {
		...
		if(!$('#auth_code').parents('li.form-item').length){
			var a = true;
		}else{
