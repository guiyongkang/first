...
function store(A,B,C,D) {
    ...
    if ( !! B) {
			...
			ZZ+='<li data-productid="' + AAA.goods_id + '"><a href="product_detail.html?goods_id=' + AAA.goods_id + '"><div class="p-img"><img src="' + AAA.url + '"></div><div class="p-info">' + CCC + '<p class="pdlist-goods_jingle">' + AAA.goods_jingle + '</p></div><div class="flag"><span class="pdlist-iwc-pdprice">￥' + AAA.goods_price + '</span>'+ (AAA.goods_promotion_type != 0 && AAA.goods_promotion_type != 1 ? ('&nbsp;<span class="product-status bg-blue">限时折扣</span>') : "") + (AAA.goods_promotion_type != 0 && AAA.goods_promotion_type != 2 ? ('&nbsp;<span class="product-status bg-yf8">抢购商品</span>') : "") + (AAA.goods_promotion_type != 1 && AAA.goods_promotion_type != 2 ? ("") : "")+(AAA.is_fcode != 0 && AAA.is_fcode == 1 ? ('&nbsp;<span class="product-status bg-fcode">F码优先</span>') : "") +(AAA.is_presell != 0 && AAA.is_presell == 1 ? ('&nbsp;<span class="product-status bg-presell">预售</span>') : "") +(AAA.is_virtual != 0 && AAA.is_virtual == 1 ? ('&nbsp;<span class="product-status bg-virtual">虚拟兑换</span>') : "") +(AAA.have_gift != 0 && AAA.have_gift == 1 ? ('&nbsp;<span class="product-status bg-fcode">赠</span>') : "") +'<div class="stores"><s class="product-price-s">￥'+ AAA.goods_marketprice + '</s><span class="flefeet">销量：<strong>' + AAA.goods_salenum + '</strong>&nbsp;件</span></div></div></a></li>'	
			}
			$("#contenttt").append(ZZ);
            }else{
		var ZZ = "";
