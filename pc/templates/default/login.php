  ...
  <div class="arrow"></div>
    <div class="nc-login-mode">
      <ul class="tabs-nav">
        ...
        <?php if ($output['setting_config']['sms_login'] == 1){?>
        <li><a href="#mobile">手机动态码登录<i></i></a></li>
        <?php } ?>
      </ul>
      <div id="tabs_container" class="tabs-container">
        <div id="default" class="tabs-content">
          <form id="login_form" class="nc-login-form" method="post" action="<?php echo APP_URL;?>/index.php?act=login&op=login">
        ...
        <div id="mobile" class="tabs-content">
          <form id="post_form" method="post" class="nc-login-form" action="<?php echo APP_URL;?>/index.php?act=connect_sms&op=login">
            <?php Security::getToken();?>
            <input type="hidden" name="form_submit" value="ok" />
            <input name="nchash" type="hidden" value="<?php echo getNchash();?>" />
            <dl>
    ...
    <div class="nc-login-api" id="demo-form-site">
      ...
      <a href="<?php echo APP_URL;?>/api.php?act=toqq" title="QQ账号登录" class="qq"><i></i>QQ</a>
      <?php } ?>
      <?php if ($output['setting_config']['sina_isuse'] == 1){?>
      <a href="<?php echo APP_URL;?>/api.php?act=tosina" title="新浪微博账号登录" class="sina"><i></i>新浪微博</a>
      <?php } ?>
      <?php if ($output['setting_config']['weixin_isuse'] == 1){?>
      <a href="javascript:void(0);" onclick="ajax_form('weixin_form', '微信账号登录', '<?php echo APP_URL;?>/index.php?act=connect_wx&op=index', 360);" title="微信账号登录" class="wx"><i></i>微信</a>
      <?php } ?>
      <?php } ?>
    </div>
  </div>
...
<?php if ($output['setting_config']['sms_login'] == 1){?>
<script type="text/javascript" src="<?php echo APP_RESOURCE_SITE_URL;?>/js/connect_sms.js" charset="utf-8"></script>
<script>
$(function(){
	$("#post_form").validate({
        errorPlacement: function(error, element){
