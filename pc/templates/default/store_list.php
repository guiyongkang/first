...
<link href="<?php echo APP_TEMPLATES_URL;?>/css/store_list.css"
...
<script type="text/javascript"
	src="<?php echo APP_RESOURCE_SITE_URL.'/js/Jquery.Query.js';?>"
...
<div class="content">
	<div class="nc-store-class">
		...
		<dl>
			...
			<dd> <a href="<?php echo urlBiz('store_list','index');?>">所有店铺</a>
				<?php foreach($output['class_list'] as $k=>$v){?>
				<?php if ($_GET['cate_id'] == $v['sc_parent_id']){?>
				<a
					href="<?php echo urlBiz('store_list','index',array('cate_id'=>$k));?>"><?php echo $v['sc_name'];?></a>
		...
		<div class="shop_con_list" id="main-nav-holder">
			<nav class="nc-gl-sort-bar" id="main-nav">
				<form id="store_list" method="GET" action="index.php">
					...
					<input type="hidden"
						name="act" value="store_list" />
					<input type="hidden"
						name="cate_id" value="<?php echo $_GET['cate_id'];?>" />
					<div class="sort-bar"> 
						<!-- 排序方式S -->
						<ul class="array">
							<!-- 默认 -->
							<li <?php if(!$_get['key']){?> class="selected" <?php }?>><a
								href="javascript:void(0)" class="nobg"
								onclick="javascript:ss_dropParam('key','order');"
								title="<?php echo $lang['goods_class_index_default_sort'];?>">默认</a></li>
							<!-- 销量 -->
							<li <?php if($_get['key'] == 'store_sales'){?> class="selected"
								<?php }?>><a href="javascript:void(0)"
								<?php if($_get['key'] == 'store_sales'){?>
								class="<?php echo $_GET['order'];?>" <?php }?>
								onclick="javascript:ss_replaceParam('store_sales','<?php echo ($_GET['order'] == 'desc' && $_GET['key'] == 'store_sales')?'asc':'desc' ?>');"
								title="<?php echo ($_GET['order'] == 'desc' && $_GET['key'] == 'store_sales')?$lang['store_class_index_sold_asc']:$lang['store_class_index_sold_desc']; ?>">销量</a></li>
							<!-- 信用 -->
							<li <?php if($_get['key'] == 'store_credit'){?> class="selected"
								<?php }?>><a href="javascript:void(0)"
								<?php if($_get['key'] == 'store_credit'){?>
								class="<?php echo $_GET['order'];?>" <?php }?>
								onclick="javascript:ss_replaceParam('store_credit','<?php  echo ($_GET['order'] == 'desc' && $_GET['key'] == 'store_credit')?'asc':'desc' ?>');"
								title="<?php  echo ($_GET['order'] == 'desc' && $_GET['key'] == 'store_credit')?$lang['store_class_index_credit_asc']:$lang['store_class_index_credit_desc']; ?>">信用</a></li>
						</ul>
						<!-- 排序方式E -->
						...
						<div class="sidebox">
							...
							<div class="selectbox">
								<input class="text" type="text" name="keyword"
									value="<?php echo $_GET['keyword'];?>" style="width: 150px;" />
							</div>
						</div>
						<div class="sidebox">
							<h5>
								<label for="area_info">店铺所在地<?php echo $lang['nc_colon'];?></label>
							</h5>
							<div class="selectbox">
	...
	<ul class="nc-store-list">
		...
		<li class="item">
			<dl class="shop-info">
				<dt class="shop-name"> <a
						href="<?php echo urlBiz('show_store','', array('store_id'=>$store['store_id']),$store['store_domain']);?>"
						target="_blank"><?php echo $store['store_name'];?></a> <img
						src="<?php echo '/shop/templates/default/images/store_grade/'.$store['grade_id'].'.png'; ?>"
						class="pngFix"> </dt>
				<dd class="shop-pic"> <a
						href="<?php echo urlBiz('show_store','', array('store_id'=>$store['store_id']),$store['store_domain']);?>"
						title="" target="_blank"><span class="size72"><img
							src="<?php echo UPLOAD_SITE_URL;?>/shop/common/loading.gif"
							rel="lazy"
							data-url="<?php echo getStoreLogo($store['store_avatar']);?>"
							alt="<?php echo $store['store_name'];?>"
							title="<?php echo $store['store_name'];?>" class="size72" /></span></a> </dd>
				<dd class="shopkeeper"><?php echo $lang['store_class_index_owner'].$lang['nc_colon'];?><?php echo $store['member_name'];?><a
						target="_blank" class="message"
						href="index.php?act=member_message&op=sendmsg&member_id=<?php echo $store['member_id'];?>"></a>
				<span>
			...
			<dl class="w200">
				...
				<dd>
					...
					<?php echo $lang['store_class_index_credit_value'].$lang['nc_colon'];?> <span
						class="seller-heart level-<?php echo $store['store_credit_average']; ?>"></span>
					<?php }?>
				</dd>
				<!-- 店铺好评率 -->
				<dd>
				...
				<!-- 店铺动态评分 -->
				<dd class="shop-rate" nc_type="shop-rate"
					store_id='<?php echo $store['store_id'];?>'><?php echo $lang['store_class_index_shop_rate'].$lang['nc_colon'];?><span><i></i></span>
			...
			<dl class="w250">
				...
				<dd> 客服：
					...
					<a target="_blank"
						href="http://wpa.qq.com/msgrd?v=3&uin=<?php echo $store['store_qq'];?>&site=qq&menu=yes"
						title="QQ: <?php echo $store['store_qq'];?>"><img border="0"
						src="http://wpa.qq.com/pa?p=2:<?php echo $store['store_qq'];?>:52"
						style="vertical-align: middle;" /></a>
					<?php }?>
					<?php if(!empty($store['store_ww'])){?>
					<a target="_blank"
						href="http://amos.im.alisoft.com/msg.aw?v=2&uid=<?php echo $store['store_ww'];?>&site=cntaobao&s=2&charset=<?php echo CHARSET;?>"><img
						border="0"
						src="http://amos.im.alisoft.com/online.aw?v=2&uid=<?php echo $store['store_ww'];?>&site=cntaobao&s=2&charset=<?php echo CHARSET;?>"
						alt="Wang Wang" style="vertical-align: middle;" /></a>
					<?php }?>
					</span> <span member_id="<?php echo $store['member_id'];?>"></span> </dd>
				<dd>
			...
			<div class="nc-shop-goodslist" nc_type='goods_<?php echo $skey;?>'>
				...
				<ul class="jcarousel-skin-tango" nc_type="jcarousel">
					...
					<li>
						<dl>
							<dt class="goods-pic"> <span class="thumb size160"> <i></i> <a
									href="index.php?act=goods&goods_id=<?php echo $v['goods_id'];?>"
									target="_blank"> <img
										onload="javascript:DrawImage(this,160,160);"
										alt="<?php echo $v['goods_name'];?>" width="160" height="160"
										src="<?php echo thumb($v,'small');?>"></a></span> </dt>
							<dd class="goods-name"> <a
									href="<?php echo urlBiz('goods','',array('goods_id'=>$v['goods_id']));?>"
									title="<?php echo $v['goods_name'];?>" target="_blank"><?php echo $v['goods_name'];?></a> </dd>
...
<script type="text/javascript"
	src="<?php echo RESOURCE_SITE_URL;?>/js/waypoints.js"></script> 
<script type="text/javascript"
