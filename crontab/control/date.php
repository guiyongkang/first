...
class date extends control
{
    ...
    private function _add_points()
    {
        ...
        //24小时之内完成了实物订单送元和经验值,每次最多处理5W个订单
        $model_order = model('order');
        $condition = array();
    ...
    private function _voucher_will_expire()
    {
        ...
        if (!empty($list)) {
            foreach ($list as $val) {
                ...
                $param['param'] = array('indate' => date('Y-m-d H:i:s', $val['voucher_end_date']), 'voucher_url' => urlShop('member_voucher', 'index'));
    ...
    private function _vr_code_will_expire()
    {
        ...
        if (!empty($list)) {
            foreach ($list as $val) {
                ...
                $param['param'] = array('indate' => date('Y-m-d H:i:s', $val['vr_indate']), 'vr_order_url' => urlShop('member_vr_order', 'index'));
                \lib\queue::push('sendMemberMsg', $param);
            }
        }
    }
    ...
    private function _arrival_notice()
    {
        ...
        for ($i = 0; $i <= $times; $i++) {
            ...
            foreach ($notice_list as $val) {
                ...
                $param['param'] = array('goods_name' => $val['goods_name'], 'goods_url' => 'javascript:;');
    ...
    private function _order_goods_cache()
    {
        ...
        if ($latest_record) {
            ...
            $start_time = strtotime(date('Y-m-d', strtotime(core\config::get('setup_date'))));
            //从系统的安装时间开始统计
        }
        for ($stime = $start_time; $stime < time(); $stime = $stime + 86400) {
            $etime = $stime + 86400 - 1;
            //避免重复统计，开始时间必须大于最后一条记录的记录时间
            $search_stime = isset($latest_record['stat_updatetime']) && $latest_record['stat_updatetime'] > $stime ? $latest_record['stat_updatetime'] : $stime;
            ...
            for ($i = 0; $i < $countnum; $i += 100) {
                ...
                if ($orderlog_list) {
                    ...
                    foreach ((array) $order_list_tmp as $k => $v) {
                        ...
                        $v['order_isvalid'] = 0;
                        if ($v['payment_code'] != 'offline' && $v['order_state'] != ORDER_STATE_NEW && $v['order_state'] != ORDER_STATE_CANCEL) {
                            //在线支付并且已支付并且未取消
                            $v['order_isvalid'] = 1;
                        } elseif ($v['order_state'] == ORDER_STATE_CANCEL && $v['refund_state'] != 0) {
                            //经过退款的取消订单
                            $v['order_isvalid'] = 1;
                        } elseif ($v['payment_code'] == 'offline' && $v['order_state'] == ORDER_STATE_SUCCESS) {
                            //货到付款订单，订单成功之后才计入统计
                            $v['order_isvalid'] = 1;
                        }
                    ...
                    foreach ((array) $ordergoods_list as $k => $v) {
                        ...
                        $tmp['goods_commonid'] = isset($goods_list[$v['goods_id']]['goods_commonid']) ? intval($goods_list[$v['goods_id']]['goods_commonid']) : 0;
                        $tmp['goods_commonname'] = isset($goods_list[$v['goods_id']]['goods_commonname']) ? $goods_list[$v['goods_id']]['goods_commonname'] : '';
                        $tmp['gc_id'] = isset($goods_list[$v['goods_id']]['gc_id']) ? intval($goods_list[$v['goods_id']]['gc_id']) : 0;
                        $tmp['gc_parentid_1'] = isset($goods_list[$v['goods_id']]['gc_id_1']) ? intval($goods_list[$v['goods_id']]['gc_id_1']) : 0;
                        $tmp['gc_parentid_2'] = isset($goods_list[$v['goods_id']]['gc_id_2']) ? intval($goods_list[$v['goods_id']]['gc_id_2']) : 0;
                        $tmp['gc_parentid_3'] = isset($goods_list[$v['goods_id']]['gc_id_3']) ? intval($goods_list[$v['goods_id']]['gc_id_3']) : 0;
                        $tmp['brand_id'] = isset($goods_list[$v['goods_id']]['brand_id']) ? intval($goods_list[$v['goods_id']]['brand_id']) : 0;
                        $tmp['brand_name'] = isset($goods_list[$v['goods_id']]['brand_name']) ? $goods_list[$v['goods_id']]['brand_name'] : '';
                        $tmp['goods_serial'] = isset($goods_list[$v['goods_id']]['goods_serial']) ? $goods_list[$v['goods_id']]['goods_serial'] : '';
                        $tmp['goods_price'] = $v['goods_price'];
                        $tmp['goods_num'] = $v['goods_num'];
                        $tmp['goods_image'] = isset($goods_list[$v['goods_id']]['goods_image']) ? $goods_list[$v['goods_id']]['goods_image'] : 0;
    ...
    private function _member_stat()
    {
        ...
        for ($stime = $start_time; $stime < time(); $stime = $stime + 86400) {
            ...
            $etime = $stime + 86400 - 1;
            //避免重复统计，开始时间必须大于最后一条记录的记录时间
            $search_stime = isset($latest_record['statm_updatetime']) && $latest_record['statm_updatetime'] > $stime ? $latest_record['statm_updatetime'] : $stime;
            //统计一天的数据，如果结束时间大于当前时间，则结束时间为当前时间，避免因为查询时间的延迟造成数据遗落
            $search_etime = ($t = $stime + 86400 - 1) > time() ? time() : $stime + 86400 - 1;
            //统计订单下单量和下单金额
            $field = ' order.order_id,add_time,buyer_id,buyer_name,order_amount';
            $where = array();
            $where['order.order_state'] = array('neq', ORDER_STATE_NEW);
            //去除未支付订单
            $where['order.refund_state'] = array('exp', "!(order.order_state = '" . ORDER_STATE_CANCEL . "' and order.refund_state = 0)");
            //没有参与退款的取消订单，不记录到统计中
            $where['order_log.log_time'] = array('between', array($search_stime, $search_etime));
            //按照订单付款的操作时间统计
            ...
            $orderlist_tmp = $model->statByOrderLog($where, $field, 0, 0, 'order_id');
            //此处由于底层的限制，仅能查询1000条，如果日下单量大于1000，则需要limit的支持
            $order_list = array();
            $orderid_list = array();
            foreach ((array) $orderlist_tmp as $k => $v) {
                $addtime = strtotime(date('Y-m-d', $v['add_time']));
                if ($addtime != $stime) {
                    //订单如果隔天支付的话，需要进行统计数据更新
                    ...
                    $order_list[$v['buyer_id']]['ordernum'] = (isset($order_list[$v['buyer_id']]['ordernum']) ? intval($order_list[$v['buyer_id']]['ordernum']) : 0) + 1;
					if(!isset($order_list[$v['buyer_id']]['orderamount'])){
						$order_list[$v['buyer_id']]['orderamount'] = 0;
					}
                    $order_list[$v['buyer_id']]['orderamount'] = floatval($order_list[$v['buyer_id']]['orderamount']) + (!empty($v['order_amount']) ? floatval($v['order_amount']) : 0);
                }
                //记录订单ID数组
            ...
            if ($orderid_list) {
                ...
                foreach ((array) $ordergoods_tmp as $k => $v) {
                    ...
                    if ($addtime != $stime) {
                        //订单如果隔天支付的话，需要进行统计数据更新
						if(!isset($update_arr[$addtime][$v['buyer_id']]['statm_goodsnum'])){
							$update_arr[$addtime][$v['buyer_id']]['statm_goodsnum'] = 0;
						}
                        $update_arr[$addtime][$v['buyer_id']]['statm_goodsnum'] = intval($update_arr[$addtime][$v['buyer_id']]['statm_goodsnum']) + (!empty($v['goods_num']) ? floatval($v['goods_num']) : 0);
                    } else {
						if(!isset($ordergoods_list[$v['buyer_id']]['goodsnum'])){
							$ordergoods_list[$v['buyer_id']]['goodsnum'] = 0;
						}
                        $ordergoods_list[$v['buyer_id']]['goodsnum'] = $ordergoods_list[$v['buyer_id']]['goodsnum'] + (!empty($v['goods_num']) > 0 ? floatval($v['goods_num']) : 0);
                    }
                }
            }
            //统计的预存款记录
            ...
            $memberidarr_ordergoods = !empty($ordergoods_list) ? array_keys($ordergoods_list) : array();
            $memberidarr_predeposit = !empty($predeposit_list) ? array_keys($predeposit_list) : array();
            $memberidarr_points = !empty($points_list) ? array_keys($points_list) : array();
            ...
            foreach ((array) $memberid_list as $k => $v) {
				...
				if(!isset($statmemberlist[$tmp['statm_memberid']]['statm_ordernum'])){
					$statmemberlist[$tmp['statm_memberid']]['statm_ordernum'] = 0;
				}
                $tmp['statm_ordernum'] = intval($statmemberlist[$tmp['statm_memberid']]['statm_ordernum']) + (!empty($order_list[$tmp['statm_memberid']]['ordernum']) ? intval($order_list[$tmp['statm_memberid']]['ordernum']) : 0);
                if(!isset($statmemberlist[$tmp['statm_memberid']]['statm_orderamount'])){
					$statmemberlist[$tmp['statm_memberid']]['statm_orderamount'] = 0;
				}
				$tmp['statm_orderamount'] = floatval($statmemberlist[$tmp['statm_memberid']]['statm_orderamount']) + (!empty($order_list[$tmp['statm_memberid']]['orderamount']) ? floatval($order_list[$tmp['statm_memberid']]['orderamount']) : 0);
                if(!isset($statmemberlist[$tmp['statm_memberid']]['statm_goodsnum'])){
					$statmemberlist[$tmp['statm_memberid']]['statm_goodsnum'] = 0;
				}
				$tmp['statm_goodsnum'] = intval($statmemberlist[$tmp['statm_memberid']]['statm_goodsnum']) + (!empty($ordergoods_list[$tmp['statm_memberid']]['goodsnum']) ? intval($ordergoods_list[$tmp['statm_memberid']]['goodsnum']) : 0);
                if(!isset($predeposit_list[$tmp['statm_memberid']]['predincrease'])){
					$predeposit_list[$tmp['statm_memberid']]['predincrease'] = 0;
				}
				$tmp['statm_predincrease'] = ($t = floatval($predeposit_list[$tmp['statm_memberid']]['predincrease'])) ? $t : 0;
                if(!isset($predeposit_list[$tmp['statm_memberid']]['predreduce'])){
					$predeposit_list[$tmp['statm_memberid']]['predreduce'] = 0;
				}
				$tmp['statm_predreduce'] = ($t = floatval($predeposit_list[$tmp['statm_memberid']]['predreduce'])) ? $t : 0;
				if(!isset($points_list[$tmp['statm_memberid']]['pointsincrease'])){
					$points_list[$tmp['statm_memberid']]['pointsincrease'] = 0;
				}
                $tmp['statm_pointsincrease'] = ($t = intval($points_list[$tmp['statm_memberid']]['pointsincrease'])) ? $t : 0;
                if(!isset($points_list[$tmp['statm_memberid']]['pointsreduce'])){
					$points_list[$tmp['statm_memberid']]['pointsreduce'] = 0;
				}
