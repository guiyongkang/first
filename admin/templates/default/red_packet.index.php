...
<div class="page">
	...
	<form method="get" name="formSearch">
		...
		<input
			type="hidden" name="op" value="index">
		<table class="tb-type1 noborder search">
			<tbody>
				<tr>
					<th>红包名称</th>
					<td><input type="text" name="packet_name" class="txt"
						value='<?php echo isset($_GET['packet_name']) ? $_GET['packet_name'] : '';?>'></td>
					<th>状态</th>
					<td><select name="state">
							<option value="">请选择</option>
							<option value="1"
								<?php if(isset($_GET['state']) && $_GET['state']==1){echo 'selected';} ?>>开启</option>
							<option value="2"
								<?php if(isset($_GET['state']) && $_GET['state']==2){echo 'selected';} ?>>关闭</option>
						</select></td>
					<td><a href="javascript:document.formSearch.submit();"
						class="btn-search " title="<?php echo $lang['nc_query']; ?>">&nbsp;</a></td>
				</tr>
			</tbody>
		</table>
	</form>
	...
	<form id="listform" action="index.php" method='post'>
		...
		<input
			type="hidden" id="listop" name="op" value="del" />
			...
			<tbody id="treet1">
				...
				<tr class="hover edit row">
					<?php
										...
										?>
					<td><a
						href="<?php echo APP_URL; ?>/tmpl/red_packet.html?id=<?php echo $v['id']; ?>"
						target="_blank"><?php echo $v['packet_name'];?></a></td>
					<td class="align-center"><?php echo $v['packet_amount'];?></td>
					<td class="align-center"><?php echo $v['packet_number'];?></td>
					<td class="align-center">
					    <a href="index.php?act=red_packet&op=uselist&id=<?php echo $v['id']; ?>"><?php echo $v['packet_numbered'];?></a>
					</td>
					...
					<td class="align-center"><a
						href="index.php?act=red_packet&op=list&id=<?php echo $v['id']; ?>">查看</a> | <a
						href="index.php?act=red_packet&op=edit&id=<?php echo $v['id']; ?>">编辑</a> |
						<?php if($v['state']==1){ ?>
						<a
						href="javascript:if(confirm('确定关闭?'))window.location.href='index.php?act=red_packet&op=state&id=<?php echo $v['id']; ?>&type=close'">关闭</a>
						<?php }else{ ?>
						<a
						href="javascript:if(confirm('确定开启?'))window.location.href='index.php?act=red_packet&op=state&id=<?php echo $v['id']; ?>&type=open'">开启</a>
						<?php } ?>
						|
						...
						<?php }else{ ?>
						<a
						href="javascript:if(confirm('确定删除?'))window.location.href='index.php?act=red_packet&op=del&id=<?php echo $v['id']; ?>'">删除</a>
						<?php } ?></td>
				</tr>
				<?php } ?>
				<?php }else { ?>
