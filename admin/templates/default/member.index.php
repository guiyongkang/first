			...
			<tbody>
				<tr>
					...
					<td><select name="search_state" >
							<option <?php if(empty($_GET['search_state'])){ ?>selected='selected'<?php } ?> value=""><?php echo $lang['member_index_state']; ?></option>
							<!--<option <?php if(isset($_GET['search_state']) && $_GET['search_state'] == 'no_informallow'){ ?>selected='selected'<?php } ?> value="no_informallow"><?php echo $lang['member_index_inform_deny']; ?></option>-->
							<option <?php if(isset($_GET['search_state']) && $_GET['search_state'] == 'no_isbuy'){ ?>selected='selected'<?php } ?> value="no_isbuy"><?php echo $lang['member_index_buy_deny']; ?></option>
							<!--<option <?php if(isset($_GET['search_state']) && $_GET['search_state'] == 'no_isallowtalk'){ ?>selected='selected'<?php } ?> value="no_isallowtalk"><?php echo $lang['member_index_talk_deny']; ?></option>-->
							<option <?php if(isset($_GET['search_state']) && $_GET['search_state'] == 'no_memberstate'){ ?>selected='selected'<?php } ?> value="no_memberstate"><?php echo $lang['member_index_login_deny']; ?></option>
						</select></td>
		...
		<table class="table tb-type2 nobdb">
			...
			<tbody>
				...
				<tr class="hover member">
					...
					<td class="w48 picture"><div class="size-44x44"><span class="thumb size-44x44"><i></i><img src="<?php if ($v['member_avatar'] != ''){ echo UPLOAD_SITE_URL.DS.ATTACH_AVATAR.DS.$v['member_avatar'];}else { echo UPLOAD_SITE_URL.'/'.ATTACH_COMMON.DS.$output['setting_config']['default_user_portrait'];}?>?<?php echo microtime();?>"  onload="javascript:DrawImage(this,44,44);"/></span></div></td>
