	...
	<form method="get" action="index.php" name="formSearch" id="formSearch">
		...
		<table class="tb-type1 noborder search">
			<tbody>
				<tr>
					<th>关键字</th>
					<td><input type="text" class="text" name="msg" value="<?php echo isset($_GET['msg']) ? trim($_GET['msg']) : ''; ?>" /></td>
	...
	<div class="chat-log">
		<ul class="chat-log-list">
			...
			<li class="from_log"><span class="avatar"><img src="<?php echo getMemberAvatarForID($val['f_id']);?>"/></span>
				<dl>
					<dt class="store_log">发消息会员：<?php echo $val['f_name']; ?> <span>(接收消息会员：<?php echo $val['t_name']; ?>)</span></span> </dt>
					<dd class="time"><?php echo date("Y-m-d H:i:s",$val['add_time']);?> <a href="index.php?act=chat_log&op=chat_log&f_name=<?php echo $val['f_name']; ?>&t_name=<?php echo $val['t_name']; ?>&add_time_from=<?php echo date("Y-m-d",$val['add_time']);?>&add_time_to=<?php echo date("Y-m-d",$val['add_time']);?>">详情</a></dd>
