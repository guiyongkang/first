			...
			<tbody>
				<tr>
					...
					<td><select name="state">
							...
							<option <?php if (isset($_GET['state']) && $_GET['state'] == '0') echo 'selected';?> value="0">待审核</option>
							<option <?php if (isset($_GET['state']) && $_GET['state'] == '1') echo 'selected';?> value="1">已经审核</option>
						</select></td>
		...
		<table class="table tb-type2">
			...
			<tbody>
				...
				<tr class="hover edit">
					...
					<td class="w72 align-center"><?php if ($v['state'] == '0') {?>
						...
						<?php } ?>
						<a href="javascript:if(confirm('<?php echo $v['state'] == '1' ? '该类目已经审核通过，删除它可能影响到商家的使用，' : null;?>确认删除吗？'))window.location = 'index.php?act=store&op=store_bind_class_applay_del&bid=<?php echo $v['bid'];?>&store_id=<?php echo $v['store_id'];?>';"><?php echo $lang['nc_del'];?></a></td>
				</tr>
				<?php } ?>
				<?php }else { ?>
				<tr class="no_data">
			...
			<tfoot>
				<tr class="tfoot">
					<td></td>
					<td colspan="15"><?php if(!empty($output['bind_list']) && is_array($output['bind_list'])){ ?>
