...
<div class="page">
	<div class="fixed-bar">
		<div class="item-title">
			...
			<ul class="tab-base">	
			<!--		
				<li><a href="<?php echo urlAdmin('goodsr', 'add_step_one', array('type' => 'waitverify'));?>"><span>增加商品</span></a></li>
				-->
			</ul>
		</div>
	</div>
		...
		<table class="tb-type1 noborder search">
			<tbody>
				<tr>
					...
					<td><input type="text" value="<?php echo isset($output['search']['search_commonid']) ? $output['search']['search_commonid'] : ''?>" name="search_commonid" id="search_commonid" class="txt" /></td>
				...
				<tr>
					...
					<td><input type="text" value="<?php echo isset($output['search']['search_store_name']) ? $output['search']['search_store_name'] : '';?>" name="search_store_name" id="search_store_name" class="txt"></td>
					<th><label>品牌</label></th>
					<td><div id="ajax_brand" class="ncsc-brand-select w180">
							<div class="selection">
								<input name="b_name" id="b_name" value="<?php echo isset($_REQUEST['b_name']) ? $_REQUEST['b_name'] : '';?>" type="text" class="txt w180" readonly="readonly" />
								<input type="hidden" name="b_id" id="b_id" value="<?php echo isset($_REQUEST['b_id']) ? $_REQUEST['b_id'] : '';?>" />
							</div>
							<div class="ncsc-brand-select-container">
								<div class="brand-index" data-url="index.php?act=common&op=ajax_get_brand">
									...
									<div class="search" nctype="search">
										...
										<a href="javascript:void(0);" class="ncsc-btn-mini" style="vertical-align: top;">Go</a>
									</div>
								</div>
					...
					<th><label>上架</label></th>
					<td><select name="search_state">
							<option value="">请选择...</option>
							<?php foreach ($output['state'] as $key => $val){?>
							<option value="<?php echo $key;?>" <?php if(!empty($output['search']['search_state']) && $output['search']['search_state'] == $key){?>selected<?php }?>><?php echo $val;?></option>
							<?php }?>
						</select></td>
					...
					<td><select name="search_verify">
							...
							<option value="<?php echo $key;?>" <?php if(!empty($output['search']['search_verify']) && $output['search']['search_verify'] == $key){?>selected<?php }?>><?php echo $val;?></option>
							<?php }?>
						</select></td>
			...
			<tbody>
				...
				<tr class="hover edit">
					...
					<td><dl class="goods-info">
							...
							<dd class="goods-type" style="position:relative;">
								...
								<i style="margin-left:10px;" class="icon-link" title="手机端商品连接"><div style="display:none;"><?=WAP_SITE_URL . '/tmpl/product_detail.html?goods_id=' . (isset($output['storage_array'][$v['goods_commonid']]['goods_id']) ? $output['storage_array'][$v['goods_commonid']]['goods_id'] : 0)?></div></i>
							</dd>
							<dd class="goods-store"><?php echo isset($output['ownShopIds'][$v['store_id']]) ? '平台' : '三方'; ?>店铺：<?php echo $v['store_name'];?></dd>
						</dl></td>
					...
					<td class="align-center"><a href="javascript:void(0);" onclick="goods_lockup(<?php echo $v['goods_commonid'];?>);">违规下架</a></td>
				</tr>
...
function goods_lockup(ids) {
    _uri = "<?php echo APP_URL;?>/index.php?act=goods&op=goods_lockup&id=" + ids;
