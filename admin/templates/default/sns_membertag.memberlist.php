		...
		<table class="table tb-type2">
			...
			<tbody>
				<tr class="hover edit">
					...
					<td class="w48 picture"><div class="size-44x44"><span class="thumb size-44x44"><i></i><img src="<?php if ($v['member_avatar'] != ''){ echo UPLOAD_SITE_URL.DS.ATTACH_AVATAR.DS.$v['member_avatar'];}else { echo APP_TEMPLATES_URL.'/images/default_user_portrait.gif';}?>"  onload="javascript:DrawImage(this,44,44);"/></span></div></td>
					<td class="w50pre name"><span><strong><?php echo $v['member_name'];?></strong></span></td>
					<td class="align-center power-onoff"><a href="JavaScript:void(0);" class=" <?php if($v['recommend'] == 0){ ?>disabled<?php }else{ ?>enabled<?php }?>" fieldvalue="<?php echo $v['recommend'];?>" fieldid="<?php echo $v['mtag_id'].','.$v['member_id'];?>" ajax_branch="mtagmember_recommend" fieldname="recommend" nc_type="inline_edit" title="<?php echo $lang['nc_editable'];?>"><img src="<?php echo APP_TEMPLATES_URL;?>/images/transparent.gif"></a></td>
					<td class="w120 align-center"><a href="index.php?act=sns_member&op=mtag_del&id=<?php echo $v['mtag_id'];?>&mid=<?php echo $v['member_id'];?>"><?php echo $lang['nc_del'];?></a></td>
				</tr>
				<?php } ?>
			</tbody>
			<tfoot>
