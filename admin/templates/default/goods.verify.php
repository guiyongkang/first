			...
			<tbody>
				<tr>
					...
					<td><select name="search_verify">
							...
							<option value="10" <?php if (!empty($output['search']['search_verify']) && $output['search']['search_verify'] == 10) {?>selected="selected"<?php }?>>等待审核</option>
							<option value="0" <?php if (!empty($output['search']['search_verify']) && $output['search']['search_verify'] == 0) {?>selected="selected"<?php }?>>未通过</option>
						</select></td>
			...
			<tbody>
				...
				<tr class="hover edit">
					...
					<td class="align-center"><p><?php echo $output['verify'][$v['goods_verify']];?></p>
						...
						<?php }?></td>
					<td class="w48 align-center"><a href="javascript:;"><?php echo $lang['nc_view'];?></a>&nbsp;|&nbsp;<a href="javascript:void(0);" onclick="goods_verify(<?php echo $v['goods_commonid'];?>);">审核</a></td>
				</tr>
...
function goods_verify(ids) {
    _uri = "<?php echo APP_URL;?>/index.php?act=goods&op=goods_verify&id=" + ids;
