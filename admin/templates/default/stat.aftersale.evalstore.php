	...
	<form method="get" action="index.php" name="formSearch" id="formSearch">
		...
		<div class="w100pre" style="width: 100%;">
			<table class="tb-type1 noborder search left">
				<tbody>
					<tr>
						<td id="scategory"><select class="querySelect" name="store_class">
								<option value="0"><?php echo $lang['nc_please_choose'];?>...</option>
								<?php if(is_array($output['class_list'])){ ?>
								<?php foreach($output['class_list'] as $k => $v){ ?>
								<option <?php if(isset($_GET['store_class']) && intval($_GET['store_class']) == $v['sc_id']){ ?>selected="selected"<?php } ?> value="<?php echo $v['sc_id']; ?>"><?php echo $v['sc_name']; ?></option>
								<?php } ?>
								<?php } ?>
							</select></td>
						<td> 店铺名称
							<input type="text" name="storename" value="<?php echo isset($output['search_arr']['storename']) ? $output['search_arr']['storename'] : '';?>"/></td>
