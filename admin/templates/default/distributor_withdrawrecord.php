...
<div class="page">
	...
	<form method="get" name="formSearch" id="formSearch">
		...
		<input type="hidden" name="op" value="withdrawrecord">
		...
		<table class="tb-type1 noborder search">
			<tbody>
				<tr>
                    ...
                    <th><label>提现方式</label></th>
					<td>
                    	<select name="code">
                        	<option value="">全部</option>
                            <?php foreach($output['method_list'] as $item){?>
                            <option value="<?php echo $item['method_code'];?>"<?php echo isset($output['search']['code']) && $output['search']['code']==$item['method_code'] ? ' selected' : '';?>><?php echo $item['method_name'];?></option>
                            <?php }?>
                        </select>
                    </td>
					...
					<td>
                    	<select name="status">
                            ...
                            <option value="1"<?php echo isset($output['search']['status']) && $output['search']['status']==1 ? ' selected' : '';?>>待审核</option>
                            <option value="2"<?php echo isset($output['search']['status']) && $output['search']['status']==2 ? ' selected' : '';?>>已执行</option>
                            <option value="3"<?php echo isset($output['search']['status']) && $output['search']['status']==3 ? ' selected' : '';?>>已驳回</option>
                        </select>
                    </td>
		...
		<table class="table tb-type2">
			<thead>
				<tr class="thead">
                    ...
                    <th class="w84 align-center border-right">总额</th>
                    <th class="w84 align-center border-right">手续费</th>
                    <th class="w84 align-center border-right">转入余额</th>
                    <th class="w84 align-center border-right">实转金额</th>
                    <th class="w150 align-center border-right">提现方式</th>
                    <th class="w84 align-center border-right">状态</th>
                    <th class="w120 align-center border-right">申请时间</th>
			...
			<tbody>
				...
				<tr class="hover edit">
                    ...
                    <td class="align-center border-right"><?php echo $value['record_total'];?></td>
                    <td class="align-center border-right"><?php echo $value['record_fee'];?></td>
                    <td class="align-center border-right"><?php echo $value['record_yue'];?></td>
                    <td class="align-center border-right"><?php echo $value['record_amount'];?></td>
                    <td class="align-center border-right"><?php echo $value['method_title'];?><?php if(!in_array($value['method_code'],array('wxzhuanzhang','wxhongbao','yue'))){?><br /><?php echo $value['method_name']?><br /><?php echo $value['method_no']?><?php }?></td>
                    <td class="align-center border-right">
                    <?php if($value['record_status']==0){?>
                    <font style="padding:8px 15px; color:#FFF; background:#D9534F; border-radius:5px;">待审核</font>
                    <?php }elseif($value['record_status']==1){?>
                    <font style="padding:8px 15px; color:#FFF; background:#5CB85C; border-radius:5px;">已执行</font>
                    <?php }elseif($value['record_status']==2){?>
					...
					<td class="align-center">
                   	<?php if($value['record_status']==0){?>
                    	<?php if($value['method_code']=='wxhongbao'){?>
						<a href="index.php?act=distributor_list&op=withdrawsend&rid=<?php echo $value['record_id'];?>">[发送红包]</a>
						<?php }elseif($value['method_code']=='wxzhuanzhang'){?>
                        <a href="index.php?act=distributor_list&op=withdrawsend&rid=<?php echo $value['record_id'];?>">[微信转账]</a>
                        <?php }else{?>
                        <a href="index.php?act=distributor_list&op=withdrawdeal&rid=<?php echo $value['record_id'];?>">[执行]</a>
                        <?php }?>&nbsp;&nbsp;
                        <a href="index.php?act=distributor_list&op=withdrawreject&rid=<?php echo $value['record_id'];?>">[驳回]</a>
                    <?php }?>
                    </td>
				</tr>
