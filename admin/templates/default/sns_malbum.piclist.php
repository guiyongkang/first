		...
		<table class="tb-type1 noborder search">
			<tbody>
				<tr>
					...
					<td><input type="text" value="<?php echo isset($_GET['pic_name']) ? $_GET['pic_name'] : '';?>" name="pic_name" id="pic_name" class="txt"></td>
	...
	<form method='post' id="form_pic">
		...
		<table class="table tb-type2">
			<tbody>
				...
				<tr>
					<td colspan="20"><ul class="thumblists">
							<?php foreach($output['pic_list'] as $val){?>
							<li class="picture">
								<div class="size-64x64"> <span class="thumb"> <i></i><img width="64" height="64" class="show_image" src="<?php echo UPLOAD_SITE_URL.DS.ATTACH_MALBUM.DS.$val['member_id'].DS.str_ireplace('.', '_240.', $val['ap_cover']);?>" /> <span class="type-file-preview" style="display: none;"> <img src="<?php echo UPLOAD_SITE_URL.DS.ATTACH_MALBUM.DS.$val['member_id'].DS.str_ireplace('.', '_240.', $val['ap_cover']);?>"> </span> </span> </div>
								<p> <span>
									<input class="checkitem" type="checkbox" name="id[]" value="<?php echo $val['ap_id'];?>" />
									</span><span><a href="javascript:void(0);" onclick="if(confirm('<?php echo $lang['nc_ensure_del'];?>')){location.href='index.php?act=sns_malbum&op=del_pic&id=<?php echo $val['ap_id'];?>';}else{return false;}"><?php echo $lang['nc_del'];?></a></span> </p>
							</li>
							<?php } ?>
						</ul></td>
				</tr>
