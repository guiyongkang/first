		...
		<table class="tb-type1 noborder search">
			<tbody>
				<tr>
					...
					<td><input type="text" id="sdate" name="sdate" class="txt date" value="<?php echo isset($_GET['sdate']) ? $_GET['sdate'] : '';?>" >
						~
						<input type="text" id="edate" name="edate" class="txt date" value="<?php echo isset($_GET['edate']) ? $_GET['edate'] : '';?>" ></td>
					<th><label><?php echo $lang['nc_state'];?></label></th>
					<td><select name="state">
							<option value="0" <?php if(isset($_GET['state']) && 0 === intval($_GET['state'])) echo 'selected';?>><?php echo $lang['nc_status'];?></option>
							<?php if(is_array($output['templatestate_arr'])) { ?>
							<?php foreach($output['templatestate_arr'] as $key=>$val) { ?>
							<option value="<?php echo $val[0];?>" <?php if(isset($_GET['state']) && intval($val[0]) === intval($_GET['state'])) echo 'selected';?>><?php echo $val[1];?></option>
							<?php } ?>
							<?php } ?>
						</select></td>
					<td><select name="recommend">
							<option value="" selected>是否推荐</option>
							<option value="1" <?php if(isset($_GET['recommend']) && '1' === $_GET['recommend']) echo 'selected';?>>已推荐</option>
							<option value="0" <?php if(isset($_GET['recommend']) && '0' === $_GET['recommend']) echo 'selected';?>>未推荐</option>
						</select></td>
	...
	<form id="list_form" method="post">
		...
		<table class="table tb-type2">
			...
			<tbody id="treet1">
				...
				<tr class="hover">
					<td>&nbsp;</td>
					<td class="align-left"><span><?php echo $val['voucher_t_storename'];?></span>
            	...
            	if($val['voucher_t_state'] == $v[0]){
					...
					<td class="align-center yes-onoff"><?php if($val['voucher_t_recommend'] == '0'){ ?>
						<a href="JavaScript:void(0);" class="disabled" ajax_branch='voucher_t_recommend' nc_type="inline_edit" fieldname="voucher_t_recommend" fieldid="<?php echo $val['voucher_t_id']?>" fieldvalue="0" title="<?php echo $lang['nc_editable'];?>"><img src="<?php echo APP_TEMPLATES_URL;?>/images/transparent.gif"></a>
						<?php }else{ ?>
						<a href="JavaScript:void(0);" class="enabled" ajax_branch='voucher_t_recommend' nc_type="inline_edit" fieldname="voucher_t_recommend" fieldid="<?php echo $val['voucher_t_id']?>" fieldvalue="1"  title="<?php echo $lang['nc_editable'];?>"><img src="<?php echo APP_TEMPLATES_URL;?>/images/transparent.gif"></a>
						<?php } ?></td>
