      ...
      <tbody>
        ...
        <tr class="hover edit">
          ...
          <td><?php $personal_image_array = getMicroshopPersonalImageUrl($value,'tiny');?>
            ...
            <a href="<?php echo APP_URL;?>/index.php?act=personal&op=detail&personal_id=<?php echo $value['personal_id'];?>" style="background:url(<?php echo $personal_image_array[$i];?>) no-repeat center center; width:60px; height:60px; float:left; margin-right:5px;" target="_blank"> 
                <img class="show_image" src="<?php echo APP_TEMPLATES_URL;?>/images/transparent.gif" style=" width:60px; height:60px; display:block;"/>
            <div class="type-file-preview"> <img  src="<?php echo $personal_image_array_240[$i];?>" /> </div>
            </a>
            <?php } ?></td>
          <td>
            <a href="<?php echo APP_URL.DS.'index.php?act=personal&op=detail&personal_id='.$value['personal_id'];?>" target="_blank"> <?php echo $value['personal_id'];?> </a>
            </td>
          <td>
            <a href="<?php echo APP_URL.DS.'index.php?act=home&member_id='.$value['commend_member_id'];?>" target="_blank"> <?php echo $value['member_name'];?> </a>
            </td>
          <td><?php echo $value['commend_message'];?></td>
          <td class="align-center"><?php echo date('Y-m-d',$value['commend_time']);?></td>
          <td class="yes-onoff align-center"><a href="JavaScript:void(0);" class=" <?php echo $value['microshop_commend']? 'enabled':'disabled'?>" ajax_branch='personal_commend'  nc_type="inline_edit" fieldname="microshop_commend" fieldid="<?php echo $value['personal_id']?>" fieldvalue="<?php echo $value['microshop_commend']?'1':'0'?>" title="<?php echo $lang['editable'];?>"><img src="<?php echo APP_TEMPLATES_URL;?>/images/transparent.gif"></a></td>
