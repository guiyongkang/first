	...
	<form method="get" action="index.php" name="formSearch" id="formSearch">
		...
		<div class="w100pre" style="width: 100%;">
			<table class="tb-type1 noborder search left">
				<tbody>
					<tr>
						<td><select name="search_sclass" id="search_sclass" class="querySelect">
								<option value="" selected >店铺分类</option>
								<?php foreach ($output['store_class'] as $k=>$v){ ?>
								<option value="<?php echo $v['sc_id'];?>" <?php echo isset($_REQUEST['search_sclass']) && $_REQUEST['search_sclass'] == $v['sc_id']?'selected':''; ?>><?php echo $v['sc_name'];?></option>
								<?php } ?>
							</select></td>
	...
	<table class="table tb-type2 nobdb">
		...
		<tbody id="datatable">
			...
			<tr class="hover">
				...
				<td class="align-center"><a href="index.php?act=stat_store&op=showstore&type=newbyday&t=<?php echo $v['seartime'];?>&scid=<?php echo isset($_REQUEST['search_sclass']) ? $_REQUEST['search_sclass'] : '';?>">查看</a></td>
			</tr>
			<?php } ?>
			<?php } else { ?>
			<tr class="no_data">
		...
		<tfoot>
			<tr class="tfoot">
				<td colspan="15" id="dataFuncs"><div class="pagination"> <?php echo isset($output['show_page']) ? $output['show_page'] : '';?> </div></td>
			</tr>
		</tfoot>
		<?php } ?>
	</table>
