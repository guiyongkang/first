		...
		<table class="tb-type1 noborder search">
			<tbody>
				<tr>
					...
					<td><input type="text" value="<?php echo isset($output['area_name']) ? $output['area_name'] : '';?>" name="area_name" id="area_name" class="txt"></td>
					<th><label for="first_letter">首字母:</label></th>
					<td><select name='first_letter'>
							<option value=""><?php echo $lang['nc_please_choose'];?>...</option>
							<?php foreach($output['letter'] as $l){?>
							<option value='<?php echo $l;?>' <?php if(isset($output['first_letter']) && $l==$output['first_letter']){ echo 'selected';}?>><?php echo $l;?></option>
							<?php }?>
						</select></td>
		...
		<table class="table tb-type2">
			...
			<tbody>
				...
				<tr class="hover edit">
					...
					<td><?php if($val['hot_city'] == '1'){?>
						<?php echo $lang['nc_yes'];?>
						<?php }else{?>
						<?php echo $lang['nc_no'];?>
						<?php }?></td>
					<td><?php echo date("Y-m-d",$val['add_time']);?></td>
					<td class='align-center'><a href="index.php?act=vr_groupbuy&op=area_view&parent_area_id=<?php echo $val['area_id'];?>">查看区域</a> | <a href="index.php?act=vr_groupbuy&op=area_edit&area_id=<?php echo $val['area_id'];?>"><?php echo $lang['nc_edit'];?></a> | <a href="javascript:submit_delete(<?php echo $val['area_id'];?>)"><?php echo $lang['nc_del'];?></a></td>
				</tr>
				<?php } ?>
				<?php }else { ?>
				<tr class="no_data">
