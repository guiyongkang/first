...
<div class="page">
	<div class="fixed-bar">
		<div class="item-title">
			...
			<ul class="tab-base">
				<li><a href="index.php?act=returned&op=return_manage"><span><?php echo '待处理';?></span></a></li>
			...
			<tbody>
				<tr>
					<th><select name="type">
							...
							<option value="refund_sn" <?php if(isset($_GET['type']) && $_GET['type'] == 'refund_sn'){?>selected<?php }?>><?php echo $lang['return_order_returnsn']; ?></option>
							<option value="store_name" <?php if(isset($_GET['type']) && $_GET['type'] == 'store_name'){?>selected<?php }?>><?php echo $lang['refund_store_name']; ?></option>
							<option value="goods_name" <?php if(isset($_GET['type']) && $_GET['type'] == 'goods_name'){?>selected<?php }?>><?php echo '商品名称'; ?></option>
							<option value="buyer_name" <?php if(isset($_GET['type']) && $_GET['type'] == 'buyer_name'){?>selected<?php }?>><?php echo $lang['refund_order_buyer']; ?></option>
						</select></th>
					<td><input type="text" class="text" name="key" value="<?php echo isset($_GET['key']) ? trim($_GET['key']) : ''; ?>" /></td>
					<th><label for="add_time_from"><?php echo '申请时间';?></label></th>
					<td><input class="txt date" type="text" value="<?php echo isset($_GET['add_time_from']) ? $_GET['add_time_from'] : '';?>" id="add_time_from" name="add_time_from">
						<label for="add_time_to">~</label>
						<input class="txt date" type="text" value="<?php echo isset($_GET['add_time_to']) ? $_GET['add_time_to'] : '';?>" id="add_time_to" name="add_time_to"/></td>
	...
	<table class="table tb-type2 nobdb">
		...
		<tbody>
			...
			<tr class="bd-line" >
				...
				<td class="align-center"><a href="index.php?act=returned&op=view&return_id=<?php echo $val['refund_id']; ?>"> <?php echo $lang['nc_view'];?> </a></td>
			</tr>
			<?php } ?>
		</tbody>
		<?php } else { ?>
