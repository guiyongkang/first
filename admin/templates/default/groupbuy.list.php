		...
		<table class="tb-type1 noborder search">
			<tbody>
				<tr>
					...
					<td><input type="text" value="<?php echo isset($_GET['groupbuy_name']) ? $_GET['groupbuy_name'] : '';?>" name="groupbuy_name" id="groupbuy_name" class="txt" style="width:100px;"></td>
					<th><label for="store_name"><?php echo $lang['store_name'];?></label></th>
					<td><input type="text" value="<?php echo isset($_GET['store_name']) ? $_GET['store_name'] : '';?>" name="store_name" id="store_name" class="txt" style="width:100px;"></td>
					<th><label for="groupbuy_state">状态</label></th>
					<td><select name="groupbuy_state" class="w90">
							<?php if(is_array($output['groupbuy_state_array'])) { ?>
							<?php foreach($output['groupbuy_state_array'] as $key=>$val) { ?>
							<option value="<?php echo $key;?>" <?php if($key == (isset($_GET['groupbuy_state']) ? $_GET['groupbuy_state'] : 0)) { echo 'selected';}?>><?php echo $val;?></option>
							<?php } ?>
							<?php } ?>
						</select></td>
	...
	<form id="list_form" method="post">
		...
		<table class="table tb-type2">
			...
			<tbody id="treet1">
				...
				<tr class="hover">
					<td class="w60 picture"><div class="size-56x56"><span class="thumb size-56x56"><i></i><img src="<?php echo gthumb($val['groupbuy_image'], 'small');?>" style=" max-width: 56px; max-height: 56px;"/></span></div></td>
					<td class="group"><p><?php echo $val['groupbuy_name'];?></p>
						<p class="goods"><?php echo $lang['groupbuy_index_goods_name'];?>:<?php echo $val['goods_name'];?></p>
						<p class="store"><?php echo $lang['groupbuy_index_store_name'];?>:<?php echo $val['store_name'];?>
					...
					<td class="yes-onoff align-center"><?php if($val['recommended'] == '0'){ ?>
						<a href="JavaScript:void(0);" class=" disabled" ajax_branch='recommended' nc_type="inline_edit" fieldname="recommended" fieldid="<?php echo $val['groupbuy_id']?>" fieldvalue="0" title="<?php echo $lang['nc_editable'];?>"><img src="<?php echo APP_TEMPLATES_URL;?>/images/transparent.gif"></a>
						<?php }else { ?>
						<a href="JavaScript:void(0);" class=" enabled" ajax_branch='recommended' nc_type="inline_edit" fieldname="recommended" fieldid="<?php echo $val['groupbuy_id']?>" fieldvalue="1" title="<?php echo $lang['nc_editable'];?>"><img src="<?php echo APP_TEMPLATES_URL;?>/images/transparent.gif"></a>
						<?php } ?></td>
					<td class="align-center"><?php echo $val['groupbuy_state_text'];?></td>
					<td class="align-center"><?php if($val['reviewable']) { ?>
						<a nctype="btn_review_pass" data-groupbuy-id="<?php echo $val['groupbuy_id'];?>" href="javascript:;">通过</a> <a nctype="btn_review_fail" data-groupbuy-id="<?php echo $val['groupbuy_id'];?>" href="javascript:;">拒绝</a>
						<?php } ?>
						<?php if($val['cancelable']) { ?>
						<a nctype="btn_cancel" data-groupbuy-id="<?php echo $val['groupbuy_id'];?>" href="javascript:;">取消</a>
						<?php } ?>
						<a nctype="btn_del" data-groupbuy-id="<?php echo $val['groupbuy_id'];?>" href="javascript:;">删除</a></td>
				</tr>
				<?php } ?>
				<?php }else { ?>
				<tr class="no_data">
