			...
			<tbody>
				<tr>
					...
					<td><select name="grade_id">
							...
							<option value="<?php echo $v['sg_id'];?>" <?php if(isset($output['grade_id']) && $output['grade_id'] == $v['sg_id']){?>selected<?php }?>><?php echo $v['sg_name'];?></option>
							<?php } ?>
							<?php } ?>
						</select></td>
					<th><label for="owner_and_name"><?php echo $lang['store_user'];?></label></th>
					<td><input type="text" value="<?php echo isset($output['owner_and_name']) ? $output['owner_and_name'] : '';?>" name="owner_and_name" id="owner_and_name" class="txt"></td>
					<td></td>
					...
					<td><select name="store_type">
							...
							<option value="<?php echo $k;?>" <?php if(isset($_GET['store_type']) && $_GET['store_type'] == $k){?>selected<?php }?>><?php echo $v;?></option>
							<?php } ?>
							<?php } ?>
						</select></td>
					<th><label for="store_name"><?php echo $lang['store_name'];?></label></th>
					<td><input type="text" value="<?php echo isset($output['store_name']) ? $output['store_name'] : '';?>" name="store_name" id="store_name" class="txt"></td>
					<td><a href="javascript:void(0);" id="ncsubmit" class="btn-search " title="<?php echo $lang['nc_query'];?>">&nbsp;</a>
						<?php if(!empty($output['owner_and_name']) or !empty($output['store_name']) or !empty($output['grade_id'])){?>
		...
		<table class="table tb-type2">
			...
			<tbody>
				...
				<tr class="hover edit <?php echo getStoreStateClassName($v);?>">
					...
					<td class="align-center"><?php echo isset($output['search_grade_list'][$v['grade_id']]) ? $output['search_grade_list'][$v['grade_id']] : '暂无等级';?></td>
					...
					<td class="align-center w200"><a href="index.php?act=store&op=store_joinin_detail&member_id=<?php echo $v['member_id'];?>">查看</a>&nbsp;&nbsp;<a href="index.php?act=store&op=store_edit&store_id=<?php echo $v['store_id']?>"><?php echo $lang['nc_edit'];?></a>&nbsp;&nbsp; <a href="index.php?act=store&op=store_bind_class&store_id=<?php echo $v['store_id']?>">经营类目</a>
						<?php if (getStoreStateClassName($v) != 'open' && cookie('remindRenewal'.$v['store_id']) == null) {?>
						<a href="<?php echo urlAdmin('store', 'remind_renewal', array('store_id'=>$v['store_id']));?>">提醒续费</a>
						<?php }?>
						&nbsp;&nbsp; <a href="index.php?act=store&op=del&id=<?php echo $v['store_id']?>&member_id=<?php echo $v['member_id']?>" onclick="return confirm('您确认要删除此店铺吗？');">删除</a></td>
				</tr>
				<?php } ?>
				<?php }else { ?>
				<tr class="no_data">
