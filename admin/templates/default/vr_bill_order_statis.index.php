...
<div class="page">
	<div class="fixed-bar">
		<div class="item-title">
			...
			<ul class="tab-base">
				...
				<li><a href="index.php?act=vr_bill&op=show_statis"><span>商家账单列表</span></a></li>
			</ul>
		</div>
	</div>
	...
	<form method="get" action="index.php" target="" name="formSearch" id="formSearch">
		...
		<table class="tb-type1 noborder search">
			<tbody>
				<tr>
					<th>按年份搜索</th>
					<td><select name="query_year" class="querySelect">
							<option value=""><?php echo $lang['nc_please_choose'];?></option>
							<?php for($i = date('Y',TIMESTAMP)-5; $i <= date('Y',TIMESTAMP)+2; $i++) { ?>
							<option value="<?php echo $i;?>" <?php if (isset($_GET['query_year']) && $_GET['query_year'] == $i) {?>selected<?php } ?>><?php echo $i;?></option>
							<?php } ?>
						</select></td>
	...
	<table class="table tb-type2 nobdb">
		...
		<tbody>
			...
			<tr class="hover">
				<td><?php echo substr($bill['os_month'],0,4).'-'.substr($bill['os_month'],4);?></td>
				...
				<td class="align-center"><a href="index.php?act=vr_bill&op=show_statis&os_month=<?php echo $bill['os_month'];?>"><?php echo $lang['nc_view'];?></a></td>
			</tr>
			<?php }?>
			<?php }else{?>
			<tr class="no_data">
