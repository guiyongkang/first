		...
		<table class="tb-type1 noborder search">
			<tbody>
				<tr>
					...
					<td><input class="txt" type="text" name="consult_content" id="consult_content" value="<?php echo isset($output['consult_content']) ? $output['consult_content'] : '';?>" /></td>
					<td><label for="consult_type">咨询类型</label></td>
					<td><select name="ctid">
							<option value="0">全部</option>
							<?php if (!empty($output['consult_type'])) {?>
							<?php foreach ($output['consult_type'] as $val) {?>
							<option <?php if (isset($output['ctid']) && ($output['ctid'] == $val['ct_id'])) {?>selected="selected"<?php }?> value="<?php echo $val['ct_id'];?>"><?php echo $val['ct_name'];?></option>
							<?php }?>
							<?php }?>
						</select></td>
		...
		<table class="table tb-type2">
			<tbody>
				...
				<tr class="space">
					...
					<th> <strong><?php echo $lang['consulting_index_object'];?>:&nbsp;</strong> <span><a target="_blank" href="<?php echo APP_URL."/index.php?act=goods&goods_id=".$consult['goods_id'].'&id='.$consult['store_id'];?>"><?php echo $consult['goods_name'];?></a></span> </th>
					<th><strong><?php echo $lang['consulting_index_store_name'];?>:</strong>&nbsp;<a href="javascript:;" class="normal" ><?php echo $consult['store_name'];?></a></th>
