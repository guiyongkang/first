		...
		<form id="upload_screen_form" class="tab-content" name="upload_screen_form" enctype="multipart/form-data" method="post" action="index.php?act=web_api&op=screen_pic" target="upload_pic">
			...
			<div class="full-screen-slides">
				<ul>
					...
					<li ap="1" screen_id="<?php echo $val['pic_id'];?>" title="可上下拖拽更改显示顺序"> 广告调用 <a class="del" href="JavaScript:del_screen(<?php echo $val['pic_id'];?>);" title="<?php echo $lang['nc_del'];?>">X</a>
					...
					<?php }else { ?>
					<li ap="0" screen_id="<?php echo $val['pic_id'];?>" title="可上下拖拽更改显示顺序"> 图片调用 <a class="del" href="JavaScript:del_screen(<?php echo $val['pic_id'];?>);" title="<?php echo $lang['nc_del'];?>">X</a>
						<div class="focus-thumb" onclick="select_screen(<?php echo $val['pic_id'];?>);" style="background-color:<?php echo $val['color'];?>;" title="点击编辑选中区域内容"> <img title="<?php echo $val['pic_name'];?>" src="<?php echo UPLOAD_SITE_URL.'/'.$val['pic_img'];?>"/></div>
						<input name="screen_list[<?php echo $val['pic_id'];?>][pic_id]" value="<?php echo $val['pic_id'];?>" type="hidden">
						<input name="screen_list[<?php echo $val['pic_id'];?>][pic_name]" value="<?php echo $val['pic_name'];?>" type="hidden">
			...
			<div class="margintop"><a href="JavaScript:void(0);" onclick="$('#upload_screen_form').submit();" class="btn"><span><?php echo $lang['web_config_save'];?></span></a> <a href="index.php?act=web_api&op=html_update&web_id=<?php echo $output['code_screen_list']['web_id'];?>" class="btn"><span><?php echo $lang['web_config_web_html'];?></span></a> <span class="web-save-succ" style="display:none;"><?php echo $lang['nc_common_save_succ'];?></span> </div>
		</form>
		<form id="upload_focus_form" class="tab-content" name="upload_screen_form" enctype="multipart/form-data" method="post" action="index.php?act=web_api&op=focus_pic" target="upload_pic" style="display:none;">
			...
			<div class="focus-trigeminy">
				...
				<div focus_id="<?php echo $key;?>" class="focus-trigeminy-group" title="<?php echo '可上下拖拽更改图片组显示顺序';?>">
					...
					广告调用 <a class="del" href="JavaScript:del_focus(<?php echo $key;?>);" title="<?php echo $lang['nc_del'];?>">X</a>
					<ul>
					...
					<?php }else { ?>
					图片调用 <a class="del" href="JavaScript:del_focus(<?php echo $key;?>);" title="<?php echo $lang['nc_del'];?>">X</a>
					<ul>
						<?php foreach($val['pic_list'] as $k => $v) { ?>
			...
			<table id="ap_focus" class="table tb-type2" style="display:none;">
				<tbody>
					...
					<tr class="noborder">
						<td class="vatop rowform"><select id="ap_id_focus" name="ap_id_focus" class=" w200" onchange="select_ap_focus();">
							</select></td>
			...
			<a href="JavaScript:void(0);" onclick="$('#upload_focus_form').submit();" class="btn"><span><?php echo $lang['web_config_save'];?></span></a> <a href="index.php?act=web_api&op=html_update&web_id=<?php echo $output['code_screen_list']['web_id'];?>" class="btn"><span><?php echo $lang['web_config_web_html'];?></span></a> <span class="web-save-succ" style="display:none;"><?php echo $lang['nc_common_save_succ'];?></span>
		</form>
	</div>
</div>
