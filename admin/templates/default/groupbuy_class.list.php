		...
		<table class="table tb-type2">
			<thead>
				<tr class="thead">
					...
					<th><?php echo isset($lang['goods_class_index_name']) ? $lang['goods_class_index_name'] : '';?></th>
			...
			<tbody>
				...
				<tr class="hover edit <?php echo $val['class_parent_id']=='0'?'':'two';?> <?php echo 'parent'.$val['class_parent_id'];?>">
					<td class="w36"><input type="checkbox"  value="<?php echo $val['class_id'];?>" class="checkitem">
						...
						<img class="node_parent" state="close" node_id="<?php echo 'parent'.$val['class_id'];?>" src="<?php echo APP_TEMPLATES_URL;?>/images/tv-expandable.gif">
						<?php } ?></td>
					...
					<td class="name"><?php if($val['class_parent_id'] != '0'){ ?>
						<img fieldid="<?php echo $val['class_id'];?>" status="close" nc_type="flex" src="<?php echo APP_TEMPLATES_URL;?>/images/tv-item1.gif">
						<?php } ?>
            ...
            --> 
				...
				<?php } ?>
				<tr>
					<td colspan="20"><a class="btn-add marginleft" href="<?php echo urlAdmin('groupbuy', 'class_add');?>">添加分类</a></td>
				</tr>
			</tbody>
			<?php if(!empty($output['list']) && is_array($output['list'])){ ?>
			<tfoot>
