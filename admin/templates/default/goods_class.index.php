			...
			<tbody>
				...
				<tr class="hover edit">
					<td class="w48"><input type="checkbox" name="check_gc_id[]" value="<?php echo $v['gc_id'];?>" class="checkitem">
						...
						<img fieldid="<?php echo $v['gc_id'];?>" status="open" nc_type="flex" src="<?php echo APP_TEMPLATES_URL;?>/images/tv-expandable.gif">
						<?php }else{ ?>
						<img fieldid="<?php echo $v['gc_id'];?>" status="close" nc_type="flex" src="<?php echo APP_TEMPLATES_URL;?>/images/tv-item.gif">
						<?php } ?></td>
					...
					<td class="w50pre name"><span title="<?php echo $lang['nc_editable'];?>" required="1" fieldid="<?php echo $v['gc_id'];?>" ajax_branch="goods_class_name" fieldname="gc_name" nc_type="inline_edit" class="editable "><?php echo $v['gc_name'];?></span> <a class="btn-add-nofloat marginleft" href="index.php?act=goods_class&op=goods_class_add&gc_parent_id=<?php echo $v['gc_id'];?>"><span><?php echo $lang['nc_add_sub_class'];?></span></a></td>
					<!--<td class="power-onoff"><?php if($v['gc_show'] == 0){ ?>
            <a href="JavaScript:void(0);" class="tooltip disabled" fieldvalue="0" fieldid="<?php echo $v['gc_id'];?>" ajax_branch="goods_class_show" fieldname="gc_show" nc_type="inline_edit" title="<?php echo $lang['nc_editable'];?>"><img src="<?php echo APP_TEMPLATES_URL;?>/images/transparent.gif"></a>
            <?php }else{ ?>
            <a href="JavaScript:void(0);" class="tooltip enabled" fieldvalue="1" fieldid="<?php echo $v['gc_id'];?>" ajax_branch="goods_class_show" fieldname="gc_show" nc_type="inline_edit" title="<?php echo $lang['nc_editable'];?>"><img src="<?php echo APP_TEMPLATES_URL;?>/images/transparent.gif"></a>
            <?php } ?></td>-->
					<td><?php echo $v['type_name'];?></td>
					<td><?php if ($v['gc_virtual'] == 1) {?>
						虚拟
						<?php }?></td>
			...
			<tfoot>
				<tr class="tfoot">
					...
					<td id="batchAction" colspan="15">
						...
						&nbsp;&nbsp;<a href="JavaScript:void(0);" class="btn" onclick="if(confirm('<?php echo $lang['goods_class_index_ensure_del'];?>')){$('#submit_type').val('del');$('form:first').submit();}"><span><?php echo $lang['nc_del'];?></span></a>
					</td>
				</tr>
			</tfoot>
			<?php } ?>
