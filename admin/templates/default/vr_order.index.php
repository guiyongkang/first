			...
			<tbody>
				<tr>
					...
					<td colspan="4"><select name="order_state" class="querySelect">
							...
							<option value="20" <?php if(isset($_GET['order_state']) && $_GET['order_state'] == '20'){?>selected<?php }?>>已付款</option>
		...
		<tbody>
			...
			<tr class="hover">
				...
				<td class="w144 align-center"><a href="index.php?act=vr_order&op=show_order&order_id=<?php echo $order['order_id'];?>"><?php echo $lang['nc_view'];?></a> 
					...
					| <a href="javascript:void(0)" onclick="if(confirm('<?php echo $lang['order_confirm_cancel'];?>')){location.href='index.php?act=vr_order&op=change_state&state_type=cancel&order_id=<?php echo $order['order_id']; ?>'}"> <?php echo $lang['order_change_cancel'];?></a>
					<?php }?>
					
					<!-- 收款 -->
					
					<?php if($order['if_system_receive_pay']) {?>
					| <a href="index.php?act=vr_order&op=change_state&state_type=receive_pay&order_id=<?php echo $order['order_id']; ?>"> <?php echo $lang['order_change_received'];?></a>
					<?php }?></td>
			</tr>
			<?php }?>
			<?php }else{?>
