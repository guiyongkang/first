			...
			<tbody>
				<tr>
					...
					<td><input type="text" id="stime" name="stime" class="txt date" value="<?php echo isset($_GET['stime']) ? $_GET['stime'] : '';?>">
						<label>~</label>
						<input type="text" id="etime" name="etime" class="txt date" value="<?php echo isset($_GET['stime']) ? $_GET['etime'] : '';?>"></td>
					<td><select name="stage">
							<option value="" <?php if (empty($_GET['stage'])){echo 'selected=selected';}?>>操作阶段</option>
							<?php foreach ($output['stage_arr'] as $k=>$v){ ?>
							<option value="<?php echo $k;?>" <?php if (isset($_GET['stage']) && $_GET['stage'] == $k){echo 'selected=selected';}?>><?php echo $v; ?></option>
							<?php } ?>
						</select></td>
					<th>描述</th>
