		...
		<table class="tb-type1 noborder search">
			<tbody>
				<tr>
					...
					<td><input class="txt" type="text" name="member_name" id="member_name" value="<?php echo isset($output['member_name']) ? $output['member_name'] : '';?>" /></td>
					<td><label for="consult_type">咨询类型</label></td>
					<td><select name="mctid">
							<option value="0">全部</option>
							<?php if (!empty($output['type_list'])) {?>
							<?php foreach ($output['type_list'] as $val) {?>
							<option <?php if (isset($output['mctid']) && $output['mctid'] == $val['mct_id']) {?>selected="selected"<?php }?> value="<?php echo $val['mct_id'];?>"><?php echo $val['mct_name'];?></option>
							<?php }?>
							<?php }?>
						</select></td>
					<td><a href="javascript:document.formSearch.submit();" class="btn-search " title="<?php echo $lang['nc_query'];?>">&nbsp;</a>
						<?php if(isset($output['form_submit']) && $output['form_submit'] == 'ok'){?>
		...
		<table class="table tb-type2">
			...
			<tbody>
				...
				<tr class="space">
					...
					<td><a href="<?php echo urlAdmin('mall_consult', 'consult_reply', array('id' => $val['mc_id']));?>">
						<?php if ($val['is_reply'] == 0) {?>
						回复
						<?php } else {?>
						编辑
						<?php }?>
						</a> | <a href="javascript:if(confirm('<?php echo $lang['nc_ensure_del'];?>')){location.href='<?php echo urlAdmin('mall_consult', 'del_consult', array('id' => $val['mc_id']));?>';}" class="normal" ><?php echo $lang['nc_del'];?></a></td>
				</tr>
				<?php }?>
				<?php }else{?>
				<tr class="no_data">
...
function checkForm(){
	...
	if(!flag)alert('<?php echo isset($lang['consulting_del_choose']) ? $lang['consulting_del_choose'] : '';?>');
	return flag;
}
</script> 
<script>
(function(){
  ...
  n.innerHTML = s.length > 50 ? '<?php echo isset($lang['consulting_index_unfold']) ? $lang['consulting_index_unfold'] : '';?>' : '';
  n.href = "###";
  n.onclick = function(){
    if (n.innerHTML == '<?php echo isset($lang['consulting_index_unfold']) ? $lang['consulting_index_unfold'] : '';?>'){
      n.innerHTML = '<?php echo isset($lang['consulting_index_retract']) ? $lang['consulting_index_retract'] : '';?>';
      p.innerHTML = s;
    }else{
      n.innerHTML = "<?php echo isset($lang['consulting_index_unfold']) ? $lang['consulting_index_unfold'] : '';?>";
      p.innerHTML = s.substring(0,50);
    }
  }
  o.innerHTML = "";
