...
<div class="page">
	...
	<form method="get" action="index.php" target="" name="formSearch" id="formSearch">
		...
		<input type="hidden" name="os_month" value="<?php echo isset($_GET['os_month']) ? $_GET['os_month'] : '';?>">
		<table class="tb-type1 noborder search">
			<tbody>
				<tr>
					<th>店铺ID/名称</th>
					<td><input class="txt-short" type="text" value="<?php echo isset($_GET['query_store']) ? $_GET['query_store'] : '';?>" name="query_store" id="query_store"/></td>
					<th>账单状态</th>
					<td><select name="bill_state">
							<option><?php echo lang('nc_please_choose');?></option>
							<option <?php if (isset($_GET['bill_state']) && $_GET['bill_state'] == BILL_STATE_CREATE) {?>selected<?php } ?> value="<?php echo BILL_STATE_CREATE;?>">已出账</option>
							<option <?php if (isset($_GET['bill_state']) && $_GET['bill_state'] == BILL_STATE_STORE_COFIRM) {?>selected<?php } ?> value="<?php echo BILL_STATE_STORE_COFIRM;?>">商家已确认</option>
							<option <?php if (isset($_GET['bill_state']) && $_GET['bill_state'] == BILL_STATE_SYSTEM_CHECK) {?>selected<?php } ?> value="<?php echo BILL_STATE_SYSTEM_CHECK?>">平台已审核</option>
							<option <?php if (isset($_GET['bill_state']) && $_GET['bill_state'] == BILL_STATE_SUCCESS) {?>selected<?php } ?> value="<?php echo BILL_STATE_SUCCESS?>">结算完成</option>
						</select></td>
	...
	<table class="table tb-type2 nobdb">
		...
		<tbody>
			...
			<tr class="hover">
				...
				<td class="align-center"><a href="index.php?act=vr_bill&op=show_bill&ob_no=<?php echo $bill_info['ob_no'];?>"><?php echo $lang['nc_view'];?></a></td>
			</tr>
			<?php }?>
			<?php }else{?>
			<tr class="no_data">
