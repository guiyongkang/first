...
<link href="<?php echo APP_TEMPLATES_URL?>/css/index.css" rel="stylesheet" type="text/css">
<link rel="stylesheet" href="<?php echo APP_TEMPLATES_URL?>/css/font/css/font-awesome.min.css">
<link type="text/css" rel="stylesheet" href="<?php echo APP_TEMPLATES_URL?>/css/jquery-ui.min.css">
<link href="<?php echo APP_TEMPLATES_URL?>/css/index.css" rel="stylesheet" type="text/css">
					...
					<dd class="opt">
						<div class="ncap-channel-category">
							...
							<dl select_class_id="<?php echo $val['gc_parent']['gc_id'];?>">
								<dt title="<?php echo $val['gc_parent']['gc_name'];?>"> <span select_id="<?php echo $val['gc_parent']['gc_id'];?>" class="pic">
									<?php if(!empty($val['gc_parent']['gc_pic'])){ ?>
									<img src="<?php echo UPLOAD_SITE_URL.'/'.$val['gc_parent']['gc_pic'];?>"/>
									<?php } ?>
									</span>
									<h4><?php echo $val['gc_parent']['gc_name'];?></h4>
									<a href="javascript:void(0);" class="ncap-btn-mini upload-pic" title="上传分类图片"><i id="form_<?php echo $val['gc_parent']['gc_id'];?>" class="fa fa-upload"></i></a> <a href="javascript:void(0);" class="ncap-btn-mini del" onclick="del_gc_parent(<?php echo $val['gc_parent']['gc_id'];?>);" title="删除该分类块"><i class="fa fa-trash"></i></a> </dt>
								<dd>
									...
									<ul>
										...
										<li gc_id="<?php echo $v['gc_id'];?>" title="<?php echo $v['gc_name'];?>" ondblclick="del_goods_class(<?php echo $v['gc_id'];?>);"><i onclick="del_goods_class(<?php echo $v['gc_id'];?>);"></i><?php echo $v['gc_name'];?>
											...
											<input name="channel_category[<?php echo $val['gc_parent']['gc_id'];?>][goods_class][<?php echo $v['gc_id'];?>][gc_name]" value="<?php echo $v['gc_name'];?>" type="hidden">
										</li>
										<?php } ?>
									</ul>
									<?php } ?>
			...
			<div id="upload_slide" class="ncap-form-default" style="display:none; overflow: visible;">
				...
				<dl class="row" style="z-index: 3;">
					...
					<dd class="opt">
						...
						<span class="err"></span>
						<p class="notic">背景色即焦点大图区域"DIV{ background-color}"值，用于页面较宽但Banner图片不能填满整个区域时背景色使用，设置请使用十六进制形式(#XXXXXX)，默认留空为白色背景。 </p>
					</dd>
				</dl>
				<dl class="row">
