		...
		<table class="tb-type1 noborder search">
			<tbody>
				<tr>
					...
					<td><input type="text" name="pg_name" id="pg_name" class="txt" value='<?php echo isset($_GET['pg_name']) ? $_GET['pg_name'] : '';?>'></td>
					<td><select name="pg_state">
							<option value="" <?php if (empty($_GET['pg_state'])){echo 'selected=selected';}?>><?php echo $lang['admin_pointprod_state']; ?></option>
							<option value="show" <?php if (isset($_GET['pg_state']) && $_GET['pg_state'] == 'show'){echo 'selected=selected';}?>><?php echo $lang['admin_pointprod_show_up']; ?></option>
							<option value="nshow" <?php if (isset($_GET['pg_state']) && $_GET['pg_state'] == 'nshow'){echo 'selected=selected';}?>><?php echo $lang['admin_pointprod_show_down']; ?></option>
							<option value="commend" <?php if (isset($_GET['pg_state']) && $_GET['pg_state'] == 'commend'){echo 'selected=selected';}?>><?php echo $lang['admin_pointprod_commend']; ?></option>
						</select></td>
					<td><a href="javascript:document.formSearch.submit();" class="btn-search " title="<?php echo $lang['nc_query'];?>">&nbsp;</a>
						<?php if(!empty($output['search_field_value']) or !empty($output['search_sort'])){?>
			...
			<tbody>
				...
				<tr class="hover">
					...
					<td class="align-center power-onoff"><?php if($v['pgoods_show'] == '0'){ ?>
						<a href="JavaScript:void(0);" class=" disabled" ajax_branch='pgoods_show' nc_type="inline_edit" fieldname="pgoods_show" fieldid="<?php echo $v['pgoods_id']?>" fieldvalue="0" title="<?php echo $lang['nc_editable'];?>"><img src="<?php echo APP_TEMPLATES_URL;?>/images/transparent.gif"></a>
						<?php }else { ?>
						<a href="JavaScript:void(0);" class=" enabled" ajax_branch='pgoods_show' nc_type="inline_edit" fieldname="pgoods_show" fieldid="<?php echo $v['pgoods_id']?>" fieldvalue="1" title="<?php echo $lang['nc_editable'];?>"><img src="<?php echo APP_TEMPLATES_URL;?>/images/transparent.gif"></a>
						<?php } ?></td>
					<td class="align-center yes-onoff"><?php if($v['pgoods_commend'] == '0'){ ?>
						<a href="JavaScript:void(0);" class=" disabled" ajax_branch='pgoods_commend' nc_type="inline_edit" fieldname="pgoods_commend" fieldid="<?php echo $v['pgoods_id']?>" fieldvalue="0" title="<?php echo $lang['nc_editable'];?>"><img src="<?php echo APP_TEMPLATES_URL;?>/images/transparent.gif"></a>
						<?php }else { ?>
						<a href="JavaScript:void(0);" class=" enabled" ajax_branch='pgoods_commend' nc_type="inline_edit" fieldname="pgoods_commend" fieldid="<?php echo $v['pgoods_id']?>" fieldvalue="1" title="<?php echo $lang['nc_editable'];?>"><img src="<?php echo APP_TEMPLATES_URL;?>/images/transparent.gif"></a>
						<?php } ?></td>
