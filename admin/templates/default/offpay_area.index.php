			...
			<tbody>
				...
				<tr>
					...
					<td><?php if (is_array($pinfo['child'])) {?>
						...
						<div class="area-list">
							<label>
								<input <?php if (!empty($output['city_checked_array'][$city_id])) echo 'checked';?> type="checkbox" nc_province="<?php echo $pid;?>" value="<?php echo $city_id;?>" name="city[]">
								<?php echo $city_name;?> </label>
							(<span city_id="<?php echo $city_id;?>" title="已选下级地区"><?php echo !empty($output['city_checked_child_array'][$city_id]) ? count($output['city_checked_child_array'][$city_id]) : 0;?></span>)<a city_id="<?php echo $city_id;?>" nc_title="<?php echo $city_name;?>" province_id="<?php echo $pid;?>" nc_type="edit" href="javascript:void(0);" title="选择下级地区"><i class="icon-pencil"></i></a> </div>
						<?php }?></td>
				</tr>
				<?php }?>
				<?php }?>
