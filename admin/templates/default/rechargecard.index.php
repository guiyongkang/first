		...
		<table class="tb-type1 noborder search">
			<tbody>
				<tr>
					...
					<td><input class="txt" type="text" name="sn" id="search-sn" value="<?php echo isset($output['sn']) ? $output['sn'] : '';?>" /></td>
					<th><label for="search-batchflag">批次标识</label></th>
					<td><input class="txt" type="text" name="batchflag" id="search-batchflag" value="<?php echo isset($output['batchflag']) ? $output['batchflag'] : '';?>" /></td>
					<th><label for="search-state">领取状态</label></th>
					<td><select name="state" id="search-state">
							<option value="">全部</option>
							<option value="0">未被领取</option>
							<option value="1">已被领取</option>
						</select>
						<script>$('#search-state').val('<?php echo isset($output['state']) ? $output['state'] : 0; ?>');</script></td>
	...
	<form method="post" action="<?php echo urlAdmin('rechargecard', 'del_card_batch');?>" onsubmit="" name="form1">
		<table class="table tb-type2">
			...
			<tbody>
				...
				<tr class="space">
					<td class="w24"><?php if ($val['state'] == 0): ?>
					...
					<td class="align-center"><?php if ($val['state'] == 1 && $val['member_id'] > 0 && $val['tsused'] > 0): ?>
						会员 <?php echo $val['member_name']; ?> 在 <?php echo date('Y-m-d H:i:s', $val['tsused']); ?> 领取
						<?php else: ?>
						未被领取
						<?php endif; ?></td>
					<td class="align-center"><?php if ($val['state'] == 0): ?>
