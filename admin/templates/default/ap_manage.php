		...
		<table class="tb-type1 noborder search">
			<tbody>
				<tr>
					...
					<td><input class="txt" type="text" name="search_name" id="search_name" value="<?php echo isset($_GET['search_name']) ? $_GET['search_name'] : '';?>" /></td>
					<td><a href="javascript:document.formSearch.submit();" class="btn-search " title="<?php echo $lang['nc_query'];?>">&nbsp;</a>
						<?php if(!empty($output['search_title']) || !empty($output['search_ac_id'])){?>
			...
			<tbody>
				...
				<tr class="hover">
					...
					<td class="align-center yes-onoff"><?php if($v['is_use'] == '0'){ ?>
						<a href="JavaScript:void(0);" class=" disabled" ajax_branch="is_use" nc_type="inline_edit" fieldname="is_use" fieldid="<?php echo $v['ap_id']?>" fieldvalue="0" title="<?php echo $lang['nc_editable'];?>"><img src="<?php echo APP_TEMPLATES_URL;?>/images/transparent.gif"></a>
						<?php }else { ?>
						<a href="JavaScript:void(0);" class=" enabled" ajax_branch="is_use" nc_type="inline_edit" fieldname="is_use" fieldid="<?php echo $v['ap_id']?>" fieldvalue="1" title="<?php echo $lang['nc_editable'];?>"><img src="<?php echo APP_TEMPLATES_URL;?>/images/transparent.gif"></a>
						<?php } ?></td>
					<td class="align-center"><a href="index.php?act=adv&op=adv&ap_id=<?php echo $v['ap_id'];?>">管理广告</a> | <a href='index.php?act=adv&op=ap_edit&ap_id=<?php echo $v['ap_id'];?>'><?php echo $lang['nc_edit'];?></a> |<a href="javascript:void(0)" onclick="copyToClipBoard('<?php echo $v['ap_id'];?>');"><?php echo $lang['ap_get_js'];?></a></td>
				</tr>
				<?php } ?>
				<?php }else { ?>
				<tr class="no_data">
