...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base"><li><a href="index.php?act=distributor&op=addlevel"><span>新增</span></a></li></ul>
    </div>
  </div>
  <div class="fixed-empty"></div>
  <form id="add_form" method="post">
  <input type="hidden" name="form_submit" value="ok" />
  <input type="hidden" name="lid" value="<?php echo $output['level_info']['level_id'];?>" />
    ...
    <table class="table tb-type2">
      <tbody>
        ...
        <tr class="noborder">
          ...
          <input id="name" name="name" value="<?php echo $output['level_info']['level_name'];?>" class="txt" type="text">
          </td>
        ...
        <tr class="noborder">
			...
			<div class="type-file-preview"><?php echo $output['level_info']['level_thumb'] ? '<img src="'. UPLOAD_SITE_URL .$output['level_info']['level_thumb'].'" />' : '';?></div>
			</span><span class="type-file-box">
			<input type='text' name='thumb' id='thumb' value="<?php echo UPLOAD_SITE_URL.$output['level_info']['level_thumb'];?>" class='type-file-text' />
        ...
        <tr class="noborder">
          <td class="required" colspan="2">成为该级别分销商门槛</td>
        </tr>
        <tr class="noborder">
          <td class="vatop rowform" colspan="2">
          	<select name="cometype">
            	<option value="6"<?php echo $output['level_info']['level_come_type']==6 ? ' selected' : '';?>>无门槛</option>
                <option value="4"<?php echo $output['level_info']['level_come_type']==4 ? ' selected' : '';?>>购买任意商品</option>
                <?php if($output['setting']['dis_come_type']==1){?>
                <option value="1"<?php echo $output['level_info']['level_come_type']==1 ? ' selected' : '';?>>一次性消费</option>
                <?php }elseif($output['setting']['dis_come_type']==2){?>
                <option value="2"<?php echo $output['level_info']['level_come_type']==2 ? ' selected' : '';?>>总销售额</option>
                <?php }elseif($output['setting']['dis_come_type']==3){?>
                <option value="3"<?php echo $output['level_info']['level_come_type']==3 ? ' selected' : '';?>>购买指定商品</option>
                <?php }elseif($output['setting']['dis_come_type']==5){?>
                <option value="5"<?php echo $output['level_info']['level_come_type']==5 ? ' selected' : '';?>>直接购买</option>
                <?php }?>
            </select>
            <div style="padding:12px 0px;<?php echo in_array($output['level_info']['level_come_type'],array(6,4)) ? ' display:none' : '';?>" class="cometype">
          	<?php if($output['setting']['dis_come_type']==1){?>
          		一次性消费<input name="comevalue" value="<?php echo $output['level_info']['level_come_type']==1 ? $output['level_info']['level_come_value'] : ''; ?>" class="txt" type="text" style="width:60px; text-align:center; margin-left:10px">积分即可成为该级别的分销商
            <?php }elseif($output['setting']['dis_come_type']==2){?>
          		总消费额达到<input name="comevalue" value="<?php echo $output['level_info']['level_come_type']==2 ? $output['level_info']['level_come_value'] : ''; ?>" class="txt" type="text" style="width:60px; text-align:center; margin-left:10px">积分即可成为该级别的分销商
            <?php }elseif($output['setting']['dis_come_type']==3){?>
                <div class="products_option">
                   ...
                   <div class="select_items">
                     ...
                     <select size='10' class="select_product1" multiple style="width:300px; height:100px; display:block; float:left">
                        <?php if($output['level_info']['level_come_type']==3){?>
                     ...
                     <input type="hidden" name="comevalue" value="<?php echo $output['level_info']['level_come_type']==3 ? ','.$output['level_info']['level_come_value'].',' : ''; ?>" />
                   </div>
            ...
            <?php }elseif($output['setting']['dis_come_type']==5){?>
          		价格：<input name="comevalue" value="<?php echo $output['level_info']['level_come_type']==5 ? $output['level_info']['level_come_value'] : ''; ?>" class="txt" type="text" style="width:60px; text-align:center; margin-left:10px">
            <?php }?>
            </div>
          </td>
        </tr>
        ...
        <tr class="noborder">
          <td colspan="2">
          <div style="padding:12px 0px;">购买分销级别获得佣金明细</div>
          <table style="width:300px; margin-top:10px; border:1px #dfdfdf solid">
            	<tr>
            		<th colspan="2" style="font-size:14px; font-weight:bold; background:#f5f5f5; text-align:center">升级时上级获得佣金明细</th>
                </tr>
                <?php for($i=0; $i<$output['setting']['dis_bonus_level']; $i++){?>
                <tr>
                	<td style="width:60px; border-right:1px #dfdfdf solid; text-align:right; padding-right:20px;"><?php echo $lang['nc_distributor_title'][$ii];?>级</td>
                    <td><input type="text" name="comecommission[<?php echo $i+1;?>]" value="<?php echo empty($output['comecommission'][$i+1]) ? '' : $output['comecommission'][$i+1];?>" class="text" style="width:60px; text-align:center; margin-left:10px" /> %</td>
                </tr>
                <?php }?>
            </table>
          </td>
