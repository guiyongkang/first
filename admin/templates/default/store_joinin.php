			...
			<tbody>
				<tr>
					...
					<td><select name="grade_id">
							...
							<option value="<?php echo $v['sg_id'];?>" <?php if($v['sg_id']==(isset($_GET['grade_id']) ? $_GET['grade_id'] : '')) { echo 'selected'; }?>><?php echo $v['sg_name'];?></option>
							<?php } ?>
							<?php } ?>
						</select></td>
					<td><select name="joinin_state">
							<option value=""><?php echo $lang['nc_please_choose'];?>...</option>
							<?php if(!empty($output['joinin_state_array']) && is_array($output['joinin_state_array'])){ ?>
							<?php foreach($output['joinin_state_array'] as $k => $v){ ?>
							<option value="<?php echo $k;?>" <?php if($k==(isset($_GET['joinin_state']) ? $_GET['joinin_state'] : '')) { echo 'selected'; }?>><?php echo $v;?></option>
							<?php } ?>
							<?php } ?>
						</select></td>
		...
		<table class="table tb-type2">
			...
			<tbody>
				...
				<tr class="hover edit">
					...
					<td class="w72 align-center"><?php if(in_array(intval($v['joinin_state']), array(STORE_JOIN_STATE_NEW, STORE_JOIN_STATE_PAY))) { ?>
			...
			<tfoot>
				<tr class="tfoot">
					<td></td>
					<td colspan="15"><?php if(!empty($output['store_list']) && is_array($output['store_list'])){ ?>
