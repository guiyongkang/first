...
<div class="page">
	...
	<form method="get" action="index.php" name="formSearch" id="formSearch">
		...
		<div class="w100pre" style="width: 100%;">
			<table class="tb-type1 noborder search left">
				<tbody>
					<tr>
						<td><select name="stat_type" id="stat_type" class="querySelect">
								<option value="" <?php echo empty($_REQUEST['stat_type'])?'selected':''; ?>>订单量统计</option>
								<option value="sale" <?php echo isset($_REQUEST['stat_type']) && $_REQUEST['stat_type']=='sale'?'selected':''; ?>>销售额统计</option>
							</select></td>
						<td>
						<td><select name="order_type" id="order_type" class="querySelect">
								<option value="" <?php echo empty($_REQUEST['order_type'])?'selected':''; ?>>请选择</option>
								...
								<option value="<?php echo ORDER_STATE_CANCEL; ?>" <?php echo !empty($_REQUEST['order_type']) && $_REQUEST['order_type']==ORDER_STATE_CANCEL?'selected':''; ?>>已取消</option>
							</select></td>
						<td><select name="search_type" id="search_type" class="querySelect">
								<option value="day" <?php echo isset($_REQUEST['search_type']) && $_REQUEST['search_type']=='day'?'selected':''; ?>>按照天统计</option>
								<option value="week" <?php echo isset($_REQUEST['search_type']) && $_REQUEST['search_type']=='week'?'selected':''; ?>>按照周统计</option>
								<option value="month" <?php echo isset($_REQUEST['search_type']) && $_REQUEST['search_type']=='month'?'selected':''; ?>>按照月统计</option>
							</select></td>
	...
	<div class="stat-info">
		<?php if(trim($output['data_null']) != 'yes' && trim($_GET['store_name'])!=''){ ?>
		<span>店铺：<strong><?php echo trim($_GET['store_name']); ?></strong></span>
		<?php } ?>
		<span>总下单量：<strong><?php echo $output['sum_data'][0]; ?></strong></span><span>总销售额：<strong><?php echo $output['sum_data'][1]?$output['sum_data'][1]:'0.00'; ?></strong>元</span></div>
