...
<div class="page">
	...
	<table class="table tb-type2 noborder search">
		<tbody>
			<tr>
				<td>店铺 - <?php echo $output['bill_info']['ob_store_name'];?>（ID：<?php echo $output['bill_info']['ob_store_id'];?>） <?php echo $output['bill_info']['ob_create_month'];?>期 结算单&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					...
					<?php }?>
					<a class="btns" target="_blank" href="index.php?act=bill&op=export_step1&bill_id=<?php echo $_GET['bill_id'];?>"><span><?php echo $lang['nc_exposrt'];?>导出结算明细</span></a></td>
			</tr>
			<tr>
				<td><?php echo $lang['order_time_from'];?>结算单号：<?php echo $output['bill_info']['ob_no'];?></td>
			</tr>
			<tr>
				<td>起止日期：<?php echo date('Y-m-d',$output['bill_info']['ob_start_date']);?> &nbsp;至&nbsp; <?php echo date('Y-m-d',$output['bill_info']['ob_end_date']);?></td>
			</tr>
			<tr>
				<td>出账日期：<?php echo date('Y-m-d',$output['bill_info']['ob_create_date']);?></td>
			</tr>
			<tr>
			...
			<tr>
				<td>结算状态：<?php echo str_replace(array('0','1','2','3','4'),array('未出账','已出账，店铺确认中','店铺已确认，平台审核中','平台已审核，正在付款','结算完成'),$output['bill_info']['ob_state']);?>
		...
		<table class="tb-type1 noborder search">
			<tbody>
				<tr>
					...
					<td><a href="javascript:void(0);" id="ncsubmit" class="btn-search " title="<?php echo $lang['nc_query'];?>">&nbsp;</a></a></td>
				</tr>
			</tbody>
		</table>
	</form>
	<table class="table tb-type2 nobdb">
        ...
        ?></td>
				<td class="align-center"><?php echo $order['buyer_name'];?>
					</rd>
				<td><a href="index.php?act=order&op=show_order&order_id=<?php echo $order['order_id'];?>"><?php echo $lang['nc_view'];?></a></td>
			</tr>
			<?php }?>
			<?php }else{?>
			<tr class="no_data">
