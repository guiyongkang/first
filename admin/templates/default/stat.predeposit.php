...
<div class="page">
				...
				<tbody>
					<tr>
						<td><select name="pd_type" id="pd_type" class="querySelect">
								<option value="recharge" <?php echo isset($_GET['pd_type']) && $_GET['pd_type']=='recharge'?'selected':''; ?>>充值</option>
								<option value="order_pay" <?php echo isset($_GET['pd_type']) && $_GET['pd_type']=='order_pay'?'selected':''; ?>>消费</option>
								<option value="cash_pay" <?php echo isset($_GET['pd_type']) && $_GET['pd_type']=='cash_pay'?'selected':''; ?>>提现</option>
								<option value="refund" <?php echo isset($_GET['pd_type']) && $_GET['pd_type']=='refund'?'selected':''; ?>>退款</option>
							</select></td>
						<td><select name="search_type" id="search_type" class="querySelect">
								<option value="day" <?php echo isset($_GET['search_type']) && $_GET['search_type']=='day'?'selected':''; ?>>按照天统计</option>
								<option value="week" <?php echo isset($_GET['search_type']) && $_GET['search_type']=='week'?'selected':''; ?>>按照周统计</option>
								<option value="month" <?php echo isset($_GET['search_type']) && $_GET['search_type']=='month'?'selected':''; ?>>按照月统计</option>
							</select></td>
	...
	<div class="stat-info"><span>存入金额： <strong><?php echo number_format($output['stat_array']['recharge_amount'],2); ?></strong> 元</span><span>消费金额： <strong><?php echo number_format($output['stat_array']['order_amount'],2); ?></strong> 元</span><span>提现金额： <strong><?php echo number_format($output['stat_array']['cash_amount'],2); ?></strong> 元</span><span>总余额： <strong><?php echo number_format($output['usable_amount'],2); ?></strong> 元</span><span>使用总人数： <strong><?php echo intval($output['user_amount']); ?></strong> </span></div>
	<div id="container" class="w100pre close_float" style="height:400px"></div>
	<div style="text-align:right;">
		<input type="hidden" id="export_type" name="export_type" data-param='{"url":"<?php echo $output['actionurl'];?>&pd_type=<?php echo isset($_GET['pd_type']) ? trim($_GET['pd_type']) : ''; ?>&exporttype=excel"}' value="excel"/>
