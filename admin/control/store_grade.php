...
class store_grade extends SystemControl
{
    public function __construct()
    {
        parent::__construct();
        core\language::read('store_grade,store');
    }
    /**
     * 店铺等级
     */
    public function store_gradeOp()
    {
        ...
        if (chksubmit()) {
            if (!empty($_POST['check_sg_id'])) {
                if (is_array($_POST['check_sg_id'])) {
                    ...
                    foreach ($_POST['check_sg_id'] as $k => $v) {
                        ...
                        if ($v == 1) {
                            ...
                            error($lang['default_store_grade_no_del'], 'index.php?act=store_grade&op=store_grade');
                        }
                ...
                dkcache('store_grade');
                $this->log(lang('nc_del,store_grade') . '[ID:' . implode(',', $_POST['check_sg_id']) . ']', 1);
                success($lang['nc_common_del_succ']);
            } else {
                error($lang['nc_common_del_fail']);
            }
        }
        $condition['like_sg_name'] = isset($_POST['like_sg_name']) ? trim($_POST['like_sg_name']) : '';
        $condition['order'] = 'sg_sort';
        $grade_list = $model_grade->getGradeList($condition);
        core\tpl::output('like_sg_name', isset($_POST['like_sg_name']) ? trim($_POST['like_sg_name']) : '');
        core\tpl::output('grade_list', $grade_list);
        core\tpl::showpage('store_grade.index');
    }
    /**
     * 新增等级
     */
    public function store_grade_addOp()
    {
        ...
        if (chksubmit()) {
            ...
            $obj_validate->validateparam = array(array('input' => $_POST['sg_name'], 'require' => 'true', 'message' => $lang['store_grade_name_no_null']), array('input' => $_POST['sg_goods_limit'], 'require' => 'true', 'validator' => 'Number', 'message' => $lang['allow_pubilsh_product_num_only_lnteger']), array('input' => $_POST['sg_sort'], 'require' => 'true', 'validator' => 'Number', 'message' => $lang['sort_only_lnteger']));
            ...
            if ($error != '') {
                ...
                if (!$this->checkGradeName(array('sg_name' => trim($_POST['sg_name'])))) {
                    error($lang['now_store_grade_name_is_there']);
                }
                //验证级别是否存在
                if (!$this->checkGradeSort(array('sg_sort' => trim($_POST['sg_sort'])))) {
                    error($lang['add_gradesortexist']);
                }
                $insert_array = array();
                ...
                $insert_array['sg_function'] = isset($_POST['sg_function']) ? implode('|', $_POST['sg_function']) : '';
                ...
                if ($result) {
                    dkcache('store_grade');
                    $this->log(lang('nc_add,store_grade') . '[' . $_POST['sg_name'] . ']', 1);
                    success($lang['nc_common_save_succ'], 'index.php?act=store_grade&op=store_grade');
                } else {
                    error($lang['nc_common_save_fail']);
                }
            }
        }
        core\tpl::showpage('store_grade.add');
    }
    /**
     * 等级编辑
     */
    public function store_grade_editOp()
    {
        ...
        if (chksubmit()) {
            if (!$_POST['sg_id']) {
                error($lang['grade_parameter_error'], 'index.php?act=store_grade&op=store_grade');
            }
            /**
             * 验证
             */
            ...
            if ($error != '') {
                ...
                if (!$this->checkGradeName(array('sg_name' => trim($_POST['sg_name']), 'sg_id' => intval($_POST['sg_id'])))) {
                    error($lang['now_store_grade_name_is_there'], 'index.php?act=store_grade&op=store_grade_edit&sg_id=' . intval($_POST['sg_id']));
                }
                //验证级别是否存在
                if (!$this->checkGradeSort(array('sg_sort' => trim($_POST['sg_sort']), 'sg_id' => intval($_POST['sg_id'])))) {
                    error($lang['add_gradesortexist'], 'index.php?act=store_grade&op=store_grade_edit&sg_id=' . intval($_POST['sg_id']));
                }
                $update_array = array();
                ...
                $update_array['sg_function'] = isset($_POST['sg_function']) ? implode('|', $_POST['sg_function']) : '';
                ...
                if ($result) {
                    dkcache('store_grade');
                    $this->log(lang('nc_edit,store_grade') . '[' . $_POST['sg_name'] . ']', 1);
        ...
        core\tpl::output('grade_array', $grade_array);
        core\tpl::showpage('store_grade.edit');
    }
    /**
     * 删除等级
     */
    public function store_grade_delOp()
    {
        ...
        if (intval($_GET['sg_id']) > 0) {
            ...
            if (!$this->isable_delGrade(intval($_GET['sg_id']))) {
                error($lang['del_gradehavestore'], 'index.php?act=store_grade&op=store_grade');
            }
            /**
             * 删除分类
             */
            ...
            dkcache('store_grade');
            $this->log(lang('nc_del,store_grade') . '[ID:' . intval($_GET['sg_id']) . ']', 1);
            success($lang['nc_common_del_succ'], 'index.php?act=store_grade&op=store_grade');
        } else {
            error($lang['nc_common_del_fail'], 'index.php?act=store_grade&op=store_grade');
        }
    }
    /**
     * 等级：设置可选模板
    ...
    public function store_grade_templatesOp()
    {
        ...
        if (chksubmit()) {
            ...
            $result = $model_grade->update($update_array);
            if ($result) {
                $this->log(lang('nc_edit,store_grade_tpl'), 1);
                success($lang['nc_common_save_succ'], 'index.php?act=store_grade&op=store_grade');
            } else {
                error($lang['nc_common_save_fail']);
            }
        ...
        core\tpl::output('grade_array', $grade_array);
        core\tpl::showpage('store_grade_template.edit');
    }
    /**
     * ajax操作
     */
    ...
    {
        $model_grade = model('store_grade');
		if(isset($param['sg_name'])){
			$condition['sg_name'] = $param['sg_name'];
		}
		if(isset($param['sg_id'])){
