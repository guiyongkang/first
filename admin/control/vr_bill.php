...
class vr_bill extends SystemControl
{
    /**
     * 每次导出订单数量
     * @var int
    ...
    private $links = array(array('url' => 'act=vr_bill&op=index', 'lang' => 'nc_manage'));
    ...
    public function indexOp()
    {
        ...
        core\tpl::showpage('vr_bill_order_statis.index');
    }
    /**
     * 某月所有店铺销量账单
     *
    ...
    public function show_statisOp()
    {
        ...
        //输出子菜单
        core\tpl::output('top_link', $this->sublink($this->links, 'index'));
        core\tpl::showpage('vr_bill_order_statis.show');
    }
    /**
     * 某店铺某月订单列表
     *
    ...
    public function show_billOp()
    {
        ...
        core\tpl::output('order_list', $order_new_list);
        core\tpl::output('code_list', $code_list);
        ...
        core\tpl::showpage('vr_bill_order_bill.show');
    }
    ...
    public function bill_payOp()
    {
        ...
        if (chksubmit()) {
            ...
            if ($update) {
                ...
                success('保存成功', 'index.php?act=vr_bill&op=show_statis&os_month=' . $bill_info['os_month']);
            } else {
                $this->log('账单付款,账单号：' . $_GET['ob_no'], 1);
                error('保存失败');
            }
        } else {
            core\tpl::showpage('vr_bill.pay');
        }
    }
    /**
     * 打印结算单
    ...
    public function bill_printOp()
    {
        ...
        core\tpl::output('bill_info', $bill_info);
        core\tpl::showpage('vr_bill.print', 'null_layout');
    }
    /**
     * 导出平台月出账单表
     *
    ...
    public function export_billOp()
    {
        ...
        if (isset($_GET['curpage']) && !is_numeric($_GET['curpage'])) {
            ...
            if ($count > self::EXPORT_SIZE) {
                ...
                core\tpl::output('murl', 'index.php?act=vr_bill&op=index');
    ...
    public function export_orderOp()
    {
        ...
        $model_bill = model('vr_bill');
        $bill_info = $model_bill->getOrderBillInfo(array('ob_no' => (isset($_GET['ob_no']) ? $_GET['ob_no'] : 0)));
        if (!$bill_info) {
            error('参数错误');
        }
        $model_order = model('vr_order');
        $condition = array();
        $condition['store_id'] = $bill_info['ob_store_id'];
        if (isset($_GET['query_type']) && $_GET['query_type'] == 'timeout') {
            //计算未使用已过期不可退兑换码列表
            $condition['vr_state'] = 0;
            $condition['vr_invalid_refund'] = 0;
            $condition['vr_indate'] = array('between', "{$bill_info['ob_start_date']},{$bill_info['ob_end_date']}");
        ...
        if (isset($_GET['curpage']) && !is_numeric($_GET['curpage'])) {
            ...
            if ($count > self::EXPORT_SIZE) {
                ...
                core\tpl::output('list', $array);
                core\tpl::output('murl', 'index.php?act=vr_bill&op=show_bill&ob_no=' . (isset($_GET['ob_no']) ? $_GET['ob_no'] : 0));
                core\tpl::showpage('export.excel');
                exit;
            } else {
                //如果数量小，直接下载
        ...
        $csv = new lib\csv();
        $export_data = $csv->charset($export_data, CHARSET, 'gbk');
        $file_name = isset($_GET['query_type']) && $_GET['query_type'] == 'timeout' ? '过期兑换码列表' : '已消费兑换码列表';
        $csv->filename = $csv->charset($file_name . '-', CHARSET) . (isset($_GET['ob_no']) ? $_GET['ob_no'] : 0);
        $csv->export($export_data);
    }
}