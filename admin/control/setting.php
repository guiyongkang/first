<?php
/**
 * 网站设置
 **/
namespace admin\control;

use core;
use lib;
defined('SAFE_CONST') or exit('Access Invalid!');
class setting extends SystemControl
{
    private $links = array(array('url' => 'act=setting&op=base', 'lang' => 'web_set'), /*array('url' => 'act=setting&op=dump', 'lang' => 'dis_dump'),*/ array('url' => 'act=setting&op=kd100', 'lang' => 'kd100Settings'));
    public function __construct()
    {
        parent::__construct();
        core\language::read('setting');
    }
    /**
     * 基本信息
     */
    public function baseOp()
    {
        $model_setting = model('setting');
        if (chksubmit()) {
            //上传网站Logo
            if (!empty($_FILES['site_logo']['name'])) {
                $upload = new lib\uploadfile();
                $upload->set('default_dir', ATTACH_COMMON);
                $result = $upload->upfile('site_logo');
                if ($result) {
                    $_POST['site_logo'] = $upload->file_name;
                } else {
                    error($upload->error);
                }
            }
            // 上传wap手机客户端LOGO
            if (!empty($_FILES['site_mobile_logo']['name'])) {
                $upload = new lib\uploadfile();
                $upload->set('default_dir', ATTACH_COMMON);
                $upload->file_name = 'home_logo.png';
                $result = $upload->upfile('site_mobile_logo');
                if ($result) {
                    $_POST['site_mobile_logo'] = $upload->file_name;
                } else {
                    error($upload->error);
                }
            }
            //二维码微信图片
            /*if (!empty($_FILES['site_logowx']['name'])) {
                $upload = new lib\uploadfile();
                $upload->set('default_dir', ATTACH_COMMON);
                $result = $upload->upfile('site_logowx');
                if ($result) {
                    $_POST['site_logowx'] = $upload->file_name;
                } else {
                    error($upload->error);
                }
            }*/
            if (!empty($_FILES['member_logo']['name'])) {
                $upload = new lib\uploadfile();
                $upload->set('default_dir', ATTACH_COMMON);
                $result = $upload->upfile('member_logo');
                if ($result) {
                    $_POST['member_logo'] = $upload->file_name;
                } else {
                    error($upload->error);
                }
            }
            if (!empty($_FILES['seller_center_logo']['name'])) {
                $upload = new lib\uploadfile();
                $upload->set('default_dir', ATTACH_COMMON);
                $result = $upload->upfile('seller_center_logo');
                if ($result) {
                    $_POST['seller_center_logo'] = $upload->file_name;
                } else {
                    error($upload->error);
                }
            }
            $list_setting = $model_setting->getListSetting();
            $update_array = array();
            $update_array['time_zone'] = $this->setTimeZone($_POST['time_zone']);
            $update_array['site_name'] = isset($_POST['site_name']) ? $_POST['site_name'] : '';
            $update_array['site_phone'] = isset($_POST['site_phone']) ? $_POST['site_phone'] : '';
            $update_array['site_bank_account'] = isset($_POST['site_bank_account']) ? $_POST['site_bank_account'] : '';
            $update_array['site_email'] = isset($_POST['site_email']) ? $_POST['site_email'] : '';
            $update_array['statistics_code'] = isset($_POST['statistics_code']) ? $_POST['statistics_code'] : '';
            if (!empty($_POST['site_logo'])) {
                $update_array['site_logo'] = $_POST['site_logo'];
            }
            if (!empty($_POST['site_mobile_logo'])) {
                $update_array['site_mobile_logo'] = $_POST['site_mobile_logo'];
            }
            if (!empty($_POST['site_logowx'])) {
                $update_array['site_logowx'] = $_POST['site_logowx'];
            }
            if (!empty($_POST['member_logo'])) {
                $update_array['member_logo'] = $_POST['member_logo'];
            }
            if (!empty($_POST['seller_center_logo'])) {
                $update_array['seller_center_logo'] = $_POST['seller_center_logo'];
            }
            $update_array['icp_number'] = isset($_POST['icp_number']) ? $_POST['icp_number'] : '';
            //设置表 400电话
            $update_array['site_tel400'] = isset($_POST['site_tel400']) ? $_POST['site_tel400'] : '';
            $update_array['site_status'] = isset($_POST['site_status']) ? $_POST['site_status'] : '';
            $update_array['closed_reason'] = isset($_POST['closed_reason']) ? $_POST['closed_reason'] : '';
            $result = $model_setting->updateSetting($update_array);
            if ($result === true) {
                //判断有没有之前的图片，如果有则删除
                if (!empty($list_setting['site_logo']) && !empty($_POST['site_logo'])) {
					if(file_exists(BASE_UPLOAD_PATH . DS . ATTACH_COMMON . DS . $list_setting['site_logo'])){
						unlink(BASE_UPLOAD_PATH . DS . ATTACH_COMMON . DS . $list_setting['site_logo']);
					}
                }
                if (!empty($list_setting['site_logowx']) && !empty($_POST['site_logowx'])) {
					if(file_exists(BASE_UPLOAD_PATH . DS . ATTACH_COMMON . DS . $list_setting['site_logowx'])){
						unlink(BASE_UPLOAD_PATH . DS . ATTACH_COMMON . DS . $list_setting['site_logowx']);
					}
                }
                if (!empty($list_setting['member_logo']) && !empty($_POST['member_logo'])) {
					if(file_exists(BASE_UPLOAD_PATH . DS . ATTACH_COMMON . DS . $list_setting['member_logo'])){
						unlink(BASE_UPLOAD_PATH . DS . ATTACH_COMMON . DS . $list_setting['member_logo']);
					}
                }
                if (!empty($list_setting['seller_center_logo']) && !empty($_POST['seller_center_logo'])) {
					if(file_exists(BASE_UPLOAD_PATH . DS . ATTACH_COMMON . DS . $list_setting['seller_center_logo'])){
						unlink(BASE_UPLOAD_PATH . DS . ATTACH_COMMON . DS . $list_setting['seller_center_logo']);
					}
                }
                $this->log(lang('nc_edit,web_set'), 1);
                success(lang('nc_common_save_succ'));
            } else {
                $this->log(lang('nc_edit,web_set'), 0);
                success(lang('nc_common_save_fail'));
            }
        }
        $list_setting = $model_setting->getListSetting();
        foreach ($this->getTimeZone() as $k => $v) {
            if ($v == $list_setting['time_zone']) {
                $list_setting['time_zone'] = $k;
                break;
            }
        }
        core\tpl::output('list_setting', $list_setting);
        //输出子菜单
        core\tpl::output('top_link', $this->sublink($this->links, 'base'));
        core\tpl::showpage('setting.base');
    }
    /**
     * 防灌水设置
     */
    public function dumpOp()
    {
        $model_setting = model('setting');
        if (chksubmit()) {
            $update_array = array();
            $update_array['guest_comment'] = $_POST['guest_comment'];
            $update_array['captcha_status_login'] = $_POST['captcha_status_login'];
            $update_array['captcha_status_register'] = $_POST['captcha_status_register'];
            $update_array['captcha_status_goodsqa'] = $_POST['captcha_status_goodsqa'];
            $result = $model_setting->updateSetting($update_array);
            if ($result === true) {
                $this->log(lang('nc_edit,dis_dump'), 1);
                success(lang('nc_common_save_succ'));
            } else {
                $this->log(lang('nc_edit,dis_dump'), 0);
                error(lang('nc_common_save_fail'));
            }
        }
        $list_setting = $model_setting->getListSetting();
        core\tpl::output('list_setting', $list_setting);
        core\tpl::output('top_link', $this->sublink($this->links, 'dump'));
        core\tpl::showpage('setting.dump');
    }
    /**
     * 快递100设置
     */
    public function kd100Op()
    {
        $model_setting = model('setting');
        if (chksubmit()) {
            $obj_validate = new lib\validate();
            if (trim($_POST['kd100_isuse']) == '1') {
                $obj_validate->validateparam = array(array("input" => $_POST["kd100_appid"], "require" => "true", "message" => '物流系统密钥不可为空'));
            }
            $error = $obj_validate->validate();
            if ($error != '') {
                error($error);
            } else {
                $update_array = array();
                $update_array['kd100_isuse'] = $_POST['kd100_isuse'];
                $update_array['kd100_appid'] = $_POST['kd100_appid'];
                $result = $model_setting->updateSetting($update_array);
                if ($result === true) {
                    $this->log(lang('nc_edit,qqSettings'), 1);
                    success(core\language::get('nc_common_save_succ'));
                } else {
                    $this->log(lang('nc_edit,qqSettings'), 0);
                    error(core\language::get('nc_common_save_fail'));
                }
            }
        }
        $list_setting = $model_setting->getListSetting();
        core\tpl::output('list_setting', $list_setting);
        //输出子菜单
        core\tpl::output('top_link', $this->sublink($this->links, 'kd100'));
        core\tpl::showpage('setting.kd100_setting');
    }
    /**
     * SEO与rewrite设置
     */
    public function seoOp()
    {
        $model_setting = model('setting');
        if (chksubmit()) {
            $update_array = array();
            $update_array['rewrite_enabled'] = $_POST['rewrite_enabled'];
            $result = $model_setting->updateSetting($update_array);
            if ($result === true) {
                $this->log(lang('nc_edit,nc_seo_set'), 1);
                success(lang('nc_common_save_succ'));
            } else {
                $this->log(lang('nc_edit,nc_seo_set'), 0);
                error(lang('nc_common_save_fail'));
            }
        }
        $list_setting = $model_setting->getListSetting();
        //读取SEO信息
        $list = model('seo')->select();
        $seo = array();
        foreach ((array) $list as $value) {
            $seo[$value['type']] = $value;
        }
        core\tpl::output('list_setting', $list_setting);
        core\tpl::output('seo', $seo);
        $category = model('goods_class')->getGoodsClassForCacheModel();
        core\tpl::output('category', $category);
        core\tpl::showpage('setting.seo_setting');
    }
    public function ajax_categoryOp()
    {
        $model = model('goods_class');
        $list = $model->field('gc_title,gc_keywords,gc_description')->find(intval($_GET['id']));
        //转码
        if (strtoupper(CHARSET) == 'GBK') {
            $list = core\language::getUTF8($list);
            //网站GBK使用编码时,转换为UTF-8,防止json输出汉字问题
        }
        echo json_encode($list);
        exit;
    }
    /**
     * SEO设置保存
     */
    public function seo_updateOp()
    {
        $model_seo = model('seo');
        if (chksubmit()) {
            $update = array();
            if (isset($_POST['SEO'][0]) && is_array($_POST['SEO'][0])) {
                $seo = $_POST['SEO'][0];
            } else {
                $seo = $_POST['SEO'];
            }
            foreach ((array) $seo as $key => $value) {
                $model_seo->where(array('type' => $key))->update($value);
            }
            dkcache('seo');
            success(lang('nc_common_save_succ'));
        } else {
            error(lang('nc_common_save_fail'));
        }
    }
    /**
     * 分类SEO保存
     *
     */
    public function seo_categoryOp()
    {
        if (chksubmit()) {
            $where = array('gc_id' => intval($_POST['category']));
            $input = array();
            $input['gc_title'] = $_POST['cate_title'];
            $input['gc_keywords'] = $_POST['cate_keywords'];
            $input['gc_description'] = $_POST['cate_description'];
            if (model('goods_class')->editGoodsClass($input, $where)) {
                dkcache('goods_class_seo');
                success(lang('nc_common_save_succ'));
            }
        }
        error(lang('nc_common_save_fail'));
    }
    /**
     * 网站功能模块开启或者关闭
     *
     */
    public function website_settingOp()
    {
        $model_setting = model('setting');
        //保存信息
        if (chksubmit()) {
            //构造更新数据数组
            $update_array = array();
            //站外分享功能
            $update_array['share_isuse'] = trim($_POST['share_isuse']);
            $result = $model_setting->updateSetting($update_array);
            if ($result === true) {
                success(core\language::get('nc_common_save_succ'));
            } else {
                error(core\language::get('nc_common_save_fail'));
            }
        }
        //读取设置内容 $list_setting
        $list_setting = $model_setting->getListSetting();
        //模板输出
        core\tpl::output('list_setting', $list_setting);
        core\tpl::showpage('setting.website_setting');
    }
    /**
     * 设置时区
     *
     * @param int $time_zone 时区键值
     */
    private function setTimeZone($time_zone)
    {
        $zonelist = $this->getTimeZone();
        return empty($zonelist[$time_zone]) ? 'Asia/Shanghai' : $zonelist[$time_zone];
    }
    private function getTimeZone()
    {
        return array('-12' => 'Pacific/Kwajalein', '-11' => 'Pacific/Samoa', '-10' => 'US/Hawaii', '-9' => 'US/Alaska', '-8' => 'America/Tijuana', '-7' => 'US/Arizona', '-6' => 'America/Mexico_City', '-5' => 'America/Bogota', '-4' => 'America/Caracas', '-3.5' => 'Canada/Newfoundland', '-3' => 'America/Buenos_Aires', '-2' => 'Atlantic/St_Helena', '-1' => 'Atlantic/Azores', '0' => 'Europe/Dublin', '1' => 'Europe/Amsterdam', '2' => 'Africa/Cairo', '3' => 'Asia/Baghdad', '3.5' => 'Asia/Tehran', '4' => 'Asia/Baku', '4.5' => 'Asia/Kabul', '5' => 'Asia/Karachi', '5.5' => 'Asia/Calcutta', '5.75' => 'Asia/Katmandu', '6' => 'Asia/Almaty', '6.5' => 'Asia/Rangoon', '7' => 'Asia/Bangkok', '8' => 'Asia/Shanghai', '9' => 'Asia/Tokyo', '9.5' => 'Australia/Adelaide', '10' => 'Australia/Canberra', '11' => 'Asia/Magadan', '12' => 'Pacific/Auckland');
    }
}