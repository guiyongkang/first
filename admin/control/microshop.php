...
class microshop extends SystemControl
{
    ...
    {
        parent::__construct();
        core\language::read('store');
        core\language::read('microshop');
    }
    ...
    public function manageOp()
    {
        ...
        core\tpl::showpage('microshop_manage');
    }
    /**
     * 微商城管理保存
     */
    public function manage_saveOp()
    {
        ...
        if (!empty($_FILES['microshop_logo']['name'])) {
            ...
            $old_image[] = BASE_UPLOAD_PATH . DS . ATTACH_MICROSHOP . DS . core\config::get('microshop_logo');
        }
        if (!empty($_FILES['microshop_header_pic']['name'])) {
            ...
            $old_image[] = BASE_UPLOAD_PATH . DS . ATTACH_MICROSHOP . DS . core\config::get('microshop_header_pic');
        }
        ...
        if ($result === true) {
            ...
            showMessage(core\language::get('nc_common_save_succ'));
        } else {
            showMessage(core\language::get('nc_common_save_fail'));
        }
    }
    /**
     * 微商城商品(随心看)分类管理
    ...
    private function class_list($type)
    {
        ...
        core\tpl::showpage("microshop_{$type}_class.list");
    }
    /**
     * 微商城商品(随心看)分类添加
     **/
    public function goodsclass_addOp()
    {
        ...
        core\tpl::output('list', $goods_class_list);
        $class_parent_id = intval($_GET['class_parent_id']);
        if (!empty($class_parent_id)) {
            core\tpl::output('class_parent_id', $class_parent_id);
        }
        $this->show_menu_goods_class('goods_class_add');
        core\tpl::showpage('microshop_goods_class.add');
    }
    /**
     * 微商城个人秀分类添加
     **/
    public function personalclass_addOp()
    {
        ...
        core\tpl::showpage('microshop_personal_class.add');
    }
    /**
     * 微商城商品(随心看)分类编辑
     **/
    ...
    private function class_edit($type)
    {
        ...
        if (empty($class_id)) {
            showMessage(core\language::get('param_error'), '', '', 'error');
        }
        $model_class = model("micro_{$type}_class");
        $condition = array();
        ...
        core\tpl::showpage("microshop_{$type}_class.add");
    }
    /**
     * 微商城商品(随心看)分类保存
     **/
    ...
    private function class_save($type)
    {
        ...
        if ($type == 'goods') {
            $validate_array[] = array('input' => $_POST['class_parent_id'], 'require' => 'true', 'validator' => 'Number', 'message' => core\language::get('parent_id_error'));
        }
        ...
        if ($result) {
            showMessage(core\language::get('class_add_success'), "index.php?act=microshop&op={$type}class_list");
        } else {
            showMessage(core\language::get('class_add_fail'), "index.php?act=microshop&op={$type}class_list", '', 'error');
        }
    }
    /*
     * ajax修改分类排序
    ...
    public function goodsclass_dropOp()
    {
        ...
        if ($result) {
            showMessage(core\language::get('class_drop_success'), '');
        } else {
            showMessage(core\language::get('class_drop_fail'), '', '', 'error');
        }
    }
    /**
     * 个人秀分类删除
    ...
    public function goodsclass_bindingOp()
    {
        ...
        core\tpl::output('class_id', $class_id);
        ...
        core\tpl::output('goods_class_root', $goods_class_root);
        core\tpl::output('goods_class', $goods_class_list);
        $model_goods_relation = model('micro_goods_relation');
        $class_binding_list = $model_goods_relation->getList(array('class_id' => $class_id));
        core\tpl::output('class_binding_list', $class_binding_list);
        ...
        core\tpl::output('class_binding_string', rtrim($class_binding_string, ','));
        $this->show_menu_goods_class('goods_class_binding');
        core\tpl::showpage('microshop_goods_class.binding');
    }
    /**
     * 分类关键字和商品分类的绑定保存
     **/
    public function goodsclass_binding_saveOp()
    {
        ...
        if ($result) {
            success(core\language::get('goods_relation_save_success'), self::MICROSHOP_CLASS_LIST);
        } else {
            error(core\language::get('goods_relation_save_fail'), self::MICROSHOP_CLASS_LIST);
        }
    }
    /**
     * 设为默认分类
    ...
    public function goods_manageOp()
    {
        ...
        core\tpl::output('show_page', $model_microshop_goods->showpage(2));
        core\tpl::output('list', $list);
        $this->show_menu('goods_manage');
        core\tpl::showpage('microshop_goods.manage');
    }
    /**
     * 随心看删除
     */
    ...
    public function personal_manageOp()
    {
        ...
        core\tpl::output('show_page', $model_personal->showpage(2));
        core\tpl::output('list', $list);
        $this->show_menu('personal_manage');
        core\tpl::showpage('microshop_personal.manage');
    }
    /**
     * 随心看删除
     */
    ...
    public function store_manageOp()
    {
        ...
        core\tpl::output('show_page', $model_store->showpage(2));
        core\tpl::showpage('microshop_store.manage');
    }
    /**
     * 店铺街添加列表
     */
    public function store_addOp()
    {
        ...
        core\tpl::output('microshop_store_array', $microshop_store_array);
        $page = new lib\page();
        $page->setEachNum(10);
        $page->setStyle('admin');
        $condition = array();
        ...
        core\tpl::showpage('microshop_store.add');
    }
    /**
     * 店铺街添加保存
     */
    ...
    private function update_microshop_sort($model_name, $key_name)
    {
        ...
        $new_sort = intval($_GET['value']);
        if ($new_sort > 255) {
            echo json_encode(array('result' => FALSE, 'message' => core\language::get('microshop_sort_error')));
            die;
        } else {
            ...
            if ($result) {
                ...
                echo json_encode(array('result' => FALSE, 'message' => core\language::get('nc_common_op_fail')));
                die;
            }
        }
    }
    ...
    public function comment_manageOp()
    {
        ...
        core\tpl::output('list', $comment_list);
        core\tpl::output('show_page', $model_comment->showpage(2));
        $this->get_channel_array();
        $this->show_menu('comment_manage');
        core\tpl::showpage('microshop_comment.manage');
    }
    /**
     * 评论删除
     */
    ...
    public function adv_manageOp()
    {
        ...
        core\tpl::showpage('microshop_adv.manage');
    }
    /**
     * 微商城广告添加
     **/
    public function adv_addOp()
    {
        ...
        core\tpl::showpage('microshop_adv.add');
    }
    ...
    {
        $obj_validate = new lib\validate();
        $validate_array = array(array('input' => $_POST['adv_sort'], 'require' => 'true', 'validator' => 'Range', 'min' => 0, 'max' => 255, 'message' => core\language::get('class_sort_error')));
        $obj_validate->validateparam = $validate_array;
        $error = $obj_validate->validate();
        if ($error != '') {
        ...
        if (!empty($_FILES['adv_image']['name'])) {
            ...
            if (empty($_POST['adv_id'])) {
                error(core\language::get('microshop_adv_image_error'));
            }
        }
        $model_adv = model("micro_adv");
        ...
        if ($result) {
            success(core\language::get('nc_common_save_succ'), "index.php?act=microshop&op=adv_manage");
        } else {
            error(core\language::get('nc_common_save_fail'), "index.php?act=microshop&op=adv_manage");
        }
    }
    /**
     * 广告删除
    ...
    {
        $adv_type_list = array();
        $adv_type_list['index'] = core\language::get('microshop_adv_type_index');
        $adv_type_list['store_list'] = core\language::get('microshop_adv_type_store_list');
        core\tpl::output('adv_type_list', $adv_type_list);
    }
    /**
     * ajax操作
     */
    ...
    {
        $channel_array = array();
        $channel_array[self::GOODS_FLAG] = array('name' => core\language::get('nc_microshop_goods'), 'key' => 'goods');
        $channel_array[self::PERSONAL_FLAG] = array('name' => core\language::get('nc_microshop_personal'), 'key' => 'personal');
        $channel_array[self::STORE_FLAG] = array('name' => core\language::get('nc_microshop_store'), 'key' => 'store');
        core\tpl::output('channel_array', $channel_array);
    }
    /**
     * 微商城菜单
     */
    ...
    {
        $menu_array = array("{$menu_key}" => array('menu_type' => 'link', 'menu_name' => core\language::get('nc_manage'), 'menu_url' => 'index.php?act=microshop&op=' . $menu_key));
        $menu_array[$menu_key]['menu_type'] = 'text';
        core\tpl::output('menu', $menu_array);
    }
    private function show_menu_goods_class($menu_key)
    {
        $menu_array = array('goods_class_list' => array('menu_type' => 'link', 'menu_name' => core\language::get('nc_manage'), 'menu_url' => 'index.php?act=microshop&op=goodsclass_list'), 'goods_class_add' => array('menu_type' => 'link', 'menu_name' => core\language::get('nc_new'), 'menu_url' => 'index.php?act=microshop&op=goodsclass_add'));
        if ($menu_key == 'goods_class_edit') {
            $menu_array['goods_class_edit'] = array('menu_type' => 'link', 'menu_name' => core\language::get('nc_edit'), 'menu_url' => '###');
        }
        if ($menu_key == 'goods_class_binding') {
            $menu_array['goods_class_binding'] = array('menu_type' => 'link', 'menu_name' => core\language::get('microshop_goods_class_binding'), 'menu_url' => '###');
        }
        $menu_array[$menu_key]['menu_type'] = 'text';
        core\tpl::output('menu', $menu_array);
    }
    ...
    {
        $menu_array = array('personal_class_list' => array('menu_type' => 'link', 'menu_name' => core\language::get('nc_manage'), 'menu_url' => 'index.php?act=microshop&op=personalclass_list'), 'personal_class_add' => array('menu_type' => 'link', 'menu_name' => core\language::get('nc_new'), 'menu_url' => 'index.php?act=microshop&op=personalclass_add'));
        if ($menu_key == 'personal_class_edit') {
            $menu_array['personal_class_edit'] = array('menu_type' => 'link', 'menu_name' => core\language::get('nc_edit'), 'menu_url' => '###');
        }
        $menu_array[$menu_key]['menu_type'] = 'text';
        core\tpl::output('menu', $menu_array);
    }
    ...
    {
        $menu_array = array('store_manage' => array('menu_type' => 'link', 'menu_name' => core\language::get('nc_manage'), 'menu_url' => 'index.php?act=microshop&op=store_manage'), 'store_add' => array('menu_type' => 'link', 'menu_name' => core\language::get('nc_new'), 'menu_url' => 'index.php?act=microshop&op=store_add'));
        $menu_array[$menu_key]['menu_type'] = 'text';
        core\tpl::output('menu', $menu_array);
    }
    private function show_menu_adv($menu_key)
    {
        $menu_array = array('adv_manage' => array('menu_type' => 'link', 'menu_name' => core\language::get('nc_manage'), 'menu_url' => 'index.php?act=microshop&op=adv_manage'), 'adv_add' => array('menu_type' => 'link', 'menu_name' => core\language::get('nc_new'), 'menu_url' => 'index.php?act=microshop&op=adv_add'));
        if ($menu_key == 'adv_edit') {
            $menu_array['adv_edit'] = array('menu_type' => 'link', 'menu_name' => core\language::get('nc_edit'), 'menu_url' => '###');
