...
class web_channel extends SystemControl
{
    private $links = array(array('url' => 'act=web_channel&op=web_channel', 'lang' => 'web_config_web_channel'), array('url' => 'act=web_channel&op=floor_list', 'lang' => 'web_config_floor_list'));
    public function __construct()
    {
        parent::__construct();
        core\language::read('web_config');
    }
    ...
    {
        core\tpl::output('top_link', $this->sublink($this->links, 'web_channel'));
        core\tpl::showpage('web_channel.list');
    }
    /**
     * 输出频道列表XML数据
     */
    public function get_channel_xmlOp()
    {
        ...
        $data['list'] = $out_list;
        echo core\tpl::flexigridXML($data);
        exit;
    }
    /**
     * 模块列表
    ...
    {
        core\tpl::output('top_link', $this->sublink($this->links, 'floor_list'));
        core\tpl::showpage('web_floor.list');
    }
    /**
     * 输出模块列表XML数据
     */
    ...
    {
        $model_channel = model('web_channel');
        $style_array = $model_channel->getFloorStyle();
        //模块类型数组
        $page = intval($_POST['rp']);
        if ($page < 1) {
            $page = 15;
        ...
        $data['total_num'] = $model_channel->gettotalnum();
        $data['list'] = $out_list;
        echo core\tpl::flexigridXMLfloor($data);
        exit;
    }
    /**
     * 中部模块数据
    ...
    public function get_channel_flOp()
    {
        ...
        core\tpl::output('floor_list', $floor_list);
        core\tpl::output('show_page', $model_channel->showpage());
        core\tpl::showpage('web_channel_fl.list', 'null_layout');
    }
    /**
     * 新增频道
     */
    public function add_channelOp()
    {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                success(core\language::get('nc_common_save_succ'), 'index.php?act=web_channel&op=web_channel');
            } else {
                error(core\language::get('nc_common_save_fail'));
            }
        }
        core\tpl::showpage('web_channel.add');
    }
    /**
     * 删除频道
     *
    ...
    public function add_floorOp()
    {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                success(core\language::get('nc_common_save_succ'), 'index.php?act=web_channel&op=floor_list');
            } else {
                error(core\language::get('nc_common_save_fail'));
            }
        }
        core\tpl::showpage('web_floor.add');
    }
    /**
     * 编辑频道
     */
    public function edit_channelOp()
    {
        ...
        core\tpl::output('channel', $channel);
        core\tpl::showpage('web_channel.edit');
    }
    /**
     * 编辑频道模块
     */
    public function set_channelOp()
    {
        ...
        if (chksubmit()) {
            ...
            if ($gc_id > 0) {
                ...
                $model_channel->editChannelGoodsClass($gc_id);
                //消除分类已绑定的频道
            }
        ...
        core\tpl::output('channel', $channel);
        core\tpl::output('gc_id', $channel['gc_id']);
        core\tpl::output('top_id', $channel['top_id']);
        $condition = array();
        $condition['web_page'] = 'channel_tp';
        $top_list = $model_channel->getFloorList($condition);
        core\tpl::output('top_list', $top_list);
        $floor_list = $model_channel->getChannelFloor($channel['floor_ids']);
        core\tpl::output('floor_list', $floor_list);
        core\tpl::showpage('web_channel.set');
    }
    /**
     * 编辑模块
     */
    public function edit_floorOp()
    {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                $model_web_config->updateWebHtml($condition['web_id']);
                //更新前台显示的html内容
        ...
        core\tpl::output('web', $web);
        $model_channel = model('web_channel');
        $style_array = $model_channel->getFloorStyle();
        //模块类型数组
        core\tpl::output('style_array', $style_array);
        core\tpl::showpage('web_floor.edit');
    }
    /**
     * 编辑顶部模块
     */
    public function edit_channel_tpOp()
    {
        ...
        core\tpl::output('floor', $floor_list[0]);
        core\tpl::showpage('web_channel_tp.edit');
    }
    /**
     * 编辑中部模块
     */
    public function edit_channel_flOp()
    {
        ...
        $floor_list = $model_web_config->getWebList(array('web_id' => "{$web_id}"));
        core\tpl::output('floor', $floor_list[0]);
        core\tpl::showpage('web_channel_fl.edit');
    }
    /**
     * 商品分类
     */
    public function get_category_listOp()
    {
        ...
        core\tpl::output('gc_parent', $gc_parent);
        $goods_class = $model_class->getGoodsClassListByParentId($gc_parent_id);
        core\tpl::output('goods_class', $goods_class);
        core\tpl::showpage('web_channel_category', 'null_layout');
    }
    /**
     * 检查分类是否已经绑定到频道
     */
    ...
    public function recommend_listOp()
    {
        ...
        core\tpl::output('show_page', $model_web_config->showpage(2));
        core\tpl::output('goods_list', $goods_list);
        core\tpl::showpage('web_goods.list', 'null_layout');
    }
    /**
     * 商品分类图片
     */
    public function upload_gc_picOp()
    {
        ...
        if ($gc_id > 0 && !empty($pic_name)) {
            core\tpl::output('pic', $pic_name);
            core\tpl::output('var_name', $gc_id);
            core\tpl::showpage('web_upload_pic', 'null_layout');
        }
    }
    /**
     * 保存焦点区切换大图
    ...
    public function channel_slideOp()
    {
        ...
        if (!empty($code)) {
            ...
            if ($pic_id > 0) {
                ...
                if (!empty($code_info[$pic_id]['pic_img'])) {
                    //原图片
    ...
    public function channel_advOp()
    {
        ...
        if (!empty($code)) {
            ...
            if ($pic_id > 0) {
                ...
                $pic_info = $_POST[$var_name];
                $pic_info['pic_id'] = $pic_id;
                if (!empty($code_info[$slide_id]['pic_list'][$pic_id]['pic_img'])) {
                    //原图片
            ...
            $code_info = $model_web_config->get_str($code_info, $code_type);
            $model_web_config->updateCode(array('code_id' => $code_id), array('code_info' => $code_info));
            core\tpl::showpage('web_upload_focus', 'null_layout');
        }
    }
    /**
     * 保存频道中部图片
    ...
    public function upload_picOp()
    {
        ...
        if (!empty($code)) {
            ...
            core\tpl::output('var_name', $var_name);
            core\tpl::output('pic', $code_info['pic']);
            $code_info = $model_web_config->get_str($code_info, $code_type);
            $state = $model_web_config->updateCode(array('code_id' => $code_id), array('code_info' => $code_info));
            core\tpl::showpage('web_upload_pic', 'null_layout');
        }
