...
class complain extends SystemControl
{
    //定义投诉状态常量
    const STATE_NEW = 10;
    ...
    private function get_complain_list($complain_state, $op)
    {
        ...
        //搜索条件
        $condition = array();
        $condition['complain_accuser'] = isset($_GET['input_complain_accuser']) ? trim($_GET['input_complain_accuser']) : '';
        $condition['complain_accused'] = isset($_GET['input_complain_accused']) ? trim($_GET['input_complain_accused']) : '';
        $condition['complain_subject_content'] = isset($_GET['input_complain_subject_content']) ? trim($_GET['input_complain_subject_content']) : '';
        $condition['complain_datetime_start'] = isset($_GET['input_complain_datetime_start']) ? strtotime($_GET['input_complain_datetime_start']) : '';
        $condition['complain_datetime_end'] = isset($_GET['input_complain_datetime_end']) ? strtotime($_GET['input_complain_datetime_end']) : '';
        ...
        $this->show_menu($op);
        core\tpl::output('op', $op);
        core\tpl::output('list', $complain_list);
        core\tpl::output('show_page', $page->show());
        core\tpl::showpage('complain.list');
    }
    /*
     * 进行中的投诉
     */
    public function complain_progressOp()
    {
        ...
        core\tpl::output('complain_goods_list', $complain_goods_list);
        if (intval($complain_info['complain_subject_id']) == 1) {
            ...
            core\tpl::output('refund_list', $list['refund']);
            //已退或处理中商品
            core\tpl::output('refund_goods', $list['goods']);
            //可退商品
        }
        $this->show_menu('complain_progress');
        core\tpl::output('order_info', $order_info);
        core\tpl::output('complain_info', $complain_info);
        core\tpl::showpage('complain.info');
    }
    /*
     * 审核提交的投诉
     */
    public function complain_verifyOp()
    {
        ...
        if (intval($complain_info['complain_state']) === self::STATE_NEW) {
            ...
            if ($model_complain->updateComplain($update_array, $where_array)) {
                ...
                success(core\language::get('complain_verify_success'), 'index.php?act=complain&op=complain_new_list');
            } else {
                error(core\language::get('complain_verify_fail'), 'index.php?act=complain&op=complain_new_list');
            }
        } else {
    ...
    public function complain_closeOp()
    {
        ...
        $obj_validate->validateparam = array(array("input" => $final_handle_message, "require" => "true", "validator" => "Length", "min" => "1", "max" => "255", "message" => core\language::get('final_handle_message_error')));
        ...
        if ($current_state !== self::STATE_FINISH) {
            ...
            if ($model_complain->updateComplain($update_array, $where_array)) {
                ...
                success(core\language::get('complain_close_success'), $this->get_complain_state_link($current_state));
            } else {
                error(core\language::get('complain_close_fail'), $this->get_complain_state_link($current_state));
            }
        } else {
            error(core\language::get('param_error'));
        }
    ...
    public function complain_subject_listOp()
    {
        ...
        core\tpl::output('list', $complain_subject_list);
        core\tpl::output('show_page', $page->show());
        core\tpl::showpage('complain_subject.list');
    }
    /*
     * 添加投诉主题页面
     */
    public function complain_subject_addOp()
    {
        ...
        core\tpl::showpage('complain_subject.add');
    }
    /*
     * 保存添加的投诉主题
     */
    public function complain_subject_saveOp()
    {
        ...
        //验证提交的内容
        $obj_validate = new lib\validate();
        $obj_validate->validateparam = array(array("input" => $input['complain_subject_content'], "require" => "true", "validator" => "Length", "min" => "1", "max" => "50", "message" => core\language::get('complain_subject_content_error')), array("input" => $input['complain_subject_desc'], "require" => "true", "validator" => "Length", "min" => "1", "max" => "100", "message" => core\language::get('complain_subject_desc_error')));
        $error = $obj_validate->validate();
        if ($error != '') {
            error($error);
        } else {
            ...
            success(core\language::get('complain_subject_add_success'), 'index.php?act=complain&op=complain_subject_list');
        }
    }
    /*
     * 删除投诉主题,伪删除只是修改标记
    ...
    public function complain_subject_dropOp()
    {
        ...
        if (empty($complain_subject_id)) {
            error(core\language::get('param_error'), 'index.php?act=complain');
        }
        $model_complain_subject = model('complain_subject');
        $update_array = array();
        ...
        if ($model_complain_subject->updateComplainSubject($update_array, $where_array)) {
            ...
            success(core\language::get('complain_subject_delete_success'), 'index.php?act=complain&op=complain_subject_list');
        } else {
            error(core\language::get('complain_subject_delete_fail'), 'index.php?act=complain&op=complain_subject_list');
        }
    }
    /*
     * 根据投诉id获取投诉对话列表
    ...
    public function get_complain_talkOp()
    {
        ...
        foreach ($complain_talk_list as $talk) {
            ...
            switch ($talk['talk_member_type']) {
                case 'accuser':
                    $talk_list[$i]['talk'] .= core\language::get('complain_accuser');
                    break;
                case 'accused':
                    $talk_list[$i]['talk'] .= core\language::get('complain_accused');
                    break;
                case 'admin':
                    $talk_list[$i]['talk'] .= core\language::get('complain_admin');
                    break;
                default:
                    $talk_list[$i]['talk'] .= core\language::get('complain_unknow');
            }
            if (intval($talk['talk_state']) === 2) {
                $talk['talk_content'] = core\language::get('talk_forbit_message');
                $forbit_link = '';
            } else {
                $forbit_link = "&nbsp;&nbsp;<a href='#' onclick=forbit_talk(" . $talk['talk_id'] . ")>" . core\language::get('complain_text_forbit') . "</a>";
            }
            $talk_list[$i]['talk'] .= '(' . $talk['talk_member_name'] . ')' . core\language::get('complain_text_say') . ':' . $talk['talk_content'] . $forbit_link;
            $i++;
        }
        if (strtoupper(CHARSET) == 'GBK') {
            $talk_list = core\language::getUTF8($talk_list);
        }
    ...
    public function publish_complain_talkOp()
    {
        ...
        if ($talk_len > 0 && $talk_len < 255) {
            ...
            if (strtoupper(CHARSET) == 'GBK') {
                $complain_talk = core\language::getGBK($complain_talk);
            }
    ...
    public function complain_settingOp()
    {
        ...
        core\tpl::showpage('complain.setting');
    }
    /**
     * 投诉设置保存
     **/
    public function complain_setting_saveOp()
    {
        ...
        if ($model_setting->updateSetting($update_array)) {
            ...
            success(core\language::get('complain_setting_save_success'));
        } else {
            error(core\language::get('complain_setting_save_fail'));
        }
    }
    /*
     * 获取订单信息
    ...
    private function get_complain_state_text($complain_state)
    {
        switch (intval($complain_state)) {
            case self::STATE_NEW:
                return core\language::get('complain_state_new');
                break;
            case self::STATE_APPEAL:
                return core\language::get('complain_state_appeal');
                break;
            case self::STATE_TALK:
                return core\language::get('complain_state_talk');
                break;
            case self::STATE_HANDLE:
                return core\language::get('complain_state_handle');
                break;
            case self::STATE_FINISH:
                return core\language::get('complain_state_finish');
                break;
            default:
                error(core\language::get('param_error'));
        }
    ...
    {
        $menu_array = array('complain_new_list' => array('menu_type' => 'link', 'menu_name' => core\language::get('complain_new_list'), 'menu_url' => 'index.php?act=complain&op=complain_new_list'), 'complain_appeal_list' => array('menu_type' => 'link', 'menu_name' => core\language::get('complain_appeal_list'), 'menu_url' => 'index.php?act=complain&op=complain_appeal_list'), 'complain_talk_list' => array('menu_type' => 'link', 'menu_name' => core\language::get('complain_talk_list'), 'menu_url' => 'index.php?act=complain&op=complain_talk_list'), 'complain_handle_list' => array('menu_type' => 'link', 'menu_name' => core\language::get('complain_handle_list'), 'menu_url' => 'index.php?act=complain&op=complain_handle_list'), 'complain_finish_list' => array('menu_type' => 'link', 'menu_name' => core\language::get('complain_finish_list'), 'menu_url' => 'index.php?act=complain&op=complain_finish_list'), 'complain_subject_list' => array('menu_type' => 'link', 'menu_name' => core\language::get('complain_subject_list'), 'menu_url' => 'index.php?act=complain&op=complain_subject_list'), 'complain_subject_add' => array('menu_type' => 'link', 'menu_name' => core\language::get('complain_subject_add'), 'menu_url' => 'index.php?act=complain&op=complain_subject_add'), 'complain_setting' => array('menu_type' => 'link', 'menu_name' => core\language::get('complain_setting'), 'menu_url' => 'index.php?act=complain&op=complain_setting'), 'complain_progress' => array('menu_type' => 'link', 'menu_name' => core\language::get('complain_progress'), 'menu_url' => '###'));
