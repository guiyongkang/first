...
class live_class extends SystemControl
{
    ...
    public function live_classOp()
    {
        ...
        core\tpl::showpage('live.groupbuyclass');
    }
    /*
     * 添加分类
     */
    public function add_classOp()
    {
        if (chksubmit()) {
            ...
            $validate_array = array(array('input' => $_POST['live_class_name'], 'require' => 'true', "validator" => "Length", "min" => "1", "max" => "10", 'message' => core\language::get('live_groupbuy_class_name_is_not_null')), array('input' => $_POST['live_class_name'], 'require' => 'true', 'validator' => 'Range', 'min' => 0, 'max' => 255, 'message' => core\language::get('live_groupbuy_class_sort_is_not_null')));
            ...
            $model_live_class = model('live_class');
            $res = $model_live_class->add($params);
            //添加分类
            if ($res) {
                H('live_class', null);
                //清除缓存
        ...
        core\tpl::showpage('live.groupbuyclass.add');
    }
    /*
     * 编辑分类
     */
    public function edit_classOp()
    {
        if (chksubmit()) {
            //数据验证
            $obj_validate = new lib\validate();
            $validate_array = array(array('input' => $_POST['live_class_name'], 'require' => 'true', "validator" => "Length", "min" => "1", "max" => "10", 'message' => core\language::get('live_groupbuy_class_name_is_not_null')), array('input' => $_POST['live_class_sort'], 'require' => 'true', 'validator' => 'Range', 'min' => 0, 'max' => 255, 'message' => core\language::get('live_groupbuy_class_sort_is_not_null')));
            $obj_validate->validateparam = $validate_array;
            $error = $obj_validate->validate();
            if ($error != '') {
                showMessage(core\language::get('error') . $error, '', '', 'error');
        ...
        core\tpl::output('live_class', $live_class);
        $list = $model_live_class->getList(array('parent_class_id' => 0));
        core\tpl::output('list', $list);
        core\tpl::showpage('live.groupbuyclass.edit');
    }
    /*
     * 删除分类
     */
