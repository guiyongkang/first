...
class web_api extends SystemControl
{
    public function __construct()
    {
        ...
        if (strtoupper(CHARSET) == 'GBK') {
            ...
            $_POST = core\language::getGBK($_POST);
        }
    ...
    public function focus_editOp()
    {
        ...
        $screen_adv_list = $model_web_config->getAdvList("screen");
        //焦点大图广告数据
        core\tpl::output('screen_adv_list', $screen_adv_list);
        $focus_adv_list = $model_web_config->getAdvList("focus");
        //四张联动区广告数据
        core\tpl::output('focus_adv_list', $focus_adv_list);
        core\tpl::showpage('web_focus.edit');
    }
    /**
     * 更新html内容
     */
    ...
    public function sale_editOp()
    {
        ...
        core\tpl::showpage('web_sale.edit');
    }
    /**
     * 商品分类
     */
    public function category_listOp()
    {
        ...
        core\tpl::output('goods_class', $goods_class);
        core\tpl::showpage('web_goods_class', 'null_layout');
    }
    /**
     * 商品推荐
     */
    ...
    public function recommends_listOp()
    {
        ...
        core\tpl::output('show_page', $model_web_config->showpage(2));
        core\tpl::output('goods_list', $goods_list);
        core\tpl::showpage('web_goods_list', 'null_layout');
    }
    /**
     * 商品排序查询
     */
    public function goods_listOp()
    {
        $model_web_config = model('web_config');
        $condition = array();
        $order = 'goods_salenum desc,goods_id desc';
        //销售数量
        ...
        core\tpl::output('show_page', $model_web_config->showpage(2));
        core\tpl::output('goods_list', $goods_list);
        core\tpl::showpage('web_goods_order', 'null_layout');
    }
    /**
     * 品牌
     */
    public function brand_listOp()
    {
        ...
        core\tpl::showpage('web_brand.list', 'null_layout');
    }
    /**
     * 保存设置
     */
    ...
    public function upload_picOp()
    {
        ...
        if (!empty($code)) {
            ...
            core\tpl::output('type', $code_info['type']);
            core\tpl::output('ap_id', $code_info['ap_id']);
            $code_info = $model_web_config->get_str($code_info, $code_type);
            $state = $model_web_config->updateCode(array('code_id' => $code_id), array('code_info' => $code_info));
    ...
    public function recommend_picOp()
    {
        ...
        if (!empty($code) && $key_id > 0 && $pic_id > 1) {
            ...
            $code_type = $code['code_type'];
            $code_info = $model_web_config->get_array($code_info, $code_type);
            //原数组
            $var_name = "pic_list";
            $pic_info = $_POST[$var_name];
            $pic_info['pic_id'] = $pic_id;
            if (!empty($code_info[$key_id]['pic_list'][$pic_id]['pic_img'])) {
                //原图片
            ...
            $code_info = $model_web_config->get_str($code_info, $code_type);
            $state = $model_web_config->updateCode(array('code_id' => $code_id), array('code_info' => $code_info));
            core\tpl::showpage('web_recommend_pic', 'null_layout');
        }
    }
    /**
     * 保存切换图片
    ...
    public function slide_advOp()
    {
        ...
        if (!empty($code)) {
            ...
            core\tpl::showpage('web_upload_slide', 'null_layout');
        }
    }
    /**
     * 保存焦点区切换大图
    ...
    public function screen_picOp()
    {
        ...
        if (!empty($code)) {
            ...
            if ($ap_pic_id > 0 && $ap_pic_id == $key) {
                ...
                core\tpl::output('ap_pic_id', $ap_pic_id);
                core\tpl::output('ap_color', $ap_color);
            }
