...
class live_groupbuy extends SystemControl
{
    ...
    public function live_groupbuyOp()
    {
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            core\tpl::output('groupbuy_state', intval($_POST['groupbuy_state']));
            core\tpl::output('is_audit', intval($_POST['audit']));
        }
        $model_live_groupbuy = model('live_groupbuy');
        $list = $model_live_groupbuy->getList($condition);
        core\tpl::output('list', $list);
        core\tpl::output('show_page', $model_live_groupbuy->showpage());
        core\tpl::showpage('livegroupbuy.list');
    }
    /*
     * 删除抢购
     */
    ...
    {
        $model_live_groupbuy = model('live_groupbuy');
        $res = $model_live_groupbuy->edit(array('groupbuy_id' => intval($_GET['groupbuy_id'])), array('is_open' => 2));
        //取消抢购
        if ($res) {
    ...
    public function groupbuyvoucherOp()
    {
        ...
        core\tpl::showpage('livevoucher.list');
    }
}