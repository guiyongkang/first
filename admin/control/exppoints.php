...
class exppoints extends SystemControl
{
    ...
    public function expsettingOp()
    {
        ...
        if (chksubmit()) {
            ...
            if ($result === true) {
                $this->log(lang('nc_edit,nc_exppoints_manage,nc_exppoints_setting'), 1);
        ...
        core\tpl::showpage('exppoints.setting');
    }
    /**
     * 积分日志列表
     */
    ...
    {
        $where = array();
        $search_mname = isset($_GET['mname']) ? trim($_GET['mname']) : '';
        $where['exp_membername'] = array('like', "%{$search_mname}%");
        if (isset($_GET['stage'])) {
            $where['exp_stage'] = trim($_GET['stage']);
        }
        $stime = isset($_GET['stime']) ? strtotime($_GET['stime']) : 0;
        $etime = isset($_GET['etime']) ? strtotime($_GET['etime']) : 0;
        if ($stime > 0 && $etime > 0) {
