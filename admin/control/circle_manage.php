...
class circle_manage extends SystemControl
{
    ...
    public function circle_listOp()
    {
        ...
        if (chksubmit()) {
            ...
            if ($_POST['submit_type'] == 'batchdel') {
                ...
                if (empty($id_array) && !is_array($id_array)) {
                    error(lang('circle_choose_del_circle'));
                }
                $model = model();
                ...
                $this->log(lang('nc_del_circle') . '[' . implode(',', $id_array) . ']');
        ...
        core\tpl::output('circle_list', $circle_list);
        core\tpl::showpage('circle.list');
    }
    /**
     * 圈子待审核列表
     */
    public function circle_verifyOp()
    {
        ...
        core\tpl::output('page', $model->showpage('2'));
        core\tpl::output('circle_list', $circle_list);
        core\tpl::showpage('circle.verify');
    }
    /**
     * 圈子新增
     */
    public function circle_addOp()
    {
        ...
        if (chksubmit()) {
            ...
            $obj_validate->validateparam = array(array("input" => $_POST["c_name"], "require" => "true", "message" => lang('circle_name_not_null')));
            ...
            if ($error != '') {
                ...
                if ($circleid) {
                    ...
                    $this->log(lang('nc_add_circle') . '[' . $circleid . ']');
                    success(lang('nc_common_op_succ'));
                } else {
    ...
    public function circle_editOp()
    {
        ...
        if (chksubmit()) {
            ...
            if ($error != '') {
                ...
                if ($rs) {
                    ...
                    $this->log(lang('nc_edit_circle') . '[' . intval($_POST['c_id']) . ']');
                    success(lang('nc_common_op_succ'), 'index.php?act=circle_manage&op=circle_list');
                } else {
                    error(lang('nc_common_op_fail'));
                }
            }
        ...
        core\tpl::output('circle_info', $circle_info);
        // 圈子分类
        $class_list = $model->table('circle_class')->where(array('class_status' => 1))->order('class_sort asc')->select();
        core\tpl::output('class_list', $class_list);
        core\tpl::showpage('circle.edit');
    }
    /**
     * 选择圈主
     */
    ...
    {
        core\tpl::output('search_url', intval($_GET['c_id']) > 0 ? urlAdmin('circle_manage', 'search_member', array('c_id' => intval($_GET['c_id']))) : urlAdmin('circle_manage', 'search_member'));
        core\tpl::showpage('circle.choose_master', 'null_layout');
    }
    /**
     * 搜索会员
     */
    public function search_memberOp()
    {
        ...
        if (!empty($count_array)) {
            foreach ($count_array as $val) {
                if (intval($val['count']) >= core\config::get('circle_createsum')) {
        ...
        if (!empty($count_array)) {
            foreach ($count_array as $val) {
                if (intval($val['count']) >= core\config::get('circle_joinsum')) {
        ...
        if (strtoupper(CHARSET) == 'GBK') {
            $member_list = core\language::getUTF8($member_list);
        }
    ...
    public function circle_delOp()
    {
        ...
        $this->log(lang('nc_del_circle') . '[' . $id . ']');
        success(lang('nc_common_op_succ'));
    }
    /**
     * 会员名称检测
    ...
    public function check_memberOp()
    {
        ...
        if (empty($member_info)) {
            ...
            if (intval($count) >= intval(core\config::get('circle_createsum'))) {
                echo 'false';
                exit;
            }
            // 允许加入圈子验证
            ...
            if (intval($count) >= intval(core\config::get('circle_joinsum'))) {
                echo 'false';
                exit;
            }
            echo 'true';
    ...
    public function ajaxOp()
    {
        switch ($_GET['branch']) {
            case 'status':
                $this->log(lang('nc_circle_pass_cerify') . '[' . intval($_GET['id']) . ']');
            case 'recommend':
                $update = array('circle_id' => intval($_GET['id']), $_GET['column'] => $_GET['value']);
                model()->table('circle')->update($update);
                echo 'true';
                break;
            case 'name':
                $where = array('circle_id' => intval($_GET['id']));
