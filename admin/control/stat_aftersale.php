...
class stat_aftersale extends SystemControl
{
    private $links = array(array('url' => 'act=stat_aftersale&op=refund', 'lang' => 'stat_refund'), array('url' => 'act=stat_aftersale&op=evalstore', 'lang' => 'stat_evalstore'));
    private $search_arr;
    //处理后的参数
    ...
    public function refundOp()
    {
        ...
        core\tpl::output('top_link', $this->sublink($this->links, 'refund'));
        core\tpl::showpage('stat.aftersale.refund');
    }
    /**
     * 退款统计
     */
    ...
    {
        //店铺分类
        core\tpl::output('class_list', rkcache('store_class', true));
        $model = model('stat');
        $where = array();
        if (isset($_GET['store_class']) && intval($_GET['store_class']) > 0) {
            $where['sc_id'] = intval($_GET['store_class']);
        }
        if (!empty($this->search_arr['storename'])) {
        ...
        $orderby = (isset($this->search_arr['orderby']) ? trim($this->search_arr['orderby']) . ',' : '') . 'seval_storeid';
        //查询评论的店铺总数
        ...
        core\tpl::output('show_page', $model->showpage(2));
        core\tpl::output('top_link', $this->sublink($this->links, 'evalstore'));
        core\tpl::showpage('stat.aftersale.evalstore');
    }
}