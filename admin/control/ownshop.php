...
class ownshop extends SystemControl
{
    ...
    public function listOp()
    {
        ...
        if (strlen($storeName) > 0) {
            ...
            core\tpl::output('store_name', $storeName);
        }
        ...
        $storesWithGoods = model('goods')->where(array('store_id' => array('in', $storeIds)))->field('distinct store_id')->key('store_id')->select();
        core\tpl::output('store_list', $storeList);
        core\tpl::output('page', $model->showpage());
        core\tpl::output('storesWithGoods', $storesWithGoods);
        core\tpl::showpage('ownshop.list');
    }
    public function addOp()
    {
        ...
        if (chksubmit()) {
			...
			$saveArray['sc_id'] = intval($_POST['sc_id']);
            ...
            success('操作成功', urlAdmin('ownshop', 'list'));
            return;
        }
		//店铺分类
    ...
    public function editOp()
    {
        ...
        if (chksubmit()) {
			...
			$saveArray['sc_id'] = intval($_POST['sc_id']);
            $storeModel->editStore($saveArray, array('store_id' => $_GET['id']));
            if ($saveArray['seller_name'] != $storeArray['seller_name']) {
                model('seller')->editSeller(array('seller_name' => $_POST['seller_name']), array('store_id' => $_GET['id'], 'seller_group_id' => 0, 'is_admin' => 1));
            }
			if(!empty($_POST['member_passwd'])){
				$memberPasswd = (string) $_POST['member_passwd'];
				model('member')->editMember(array('member_id'=>$storeArray['member_id']), array('member_passwd' => md5(trim($memberPasswd))));
			}
            /* 自营店不下架商品
        ...
        $parent_list = $model_store_class->getStoreClassList(array(), '', false);
		core\tpl::output('class_list', $parent_list);
        core\tpl::output('store_array', $storeArray);
        core\tpl::showpage('ownshop.edit');
    }
    public function check_seller_nameOp()
    {
        echo json_encode($this->checkSellerName(trim($_GET['seller_name']), isset($_GET['id']) ? intval($_GET['id']) : 0));
        exit;
    }
    ...
    public function bind_classOp()
    {
        ...
        core\tpl::output('showpage', $model_store_bind_class->showpage());
        core\tpl::showpage('ownshop.bind_class');
    }
    /**
     * 添加经营类目
     */
