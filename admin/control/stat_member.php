...
class stat_member extends SystemControl
{
    private $links = array(array('url' => 'act=stat_member&op=newmember', 'lang' => 'stat_newmember'), array('url' => 'act=stat_member&op=analyze', 'lang' => 'stat_memberanalyze'), array('url' => 'act=stat_member&op=scale', 'lang' => 'stat_scaleanalyze'), array('url' => 'act=stat_member&op=area', 'lang' => 'stat_areaanalyze'), array('url' => 'act=stat_member&op=buying', 'lang' => 'stat_buying'));
    private $search_arr;
    //处理后的参数
    ...
    public function newmemberOp()
    {
        ...
        if ($this->search_arr['search_type'] == 'day') {
            ...
            core\tpl::output('actionurl', 'index.php?act=stat_member&op=newmember&search_type=day&search_time=' . date('Y-m-d', $this->search_arr['day']['search_time']));
        }
        if ($this->search_arr['search_type'] == 'week') {
            ...
            core\tpl::output('actionurl', 'index.php?act=stat_member&op=newmember&search_type=week&searchweek_year=' . $this->search_arr['week']['current_year'] . '&searchweek_month=' . $this->search_arr['week']['current_month'] . '&searchweek_week=' . $this->search_arr['week']['current_week']);
        }
        if ($this->search_arr['search_type'] == 'month') {
            ...
            core\tpl::output('actionurl', 'index.php?act=stat_member&op=newmember&search_type=month&searchmonth_year=' . $this->search_arr['month']['current_year'] . '&searchmonth_month=' . $this->search_arr['month']['current_month']);
        }
        //计算同比
        ...
        if (isset($_GET['exporttype']) && $_GET['exporttype'] == 'excel') {
            ...
            core\tpl::output('count_arr', $count_arr);
            core\tpl::output('top_link', $this->sublink($this->links, 'newmember'));
            core\tpl::showpage('stat.newmember');
        }
    }
    /**
     * 会员分析
    ...
    public function analyzeOp()
    {
        ...
        for ($i = 0; $i < 15; $i++) {
            $stat_ordernum_arr['series'][0]['data'][] = array('name' => isset($ordernum_listtop15[$i]['statm_membername']) ? strval($ordernum_listtop15[$i]['statm_membername']) : '', 'y' => isset($ordernum_listtop15[$i]['ordernum']) ? intval($ordernum_listtop15[$i]['ordernum']) : 0);
        }
        $stat_ordernum_arr['yAxis'] = '下单量';
        $statordernum_json = getStatData_Column2D($stat_ordernum_arr);
        unset($stat_ordernum_arr);
        core\tpl::output('statordernum_json', $statordernum_json);
        core\tpl::output('ordernum_listtop15', $ordernum_listtop15);
        //下单商品件数
        ...
        for ($i = 0; $i < 15; $i++) {
            $stat_goodsnum_arr['series'][0]['data'][] = array('name' => isset($goodsnum_listtop15[$i]['statm_membername']) ? strval($goodsnum_listtop15[$i]['statm_membername']) : '', 'y' => isset($goodsnum_listtop15[$i]['goodsnum']) ? intval($goodsnum_listtop15[$i]['goodsnum']) : 0);
        }
        $stat_goodsnum_arr['yAxis'] = '下单商品件数';
        $statgoodsnum_json = getStatData_Column2D($stat_goodsnum_arr);
        unset($stat_goodsnum_arr);
        core\tpl::output('statgoodsnum_json', $statgoodsnum_json);
        core\tpl::output('goodsnum_listtop15', $goodsnum_listtop15);
        //下单金额
        ...
        for ($i = 0; $i < 15; $i++) {
            $stat_orderamount_arr['series'][0]['data'][] = array('name' => isset($orderamount_listtop15[$i]['statm_membername']) ? strval($orderamount_listtop15[$i]['statm_membername']) : '', 'y' => isset($orderamount_listtop15[$i]['orderamount']) ? floatval($orderamount_listtop15[$i]['orderamount']) : 0);
        }
        $stat_orderamount_arr['yAxis'] = '下单金额';
        $statorderamount_json = getStatData_Column2D($stat_orderamount_arr);
        unset($stat_orderamount_arr);
        core\tpl::output('statorderamount_json', $statorderamount_json);
        core\tpl::output('orderamount_listtop15', $orderamount_listtop15);
        core\tpl::output('searchtime', implode('|', $searchtime_arr));
        core\tpl::output('top_link', $this->sublink($this->links, 'analyze'));
        core\tpl::showpage('stat.memberanalyze');
    }
    /**
     * 会员分析异步详细列表
     */
    public function analyzeinfoOp()
    {
        ...
        $curpage = ($t = (isset($_REQUEST['curpage']) && intval($_REQUEST['curpage']))) ? $t : 1;
        ...
        if (isset($_GET['exporttype']) && $_GET['exporttype'] == 'excel') {
            ...
            core\tpl::output('caption', $caption);
            core\tpl::output('stat_field', $_GET['type']);
            core\tpl::output('memberlist', $memberlist);
            core\tpl::output('show_page', $model->showpage(2));
            core\tpl::showpage('stat.memberanalyze.info', 'null_layout');
        }
    }
    /**
     * 查看会员列表
    ...
    public function showmemberOp()
    {
        ...
        $model = model('stat');
        $where = array();
        if (isset($_GET['type']) && in_array($_GET['type'], array('newbyday', 'newbyweek', 'newbymonth'))) {
        ...
        if ($this->search_arr['exporttype'] == 'excel') {
            ...
            //data
            foreach ($member_list as $k => $v) {
                $excel_data[$k + 1][] = array('data' => $v['member_name'] . '(' . lang('member_index_true_name,nc_colon') . $v['member_truename'] . ')');
                ...
                $excel_data[$k + 1][] = array('data' => lang('member_index_available,nc_colon') . $v['available_predeposit'] . lang('currency_zh') . '(' . lang('member_index_frozen,nc_colon') . $v['freeze_predeposit'] . lang('currency_zh') . ')');
            }
            $excel_data = $excel_obj->charset($excel_data, CHARSET);
            $excel_obj->addArray($excel_data);
        ...
        core\tpl::output('actionurl', $actionurl);
        core\tpl::output('member_list', $member_list);
        core\tpl::output('show_page', $model->showpage(2));
        $this->links[] = array('url' => 'act=stat_member&op=showmember', 'lang' => 'stat_memberlist');
        core\tpl::output('top_link', $this->sublink($this->links, 'showmember'));
        core\tpl::showpage('stat.info.memberlist');
    }
    /**
     * 会员规模
     */
    public function scaleOp()
    {
        ...
        $where = array();
        $where['statm_time'] = array('between', $searchtime_arr);
        if (isset($this->search_arr['membername']) && trim($this->search_arr['membername'])) {
        ...
        $orderby = (isset($this->search_arr['orderby']) ? trim($this->search_arr['orderby']) . ',' : '') . 'statm_memberid desc';
        //查询记录总条数
        $count_arr = $model->statByStatmember($where, 'COUNT(DISTINCT statm_memberid) as countnum');
        $countnum = intval($count_arr[0]['countnum']);
        if (isset($_GET['exporttype']) && $_GET['exporttype'] == 'excel') {
        ...
        core\tpl::output('top_link', $this->sublink($this->links, 'scale'));
        core\tpl::showpage('stat.memberscale');
    }
    /**
     * 区域分析
     */
    public function areaOp()
    {
        ...
        $searchtime_arr = $model->getStarttimeAndEndtime($this->search_arr);
        core\tpl::output('searchtime', implode('|', $searchtime_arr));
        core\tpl::output('top_link', $this->sublink($this->links, 'area'));
        core\tpl::showpage('stat.memberarea');
    }
    /**
     * 区域分析之详细列表
     */
    public function area_listOp()
    {
        ...
        $orderby = (isset($this->search_arr['orderby']) ? trim($this->search_arr['orderby']) . ',' : '') . 'reciver_province_id';
        ...
        core\tpl::output('statlist', $statlist);
        core\tpl::output('statheader', $statheader);
        core\tpl::output('orderby', isset($this->search_arr['orderby']) ? $this->search_arr['orderby'] : '');
        core\tpl::output('actionurl', "index.php?act={$this->search_arr['act']}&op={$this->search_arr['op']}&t={$this->search_arr['t']}");
        core\tpl::output('show_page', $model->showpage(2));
        core\tpl::output('top_link', $this->sublink($this->links, 'area'));
        core\tpl::showpage('stat.listandorder', 'null_layout');
    }
    /**
     * 区域分析之地图数据
     */
    public function area_mapOp()
    {
        ...
        core\tpl::output('stat_field', isset($_GET['type']) ? $_GET['type'] : '');
        core\tpl::output('stat_json', $stat_json);
        core\tpl::showpage('stat.map', 'null_layout');
    }
    /**
     * 购买分析
     */
    public function buyingOp()
    {
        ...
        $where['order_add_time'] = array('between', $searchtime_arr);
        $field = '1';
        $pricerange_arr = ($t = trim(core\config::get('stat_orderpricerange'))) ? unserialize($t) : '';
        if ($pricerange_arr) {
        ...
        core\tpl::output('hour_statjson', $hour_statjson);
        core\tpl::output('stattimes_arr', $stattimes_arr);
        core\tpl::output('guestprice_statjson', $guestprice_statjson);
        core\tpl::output('top_link', $this->sublink($this->links, 'buying'));
        core\tpl::showpage('stat.buying');
    }
}