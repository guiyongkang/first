...
class SystemControl
{
    ...
    {
        core\language::read('common,layout');
        /**
         * 验证用户是否登录
        ...
        if ((!empty($_GET['branch']) || $_GET['op'] == 'ajax') && strtoupper(CHARSET) == 'GBK') {
    ...
    protected final function checkPermission($link_nav = null)
    {
        ...
        $act = !empty($_GET['act']) ? $_GET['act'] : (!empty($_POST['act']) ? $_POST['act'] : '');
        ...
        dispatchJump_admin(lang('nc_assign_right'));
    }
    /**
     * 取得后台菜单
     *
    ...
    protected final function getNav($permission = '', &$top_nav, &$left_nav, &$map_nav)
    {
        ...
        foreach ($array['left'] as $k => $v) {
            ...
            foreach ($v['list'] as $key => $value) {
                ...
                $tmp_list .= str_ireplace(array('_args_', '_text_', '_opact_', '_pkey_'), array($value['args'], $value['text'], $value['op'] . $value['act']), $model_list_parent . $model_list);
            }
    ...
    {
        if (!core\config::get('sys_log') || !is_string($lang)) {
            return;
        }
        if ($admin_name == '') {
        ...
        if (is_null($state)) {
            ...
            //$state = $state ? lang('nc_succ') : lang('nc_fail');
