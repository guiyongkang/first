...
class stat_store extends SystemControl
{
    private $links = array(array('url' => 'act=stat_store&op=newstore', 'lang' => 'stat_newstore'), array('url' => 'act=stat_store&op=hotrank', 'lang' => 'stat_storehotrank'), array('url' => 'act=stat_store&op=storesales', 'lang' => 'stat_storesales'), array('url' => 'act=stat_store&op=degree', 'lang' => 'stat_storedegree'), array('url' => 'act=stat_store&op=storearea', 'lang' => 'stat_storearea'));
    private $search_arr;
    //处理后的参数
    private $store_class;
    //店铺分类
    public function __construct()
    {
        ...
        core\tpl::output('store_class', $this->store_class);
    }
    /**
     * 新增店铺
     */
    public function newstoreOp()
    {
        ...
        $where = array();
        //所属店铺分类
        $search_sclass = isset($_REQUEST['search_sclass']) ? intval($_REQUEST['search_sclass']) : '';
        if ($search_sclass > 0) {
        ...
        //搜索的时间
        core\tpl::output('search_time', $_REQUEST['search_time']);
        //周
        if (empty($_REQUEST['search_time_year'])) {
            $_REQUEST['search_time_year'] = date('Y', time());
        }
        if (empty($_REQUEST['search_time_month'])) {
            $_REQUEST['search_time_month'] = date('m', time());
        }
        if (empty($_REQUEST['search_time_week'])) {
        ...
        core\tpl::output('current_year', $current_year);
        core\tpl::output('current_month', $current_month);
        core\tpl::output('current_week', $current_week);
        core\tpl::output('year_arr', $year_arr);
        core\tpl::output('month_arr', $month_arr);
        core\tpl::output('week_arr', $week_arr);
        $model = model('stat');
        ...
        if ($_REQUEST['search_type'] == 'day') {
            ...
            core\tpl::output('actionurl', $actionurl . '&search_type=day&search_time=' . date('Y-m-d', $search_time));
        }
        if ($_REQUEST['search_type'] == 'week') {
            ...
            core\tpl::output('actionurl', $actionurl . '&search_type=week&search_time_year=' . $current_year . '&search_time_month=' . $current_month . '&search_time_week=' . $current_week);
        }
        if ($_REQUEST['search_type'] == 'month') {
            ...
            core\tpl::output('actionurl', $actionurl . '&search_type=month&search_time_year=' . $current_year . '&search_time_month=' . $current_month);
        }
        //计算同比
        ...
        if (isset($_GET['exporttype']) && $_GET['exporttype'] == 'excel') {
            ...
            core\tpl::output('top_link', $this->sublink($this->links, 'newstore'));
            core\tpl::showpage('stat.newstore');
        }
    }
    /**
     * 热卖排行
    ...
    public function hotrankOp()
    {
        ...
        core\tpl::output('top_link', $this->sublink($this->links, 'hotrank'));
        core\tpl::showpage('stat.store.hotrank');
    }
    /**
     * 热卖排行列表
     */
    public function hotrank_listOp()
    {
        ...
        core\tpl::output('soaring_statlist', $soaring_statlist);
        core\tpl::output('statlist', $statlist);
        core\tpl::output('sort_text', $sort_text);
        core\tpl::output('stat_field', $_GET['type']);
        core\tpl::showpage('stat.store.hotrank.list', 'null_layout');
    }
    /**
     * 店铺等级
     */
    public function degreeOp()
    {
        ...
        $model = model('stat');
        //查询店铺分类下的店铺
        $search_sclass = isset($_REQUEST['search_sclass']) ? intval($_REQUEST['search_sclass']) : 0;
        if ($search_sclass > 0) {
            $where['sc_id'] = $search_sclass;
        }
        ...
        if (!empty($storelist)) {
            ...
            core\tpl::output('stat_json', getStatData_Pie($data));
        }
        core\tpl::output('top_link', $this->sublink($this->links, 'degree'));
        core\tpl::showpage('stat.storedegree');
    }
    /**
     * 查看店铺列表
     */
    public function showstoreOp()
    {
        ...
        if (in_array($_GET['type'], array('newbyday', 'newbyweek', 'newbymonth', 'storearea'))) {
            $actionurl = 'index.php?act=stat_store&op=showstore&type=' . (isset($_GET['type']) ? $_GET['type'] : '') . '&t=' . (isset($this->search_arr['t']) ? $this->search_arr['t'] : '');
            $searchtime_arr_tmp = isset($this->search_arr['t']) ? explode('|', $this->search_arr['t']) : array();
        ...
        //商品分类
        $sc_id = isset($_GET['scid']) ? intval($_GET['scid']) : 0;
        if ($sc_id > 0) {
            $where['sc_id'] = $sc_id;
        ...
        core\tpl::output('top_link', $this->sublink($this->links, 'showstore'));
        core\tpl::showpage('stat.info.storelist');
    }
    /**
     * 销售统计
     */
    public function storesalesOp()
    {
        ...
        $searchtime_arr = $model->getStarttimeAndEndtime($this->search_arr);
        core\tpl::output('searchtime', implode('|', $searchtime_arr));
        core\tpl::output('top_link', $this->sublink($this->links, 'storesales'));
        core\tpl::showpage('stat.store.sales');
    }
    /**
     * 店铺销售统计列表
     */
    public function storesales_listOp()
    {
        ...
        $orderby = isset($this->search_arr['orderby']) ? trim($this->search_arr['orderby']) . ',' : '' . 'store_id asc';
        ...
        core\tpl::output('actionurl', 'index.php?act=' . $this->search_arr['act'] . '&op=' . $this->search_arr['op'] . '&t=' . $this->search_arr['t'] . '&search_sclass=' . $search_sclass . '&search_sname=' . $_GET['search_sname']);
        core\tpl::output('show_page', $model->showpage(2));
    ...
    public function storesales_trendsOp()
    {
        ...
        if ($storeid <= 0) {
            core\tpl::output('stat_error', '走势图加载错误');
            core\tpl::showpage('stat.store.salestrends');
            exit;
        }
        ...
        core\tpl::output('storename', $storename);
        core\tpl::output('stat_json', $stat_json);
        core\tpl::showpage('stat.store.salestrends', 'null_layout');
    }
    /**
     * 地区分布
     */
    public function storeareaOp()
    {
        ...
        $searchtime_arr = $model->getStarttimeAndEndtime($this->search_arr);
        $where = array();
        if (isset($_GET['search_time']) && trim($_GET['search_time'])) {
        ...
        foreach ((array) $statlist_tmp as $k => $v) {
            ...
            $v['sort'] = $k + 1;
            $v['provincename'] = ($t = (isset($province_array[$province_id]) ? $province_array[$province_id] : '')) ? $t : '其他';
        ...
        core\tpl::output('actionurl', $actionurl);
        core\tpl::output('top_link', $this->sublink($this->links, 'storearea'));
        core\tpl::showpage('stat.storearea');
    }
}