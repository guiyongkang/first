...
class store extends SystemControl
{
    const EXPORT_SIZE = 1000;
    public function __construct()
    {
        parent::__construct();
        core\language::read('store,store_grade');
    }
    /**
     * 店铺
     */
    public function storeOp()
    {
        ...
        if (isset($_GET['owner_and_name']) && trim($_GET['owner_and_name']) != '') {
            $condition['member_name'] = array('like', '%' . $_GET['owner_and_name'] . '%');
            core\tpl::output('owner_and_name', $_GET['owner_and_name']);
        }
        if (isset($_GET['store_name']) && trim($_GET['store_name']) != '') {
            $condition['store_name'] = array('like', '%' . trim($_GET['store_name']) . '%');
            core\tpl::output('store_name', $_GET['store_name']);
        }
        if (isset($_GET['grade_id']) && intval($_GET['grade_id']) > 0) {
            $condition['grade_id'] = intval($_GET['grade_id']);
            core\tpl::output('grade_id', intval($_GET['grade_id']));
        }
        ...
        core\tpl::output('search_grade_list', $search_grade_list);
        core\tpl::output('grade_list', $grade_list);
        core\tpl::output('store_list', $store_list);
        core\tpl::output('store_type', $this->_get_store_type_array());
        core\tpl::output('page', $model_store->showpage('2'));
        core\tpl::showpage('store.index');
    }
    private function _get_store_type_array()
    {
        return array('open' => '开启', 'close' => '关闭', 'expire' => '即将到期', 'expired' => '已到期');
    }
    /**
     * 店铺编辑
     */
    public function store_editOp()
    {
        ...
        if (chksubmit()) {
            ...
            if (empty($grade_array)) {
                error($lang['please_input_store_level']);
            }
            //结束时间
            $time = '';
            ...
            $update_array['store_baozh'] = trim($_POST['store_baozh']);
            //保障服务开关
            $update_array['store_baozhopen'] = trim($_POST['store_baozhopen']);
            //保证金显示开关
            $update_array['store_baozhrmb'] = trim($_POST['store_baozhrmb']);
            //新加保证金-金额
            $update_array['store_qtian'] = trim($_POST['store_qtian']);
            //保障服务-七天退换
            $update_array['store_zhping'] = trim($_POST['store_zhping']);
            //保障服务-正品保证
            $update_array['store_erxiaoshi'] = trim($_POST['store_erxiaoshi']);
            //保障服务-两小时发货
            $update_array['store_tuihuo'] = trim($_POST['store_tuihuo']);
            //保障服务-退货承诺
            $update_array['store_shiyong'] = trim($_POST['store_shiyong']);
            //保障服务-试用
            $update_array['store_xiaoxie'] = trim($_POST['store_xiaoxie']);
            //保障服务-消协
            $update_array['store_huodaofk'] = trim($_POST['store_huodaofk']);
            //保障服务-货到付款
            $update_array['store_shiti'] = trim($_POST['store_shiti']);
            //保障服务-实体店铺
            if ($update_array['store_state'] == 0) {
                ...
                $update_array['store_recommend'] = isset($_POST['store_recommend']) ? intval($_POST['store_recommend']) : 0;
            }
            ...
            if ($result) {
                ...
                $this->log(lang('nc_edit,store') . '[' . $_POST['store_name'] . ']', 1);
    ...
    public function edit_save_joininOp()
    {
        if (chksubmit()) {
            ...
            if ($result) {
                ...
                success(lang('nc_common_op_succ'), 'index.php?act=store&op=store');
            } else {
    ...
    public function store_bind_classOp()
    {
        ...
        for ($i = 0, $j = count($store_bind_class_list); $i < $j; $i++) {
            ...
            $store_bind_class_list[$i]['class_2_name'] = isset($goods_class[$store_bind_class_list[$i]['class_2']]['gc_name']) ? $goods_class[$store_bind_class_list[$i]['class_2']]['gc_name'] : '';
            $store_bind_class_list[$i]['class_3_name'] = isset($goods_class[$store_bind_class_list[$i]['class_3']]['gc_name']) ? $goods_class[$store_bind_class_list[$i]['class_3']]['gc_name'] : '';
        }
        core\tpl::output('store_bind_class_list', $store_bind_class_list);
        core\tpl::showpage('store_bind_class');
    }
    /**
     * 添加经营类目
     */
    public function store_bind_class_addOp()
    {
        ...
        $commis_rate = isset($_POST['commis_rate']) ? intval($_POST['commis_rate']) : 0;
    ...
    public function store_bind_class_updateOp()
    {
        ...
        if ($new_commis_rate < 0 || $new_commis_rate >= 100) {
            ...
            if ($result) {
                ...
                echo json_encode(array('result' => FALSE, 'message' => lang('nc_common_op_fail')));
                die;
            }
        }
    }
    ...
    public function store_joininOp()
    {
        ...
        core\tpl::output('store_list', $store_list);
        core\tpl::output('joinin_state_array', $this->get_store_joinin_state());
        //店铺等级
        $model_grade = model('store_grade');
        $grade_list = $model_grade->getGradeList();
        core\tpl::output('grade_list', $grade_list);
        core\tpl::output('page', $model_store_joinin->showpage('2'));
    ...
    public function store_bind_class_applay_listOp()
    {
        ...
        $store_ids = array();
        for ($i = 0, $j = count($store_bind_class_list); $i < $j; $i++) {
            $store_bind_class_list[$i]['class_1_name'] = isset($goods_class[$store_bind_class_list[$i]['class_1']]) ? $goods_class[$store_bind_class_list[$i]['class_1']]['gc_name'] : '';
            $store_bind_class_list[$i]['class_2_name'] = isset($goods_class[$store_bind_class_list[$i]['class_2']]) ? $goods_class[$store_bind_class_list[$i]['class_2']]['gc_name'] : '';
            $store_bind_class_list[$i]['class_3_name'] = isset($goods_class[$store_bind_class_list[$i]['class_3']]) ? $goods_class[$store_bind_class_list[$i]['class_3']]['gc_name'] : '';
        ...
        core\tpl::output('bind_list', $store_bind_class_list);
        core\tpl::output('bind_store_list', $bind_store_list);
        core\tpl::output('page', $model_store_bind_class->showpage('2'));
        core\tpl::showpage('store_bind_class_applay.list');
    }
    /**
     * 审核经营类目申请
     */
    ...
    public function store_bind_class_applay_delOp()
    {
        ...
        if ($del) {
            ...
            success('删除成功', getReferer());
        } else {
            error('删除失败', getReferer());
        }
    }
    private function get_store_joinin_state()
    {
        $joinin_state_array = array(STORE_JOIN_STATE_NEW => '新申请', STORE_JOIN_STATE_PAY => '已付款', STORE_JOIN_STATE_VERIFY_SUCCESS => '待付款', STORE_JOIN_STATE_VERIFY_FAIL => '审核失败', STORE_JOIN_STATE_PAY_FAIL => '付款审核失败', STORE_JOIN_STATE_FINAL => '开店成功');
    ...
    public function reopen_listOp()
    {
        ...
        core\tpl::output('reopen_list', $reopen_list);
        core\tpl::output('page', $model_store_reopen->showpage('2'));
        core\tpl::showpage('store_reopen.list');
    }
    /**
     * 审核店铺续签申请
     */
    ...
    {
        if (!isset($_GET['re_id']) || intval($_GET['re_id']) <= 0) {
            exit;
        }
        $model_store_reopen = model('store_reopen');
        $condition = array();
        ...
        if ($update) {
            ...
            $this->log($msg, 1);
            success('续签成功，店铺有效成功延续到了' . date('Y-m-d', $data['re_end_time']) . '日', getReferer());
        } else {
            error('审核失败', getReferer());
        }
    }
    ...
    public function store_joinin_detailOp()
    {
        ...
        core\tpl::output('joinin_detail_title', $joinin_detail_title);
        core\tpl::output('joinin_detail', $joinin_detail);
        core\tpl::showpage('store_joinin.detail');
    }
    /**
     * 审核
     */
    ...
    private function store_joinin_verify_pass($joinin_detail)
    {
        ...
        if ($param['paying_amount'] > 0) {
            success('店铺入驻申请审核完成', 'index.php?act=store&op=store_joinin');
        } else {
    ...
    private function store_joinin_verify_open($joinin_detail)
    {
        ...
        if ($_POST['verify_type'] === 'pass') {
            ...
            if ($state) {
                ...
                $album_arr = array();
                $album_arr['aclass_name'] = core\language::get('store_save_defaultalbumclass_name');
                ...
                $msg = core\language::get('store_save_create_success');
                //插入店铺绑定分类表
                ...
                for ($i = 0, $length = count($store_bind_class); $i < $length; $i++) {
                    ...
                    $store_bind_class_array[] = array('store_id' => $store_id, 'commis_rate' => $store_bind_commis_rates[$i], 'class_1' => $class1, 'class_2' => $class2, 'class_3' => $class3, 'state' => 1);
                }
                $model_store_bind_class = model('store_bind_class');
                $model_store_bind_class->addStoreBindClassAll($store_bind_class_array);
                success('店铺开店成功', 'index.php?act=store&op=store_joinin');
            } else {
                error('店铺开店失败', 'index.php?act=store&op=store_joinin');
            }
        } else {
            error('店铺开店拒绝', 'index.php?act=store&op=store_joinin');
        }
    }
    /**
     * 提醒续费
    ...
    public function newshop_addOp()
    {
        if (chksubmit()) {
            ...
            if (!$this->checkSellerName($_POST['seller_name'])) {
                error('店主卖家账号名称已被其它店铺占用');
            }
            try {
                $memberId = model('member')->addMember(array('member_name' => $memberName, 'member_passwd' => $memberPasswd, 'member_email' => ''));
            } catch (\Exception $ex) {
                error('店主账号新增失败');
            }
            $storeModel = model('store');
            ...
            model('seller')->addSeller(array('seller_name' => $_POST['seller_name'], 'member_id' => $memberId, 'store_id' => $storeId, 'seller_group_id' => 0, 'is_admin' => 1));
            model('store_joinin')->save(array('seller_name' => $_POST['seller_name'], 'store_name' => $_POST['store_name'], 'member_name' => $memberName, 'member_id' => $memberId, 'joinin_state' => 40, 'company_province_id' => 0, 'sc_bail' => 0, 'joinin_year' => 1));
            // 添加相册默认
            $album_model = model('album');
            $album_arr = array();
            ...
            success('操作成功', urlAdmin('store', 'store'));
            return;
        }
        core\tpl::showpage('store.newshop.add');
    }
    ...
    private function checkSellerName($sellerName, $storeId = 0)
    {
        ...
        $count = (int) model('store_joinin')->getStoreJoininCount(array('seller_name' => $sellerName));
        if ($count > 0) {
            return false;
        }
        $seller = model('seller')->getSellerInfo(array('seller_name' => $sellerName));
        if (empty($seller)) {
            return true;
        }
        if (!$storeId) {
    ...
    {
        // 判断store_joinin是否存在记录
        $count = (int) model('store_joinin')->getStoreJoininCount(array('member_name' => $memberName));
        if ($count > 0) {
            return false;
        }
        return !model('member')->getMemberCount(array('member_name' => $memberName));
    }
    /**
     * 验证店铺名称是否存在
     */
    ...
    public function store_import_csvOp()
    {
        if (isset($_POST['import'])) {
            ...
            for ($i = 1; $i < $rows; $i++) {
                ...
                try {
                    $memberId = model('member')->addMember(array('member_name' => $member_name, 'member_passwd' => $password, 'member_email' => ''));
                } catch (\Exception $ex) {
                    error('店主账号新增失败');
                }
                $storeModel = model('store');
                ...
                $storeId = $storeModel->addStore($saveArray);
                model('seller')->addSeller(array('seller_name' => $seller_name, 'member_id' => $memberId, 'store_id' => $storeId, 'seller_group_id' => 0, 'is_admin' => 1));
            ...
            fclose($handle);
            //关闭指针
            success('操作成功,成功导入 ' . strval($scounter) . ' 条数据', urlAdmin('store', 'store'));
            return;
            /*
            $row = 0;    		
