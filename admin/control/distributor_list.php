	...
	public function showdetailOp(){
		...
		switch($type){
			...
			case 'com':
				...
				$result = $model_dis->getInfoOne('distributor_goodsrecord_detail',array('member_id'=>$aid,'points_status'=>0),'sum(detail_bonus) as money');
				$html .= $result['money'] ? $result['money'] : '0';
				$html .= ' 元</td></tr>';
				...
				//获取全返总额
				$html .= '<tr><td style="height:40px; width:150px; line-height:40px; font-size:16px;">全返获得总额</td><td style="height:40px; line-height:40px; font-size:16px; color:red">&yen; ';
				$result = $model_dis->getInfoOne('distributor_goodsrecord_detail',array('member_id'=>$aid,'points_status'=>1),'sum(detail_bonus) as money');
				$html .= $result['money'] ? $result['money'] : '0';
				$html .= ' 元</td></tr>';
