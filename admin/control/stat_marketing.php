...
class stat_marketing extends SystemControl
{
    private $links = array(array('url' => 'act=stat_marketing&op=promotion', 'lang' => 'stat_promotion'), array('url' => 'act=stat_marketing&op=group', 'lang' => 'stat_group'));
    private $search_arr;
    //处理后的参数
    ...
    public function promotionOp()
    {
        ...
        core\tpl::output('statcount', $statcount);
        core\tpl::output('statlist', $statlist);
        core\tpl::output('stat_json', $stat_json);
        core\tpl::output('searchtime', implode('|', $searchtime_arr));
        core\tpl::output('top_link', $this->sublink($this->links, 'promotion'));
        core\tpl::showpage('stat.marketing.promotion');
    }
    /**
     * 促销销售趋势分析
     */
    ...
    public function groupOp()
    {
        ...
        $searchtime_arr = $model->getStarttimeAndEndtime($this->search_arr);
        core\tpl::output('searchtime', implode('|', $searchtime_arr));
        core\tpl::output('top_link', $this->sublink($this->links, 'group'));
        core\tpl::showpage('stat.marketing.group');
    }
    /**
     * 抢购统计
     */
    public function grouplistOp()
    {
        ...
        $gname = isset($_GET['gname']) ? trim($_GET['gname']) : '';
        if ($gname) {
        ...
        core\tpl::output('grouplist', $grouplist);
        core\tpl::output('show_page', $model->showpage(2));
        core\tpl::output('searchtime', $_GET['t']);
        core\tpl::showpage('stat.marketing.grouplist', 'null_layout');
    }
    /**
     * 抢购商品统计
     */
    public function groupgoodsOp()
    {
        ...
        $orderby = (isset($this->search_arr['orderby']) ? trim($this->search_arr['orderby']) . ',' : '') . 'goods_id desc';
        //统计记录总条数
