...
class live_order extends SystemControl
{
    ...
    public function live_orderOp()
    {
        ...
        core\tpl::output('show_page', $model_live_order->showpage(2));
        //显示支付接口列表(搜索)
        $payment_list = model('payment')->getPaymentOpenList();
        core\tpl::output('payment_list', $payment_list);
        core\tpl::showpage('live_order.list');
    }
    /*
     * 订单详情
     */
    public function order_detailOp()
    {
        ...
        core\tpl::output('live_order', $live_order[0]);
        $pwd_condition = array();
        $pwd_condition['order_id'] = $_GET['order_id'];
        $live_order_pwd = $model_live_order->getLiveOrderPwd($pwd_condition);
        core\tpl::output('live_order_pwd', $live_order_pwd);
        core\tpl::showpage('live_order.detail');
    }
    /*
     * 结算订单
     */
    public function live_area_accountOp()
    {
        ...
        $condition['live_order.state'] = array('in', array(2, 3));
        //2.已支付 3.已消费
        ...
        core\tpl::output('list', $list);
        core\tpl::output('show_page', $model_live_order->showpage(2));
        core\tpl::showpage('live_order.account');
    }
    /*
     * 订单删除
     */
