...
class goodsr extends SystemControl
{
    ...
    public function getfanyongOp()
    {
        ...
        if (!empty($_GET['buyer_name'])) {
            ...
            $member_info = $obj_member->getMemberInfo(array('member_name' => $_GET['buyer_name']));
            if($member_info)
        ...
        if ($start_unixtime || $end_unixtime) {
            $condition['points_time'] = array('time', array($start_unixtime, $end_unixtime));
        }
        
        $order_list = $model_points_fanyong->getfanyongList($condition, 30);
      
        //显示支付接口列表(搜索)
      //  $order_list = model('payment')->getPaymentOpenList();     
        core\tpl::output('order_list', $order_list);
        core\tpl::output('show_page', $model_points_fanyong->showpage());
      //  core\tpl::showpage('order.index');
        core\tpl::showpage('goodsr.fanyong');
    }
    /**
     * 商品管理
     */
    public function goodsrOp()
    {
