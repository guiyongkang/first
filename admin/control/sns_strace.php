...
class sns_strace extends SystemControl
{
    public function __construct()
    {
        parent::__construct();
        core\language::read('snstrace,sns_strace');
    }
    /**
     * 动态列表
     */
    public function stracelistOp()
    {
        ...
        if (!empty($_GET['search_stime']) || !empty($_GET['search_etime'])) {
                ...
                if ($val['strace_content'] == '') {
                    ...
                    if (CHARSET == 'GBK') {
                        foreach ((array) $data as $k => $v) {
                            $data[$k] = core\language::getGBK($v);
                        }
                    }
                    $content = $model_stracelog->spellingStyle($val['strace_type'], $data);
                    $strace_list[$key]['strace_content'] = str_replace("%siteurl%", APP_URL . DS, $content);
                }
            }
        }
        core\tpl::output('show_page', $model_stracelog->showpage(2));
        core\tpl::output('strace_list', $strace_list);
        core\tpl::showpage('sns_strace.index');
    }
    /**
     * 删除动态
     */
    ...
    public function scomm_listOp()
    {
        // where 条件
        $where = array();
        $st_id = isset($_GET['st_id']) ? intval($_GET['st_id']) : 0;
        if ($st_id > 0) {
        ...
        core\tpl::output('show_page', $model_storesnscomment->showpage(2));
        core\tpl::output('scomm_list', $scomm_list);
        core\tpl::showpage('sns_scomment.index');
    }
    /**
     * 删除评论
     */
