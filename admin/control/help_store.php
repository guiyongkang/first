...
class help_store extends SystemControl
{
    ...
    public function help_storeOp()
    {
        ...
        core\tpl::output('show_page', $model_help->showpage());
        $type_list = $model_help->getStoreHelpTypeList();
        core\tpl::output('type_list', $type_list);
        core\tpl::showpage('help_store.list');
    }
    /**
     * 帮助类型
     */
    public function help_typeOp()
    {
        ...
        core\tpl::output('type_list', $type_list);
        core\tpl::output('show_page', $model_help->showpage());
        core\tpl::showpage('help_store_type.list');
    }
    /**
     * 新增帮助
     *
    ...
    public function add_helpOp()
    {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                success(core\language::get('nc_common_save_succ'), 'index.php?act=help_store&op=help_store');
            } else {
        ...
        core\tpl::showpage('help_store.add');
    }
    /**
     * 编辑帮助
     *
    ...
    public function edit_helpOp()
    {
        ...
        $pic_list = $model_help->getHelpPicList($condition);
        core\tpl::output('pic_list', $pic_list);
        core\tpl::showpage('help_store.edit');
    }
    /**
     * 删除帮助
     *
    ...
    public function del_helpOp()
    {
        ...
        if ($state) {
            ...
            success(core\language::get('nc_common_del_succ'), 'index.php?act=help_store&op=help_store');
        } else {
    ...
    public function add_typeOp()
    {
        ...
        if (chksubmit()) {
            ...
            $type_array['help_show'] = intval($_POST['help_show']);
            //是否显示,0为否,1为是
            $type_array['page_show'] = '1';
            //页面类型:1为店铺,2为会员
            $state = $model_help->addHelpType($type_array);
            if ($state) {
                $this->log('新增店铺帮助类型，编号' . $state);
                success(core\language::get('nc_common_save_succ'), 'index.php?act=help_store&op=help_type');
            } else {
                error(core\language::get('nc_common_save_fail'));
            }
        }
        core\tpl::showpage('help_store_type.add');
    }
    /**
     * 编辑帮助类型
     *
    ...
    public function edit_typeOp()
    {
        ...
        core\tpl::output('type', $type);
        core\tpl::showpage('help_store_type.edit');
    }
    /**
     * 删除帮助类型
     *
    ...
    public function del_typeOp()
    {
        ...
        if ($state) {
            ...
            success(core\language::get('nc_common_del_succ'), 'index.php?act=help_store&op=help_type');
        } else {
            error(core\language::get('nc_common_del_fail'));
        }
    }
