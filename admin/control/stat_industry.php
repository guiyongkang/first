...
class stat_industry extends SystemControl
{
    private $links = array(array('url' => 'act=stat_industry&op=scale', 'lang' => 'stat_industryscale'), array('url' => 'act=stat_industry&op=rank', 'lang' => 'stat_industryrank'), array('url' => 'act=stat_industry&op=price', 'lang' => 'stat_industryprice'), array('url' => 'act=stat_industry&op=general', 'lang' => 'stat_industrygeneral'));
    private $search_arr;
    //处理后的参数
    private $gc_arr;
    //分类数组
    ...
    public function __construct()
    {
        ...
        $show_depth = 3;
        //select需要展示的深度
        if (in_array($this->search_arr['op'], array('scale', 'general'))) {
            //仅显示前两级分类
            $show_depth = 2;
        }
        $this->choose_gcid = ($t = (isset($_REQUEST['choose_gcid']) && intval($_REQUEST['choose_gcid']))) > 0 ? $t : 0;
    ...
    public function scaleOp()
    {
        ...
        core\tpl::showpage('stat.industry.scale');
    }
    /**
     * 行业规模列表
     */
    public function scale_listOp()
    {
        ...
        if ($gc_childarr) {
            ...
            core\tpl::output('stat_json', $statjson);
            core\tpl::output('stattype', $_GET['stattype']);
            core\tpl::showpage('stat.linelabels', 'null_layout');
        }
    }
    /**
     * 行业排行
    ...
    public function rankOp()
    {
        ...
        //计入统计的有效订单
        $where['order_add_time'] = array('between', $searchtime_arr);
        $gc_id_depth = isset($this->gc_arr[$this->choose_gcid]['depth']) ? $this->gc_arr[$this->choose_gcid]['depth'] : 0;
        if ($this->choose_gcid > 0) {
        ...
        core\tpl::output('goods_statjson', $goods_statjson);
        core\tpl::output('goods_list', $goods_list);
        core\tpl::output('store_statjson', $store_statjson);
        core\tpl::output('store_list', $store_list);
        core\tpl::output('top_link', $this->sublink($this->links, 'rank'));
        core\tpl::showpage('stat.industry.rank');
    }
    /**
     * 价格分布
     */
    public function priceOp()
    {
        ...
        core\tpl::output('goodsnum_stat_json', $goodsnum_stat_json);
        core\tpl::output('orderamount_stat_json', $orderamount_stat_json);
        core\tpl::output('top_link', $this->sublink($this->links, 'price'));
        core\tpl::showpage('stat.industry.price');
    }
    /**
     * 销售统计
     */
    public function generalOp()
    {
        ...
        core\tpl::showpage('stat.industry.general');
    }
    /**
     * 概况总览
     */
    public function general_listOp()
    {
        ...
        if ($gc_childarr) {
            ...
            foreach ($gc_childarr as $k => $v) {
                ...
                $tmp['ordergcount'] = ($t = (isset($ordergoods_list[$v['gc_id']]['ordergcount']) ? $ordergoods_list[$v['gc_id']]['ordergcount'] : 0)) ? $t : 0;
                $tmp['ordergnum'] = ($t = (isset($ordergoods_list[$v['gc_id']]['ordergnum']) ? $ordergoods_list[$v['gc_id']]['ordergnum'] : 0)) ? $t : 0;
                $tmp['orderamount'] = ($t = (isset($ordergoods_list[$v['gc_id']]['orderamount']) ? $ordergoods_list[$v['gc_id']]['orderamount'] : 0)) ? $t : 0;
                $tmp['goodscount'] = ($t = (isset($goods_list[$v['gc_id']]['goodscount']) ? $goods_list[$v['gc_id']]['goodscount'] : 0)) ? $t : 0;
                $tmp['priceavg'] = ncPriceFormat(($t = (isset($goods_list[$v['gc_id']]['priceavg']) ? $goods_list[$v['gc_id']]['priceavg'] : 0)) ? $t : 0);
                $tmp['unordergcount'] = (isset($goods_list[$v['gc_id']]['goodscount']) ? intval($goods_list[$v['gc_id']]['goodscount']) : 0) - (isset($ordergoods_list[$v['gc_id']]['ordergcount']) ? intval($ordergoods_list[$v['gc_id']]['ordergcount']) : 0);
                //计算无销量商品数
                $statlist_tmp[] = $tmp;
            }
            $statlist = array();
        ...
        core\tpl::output('orderby', $orderby);
        core\tpl::output('actionurl', "index.php?act={$this->search_arr['act']}&op={$this->search_arr['op']}&choose_gcid=" . $this->choose_gcid);
