...
class bill extends SystemControl
{
    /**
     * 每次导出订单数量
     * @var int
    ...
    private $links = array(array('url' => 'act=bill&op=index', 'lang' => 'nc_manage'));
    ...
    public function indexOp()
    {
        ...
        $condition = array();
        if (isset($_GET['query_year']) && preg_match('/^\\d{4}$/', $_GET['query_year'], $match)) {
            $condition['os_year'] = $_GET['query_year'];
        }
        $model_bill = model('bill');
        $bill_list = $model_bill->getOrderStatisList($condition, '*', 12, 'os_month desc');
        core\tpl::output('bill_list', $bill_list);
        core\tpl::output('show_page', $model_bill->showpage());
        //输出子菜单
        core\tpl::output('top_link', $this->sublink($this->links, 'index'));
        core\tpl::showpage('bill_order_statis.index');
    }
    /**
     * 某月所有店铺销量账单
     *
    ...
    public function show_statisOp()
    {
        ...
        if (isset($_GET['bill_state']) && is_numeric($_GET['bill_state'])) {
            $condition['ob_state'] = intval($_GET['bill_state']);
        }
        if (isset($_GET['query_store']) && preg_match('/^\\d{1,8}$/', $_GET['query_store'])) {
            $condition['ob_store_id'] = $_GET['query_store'];
        } elseif (!empty($_GET['query_store'])) {
        ...
        //输出子菜单
        core\tpl::output('top_link', $this->sublink($this->links, 'index'));
        core\tpl::showpage('bill_order_statis.show');
    }
    /**
     * 某店铺某月订单列表
     *
    ...
    {
        if (isset($_GET['ob_no']) && !preg_match('/^20\\d{5,12}$/', $_GET['ob_no'], $match)) {
            error('参数错误');
        }
        $model_bill = model('bill');
        ...
        $if_start_date = isset($_GET['query_start_date']) ? preg_match('/^20\\d{2}-\\d{2}-\\d{2}$/', $_GET['query_start_date']) : '';
        $if_end_date = isset($_GET['query_end_date']) ? preg_match('/^20\\d{2}-\\d{2}-\\d{2}$/', $_GET['query_end_date']) : '';
        ...
        if (isset($_GET['query_type']) && $_GET['query_type'] == 'refund') {
            //退款订单列表
        ...
        } elseif (isset($_GET['query_type']) && $_GET['query_type'] == 'cost') {
            //店铺费用
            ...
            core\tpl::output('cost_list', $store_cost_list);
            core\tpl::output('store_info', $store_info);
            core\tpl::output('show_page', $model_store_cost->showpage());
            ...
            core\tpl::output('commis_list', $commis_list);
        ...
        core\tpl::output('tpl_name', $sub_tpl_name);
        core\tpl::output('bill_info', $bill_info);
        core\tpl::showpage('bill_order_bill.show');
    }
    ...
    public function bill_payOp()
    {
        ...
        if (chksubmit()) {
            ...
            if ($update) {
                ...
                success('保存成功', 'index.php?act=bill&op=show_statis&os_month=' . $bill_info['os_month']);
            } else {
                $this->log('账单付款,账单号：' . $_GET['ob_no'], 1);
                error('保存失败');
            }
        } else {
            core\tpl::showpage('bill.pay');
        }
    }
    /**
     * 打印结算单
    ...
    public function bill_printOp()
    {
        ...
        core\tpl::output('bill_info', $bill_info);
        core\tpl::showpage('bill.print', 'null_layout');
    }
    /**
     * 导出平台月出账单表
     *
    ...
    public function export_billOp()
    {
        ...
        if (isset($_GET['curpage']) && !is_numeric($_GET['curpage'])) {
            ...
            if ($count > self::EXPORT_SIZE) {
                ...
                core\tpl::output('murl', 'index.php?act=bill&op=index');
        ...
        $csv->filename = $csv->charset('账单-', CHARSET) . (isset($_GET['os_month']) ? $_GET['os_month'] : '');
    ...
    public function export_orderOp()
    {
        ...
        if (isset($_GET['curpage']) && !is_numeric($_GET['curpage'])) {
            ...
            if ($count > self::EXPORT_SIZE) {
                ...
                core\tpl::output('list', $array);
                core\tpl::output('murl', 'index.php?act=bill&op=show_bill&ob_no=' . $_GET['ob_no']);
                core\tpl::showpage('export.excel');
                exit;
            } else {
                //如果数量小，直接下载
    ...
    public function export_refund_orderOp()
    {
        ...
        if (isset($_GET['curpage']) && !is_numeric($_GET['curpage'])) {
            ...
            if ($count > self::EXPORT_SIZE) {
                ...
                core\tpl::output('list', $array);
                core\tpl::output('murl', 'index.php?act=bill&op=show_bill&query_type=refund&ob_no=' . (isset($_GET['ob_no']) ? $_GET['ob_no'] : ''));
                core\tpl::showpage('export.excel');
                exit;
            } else {
                //如果数量小，直接下载
