...
class cms_article extends SystemControl
{
    //文章状态草稿箱
    ...
    private function get_cms_article_list($condition, $menu_key)
    {
        ...
        core\tpl::output('show_page', $model_article->showpage(2));
        core\tpl::output('list', $article_list);
        core\tpl::output('article_state_list', $this->get_article_state_list());
        core\tpl::showpage("cms_article.list");
    }
    /**
     * cms文章审核
     */
    ...
    public function cms_article_dropOp()
    {
        ...
        if ($result) {
            $this->log(core\language::get('cms_log_article_drop') . $article_id, 1);
            success(core\language::get('nc_common_del_succ'));
        } else {
            $this->log(core\language::get('cms_log_article_drop') . $article_id, 0);
            error(core\language::get('nc_common_del_fail'), '', '', 'error');
        }
    }
    /**
     * ajax操作
    ...
    {
        $array = array();
        $array[self::ARTICLE_STATE_DRAFT] = array('text' => core\language::get('cms_text_draft'));
        $array[self::ARTICLE_STATE_VERIFY] = array('text' => core\language::get('cms_text_verify'));
        $array[self::ARTICLE_STATE_PUBLISHED] = array('text' => core\language::get('cms_text_published'));
        $array[self::ARTICLE_STATE_RECYCLE] = array('text' => core\language::get('cms_text_recycle'));
        return $array;
    }
    private function show_menu($menu_key)
    {
        $menu_array = array('list' => array('menu_type' => 'link', 'menu_name' => core\language::get('nc_list'), 'menu_url' => 'index.php?act=cms_article&op=cms_article_list'), 'list_verify' => array('menu_type' => 'link', 'menu_name' => core\language::get('cms_article_list_verify'), 'menu_url' => 'index.php?act=cms_article&op=cms_article_list_verify'), 'list_published' => array('menu_type' => 'link', 'menu_name' => core\language::get('cms_article_list_published'), 'menu_url' => 'index.php?act=cms_article&op=cms_article_list_published'));
