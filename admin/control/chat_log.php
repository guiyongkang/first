...
class chat_log extends SystemControl
{
    ...
    public function chat_logOp()
    {
        ...
        if (!empty($f_name)) {
            ...
            core\tpl::output('f_member', $f_member);
        }
        ...
        if (!empty($t_name)) {
            ...
            core\tpl::output('t_member', $t_member);
        }
        if (isset($f_member['member_id']) && $f_member['member_id'] > 0 && $t_member['member_id'] > 0) {
            //验证账号
            $condition = array();
            $condition['add_time_from'] = isset($_GET['add_time_from']) ? trim($_GET['add_time_from']) : '';
            $condition['add_time_to'] = isset($_GET['add_time_to']) ? trim($_GET['add_time_to']) : '';
        ...
        core\tpl::showpage('chat_log.list');
    }
    /**
     * 聊天内容查询
     */
    public function msg_logOp()
    {
        if (!empty($_GET['msg'])) {
            $model_chat = model('web_chat');
            $condition = array();
            $add_time_from = isset($_GET['add_time_from']) ? strtotime($_GET['add_time_from']) : '';
            $add_time_to = isset($_GET['add_time_to']) ? strtotime($_GET['add_time_to']) : '';
        ...
        core\tpl::showpage('chat_msg_log.list');
    }
}