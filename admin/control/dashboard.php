...
class dashboard extends SystemControl
{
    public function __construct()
    {
        parent::__construct();
        core\language::read('dashboard');
    }
    /**
     * 欢迎页面
     */
    public function welcomeOp()
    {
        ...
        $setup_date = core\config::get('setup_date');
        $statistics['os'] = PHP_OS;
        $statistics['web_server'] = $_SERVER['SERVER_SOFTWARE'];
        $statistics['php_version'] = PHP_VERSION;
        $statistics['sql_version'] = \db\mysqli::getServerInfo();
        $statistics['shop_version'] = $version;
        $statistics['setup_date'] = substr($setup_date, 0, 10);
        core\tpl::output('statistics', $statistics);
        core\tpl::output('admin_info', $admin_info);
        core\tpl::showpage('welcome');
    }
    /**
     * 关于我们
     */
