...
class upload extends SystemControl
{
    private $links = array(array('url' => 'act=upload&op=param', 'lang' => 'upload_param'), array('url' => 'act=upload&op=default_thumb', 'lang' => 'default_thumb'), /*array('url' => 'act=upload&op=login', 'lang' => 'loginSettings'),*/ array('url' => 'act=upload&op=tool', 'lang' => 'image_thumb_tool'), array('url' => 'act=upload&op=font', 'lang' => 'font_set'));
    ...
    public function paramOp()
    {
        if (chksubmit()) {
            ...
            $obj_validate->validateparam = array(array("input" => $_POST["image_max_filesize"], "require" => "true", "validator" => "Number", "message" => lang('upload_image_filesize_is_number')), array("input" => trim($_POST["image_allow_ext"]), "require" => "true", "message" => lang('image_allow_ext_not_null')));
            ...
            if ($error != '') {
                ...
                if ($result) {
                    $this->log(lang('nc_edit,upload_param'), 1);
                    success(lang('nc_common_save_succ'));
                } else {
                    $this->log(lang('nc_edit,upload_param'), 0);
        ...
        //输出子菜单
        core\tpl::output('top_link', $this->sublink($this->links, 'param'));
        core\tpl::showpage('upload.param');
    }
    /**
     * 默认图设置
     */
    public function default_thumbOp()
    {
        ...
        if (chksubmit()) {
            ...
            if ($result === true) {
                ...
                $this->log(lang('nc_edit,default_thumb'), 1);
                success(lang('nc_common_save_succ'));
            } else {
                $this->log(lang('nc_edit,default_thumb'), 0);
                error(lang('nc_common_save_fail'));
            }
        }
        $list_setting = $model_setting->getListSetting();
        ...
        core\tpl::output('list_setting', $list_setting);
        //输出子菜单
        core\tpl::output('top_link', $this->sublink($this->links, 'default_thumb'));
        core\tpl::showpage('upload.thumb');
    }
    /**
     * 登录主题图片
     */
    public function loginOp()
    {
        ...
        if (chksubmit()) {
            ...
            $result = $model_setting->updateSetting($update_array);
            if ($result === true) {
                $this->log(lang('nc_edit,loginSettings'), 1);
                success(lang('nc_common_save_succ'));
            } else {
                $this->log(lang('nc_edit,loginSettings'), 0);
                error(lang('nc_common_save_fail'));
            }
        }
        $list_setting = $model_setting->getListSetting();
        ...
        core\tpl::output('top_link', $this->sublink($this->links, 'login'));
        core\tpl::showpage('upload.login');
    }
    /**
     * 水印字体
     *
    ...
    public function fontOp()
    {
        ...
        core\tpl::output('top_link', $this->sublink($this->links, 'font'));
        core\tpl::showpage('upload.font');
    }
    /**
     * 压缩工具
     *
    ...
    {
        core\tpl::output('top_link', $this->sublink($this->links, 'tool'));
        core\tpl::showpage('upload.tool');
    }
}