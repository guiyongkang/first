...
class stat_trade extends SystemControl
{
    private $links = array(array('url' => 'act=stat_trade&op=income', 'lang' => 'stat_sale_income'), array('url' => 'act=stat_trade&op=predeposit', 'lang' => 'stat_predeposit'), array('url' => 'act=stat_trade&op=sale', 'lang' => 'stat_sale'));
    private $search_arr;
    //处理后的参数
    ...
    {
        $model = model('stat');
        if (empty($_GET['search_year']) || empty($_GET['search_month'])) {
            ...
            if ($now_month == 1) {
                ...
                /*if ($now_month > 10) {
                    $_GET['search_month'] = $now_month - 1;
                } else {
                    $_GET['search_month'] = '0' . ($now_month - 1);
                }*/
            }
        }
        $year = isset($_GET['search_year']) ? intval($_GET['search_year']) : '';
        $month = isset($_GET['search_month']) ? trim($_GET['search_month']) : '';
        ...
        if (isset($_GET['exporttype']) && $_GET['exporttype'] == 'excel') {
            ...
            core\tpl::output('plat_data', $plat_data[0]);
            //店铺数据
            core\tpl::output('store_list', $model->getBillList($condition, 'ob'));
            core\tpl::output('show_page', $model->showpage());
            core\tpl::output('top_link', $this->sublink($this->links, 'income'));
            core\tpl::showpage('stat.income');
        }
    }
    /**
     * 预存款统计
    ...
    {
        $where = array();
        if (isset($_GET['pd_type']) && trim($_GET['pd_type']) == 'cash_pay') {
        ...
        $where['lg_type'] = empty($_GET['pd_type']) ? 'recharge' : trim($_GET['pd_type']);
        //初始化时间
        //天
        ...
        if ($_REQUEST['search_type'] == 'day') {
            ...
            core\tpl::output('actionurl', 'index.php?act=stat_trade&op=predeposit&search_type=day&search_time=' . date('Y-m-d', $search_time));
        }
        if ($_REQUEST['search_type'] == 'week') {
            ...
            core\tpl::output('actionurl', 'index.php?act=stat_trade&op=predeposit&search_type=week&search_time_year=' . $current_year . '&search_time_month=' . $current_month . '&search_time_week=' . $current_week);
        }
        ...
        if (isset($_GET['exporttype']) && $_GET['exporttype'] == 'excel') {
            ...
            core\tpl::output('stat_array', array('recharge_amount' => $recharge_amount[0]['allnum'], 'order_amount' => abs($order_amount[0]['allnum']), 'cash_amount' => abs($cash_amount[0]['allnum'])));
            $user_amount = $model->getPredepositInfo(true, 'distinct lg_member_id');
            core\tpl::output('user_amount', count($user_amount));
            $usable_amount = $model->getPredepositInfo(true, 'sum(lg_av_amount+lg_freeze_amount) as allnum');
            core\tpl::output('usable_amount', $usable_amount[0]['allnum']);
            //得到统计图数据
            ...
            core\tpl::output('top_link', $this->sublink($this->links, 'predeposit'));
            core\tpl::showpage('stat.predeposit');
        }
    }
    /**
     * 订单统计
    ...
    public function saleOp()
    {
        ...
        if (isset($_GET['exporttype']) && $_GET['exporttype'] == 'excel') {
            ...
            core\tpl::output('statcount_arr', $statcount_arr);
            core\tpl::output('searchtime', implode('|', array($stime, $etime)));
            core\tpl::output('statlist', $statlist);
            core\tpl::output('show_page', $model->showpage());
            core\tpl::output('top_link', $this->sublink($this->links, 'sale'));
            core\tpl::showpage('stat.sale');
        }
    }
    /**
     * 订单走势
    ...
    public function sale_trendOp()
    {
        ...
        $stattype = isset($_GET['type']) ? trim($_GET['type']) : '';
        ...
        $stat_json = getStatData_LineLabels($stat_arr);
        core\tpl::output('stat_json', $stat_json);
        core\tpl::output('stattype', $stattype);
