...
class pointprod extends SystemControl
{
    public function __construct()
    {
        parent::__construct();
        core\language::read('pointprod');
    }
    /**
     * 积分礼品列表
     */
    public function pointprodOp()
    {
        ...
        //条件
        $where = array();
        $pgoods_name = isset($_GET['pg_name']) ? trim($_GET['pg_name']) : '';
        if ($pgoods_name) {
            $where['pgoods_name'] = array('like', '%' . $pgoods_name . '%');
        }
		$_GET['pg_state'] = isset($_GET['pg_state']) ? trim($_GET['pg_state']) : '';
        switch ($_GET['pg_state']) {
            case 'show':
        ...
        core\tpl::output('show_page', $pointprod_model->showpage());
        core\tpl::showpage('pointprod.list');
    }
    /**
     * 积分礼品添加
     */
    public function prod_addOp()
    {
        ...
        if (chksubmit()) {
            ...
            $validate_arr[] = array("input" => $_POST["goodsname"], "require" => "true", "message" => lang('admin_pointprod_add_goodsname_error'));
            $validate_arr[] = array("input" => $_POST["goodsprice"], "require" => "true", "validator" => "DoublePositive", "message" => lang('admin_pointprod_add_goodsprice_number_error'));
            $validate_arr[] = array('input' => $_POST['goodspoints'], 'require' => 'true', 'validator' => 'IntegerPositive', 'message' => lang('admin_pointprod_add_goodspoint_number_error'));
            $validate_arr[] = array('input' => $_POST['goodsserial'], 'require' => 'true', 'message' => lang('admin_pointprod_add_goodsserial_null_error'));
            $validate_arr[] = array('input' => $_POST['goodsstorage'], 'require' => 'true', 'validator' => 'IntegerPositive', 'message' => lang('admin_pointprod_add_storage_number_error'));
            $validate_arr[] = array('input' => $_POST['sort'], 'require' => 'true', 'validator' => 'IntegerPositive', 'message' => lang('admin_pointprod_add_sort_number_error'));
            if ($_POST['islimit'] == 1) {
                $validate_arr[] = array('input' => $_POST['limitnum'], 'validator' => 'IntegerPositive', 'message' => lang('admin_pointprod_add_limitnum_digits_error'));
            }
            if ($_POST['islimittime']) {
                $validate_arr[] = array('input' => $_POST['starttime'], 'require' => 'true', 'message' => lang('admin_pointprod_add_limittime_null_error'));
                $validate_arr[] = array('input' => $_POST['endtime'], 'require' => 'true', 'message' => lang('admin_pointprod_add_limittime_null_error'));
            }
            $obj_validate->validateparam = $validate_arr;
            $error = $obj_validate->validate();
            if ($error != '') {
                showDialog(lang('error') . $error, '', 'error');
            }
            ...
            $goods_array['pgoods_state'] = isset($_POST['forbidstate']) ? trim($_POST['forbidstate']) : '';
            ...
            if ($state) {
                ...
                showDialog(lang('admin_pointprod_add_success'), 'index.php?act=pointprod&op=pointprod', 'succ');
            }
        }
        //模型实例化
        $where = array();
    ...
    public function prod_editOp()
    {
        ...
        if (!$pg_id) {
            showDialog(lang('admin_pointprod_parameter_error'), 'index.php?act=pointprod&op=pointprod', 'error');
        }
        $model_pointprod = model('pointprod');
        //查询礼品记录是否存在
        $prod_info = $model_pointprod->getPointProdInfo(array('pgoods_id' => $pg_id));
        if (!$prod_info) {
            showDialog(lang('admin_pointprod_record_error'), 'index.php?act=pointprod&op=pointprod', 'error');
        }
        if (chksubmit()) {
            //验证表单
            $obj_validate = new lib\validate();
            ...
            if ($state) {
                ...
                $this->log(lang('nc_edit,admin_pointprodp') . '[' . $_POST['goodsname'] . ']');
                showDialog(lang('admin_pointprod_edit_success'), 'index.php?act=pointprod&op=pointprod', 'succ');
            }
        } else {
            $where = array();
            $where['upload_type'] = '6';
            ...
            core\tpl::output('PHPSESSID', session_id());
            core\tpl::output('hourarr', $hourarr);
            core\tpl::output('prod_info', $prod_info);
            core\tpl::showpage('pointprod.edit');
        }
    }
    /**
     * 删除积分礼品
    ...
    public function prod_dropOp()
    {
        ...
        if ($result) {
            $this->log(lang('nc_del,admin_pointprodp') . '[ID:' . $pg_id . ']');
            showDialog(lang('admin_pointprod_del_success'), 'index.php?act=pointprod&op=pointprod', 'succ');
        } else {
            showDialog(lang('admin_pointprod_del_fail'), 'index.php?act=pointprod&op=pointprod', 'error');
        }
    }
    /**
     * 批量删除积分礼品
    ...
    public function prod_dropallOp()
    {
        ...
        if ($result) {
            $this->log(lang('nc_del,admin_pointprodp') . '[ID:' . implode(',', $pg_id) . ']');
            showDialog(lang('admin_pointprod_del_success'), 'index.php?act=pointprod&op=pointprod', 'succ');
        } else {
            showDialog(lang('admin_pointprod_del_fail'), 'index.php?act=pointprod&op=pointprod', '', 'error');
        }
    ...
    public function pointprod_pic_uploadOp()
    {
        ...
        $insert_array['file_size'] = isset($_FILES['Filedata']['size']) ? $_FILES['Filedata']['size'] : 0;
        $insert_array['upload_time'] = time();
