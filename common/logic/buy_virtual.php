...
class buy_virtual
{
     ...
     * 
     * @param array $post
     *        	接收POST数据，必须传入goods_id:商品ID，quantity:购买数量,buyer_phone:接收手机,buyer_msg:买家留言
     * @param int $member_id        	
     * @return array
     */
    public function buyStep3($post, $member_id)
    {
        ...
        // 变更库存和销量
        lib\queue::push('createOrderUpdateStorage', array($goods_info['goods_id'] => $goods_info['quantity']));
        // 更新抢购信息
    ...
    private function _createOrder($input, $goods_info, $member_info)
    {
		...
		$order['owner_id'] = $member_info['inviter_id'];
        ...
        if ($goods_info['goods_storage_alarm'] >= $goods_info['goods_storage'] - $input['quantity']) {
            ...
            lib\queue::push('sendStoreMsg', array('code' => 'goods_storage_alarm', 'store_id' => $goods_info['store_id'], 'param' => $param));
        }
		
		if($member_info['inviter_id']>0){
			$order_goods = array();
			$goods_info['goods_pay_price'] = $goods_info['goods_price'];
			$goods_info['goods_num'] = $input['quantity'];
			$order_goods[] = $goods_info;
			$flag = logic('distributor')->add_distributor_good_commission($order_goods,$member_info['inviter_id'],$member_info['member_id'],$order_id,$member_info['member_name'],1);
			...
			$flag = logic('weixin_message')->addorder($access_token, $disconfig ,$weixin_config, $member_info['member_name'], $pay_total, $order_id, 0);
    ...
    {
        if ((isset($goods_info['ifgroupbuy']) && $goods_info['ifgroupbuy']) && (isset($goods_info['groupbuy_id']) && $goods_info['groupbuy_id'])) {
            $groupbuy_info = array();
    ...
    {
        if (!core\config::get('groupbuy_allow') || empty($goods_info) || !is_array($goods_info)) {
            return $goods_info;
        }
