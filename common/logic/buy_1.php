...
class buy_1
{
    ...
    {
        // 处理预定订单
        if (isset($order_list[0]['order_type']) && $order_list[0]['order_type'] == 2) {
            ...
            //$update = $model_order_book->editOrderBook($data, $condition);//Mr.范
            //if (!$update) {
     ...
     * 
     * @param unknown $store_goods_total
    ...
    {
        if (!core\config::get('promotion_allow') || empty($store_goods_total) || !is_array($store_goods_total)) {
     ...
     *        	店铺商品总金额
     * @param array $preferential_array
     *        	店铺优惠活动内容
     * @param string $preferential_type
     *        	优惠类型
    ...
    public function reCalcGoodsTotal($store_goods_total, $preferential_array = array(), $preferential_type)
    {
        ...
        switch ($preferential_type) {
            case 'mansong':
                if (!core\config::get('promotion_allow')) {
                    return $store_goods_total;
                }
                ...
                break;
            case 'voucher':
                if (!core\config::get('voucher_allow')) {
                    return $store_goods_total;
                }
     ...
     * 
     * @param array $input_voucher_list
     *        	代金券列表
     * @param array $store_goods_total
     *        	(店铺ID=>商品总金额)
     * @return array
    ...
    public function getXianshiInfo(&$goods_info, $quantity)
    {
        ...
        if (!core\config::get('promotion_allow') || empty($goods_info['xianshi_info'])) {
            return;
        }
    ...
    public function appendPremiumsToCartList($store_cart_list, $store_premiums_list = array(), $store_mansong_rule_list = array(), $member_id)
    {
        ...
        if (is_array($store_premiums_list)) {
            foreach ($store_premiums_list as $store_id => $goods_list) {
                ...
                foreach ($goods_list as $goods_info) {
                    ...
                    if ($goods_storage_quantity[$goods_info['goods_id']] - $goods_info['goods_num'] >= 0) {
						if(!isset($goods_buy_quantity[$goods_info['goods_id']])){
							$goods_buy_quantity[$goods_info['goods_id']] = 0;
						}
				...
				if(!isset($store_mansong_rule_list[$store_id]['desc'])){
					$store_mansong_rule_list[$store_id]['desc'] = '';
				}
    ...
    public function pdPay($order_list, $input, $buyer_info)
    {
        ...
        foreach ($order_list as $order_info) {
            ...
            if ($available_pd_amount >= $order_amount) {
                ...
                $insert = $model_order->addOrderLog($data);
                if (!$insert) {
                    throw new \Exception('记录订单元支付日志出现错误');
                }
                // 订单状态 置为已支付
                $data_order = array();
    ...
    public function editGoodsNum($buy_items)
    {
        foreach ($buy_items as $goods_id => $buy_num) {
            ...
            if (!$result) {
                throw new \Exception(lang('cart_step2_submit_fail'));
            }
        }
    }
    /**
     ...
     * 
     * @param unknown $store_id_array
     *        	店铺ID数组
     */
    public function getMansongRuleList($store_id_array)
    {
        if (!core\config::get('promotion_allow') || empty($store_id_array) || !is_array($store_id_array)) {
            return array();
        }
        $model_mansong = model('p_mansong');
        $mansong_rule_list = array();
    ...
    public function createInvoiceData($invoice_info)
    {
		...
		if (!empty($invoice_info)){
			if ($invoice_info['inv_state'] == 1) {
				$inv['类型'] = '普通发票 ';
				$inv['抬头'] = isset($invoice_info['inv_title_select']) && $invoice_info['inv_title_select'] == 'person' ? '个人' : $invoice_info['inv_title'];
    ...
    private function _getGoodsOnlineInfo($goods_id, $quantity)
    {
        ...
        $new_array['groupbuy_info'] = !empty($goods_info['groupbuy_info']) ? $goods_info['groupbuy_info'] : '';
        $new_array['xianshi_info'] = isset($goods_info['xianshi_info']) ? $goods_info['xianshi_info'] : '';
    ...
    {
        if (!core\config::get('groupbuy_allow') || empty($goods_info['groupbuy_info'])) {
            return;
        }
        ...
        if (!empty($ordergoods) && intval($ordergoods) > 0) {
            $tnum = intval($groupbuy_info['upper_limit']) - intval($ordergoods);
            // -intval($goods_info['goods_num']);
            if ($tnum <= 0) {
                $goods_info = null;
            } else {
    ...
    {
        if (!core\config::get('promotion_allow') || empty($cart_list)) {
            return;
        }
        $model_goods = model('goods');
        foreach ($cart_list as $key => $cart_info) {
            ...
            foreach ($store_cart as $cart_info) {
                if (!intval($cart_info['bl_id'])) {
					...
					if(!isset($goods_buy_quangity[$cart_info['goods_id']])){
						$goods_buy_quangity[$cart_info['goods_id']] = 0;
					}
    ...
    private function _parseMansongRuleDesc($rule_info)
    {
        ...
        $goods_desc = !empty($rule_info['mansong_goods_name']) && !empty($rule_info['goods_storage']) ? " 送<a href='" . urlShop('goods', 'index', array('goods_id' => $rule_info['goods_id'])) . "' title='{$rule_info['mansong_goods_name']}' target='_blank'>[赠品]</a>" : '';
