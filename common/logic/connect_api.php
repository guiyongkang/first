...
class connect_api
{
    ...
    public function smsRegister($phone, $captcha, $password, $client)
    {
        if ($this->check_captcha($phone, $captcha)) {
            if (core\config::get('sms_register') != 1) {
            ...
            $member = array();
			$member['inviter_id'] = core\session::get('uid') ?: 0;
			if($member['inviter_id'] == 0){
				$dis_setting = model('distributor_setting')->field('member_inviter')->find();
				if($dis_setting['member_inviter']==1){
					return array('state' => 0, 'msg' => '必须通过邀请人才能成为会员');
				}
			}
            ...
            if ($result) {
                $member = $model_member->getMemberInfo(array('member_mobile' => $phone));
                $key = '';
                // $model_member->_get_token($member['member_id'],$member['member_name'],$client);
    ...
    {
        if ($this->check_captcha($phone, $captcha)) {
            if (core\config::get('sms_password') != 1) {
