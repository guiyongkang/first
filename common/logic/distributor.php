...
class distributor
{
	...
	*/	
	public function add_distributor_account($member_info,$level_id){
		...
		if($member_info['inviter_id']>0){
			$inviter = $model_dis->getInfoOne('distributor_account',array('member_id'=>$member_info['inviter_id']), 'dis_path,distributor_id,inviter_num,team_id','distributor_id desc');
			if(empty($inviter)){
		...
		if($member_info['inviter_id']>0){
			$inviter_data = array();
			$inviter_data['inviter_num'] = $inviter['inviter_num']+1;
			
			//查询股东级别
			$where = array();
			$where['team_invitenum'] = array('elt',$inviter['inviter_num']+1);
			$teaminfo = $model_dis->getInfoOne('distributor_team',$where, 'MAX(team_id) as team_id');
			if(!empty($teaminfo['team_id'])){
				if($teaminfo['team_id']>$inviter['team_id']){
					$inviter_data['team_id'] = $teaminfo['team_id'];
				}
			}
			unset($where);
			$result = $model_dis->editInfo('distributor_account',$inviter_data,array('distributor_id'=>$inviter['distributor_id']));
			$flag = $flag && $result;
		}
		if($flag){
			$model_dis->commit();
	...
	*/
	public function deal_public($member_info, $extend = array()){
		...
		//获得应得公排分区
		$public_area = array();
		$result = $model_dis->getInfoList('distributor_gp_area','','','item_id asc','item_id');
		if(empty($result)){
			return true;
		}
		foreach($result as $r){
			$public_area[] = $r['item_id'];
		}
		$area_id = $public_area[0];		
		
		$public_check = $model_dis->getInfoOne('distributor_gp',array('member_id'=>$member_info['member_id']),'status','ralate_id desc');
	...
	* 增加排位
	*/
	public function add_public_account($member_info,$dis_config=array(),$area_id, $public_area=array()){
		...
		//最多排位限制
		$result = $model_dis->getInfoOne('distributor_gp',array('member_id'=>$member_info['member_id']),'count(ralate_id) as num');
		if($result['num']>=101){
			return false;
		}
		unset($result);
		...
		//感恩奖
		$is_out_level = array();//出局人信息		
		$m = pow($dis_config['public_times'],$dis_config['public_out_level']);		
		if($position['distributor_x']%$m==0 && ($position['distributor_y'] - $dis_config['public_out_level'])>1){
			$condition_1['distributor_y'] = $position['distributor_y'] - $dis_config['public_out_level'];
			$condition_1['distributor_x'] = intval($position['distributor_x']/$m);
			$condition_1['rootid'] = $position['rootid'];
			$condition_1['status'] = 1;
			$condition_1['area_id'] = $area_id;
			$_gpinfo = $model_dis->getInfoOne('distributor_gp',$condition_1,'ralate_id,member_id');
			if(!empty($_gpinfo)){
				$result = $model_dis->editInfo('distributor_gp',array('status'=>0),array('ralate_id'=>$_gpinfo['ralate_id']));
				$flag = $flag && $result;
				$is_out_level['member_id'] = $_gpinfo['member_id'];
				$is_out_level['ralate_id'] = $_gpinfo['ralate_id'];
				$message_data[] = array(
					'member_id'=>$_gpinfo['member_id'],
					'text'=>'你已出局!欢迎复投,重新卡位'
				);
				$result = $model_member->getMemberInfo(array('member_id'=>$_gpinfo['member_id']), 'member_name,inviter_id');
		...
		if($flag){
			...
			if(!empty($is_out_level)){
				...
				if(!empty($public_area) && in_array($area_id,$public_area)){
					if($area_id>$public_area[0] && $area_id<$public_area[count($public_area)-1]){//达到升级条件
						//找到要升级的分区ID
						$p_0 = array_search($area_id,$public_area);
						$next_area_id = $public_area[$p_0+1];
						//继续卡位
						$member_info = $model_member->getMemberInfo(array('member_id'=>$is_out_level['member_id']), '*');
						$f_g = $this->add_public_account($member_info,$dis_config,$next_area_id, $public_area);
					}else{
						if($area_id==$public_area[0]){//更新第一个分区升级第二步
							$result = $model_dis->editInfo('distributor_gp',array('flag_update'=>1),array('ralate_id'=>$is_out_level['ralate_id']));							
						}					
					}
				}
			}
	...
	*/
	...
	public function public_update($member_id){
		...
		//获得用户的公排最高级区域
		$result = $model_dis->getInfoOne('distributor_gp',array('member_id'=>$member_info['member_id'],'flag_update'=>1),'area_id,ralate_id','ralate_id desc');
		if(empty($result)){
			return false;
		}
		$current_area_id = $result['area_id'];
		$ralate_id = $result['ralate_id'];
		unset($result);
		...
		if($current_area_id==1){
			...
			$condition['area_id'] = $current_area_id;
		...
		$flag = $model_dis->editInfo('distributor_gp',array('flag_update'=>0), array('ralate_id'=>$ralate_id));
		
		//达到升级条件
		$this->add_public_account($member_info,array(),$next_area_info['item_id'],array());
	}
	
	//股东分红
	public function deal_team_commission($order_list=array(),$order_info=array()){
		$model_dis = model('distributor');
		$goods_info = $goodids = $order_good_info = array();
		if(!empty($order_list)){//实物商品
			$order_type = 0;
			$order_ids = array();
			foreach($order_list as $value){
				$order_ids[] = $value['order_id'];
			}
			$result = model()->query('SELECT order_id,goods_id,goods_price,goods_num,goods_name FROM shop_order_goods WHERE order_id IN('.implode(',',$order_ids).')');
			foreach($result as $r){
				if(!in_array($r['goods_id'],$goodids)){
					$goodids[] = $r['goods_id'];
				}
				$order_good_info[] = $r;
			}
			...
			$order_type = 1;
			$order_good_info[] = $order_info;
			$goodids[] = $order_info['goods_id'];
		}
		...
		//获取商品相关信息
		$goods_common = array();
		$result = model()->query('SELECT gc.goods_costprice,ds.good_team_commission,g.goods_id FROM shop_goods AS g LEFT JOIN shop_goods_common AS gc ON gc.goods_commonid=g.goods_commonid RIGHT JOIN shop_distributor_goods AS ds ON gc.goods_commonid=ds.good_id WHERE g.goods_id IN('.implode(',',$goodids).') AND ds.good_team_commission<>""');
		if(empty($result)){
			return true;
		}
		
		foreach($result as $r){
			$r['team_commission'] = json_decode($r['good_team_commission'],true);
			$goods_common[$r['goods_id']] = $r;
		}
		...
		//获得股东会员相关数组
		$teams = array();
		$result = model()->query('SELECT member_id,team_id FROM shop_distributor_account WHERE team_id>0');
		if(empty($result)){
			return true;
		}
		
		foreach($result as $t){
			$teams[$t['team_id']][] = $t['member_id'];
		}
		
		//组装数据库信息
		$detail_data = array();
		foreach($order_good_info as $goodinfo){
			if(empty($goods_common[$goodinfo['goods_id']])){
				continue;
			}
			$gid = $goodinfo['goods_id'];
			$orderid = $goodinfo['order_id'];
			$desc = '会员购买['.$goodinfo['goods_name'].']，价格：'.$goodinfo['goods_price'].'元，数量：'.$goodinfo['goods_num'].'件';
			$goodinfo['team_commission'] = $goods_common[$goodinfo['goods_id']]['team_commission'];
			$goodinfo['profit'] = number_format(($goodinfo['goods_price'] - $goods_common[$goodinfo['goods_id']]['goods_costprice']),2,'.','') * $goodinfo['goods_num'];
			if($goodinfo['profit']<0.01){
				continue;
			}
			foreach($teams as $tid=>$teaminfo){
				if(empty($goodinfo['team_commission'][$tid])){
					continue;
				}
				$total = number_format(($goodinfo['profit'] * $goodinfo['team_commission'][$tid] * 0.01),2,'.','');
				if($total<0.01){
					continue;
				}
				$number = count($teaminfo);
				$bonus = number_format(($total/$number),2,'.','');
				if($bonus<0.01){
					continue;
				}
				
				foreach($teaminfo as $mid){
					$detail_data[] = array(
						'member_id'=>$mid,
						'team_id'=>$tid,
						'goods_id'=>$gid,
		...
		if(!empty($detail_data)){
			$flag = $model_dis->addAll('distributor_fenhong',$detail_data);
		}
	}
}