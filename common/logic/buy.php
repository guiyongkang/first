...
class buy
{
    ...
    public function getBuyStep1Data($member_id, $data)
    {
        ...
        if (isset($inv_info['inv_state']) && $inv_info['inv_state'] == '2' && !$vat_deny) {
            $inv_info['content'] = '增值税发票 ' . $inv_info['inv_company'] . ' ' . $inv_info['inv_code'] . ' ' . $inv_info['inv_reg_addr'];
        } elseif (isset($inv_info['inv_state']) && $inv_info['inv_state'] == '2' && $vat_deny) {
            $inv_info = array();
     ...
     * @return array
     */
    public function buyStep2($post, $member_id, $member_name, $member_email, $owner_id)
    {
		...
		$this->_member_info['owner_id'] = $owner_id;
    ...
    public function delCart($ifcart, $member_id, $cart_ids)
    {
        ...
        if (preg_match('/^[\\d,]+$/', $cart_id_str)) {
            lib\queue::push('delCart', array('buyer_id' => $member_id, 'cart_ids' => $cart_ids));
        }
    }
    /**
    ...
    public function changeAddr($freight_hash, $city_id, $area_id, $member_id)
    {
        ...
        // 将hash解密，得到运费信息(店铺ID，运费,售卖区域ID),hash内容有效期为1小时
    ...
    private function _createOrderStep1()
    {
        ...
        if (!in_array($input_if_vat, array('allow_vat', 'deny_vat'))) {
            throw_exception('订单保存出现异常[值税发票出现错误]，请重试');
        }
        ...
        if (!in_array($input_if_offpay, array('allow_offpay', 'deny_offpay'))) {
            throw_exception('订单保存出现异常[货到付款验证错误]，请重试');
        }
    ...
    private function _createOrderStep3()
    {
        ...
        if ($append_premiums_to_cart_list === false) {
            throw_exception('抱歉，您购买的商品库存不足，请重购买');
        } else {
    ...
    private function _createOrderStep4()
    {
		...
		$owner_id = $this->_member_info['owner_id'];
		$amount = 0;
        $model_order = model('order');
        $model_goodsinfo = model('goods');
        // 存储生成的订单数据
        $order_list = array();
        // 存储通知信息
        $notice_list = array();
        ...
        foreach ($store_cart_list as $store_id => $goods_list) {
            ...
            if (!$order_id) {
                throw_exception('订单保存失败[未生成订单数据]');
            }
            ...
            if (!$order_id) {
                throw_exception('订单保存失败[未生成订单扩展数据]');
            }
            // 生成order_goods订单商品数据
            $i = 0;
            foreach ($goods_list as $goods_info) {
                if (!$goods_info['state'] || !$goods_info['storage_state']) {
                    throw_exception('部分商品已经下架或库存不足，请重新选择');
                }
                if (!intval($goods_info['bl_id'])) {
                    ...
                    $condition['goods_commonid']=$goods_info['goods_id'];
                    $goodsinfo=$model_goodsinfo->getGoodeCommonInfo($condition);
                    $order_goods[$i]['points'] = $goodsinfo['points'];
                    $order_goods[$i]['pointsdays'] = $goodsinfo['pointsdays'];
                    
                    if (!empty($goods_info['ifgroupbuy'])) {
                        $ifgroupbuy = true;
                        $order_goods[$i]['goods_type'] = 2;
                    } elseif (!empty($goods_info['ifxianshi'])) {
                        $order_goods[$i]['goods_type'] = 3;
                    } elseif (!empty($goods_info['ifzengpin'])) {
                        $order_goods[$i]['goods_type'] = 5;
                    } else {
                        $order_goods[$i]['goods_type'] = 1;
                    }
                    $order_goods[$i]['promotions_id'] = !empty($goods_info['promotions_id']) ? $goods_info['promotions_id'] : 0;
                    $order_goods[$i]['commis_rate'] = isset($store_gc_id_commis_rate_list[$store_id][$goods_info['gc_id']]) ? floatval($store_gc_id_commis_rate_list[$store_id][$goods_info['gc_id']]) : 0;
                    //佣金比例
                    ...
                    foreach ($goods_info['bl_goods_list'] as $bl_goods_info) {
                        ...
                        $condition['goods_commonid']=$bl_goods_info['goods_id'];
                        $goodsinfo=$model_goodsinfo->getGoodeCommonInfo($condition);
                        $order_goods[$i]['points'] = $goodsinfo['points'];
                        $order_goods[$i]['pointsdays'] = $goodsinfo['pointsdays'];
            ...
            if (!$insert) {
                throw_exception('订单保存失败[未生成商品数据]');
            }
            // 存储商家发货提醒数据
			...
			//分销处理
			if($owner_id>0){
				$flag = logic('distributor')->add_distributor_good_commission($order_goods,$owner_id,$member_id,$order_id,$member_name,0);
				...
				$flag = logic('weixin_message')->addorder($access_token, $disconfig ,$weixin_config, $member_name, $store_final_order_total[$store_id], $order_id, 0);
        ...
        $this->_order_data['ifgroupbuy'] = isset($ifgroupbuy) ? $ifgroupbuy : false;
    }
    /**
     * 充值卡、预存款支付
     */
    ...
    private function _createOrderStep6()
    {
        ...
        // 变更库存和销量
        lib\queue::push('createOrderUpdateStorage', $goods_buy_quantity);
        // 更新使用的代金券状态
        if (!empty($input_voucher_list) && is_array($input_voucher_list)) {
            lib\queue::push('editVoucherState', $input_voucher_list);
        }
        // 更新F码使用状态
        if ($fc_id) {
            lib\queue::push('updateGoodsFCode', $fc_id);
        }
        // 更新抢购购买人数和数量
        if ($ifgroupbuy) {
            foreach ($store_cart_list as $goods_list) {
                foreach ($goods_list as $goods_info) {
                    if ($goods_info['ifgroupbuy'] && $goods_info['groupbuy_id']) {
                        ...
                        lib\queue::push('editGroupbuySaleCount', $groupbuy_info);
                    }
                }
            }
        }
        ...
        // 保存订单自提点信息
        if (core\config::get('delivery_isuse') && intval($input_address_info['dlyp_id'])) {
            $data = array();
            ...
            lib\queue::push('saveDeliveryOrder', $data);
        }
        // 发送提醒类信息
        if (!empty($notice_list)) {
            foreach ($notice_list as $code => $value) {
                lib\queue::push('sendStoreMsg', array('code' => $code, 'store_id' => key($value), 'param' => current($value)));
            }
        }
    }
    /**
    ...
    public function buyDecrypt($string, $member_id, $ttl = 0)
    {
        ...
        return ($tmp = is_serialized($string) ? unserialize($string) : false) !== false ? $tmp : $string;
    }
