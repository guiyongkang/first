...
class order
{
    ...
    public function changeOrderStateCancel($order_info, $role, $user = '', $msg = '', $if_update_account = true, $if_quque = true)
    {
        try {
            ...
            if ($if_quque) {
                lib\queue::push('cancelOrderUpdateStorage', $data);
            } else {
    ...
    public function changeOrderStateReceive($order_info, $role, $user = '', $msg = '')
    {
        try {
            ...
            $rebate_amount = ceil(0.01 * $order_info['order_amount'] * core\config::get('points_rebate'));
    ...
    public function changeOrderSend($order_info, $role, $user = '', $post = array())
    {
        ...
        //更新表发货信息
        if ($post['shipping_express_id'] && (isset($order_info['extend_order_common']['reciver_info']['dlyp']) && $order_info['extend_order_common']['reciver_info']['dlyp'])) {
            $data = array();
        ...
        $param['param'] = array('order_sn' => $order_info['order_sn'], 'order_url' => urlShop('member_order', 'show_order', array('order_id' => $order_id)));
