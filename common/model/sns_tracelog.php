...
class sns_tracelog
{
    ...
    public function tracelogAdd($param)
    {
        ...
        $result = db\mysqli::insert('sns_tracelog', $param);
        return $result;
    }
    /**
     * 动态记录列表
    ...
    public function getTracelogList($condition, $page = '', $field = '*')
    {
        ...
        $param['field'] = $field;
        $param['order'] = isset($condition['order']) ? $condition['order'] : 'sns_tracelog.trace_id desc';
    ...
    public function getTracelogRow($condition, $field = '*')
    {
        ...
        return db\mysqli::getRow($param, $field);
    }
    /**
     * 更新动态记录
    ...
    public function tracelogEdit($param, $condition)
    {
        ...
        //得到条件语句
        $condition_str = $this->getCondition($condition);
        $result = db\mysqli::update('sns_tracelog', $param, $condition_str);
        return $result;
    }
    /**
     * 删除动态
    ...
    public function delTracelog($condition)
    {
        ...
        if (!empty($condition['trace_id_in'])) {
            $condition_str .= " and trace_id in('{$condition['trace_id_in']}') ";
        }
        if (!empty($condition['trace_memberid'])) {
            $condition_str .= " and trace_memberid='{$condition['trace_memberid']}' ";
        }
        return db\mysqli::delete('sns_tracelog', $condition_str);
    }
    /**
     * 动态总数
     */
    public function countTrace($condition)
    {
        //得到条件语句
        $condition_str = $this->getCondition($condition);
        $count = db\mysqli::getCount('sns_tracelog', $condition_str);
        return $count;
    }
    /**
     * 将条件数组组合为SQL语句的条件部分
