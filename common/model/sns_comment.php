...
class sns_comment
{
    ...
    public function commentAdd($param)
    {
        ...
        if (is_array($param)) {
            $result = db\mysqli::insert('sns_comment', $param);
            return $result;
        } else {
            return false;
        }
    ...
    public function getCommentList($condition, $page = '', $field = '*')
    {
        ...
        $param['field'] = $field;
        $param['order'] = isset($condition['order']) ? $condition['order'] : 'sns_comment.comment_id desc';
    ...
    {
        $condition_str = $this->getCondition($condition);
        return db\mysqli::getCount("sns_comment", $condition_str);
    }
    /**
     * 获取评论详细
     * 
    ...
    public function commentEdit($param, $condition)
    {
        ...
        //得到条件语句
        $condition_str = $this->getCondition($condition);
        $result = db\mysqli::update('sns_comment', $param, $condition_str);
        return $result;
    }
    /**
     * 将条件数组组合为SQL语句的条件部分
