...
class store extends core\model
{
    /**
     * 自营店铺的ID
     *
    ...
    public function delStore($condition)
    {
        ...
        //删除店铺相关图片
		if(file_exists(BASE_UPLOAD_PATH . DS . ATTACH_STORE . DS . $store_info['store_label']) && $store_info['store_label']){
			unlink(BASE_UPLOAD_PATH . DS . ATTACH_STORE . DS . $store_info['store_label']);
		}
        if(file_exists(BASE_UPLOAD_PATH . DS . ATTACH_STORE . DS . $store_info['store_banner']) && $store_info['store_banner']){
        ...
        if ($store_info['store_slide'] != '') {
            foreach (explode(',', $store_info['store_slide']) as $val) {
				if(file_exists(BASE_UPLOAD_PATH . DS . ATTACH_SLIDE . DS . $val) && $val){
    ...
    public function delStoreEntirely($condition)
    {
        ...
        model('album')->delAlbum($condition);
