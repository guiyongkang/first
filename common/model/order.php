...
class order extends core\model
{
     ...
     * @return Ambigous <multitype:boolean Ambigous <string, mixed> , unknown>
     */
    public function getOrderList($condition, $page = 0, $field = '*', $order = 'order_id desc', $limit = 0, $extend = array(), $master = false)
    {
		$list = $this->table('order')->field($field)->where($condition)->limit($limit)->page($page)->order($order)->master($master)->select();
        if (empty($list)) {
            return array();
        }
        $order_list = array();
    ...
    public function addOrder($data)
    {
        ...
        if ($insert) {
            //更新缓存
            lib\queue::push('delOrderCountCache', array('buyer_id' => $data['buyer_id'], 'store_id' => $data['store_id']));
        }
        return $insert;
    }
    /**
    ...
    public function editOrder($data, $condition, $limit = '')
    {
        ...
        if ($update) {
            //更新缓存
            lib\queue::push('delOrderCountCache', $condition);
        }
        return $update;
    }
    /**
    ...
    public function getOrderOperateState($operate, $order_info)
    {
        ...
        switch ($operate) {
            ...
            //买家投诉
            case 'complain':
                $state = in_array($order_info['order_state'], array(ORDER_STATE_PAY, ORDER_STATE_SEND)) || intval($order_info['finnshed_time']) > TIMESTAMP - core\config::get('complain_time_limit');
                break;
            case 'payment':
                $state = $order_info['order_state'] == ORDER_STATE_NEW && $order_info['payment_code'] == 'online';
                break;
