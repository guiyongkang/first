...
class sns_sharegoods
{
    ...
    public function sharegoodsAdd($param)
    {
        ...
        if (is_array($param)) {
            $result = db\mysqli::insert('sns_sharegoods', $param);
            return $result;
        } else {
            return false;
        }
    ...
    public function editSharegoods($param, $condition)
    {
        ...
        $result = db\mysqli::update('sns_sharegoods', $param, $condition_str);
        return $result;
    }
    /**
     * 分享商品记录列表
    ...
    public function getSharegoodsList($condition, $page = '', $field = '*', $type = 'simple')
    {
        ...
        switch ($type) {
            case 'detail':
                ...
                $param['join_on'] = array('sns_sharegoods.share_goodsid=sns_goods.snsgoods_goodsid');
                break;
            default:
                $param['table'] = 'sns_sharegoods';
        }
    ...
    public function delSharegoods($condition)
    {
        ...
        $condition_str = '';
        if (!empty($condition['share_id'])) {
            $condition_str .= " and share_id='{$condition['share_id']}' ";
        }
        if (!empty($condition['share_memberid'])) {
            $condition_str .= " and share_memberid='{$condition['share_memberid']}' ";
        }
        return db\mysqli::delete('sns_sharegoods', $condition_str);
    }
    /**
     * 将条件数组组合为SQL语句的条件部分
     *
