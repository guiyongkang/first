...
class goods_class_tag extends core\model
{
    ...
    public function getTagList($condition = array(), $page = '', $field = '*')
    {
        ...
        $param['order'] = isset($condition['order']) ? $condition['order'] : 'gc_tag_id asc';
    ...
    public function tagAdd($param)
    {
        ...
        if (!empty($param) && is_array($param)) {
            ...
            foreach ($param as $value) {
                ...
                $class_name_2 = '';
                $class_name_3 = '';
                if (!empty($value['sub_class']) && is_array($value['sub_class'])) {
                    //二级
                    foreach ($value['sub_class'] as $val) {
                        ...
                        $class_id = $val['gc_id'];
                        $type_id = $val['type_id'];
                        if (!empty($val['sub_class']) && is_array($val['sub_class'])) {
                            //三级
        ...
        return db\mysqli::query("insert into `" . DBPRE . "goods_class_tag` (`gc_id_1`,`gc_id_2`,`gc_id_3`,`gc_tag_name`,`gc_tag_value`,`gc_id`,`type_id`) values " . $condition_str);
    }
    /**
     * TAG清空
     */
    ...
    {
        return db\mysqli::query("TRUNCATE TABLE `" . DBPRE . "goods_class_tag`");
    }
    /**
     * TAG更新
     * 
    ...
    public function updateTag($param)
    {
        ...
        if (is_array($param)) {
            ...
            $result = db\mysqli::update('goods_class_tag', $tmp, $where);
            return $result;
        } else {
            return false;
        }
    ...
    {
        if (!empty($id)) {
            return db\mysqli::delete('goods_class_tag', ' gc_tag_id in (' . $id . ')');
        } else {
            return false;
        }
    }
    ...
    {
        return db\mysqli::delete('goods_class_tag', $this->_condition($condition));
    }
    /**
     * TAG添加
     */
    public function addOneTag($param)
    {
        ...
        if (is_array($param)) {
            ...
            $result = db\mysqli::insert('goods_class_tag', $tmp);
            return $result;
        } else {
            return false;
        }
