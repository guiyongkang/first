...
class database
{
    ...
    {
        $table_list = core\session::get('db_backup.backup_tables');
        if ($step == 1 && $table_list) {
            $this->back_content .= "\r\n" . 'SET SQL_MODE=\'NO_AUTO_VALUE_ON_ZERO\';' . "\r\n\r\n";
			core\session::set('db_backup.table_name', $table_list[0]);
        }
        $this->step = $step;
        if (core\session::has('db_backup.table_name')) {
            $key = array_search(core\session::get('db_backup.table_name'), $table_list);
            if ($key > 0) {
                for ($i = 0; $i < $key; $i++) {
                    unset($table_list[$i]);
                }
         ...
         * 创建表sql
         */
        if (core\session::get('db_backup.op') == 'create') {
            foreach ($table_list as $k => $v) {
                /**
                 * 写入session
                 */
				core\session::set('db_backup.table_name', $v);
                ...
                if (empty($table_list[$k + 1])) {
                     ...
                     * 写入session op
                     */
					core\session::set('db_backup.op', 'insert');
					core\session::set('db_backup.table_name', $v);
					core\session::set('db_backup.table_name', $table_list[0]);
                    return true;
                }
         ...
         * 表信息sql
         */
        if (core\session::get('db_backup.op') == 'insert') {
            foreach ($table_list as $k => $v) {
                /**
                 * 写入session
                 */
                 ...
                 * 判断是否最后结束
                 */
                if (empty($table_list[$k + 1]) && core\session::get('db_backup.limit') == 0) {
                    /**
                     * 写入文件
                     */
                    $this->writeBackFile();
    ...
    {
        $table_list = array();
        $tmp = db\mysqli::showTables();
        $table = 'Tables_in_' . strtolower(DBNAME);
        if ($type == 'all') {
            if (is_array($tmp)) {
                foreach ($tmp as $k => $v) {
					if(isset($v[$table])){
    ...
    private function getCreateContent($table)
    {
        ...
        $tmp_create .= db\mysqli::showCreateTable(substr($table, strlen(DBPRE), strlen($table) - 1));
        $tmp_create .= ";\r\n\r\n";
        /**
         * 判断长度
         */
        if (strlen($this->back_content . $tmp_create) >= core\session::get('db_backup.size')) {
            /**
             * 写入文件，跳出
             */
            $this->writeBackFile();
    ...
    private function getInsertContent($table)
    {
         ...
         * 开始条数
         */
        $limit = core\session::get('db_backup.limit') ? core\session::get('db_backup.limit') : 0;
        /**
         * 当前语句大小
         */
         ...
         * 字段信息
         */
        $columns_array = db\mysqli::showColumns(substr($table, strlen(DBPRE), strlen($table) - 1));
        /**
         * 生成sql语句
         */
        $result = '';
        foreach ($list as $k => $v) {
             ...
             * 判断字符串大小限制
             */
            if (strlen($this->back_content . $tmp_sql) >= core\session::get('db_backup.size')) {
                /**
                 * 备份文件
                 */
                $this->writeBackFile();
                 ...
                 * 增加session中的limit计数
                 */
				core\session::set('db_backup.limit', core\session::get('db_backup.limit')+1);
    ...
    public function getBackDir()
    {
        ...
        $return = $tmp . '_' . ((isset($check_array[(count($check_array)?:1) - 1]) ? $check_array[(count($check_array)?:1) - 1] : 0) + 1);
        return $return;
    }
    /**
     * 写入文件
    ...
    {
        core\language::read('model_lang_index');
        $lang = core\language::getLangContent();
        $step = $this->step;
        try {
            if (!is_dir(BASE_DATA_PATH . DS . 'sql_back' . DS . core\session::get('db_backup.back_file'))) {
                if (!$this->mkdirs(BASE_DATA_PATH . DS . 'sql_back' . DS . core\session::get('db_backup.back_file'), 0755)) {
                    $error = $lang['db_backup_mkdir_fail'];
                    throw_exception($error);
                } else {
                    $fp = fopen(BASE_DATA_PATH . DS . 'sql_back' . DS . core\session::get('db_backup.back_file') . DS . 'index.html', 'w+');
                    fclose($fp);
                }
            }
            $file_name = BASE_DATA_PATH . DS . 'sql_back' . DS . core\session::get('db_backup.back_file') . DS . core\session::get('db_backup.back_file') . '_' . $step . '_' . core\session::get('db_backup.md5') . '.sql';
