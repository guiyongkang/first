...
class groupbuy extends core\model
{
    ...
    private $groupbuy_state_array = array(0 => '全部', self::GROUPBUY_STATE_REVIEW => '审核中', self::GROUPBUY_STATE_NORMAL => '正常', self::GROUPBUY_STATE_CLOSE => '已结束', self::GROUPBUY_STATE_REVIEW_FAIL => '审核失败', self::GROUPBUY_STATE_CANCEL => '管理员关闭');
    ...
    public function getGroupbuyInfoByGoodsCommonID($goods_commonid)
    {
        ...
        if (empty($info)) {
            ...
            $info['info'] = !empty($groupbuy_goods_list[0]) ? serialize($groupbuy_goods_list[0]) : array();
            $this->_wGoodsGroupbuyCache($goods_commonid, $info);
        }
        $groupbuy_goods_info = !empty($info['info']) ? unserialize($info['info']) : array();
    ...
    private function _unlockGoods($goods_commonid)
    {
        ...
        // 添加对列 更新商品促销价格
        lib\queue::push('updateGoodsPromotionPriceByGoodsCommonId', $goods_commonid);
    }
    /**
     * 更新
     * @param array $update
    ...
    public function editExpireGroupbuy($condition)
    {
        ...
        if (!empty($expire_groupbuy_list)) {
            ...
            lib\queue::push('updateGoodsPromotionPriceByGoodsCommonId', $goodscommonid_array);
        }
        $groupbuy_id_string = '';
        if (!empty($expire_groupbuy_list)) {
    ...
    protected function getCachingDataByQuery($key)
    {
        ...
        switch ($key) {
            ...
            case 'groupbuy_vr_classes':
                ...
                break;
            case 'groupbuy_vr_cities':
                // 虚拟抢购地区
                // 一级地区 城市
                $arr = (array) model()->table('vr_groupbuy_area')->where(array('hot_city' => 1, 'parent_area_id' => 0))->order('area_id asc')->limit(false)->key('area_id')->select();
                foreach ($arr as $v) {
                    $id = $v['area_id'];
                ...
                if ($pids = array_keys($arr)) {
                    // 二级地区 区域
                    $arr = (array) model()->table('vr_groupbuy_area')->where(array('parent_area_id' => array('in', $pids)))->order('area_id asc')->limit(false)->key('area_id')->select();
                    foreach ($arr as $v) {
                        $id = $v['area_id'];
                        $pid = $v['parent_area_id'];
                        $data['name'][$id] = $v['area_name'];
