...
class flea_consult {
	...
	public function addConsult($input){
		...
		$result	= db\mysqli::insert('flea_consult',$consult);
		if($result) {
			return $result;
		} else {
			return false;
	...
	public function getConsultList($condition,$obj_page='',$type="simple",$ctype='goods'){
		...
		$consult_list = db\mysqli::select($param,$obj_page);
		return $consult_list;
	}
	/**
	 * 删除咨询
	...
	public function dropConsult($id){
		return db\mysqli::delete('flea_consult',"where consult_id in ({$id})");
	}
	/**
	 * 回复咨询
	 * 
	...
	public function replyConsult($input){
		...
		return db\mysqli::update('flea_consult',$input,'consult_id='.$input['consult_id']);
	}
	/**
	 * 构造查询条件
	 * 
	...
	private function getCondition($condition_array){
		...
		if(!empty($condition_array['seller_id'])) {
