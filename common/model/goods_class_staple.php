...
class goods_class_staple extends core\model
{
    ...
    {
        $where = array('gc_id_1' => intval($param['gc_id_1']), 'gc_id_2' => (isset($param['gc_id_2']) ? intval($param['gc_id_2']) : 0), 'gc_id_3' => (isset($param['gc_id_3']) ? intval($param['gc_id_3']) : 0), 'member_id' => $member_id);
        $staple_info = $this->getStapleInfo($where);
        if (empty($staple_info)) {
            $insert = array('staple_name' => $param['gc_tag_name'], 'gc_id_1' => intval($param['gc_id_1']), 'gc_id_2' => (isset($param['gc_id_2']) ? intval($param['gc_id_2']) : 0), 'gc_id_3' => (isset($param['gc_id_3']) ? intval($param['gc_id_3']) : 0), 'type_id' => $param['type_id'], 'member_id' => $member_id);
