...
class goods extends core\model
{
    ...
    {
        $where = array('store_id' => (int) $storeId, 'gc_id_1' => (int) $gcId);
        if ($notEqualGoodsId > 0) {
            $where['goods_id'] = array('neq', (int) $notEqualGoodsId);
        }
        return $this->table('goods')->where($where)->order('rand()')->limit($size)->select();
    }
    /**
    ...
    public function getGoodsListByColorDistinct($condition, $field = '*', $order = 'goods_id asc', $page = 0)
    {
        ...
        $goods_list = array();
        if ($count != 0) {
            $goods_list = $this->getGoodsOnlineList($condition, $field, $page, $order, 0, 'CONCAT(goods_commonid)', false, $count);
        }
        return $goods_list;
    }
    /**
    ...
    public function getGoodsInfoAndPromotionById($goods_id)
    {
        ...
        // 限时折扣
        if (core\config::get('promotion_allow') && empty($goods_info['groupbuy_info'])) {
     ...
     * @param string $fields
     *        	需要取得的缓存键值, 例如：'*','goods_name,store_name'
     * @return array
     */
    ...
    public function delGoods($condition)
    {
        ...
        if (!empty($goods_list)) {
            ...
            foreach ($goods_list as $val) {
				...
				if(file_exists(BASE_UPLOAD_PATH . DS . ATTACH_STORE . DS . $val['store_id'] . DS . $val['goods_id'] . '.png')){
    ...
    {
        lib\queue::push('sendStoreMsg', array('code' => $code, 'store_id' => $store_id, 'param' => $param));
    }
    /**
     * 获得商品子分类的ID
     * 
    ...
    {
        if (isset($goods['is_virtual']) && $goods['is_virtual'] == 1) {
            return false;
        }
        return true;
    }
    ...
    {
        if ((isset($goods['is_virtual']) && $goods['is_virtual'] == 1) || (isset($goods['is_fcode']) && $goods['is_fcode'] == 1) || (isset($goods['is_presell']) && $goods['is_presell'] == 1) || (isset($goods['is_appoint']) && $goods['is_appoint'] == 1)) {
            return false;
        }
        return true;
    }
    ...
    public function getMobileBodyByCommonID($goods_commonid, $fields = 'mobile_body')
    {
        ...
        // 手机商品描述
        if (!empty($common_info['mobile_body'])) {
