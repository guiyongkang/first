...
class waybill extends core\model
{
    ...
    public function getWaybillDesignInfo($waybill_id, $store_id = 0)
    {
        ...
        return array('waybill_info' => $waybill_info, 'waybill_info_data' => $waybill_info_data, 'waybill_item_list' => $waybill_item_list);
    }
    /*
     * 增加 
     * @param array $param
    ...
    public function validWaybill($input)
    {
        ...
        $obj_validate->validateparam = array(array("input" => $input['waybill_name'], "require" => "true", "message" => '模板名称不能为空'), array("input" => $input['waybill_width'], "require" => "true", "message" => '宽度不能为空'), array("input" => $input['waybill_height'], "require" => "true", "message" => '高度不能为空'));
    ...
    public function getPrintInfoByOrderInfo($order_info)
    {
        ...
        $print_info['seller_name'] = empty($daddress_info['seller_name']) ? '' : $daddress_info['seller_name'];
        $print_info['seller_area'] = empty($daddress_info['area_info']) ? '' : $daddress_info['area_info'];
        $print_info['seller_address'] = empty($daddress_info['address']) ? '' : $daddress_info['address'];
        $print_info['seller_phone'] = empty($daddress_info['telphone']) ? '' : $daddress_info['telphone'];
        $print_info['seller_company'] = empty($daddress_info['company']) ? '' : $daddress_info['company'];
        return $print_info;
    }
}