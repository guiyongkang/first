...
class flea
{
    ...
    public function saveGoods($param)
    {
        ...
        $result = db\mysqli::insert('flea', $goods_array);
        return $result;
    }
    /**
     * 商品列表
    ...
    public function listGoods($param, $page = '', $field = '*')
    {
        ...
        $list_goods = db\mysqli::select($array, $page);
        return $list_goods;
    }
    /**
     * 他们正在卖的
    ...
    public function statistic($member_id)
    {
        ...
        $goods_array = db\mysqli::select($array);
    ...
    public function getListImageGoods($param, $field = '*')
    {
        ...
        $array['where'] = $condition_str;
        $array['field'] = $field;
        $list_image = db\mysqli::select($array);
        return $list_image;
    }
    /**
     * 得到商品所有缩略图，带商品路径
    ...
    public function updateGoods($param, $goods_id)
    {
        ...
        $update = db\mysqli::update('flea', $param, $condition_str);
        return $update;
    }
    /**
     * 闲置物品数量
    ...
    public function dropGoods($goods_id)
    {
        ...
        $del_state = db\mysqli::delete('flea', 'where goods_id in (' . $goods_id . ')');
        if ($del_state) {
            $image_more = db\mysqli::select(array('table' => 'flea_upload', 'field' => 'file_name', 'where' => ' where item_id in (' . $goods_id . ') and upload_type in ("12","13")'));
            ...
            db\mysqli::delete('flea_upload', 'where item_id in (' . $goods_id . ') and upload_type in ("12","13")');
        }
        return true;
    }
    /**
    ...
    public function dropImageGoods($param)
    {
        ...
        //得到条件语句
        $condition_str = $this->getCondition($param);
        $image_more = db\mysqli::select(array('table' => 'flea_upload', 'where' => $condition_str, 'field' => 'file_name'));
        if (is_array($image_more) && !empty($image_more)) {
            foreach ($image_more as $v) {
                @unlink(UPLOAD_SITE_URL . DS . ATTACH_MALBUM . DS . $_SESSION['member_id'] . DS . $v['file_name']);
            }
        }
        $state = db\mysqli::delete('flea_upload', $condition_str);
        return $state;
    }
    // 	/**
    // 	 *	查询拥有闲置物品数量从多到少会员列表
    ...
    {
        $flea_ids = db\mysqli::select(array('table' => 'flea', 'field' => 'goods_id', 'limit' => '27'));
        return $flea_ids;
    }
    /**
     * 将条件数组组合为SQL语句的条件部分
    ...
    {
        $condition_sql = '';
        if (!empty($condition_array['member_id'])) {
            $condition_sql .= " and member_id = " . $condition_array['member_id'];
        }
        if (!empty($condition_array['image_store_id'])) {
        ...
        if (!empty($condition_array['gc_id_list'])) {
        ...
        if (!empty($condition_array['keyword'])) {
            $condition_sql .= " and goods_name LIKE '%" . $condition_array['keyword'] . "%'";
        }
        if (!empty($condition_array['upload_id'])) {
        ...
        if (!empty($condition_array['gc_id'])) {
        ...
        if (!empty($condition_array['seller_input'])) {
            $condition_sql .= " and member_id = " . $condition_array['seller_input'];
        }
        if (!empty($condition_array['quality_input'])) {
    ...
    private function _getRecursiveClass($class_id)
    {
        ...
        $id = implode(',', $class_id);
        $class_list .= ',' . $id;
        $temp_list = db\mysqli::select(array('table' => 'flea_class', 'where' => 'gc_parent_id in (' . $id . ')', 'field' => 'gc_id'));
        if (!empty($temp_list)) {
            $_tmp = array();
            //取得ID组成的一维数组
