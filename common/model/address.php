...
class address extends core\model
{
    ...
    public function getAddressInfo($condition, $order = '')
    {
        ...
        if (core\config::get('delivery_isuse') && !empty($addr_info['dlyp_id'])) {
    ...
    public function getAddressList($condition, $order = 'address_id desc')
    {
        ...
        if (core\config::get('delivery_isuse')) {
            $dlyp_ids = array();
            $dlyp_new_list = array();
            ...
            if (!empty($dlyp_new_list)) {
                foreach ($address_list as $k => $v) {
                    ...
                    $address_list[$k]['address'] = '（' . $dlyp_info['dlyp_address_name'] . ') ' . $dlyp_info['dlyp_address'] . '，电话：' . trim($dlyp_info['dlyp_mobile'] . '，' . $dlyp_info['dlyp_telephony'], '，');
                }
            }
        }
        return $address_list;
