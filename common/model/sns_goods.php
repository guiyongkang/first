...
class sns_goods extends core\model
{
    ...
    public function goodsAdd($param)
    {
        ...
        if (is_array($param)) {
            $result = db\mysqli::insert('sns_goods', $param);
            return $result;
        } else {
            return false;
        }
    ...
    public function editGoods($param, $condition)
    {
        ...
        $result = db\mysqli::update('sns_goods', $param, $condition_str);
        return $result;
    }
    /**
     * 将条件数组组合为SQL语句的条件部分
    ...
    private function getCondition($condition_array)
    {
        $condition_sql = '';
        //商品ID
        if (!empty($condition_array['snsgoods_goodsid'])) {
