...
class trade extends core\model
{
    ...
    {
        $max_data = array('order_refund' => 15, 'refund_confirm' => 7, 'return_confirm' => 7, 'return_delay' => 5);
        if ($day_type == 'all') {
            return $max_data;
        }
        //返回所有
    ...
    {
        $state_data = array('order_cancel' => ORDER_STATE_CANCEL, 'order_default' => ORDER_STATE_NEW, 'order_paid' => ORDER_STATE_PAY, 'order_shipped' => ORDER_STATE_SEND, 'order_completed' => ORDER_STATE_SUCCESS);
        if ($type == 'all') {
            return $state_data;
        }
        //返回所有
