...
class distributor_chat extends core\model
{
    /**
     * get chat msg
     *
    ...
    {
        $result = $this->table('distributor_chat_msg')->where($condition)->page($page)->order('m_id')->select();
        return $result;
    }
    /**
     * add chat msg
    ...
    public function addMsg($msg)
    {
        ...
        $msg['add_time'] = time();
        $m_id = $this->table('distributor_chat_msg')->insert($msg);
        if ($m_id > 0) {
            $msg['m_id'] = $m_id;
            unset($msg['r_state']);
            ...
            $goods_id = 0;
            $goods_info = array();
            $pattern = '#' . APP_URL . '/index.php\\?act=goods&amp;op=index&amp;goods_id=(\\d+)$#';
            preg_match($pattern, $t_msg, $matches);
            if (isset($matches[1]) && intval($matches[1]) < 1) {
                //伪静态
                $pattern = '#' . APP_URL . '/item-(\\d+)\\.html$#';
                preg_match($pattern, $t_msg, $matches);
            }
    ...
    public function updateMsg($condition, $data)
    {
        ...
        if (is_array($data)) {
            $result = $this->table('distributor_chat_msg')->where($condition)->update($data);
            return $result;
        } else {
            return false;
        }
    ...
    {
        $list = array();
        $msg = $this->table('distributor_chat_msg')->field('count(DISTINCT t_id) as count')->where($condition)->find();
        if ($msg['count'] > 0) {
            $count = intval($msg['count']);
            $field = 't_id,t_name,max(add_time) as addtime';
            $list = $this->table('distributor_chat_msg')->field($field)->group('t_id')->where($condition)->limit($limit)->page($page, $count)->order('addtime desc')->select();
        }
        return $list;
    }
    /**
    ...
    {
        $list = array();
        $msg = $this->table('distributor_chat_msg')->field('count(DISTINCT f_id) as count')->where($condition)->find();
        if ($msg['count'] > 0) {
            $count = intval($msg['count']);
            $field = 'f_id,f_name,max(add_time) as addtime';
            $list = $this->table('distributor_chat_msg')->field($field)->group('f_id')->where($condition)->limit($limit)->page($page, $count)->order('addtime desc')->select();
        }
        return $list;
    }
    /**
    ...
    {
        return (int) $this->table('distributor_chat_msg')->where($condition)->count();
    }
    /**
     * 删除消息
     *
    ...
    {
        return $this->table('distributor_chat_msg')->where($condition)->delete();
    }
    /**
     * 删除消息
     *
