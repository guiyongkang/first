...
class buy
{
    ...
    public function addOrderLog($order_list = array())
    {
        ...
        foreach ($order_list as $order_id => $order) {
            ...
            $data['log_role'] = 'buyer';
            $data['log_msg'] = lang('order_log_new');
    ...
    public function pdPay($order_list, $input, $member_id, $member_name)
    {
        ...
        if ($buyer_info['member_paypwd'] == '' || $buyer_info['member_paypwd'] != md5($input['password'])) {
            return;
        }
    ...
    public function buyStep2($post, $member_id, $member_name, $member_email)
    {
        ...
        //更新收货人所在省份
        lib\queue::push('editReciverProid', array('order_ids' => array_keys($order_list), 'area_id' => $input_city_id));
        //删除购物车中的商品
        if ($post['ifcart']) {
            $model_cart->delCart('db', array('buyer_id' => $member_id, 'cart_id' => array('in', array_keys($input_buy_items))));
            //lib\queue::push('delCart', array('buyer_id'=>$member_id,'cart_ids'=>array_keys($input_buy_items)));
        }
