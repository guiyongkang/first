...
class exppoints extends core\model
{
    /**
     * 操作经验值
    ...
    function saveExppointsLog($stage, $insertarr)
    {
        ...
        $exppoints_rule = core\config::get("exppoints_rule") ? unserialize(core\config::get("exppoints_rule")) : array();
        //记录原因文字
        switch ($stage) {
            case 'login':
                if (empty($insertarr['exp_desc'])) {
