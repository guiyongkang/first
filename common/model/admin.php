...
class admin
{
    ...
    public function getAdminList($condition, $obj_page)
    {
        ...
        $param = array('table' => 'admin', 'field' => '*', 'where' => $condition_str);
    ...
    public function _condition($condition)
    {
        ...
        if (!empty($condition['admin_name'])) {
            $condition_str .= " and admin_name = '" . $condition['admin_name'] . "'";
        }
        if (!empty($condition['admin_password'])) {
    ...
    public function infoAdmin($param, $field = '*')
    {
        ...
        $param['field'] = $field;
        $admin_info = db\mysqli::select($param);
		if($admin_info){
			return $admin_info[0];
		}else{
			return array();
    ...
    public function addAdmin($param)
    {
        ...
        if (is_array($param)) {
            ...
            $result = db\mysqli::insert('admin', $tmp);
            return $result;
        } else {
            return false;
        }
    ...
    public function updateAdmin($param)
    {
        ...
        if (is_array($param)) {
            ...
            $result = db\mysqli::update('admin', $tmp, $where);
            return $result;
        } else {
            return false;
        }
    ...
    public function delAdmin($id)
    {
        if (intval($id) > 0) {
            ...
            $result = db\mysqli::delete('admin', $where);
            return $result;
        } else {
            return false;
        }
