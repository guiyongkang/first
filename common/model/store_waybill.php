...
class store_waybill extends core\model
{
    ...
    public function getStoreWaybillListWithWaybillInfo($store_id, $store_express)
    {
        ...
        return $this->table('store_waybill,waybill')->join('inner')->on('store_waybill.waybill_id=waybill.waybill_id')->where($condition)->field($field)->order('')->select();
    }
    /**
     * 读取单条记录
     * @param array $condition
