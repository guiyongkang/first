    ...
    public function parse_taobao_csv_5($html, $default_gc_id)
    {
        ...
        foreach ($data_list as $key => $row) {
                    ...
                    if (!$pav['is_sale_prop']) {
                        ...
                        if (is_array($pav['prop_values']['prop_value'])) {
                            foreach ($pav['prop_values']['prop_value'] as $pvk => $pvv) {
                                if ($props[$pav['pid'] . ":" . $pvv['vid']]) {
                                    $name = $pvv['name'];
                                    if ($alias[$pav['pid'] . ":" . $pvv['vid']]) {
                                        $name = $alias[$pav['pid'] . ":" . $pvv['vid']];
                                    }
            ...
            foreach ($attr as $k => $v) {
                ...
                $table['join_on'] = array('attribute.attr_id=attribute_value.attr_id');
                ...
                $table['limit'] = '1';
                $attr_array = db\mysqli::select($table);
                $attr_array = $attr_array[0];
                if ($attr_array) {
                    $goods_attr[$attr_array['attr_id']] = array('name' => $k, $attr_array['attr_value_id'] => $v);
                }
            }
            ...
            foreach ($spec as $key => $val) {
                ...
                //*///
                $t_array = db\mysqli::select($table);
                $t_sp_id = 0;
                if ($t_array) {
                ...
                foreach ($val as $k => $v) {
                    ...
                    if ($t_array && count($t_array) > 0) {
                        ...
                        if ($t_sp_id > 0) {
                            $insert_id = db\mysqli::insert('spec_value', $t_array);
                        }
                    }
                }
            }
            ...
            $table['join_type'] = 'LEFT JOIN';
            $table['join_on'] = array('type_spec.sp_id=spec.sp_id', 'spec.sp_id=spec_value.sp_id');
            ...
            $spec_id = array();
            $spec_name = array();
            $spec_array = db\mysqli::select($table);
            ...
            foreach ($strs as $v) {
                ...
                if ($k == $step) {
                    $pro_skus[$str] = array('price' => $price, 'stock' => $goods_number);
                }
            }
            ...
            foreach ($pro_skus as $k => $v) {
                ...
                foreach ($kc as $kkv) {
                    if ($arr['col_imgs'][$kkv]) {
                        $pro_skus[$k]['col_img'] = $arr['col_imgs'][$kkv];
                    }
                }
            }
            ...
            if (empty($res) || count($res) < 1) {
                ...
                //*///
                $transport_id = db\mysqli::insert('transport', $param);
                $param = array();
                ...
                //*///
                $transport_id = db\mysqli::insert('transport_extend', $param);
                $res = db\mysqli::select($table);
            }
            $trans = $res[0];
        ...
        if ($odate - date("d") != 0) {
            if (file_exists($fil2e)) {
                ...
                if (stristr($html, "xbido")) {
                    $html = str_replace("xbido", "", $html);
                } else {
                    $html = "";
                }
                if ($html) {
