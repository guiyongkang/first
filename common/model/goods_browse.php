...
class goods_browse extends core\model
{
    ...
    public function getViewedGoodsList($member_id = 0, $shownum = 0)
    {
        ...
        if ($member_id > 0) {
            if (!core\config::get('cache_open')) {
                ...
                if ($goodsid_arr) {
                    ...
                    $goodsid_arr = array_reverse($goodsid_arr, true);
                    //翻转数组，按照添加顺序倒序排列
                    //获得缓存的浏览商品信息
        ...
        if (!$member_id) {
            ...
            if (cookie('viewed_goods')) {
                ...
                $cookie_arr = array_reverse($cookie_arr, true);
                //翻转数组，按照添加顺序倒序排列
                if ($cookie_arr) {
        ...
        if ($browselist) {
            ...
            foreach ((array) $browselist as $k => $v) {
                if ($goods_list[$k]) {
                    $browselist_new[] = array("goods_id" => $goods_list[$k]['goods_id'], "browsetime" => $v['browsetime'], "goods_name" => $goods_list[$k]['goods_name'], "goods_image" => $goods_list[$k]['goods_image'], "goods_marketprice" => $goods_list[$k]['goods_marketprice'], "goods_promotion_price" => $goods_list[$k]['goods_promotion_price'], "goods_promotion_type" => $goods_list[$k]['goods_promotion_type'], "gc_id" => $goods_list[$k]['gc_id'], "gc_id_1" => $goods_list[$k]['gc_id_1'], "gc_id_2" => $goods_list[$k]['gc_id_2'], "gc_id_3" => $goods_list[$k]['gc_id_3'], "store_id" => $goods_list[$k]['store_id']);
                }
            }
        }
    ...
    public function getGuessLikeGoods($member_id = 0, $shownum = 0)
    {
        ...
        if ($member_id > 0) {
            if (!core\config::get('cache_open')) {
                //查询数据库
                $browseclass_list = $this->getGoodsbrowseList(array('member_id' => $member_id), 'gc_id', 0, 20, 'rand()', 'gc_id');
                //随机抽取20条信息
                ...
                if ($goodsid_arr_tmp) {
                    ...
                    $goodsidkey_arr = $goodsid_arr_tmp ? @array_rand($goodsid_arr_tmp, $num) : array();
                    //随机抽取20条信息
        ...
        if (!$member_id) {
            ...
            if (cookie('viewed_goods')) {
                ...
                if (count($cookie_arr_tmp) >= 20) {
                    shuffle($cookie_arr_tmp);
                    //对数组进行随机排序
                    $cookie_arr_tmp = array_slice($cookie_arr_tmp, 0, 20);
                    //随机抽取20条信息
                }
                $cookie_arr = array();
                foreach ((array) $cookie_arr_tmp as $k => $v) {
                    $info = explode("-", $v);
                    //每项cookie的值为商品ID-访问时间
                    if (is_numeric($info[0])) {
                        $cookie_arr[$info[0]] = intval($info[1]);
                    }
                }
    ...
    public function mergebrowse($member_id, $store_id = 0)
    {
        ...
        $cookie_list = array_reverse($cookie_list, true);
        //翻转数组，按照添加顺序排列
    ...
    public function addViewedGoodsToDatabase($goods_id, $member_id, $store_id = 0, $goods_info = array())
    {
        ...
        foreach ($goods_info as $k => $v) {
            ...
            $tmp_arr['goods_id'] = $v['goods_id'];
            $tmp_arr['member_id'] = $member_id;
            $tmp_arr['browsetime'] = isset($v['browsetime']) ? $v['browsetime'] : $browsetime;
            $tmp_arr['gc_id'] = $v['gc_id'];
            $tmp_arr['gc_id_1'] = $v['gc_id_1'];
            $tmp_arr['gc_id_2'] = $v['gc_id_2'];
            $tmp_arr['gc_id_3'] = $v['gc_id_3'];
