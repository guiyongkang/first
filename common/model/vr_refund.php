...
class vr_refund extends core\model
{
    ...
    public function addRefund($refund_array, $order = array())
    {
        ...
        try {
            ...
            $model_vr_order->editOrderCode(array('refund_lock' => 1), array('vr_code' => array('in', $code_array)));
            //退款锁定
            $this->commit();
            return $refund_id;
        } catch (\Exception $e) {
    ...
    public function editOrderRefund($refund)
    {
        ...
        try {
            ...
            $state = $this->editRefund(array('refund_id' => $refund_id), $refund_array);
            ////更新退款
            if ($state && $refund['admin_state'] == '2') {
                ...
                if ($predeposit_amount > 0) {
                    ...
                    $pay_amount = floatval($refund['pay_amount']);
                    //已完成在线退款金额
                    if ($pay_amount > 0) {
    ...
    {
        $admin_array = array('1' => '待审核', '2' => '同意', '3' => '不同意');
        //退款状态:1为待审核,2为同意,3为不同意
        core\tpl::output('admin_array', $admin_array);
        $state_data = array('admin' => $admin_array);
        if ($type == 'all') {
            return $state_data;
        }
        //返回所有
