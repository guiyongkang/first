...
class help extends core\model
{
        ...
        if (empty($condition)) {
            ...
            if (!empty($upload_list) && is_array($upload_list)) {
                foreach ($upload_list as $key => $value) {
					if(file_exists(BASE_UPLOAD_PATH . DS . ATTACH_ARTICLE . DS . $value['file_name'])){
    ...
    public function getShowStoreHelpList($condition = array())
    {
        ...
        //是否显示,0为否,1为是
        $list = $this->getStoreHelpTypeList($condition);
        //帮助类型
        if (!empty($list) && is_array($list)) {
            $type_ids = array_keys($list);
            //类型编号数组
            $condition = array();
