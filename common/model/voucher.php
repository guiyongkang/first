...
class voucher extends core\model
{
    ...
    //默认领取方式
    private $voucher_gettype_array = array('points' => array('sign' => 1, 'name' => '元兑换'), 'pwd' => array('sign' => 2, 'name' => '卡密兑换'), 'free' => array('sign' => 3, 'name' => '免费领取'));
    ...
    {
        return array('unused' => array(1, core\language::get('voucher_voucher_state_unused')), 'used' => array(2, core\language::get('voucher_voucher_state_used')), 'expire' => array(3, core\language::get('voucher_voucher_state_expire')));
    }
    /**
    ...
    public function getCurrentAvailableVoucherCount($member_id)
    {
        ...
        if (empty($info['voucher_count'])) {
            ...
            $voucher_count = isset($info['voucher_count']) ? intval($info['voucher_count']) : 0;
        }
    ...
    public function getCanChangeTemplateInfo($vid, $member_id, $store_id = 0)
    {
        ...
        if ($template_info['voucher_t_gettype'] == $this->voucher_gettype_array['points']['sign'] && $template_info['voucher_t_points'] > 0) {
            if (intval($member_info['member_points']) < intval($template_info['voucher_t_points'])) {
                return array('state' => false, 'msg' => '您的元不足，暂时不能兑换该代金券');
            }
        }
        //验证会员级别
        ...
        if (!empty($voucher_list)) {
            ...
            if ($voucher_count >= intval(core\config::get('promotion_voucher_buyertimes_limit'))) {
                $message = sprintf('您的可用代金券已有%s张,不可再兑换了', core\config::get('promotion_voucher_buyertimes_limit'));
