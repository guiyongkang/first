...
class web_config extends core\model
{
    /**
     * 读取模块内容记录
     *
    ...
    public function updateWebHtml($web_id = 1, $style_name = 'orange')
    {
        ...
        if (!empty($code_list) && is_array($code_list)) {
            core\language::read('web_config,home_index_index');
    ...
    public function getWebHtml($web_page = 'index', $update_all = 0)
    {
        ...
        if (!empty($web_list) && is_array($web_list)) {
            foreach ($web_list as $k => $v) {
                $key = $v['web_page'];
				$web_array[$key] = isset($web_array[$key]) ? $web_array[$key] : '';
    ...
    {
        $condition = array();
        $condition['screen'] = array('ap_class' => '0', 'is_use' => '1', 'ap_width' => '1920', 'ap_height' => '481');
        $condition['focus'] = array('ap_class' => '0', 'is_use' => '1', 'ap_width' => '259', 'ap_height' => '180');
    ...
    {
        $style_data = array('red' => core\language::get('web_config_style_red'), 'pink' => core\language::get('web_config_style_pink'), 'orange' => core\language::get('web_config_style_orange'), 'green' => core\language::get('web_config_style_green'), 'blue' => core\language::get('web_config_style_blue'), 'purple' => core\language::get('web_config_style_purple'), 'brown' => core\language::get('web_config_style_brown'), 'default' => core\language::get('web_config_style_default'));
    ...
    public function get_array($code_info, $code_type)
    {
        ...
        switch ($code_type) {
            case 'array':
                if (!empty($code_info) && is_string($code_info)) {
    ...
    public function getGoodsList($condition = array(), $order = 'goods_id desc', $page = '')
    {
        ...
        if (!empty($goods_list) && is_array($goods_list)) {
            ...
            $common_ids = array_keys($goods_commonlist);
            //商品公共ID数组
    ...
    public function updateWebGoods($condition = array('web_show' => '1'))
    {
        ...
        $web_list = $this->getWebList($condition);
        //板块列表
        if (!empty($web_list) && is_array($web_list)) {
            foreach ($web_list as $k => $v) {
                $web_id = $v['web_id'];
                    ...
                    if (!empty($recommend_list) && is_array($recommend_list)) {
                        foreach ($recommend_list as $k => $v) {
                            if (!empty($v['goods_list']) && is_array($v['goods_list'])) {
                                //商品列表
                                $goods_id_array = array_keys($v['goods_list']);
                                //商品ID
