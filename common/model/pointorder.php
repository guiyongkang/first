...
class pointorder extends core\model
{
    private $product_sn;
    // 订单编号
    /**
     * 生成元兑换订单编号(两位随机 + 从2000-01-01 00:00:00 到现在的秒数+微秒+会员ID%1000)，该值会传给第三方支付接口
     * 长度 =2位 + 10位 + 3位 + 3位 = 18位
    ...
    {
        return $this->product_sn = mt_rand(10, 99) . sprintf('%010d', time() - 946656000) . sprintf('%03d', (double) microtime() * 1000) . sprintf('%03d', (int) $member_id % 1000);
    }
    /**
     * 通过状态标识获取兑换订单状态
     */
    ...
    public function receivingPointOrder($point_orderid, $member_id = 0)
    {
        ...
        $where['point_orderstate'] = $pointorderstate_arr['waitreceiving'][0];
        // 已发货待收货
        // 更新运费
    ...
    public function createOrder($post_arr, $pointprod_arr, $member_info)
    {
        ...
        if ($address_info) {
            ...
            $address_array['point_telphone'] = !isset($address_info['tel_phone']) ? $address_info['mob_phone'] : $address_info['tel_phone'];
    ...
    public function delPointOrderByOrderID($order_id)
    {
        ...
        $where['point_orderstate'] = $pointorderstate_arr['canceled'][0];
        // 只有取消的订单才能删除
    ...
    public function shippingPointOrder($order_id, $postarr, $order_info = array())
    {
        ...
        if ($order_info['point_orderstate'] == $pointorderstate_arr['waitship'][0]) {
            ...
            $update_arr['point_orderstate'] = $pointorderstate_arr['waitreceiving'][0];
            // 已经发货,待收货
        }
