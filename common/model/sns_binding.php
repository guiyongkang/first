...
class sns_binding extends core\model
{
    ...
    public function getUsableApp($member_id)
    {
        ...
        foreach ($app_arr as $k => $v) {
            if (core\config::get('share_' . $k . '_isuse') == 1) {
                $v['isbind'] = false;
                $app_arrnew[$k] = $v;
            }
        }
    ...
    {
        $app_arr = array();
        $app_arr['qqweibo'] = array('name' => core\language::get('nc_shareset_qqweibo'), 'url' => "http://t.qq.com", 'applyurl' => 'http://dev.t.qq.com');
        $app_arr['sinaweibo'] = array('name' => core\language::get('nc_shareset_sinaweibo'), 'url' => "http://www.weibo.com", 'applyurl' => 'http://open.weibo.com/developers');
        return $app_arr;
    }
    /**
    ...
    public function addSinaWeiboUpload($bindinfo, $params)
    {
        ...
        $c = new SaeTClientV2(core\config::get('share_sinaweibo_appid'), core\config::get('share_sinaweibo_appkey'), $bindinfo['snsbind_accesstoken']);
        if ($params['images']) {
