...
class circle_exp extends core\model
{
    ...
    private function relOperation($param)
    {
        $exp = 0;
        $today = date('Y-m-d');
        $dexp = $param['type'] == 'release' ? intval(core\config::get('circle_exprelease')) : intval(core\config::get('circle_expreply'));
        if (intval(core\config::get('circle_expreleasemax')) != 0) {
            $exp_info = $this->table('circle_expmember')->find($param['member_id']);
            if (!empty($exp_info) && $exp_info['em_time'] == $today) {
                // there are data
                $exp = $dexp > intval(core\config::get('circle_expreleasemax')) - intval($exp_info['em_exp']) ? intval(core\config::get('circle_expreleasemax')) - intval($exp_info['em_exp']) : $dexp;
            } else {
                // There is no data
                $exp = $dexp > intval(core\config::get('circle_expreleasemax')) ? intval(core\config::get('circle_expreleasemax')) : $dexp;
            }
            $insert = array();
    ...
    private function repdOperation($param)
    {
        $exp = 0;
        $today = date('Y-m-d');
        if (intval(core\config::get('circle_exprepliedmax')) != 0) {
            $exp_info = $this->table('circle_exptheme')->find($param['theme_id']);
            if (!empty($exp_info) && $exp_info['et_time'] == $today) {
                // there are data
                $exp = intval(core\config::get('circle_expreplied')) > intval(core\config::get('circle_exprepliedmax')) - intval($exp_info['et_exp']) ? intval(core\config::get('circle_exprepliedmax')) - intval($exp_info['et_exp']) : intval(core\config::get('circle_expreplied'));
            } else {
                // There is no data
                $exp = intval(core\config::get('circle_expreplied')) > intval(core\config::get('circle_exprepliedmax')) ? intval(core\config::get('circle_exprepliedmax')) : intval(core\config::get('circle_expreplied'));
            }
            $insert = array();
            ...
            $exp = intval(core\config::get('circle_expelease'));
        }
        if ($exp != 0) {
            $param['exp'] = $exp;
            return $this->expOperation(1, $param);
    ...
    private function expOperation($type, $param)
    {
        ...
        if ($type) {
            ...
            foreach ($data as $val) {
                ...
                if (intval($val['mld_exp']) > $update['cm_exp']) {
                    break;
                }
            }
            // upgrade
    ...
    private function recordsLog($type, $param)
    {
        ...
        switch ($param['type']) {
            case 'release':
                ...
                $insert['el_type'] = 2;
                $insert['el_desc'] = lang('circle_exp_release_theme');
                break;
            case 'reply':
                $update = array();
                ...
                $insert['el_type'] = 3;
                $insert['el_desc'] = lang('circle_exp_reply');
                break;
            case 'replied':
                $update = array();
                ...
                $this->table('circle_theme')->update($update);
                $insert['el_type'] = 4;
                $insert['el_desc'] = lang('circle_exp_theme_replied');
                break;
            case 'delRelease':
                $insert['el_type'] = 5;
                $insert['el_desc'] = lang('circle_exp_theme_delete');
                break;
            case 'delReplied':
                $insert['el_type'] = 6;
                $insert['el_desc'] = lang('circle_exp_reply_delete');
                break;
            case 'master':
                $insert['el_type'] = 1;
