...
class search
{
    ...
    public function getIndexerList($condition = array(), $order = array(), $pagesize = 24)
    {
        try {
            //全文搜索初始化
            $this->_createXS($pagesize, core\config::get('fullindexer.appname'));
            //设置搜索内容
            ...
            //             } else {
            core\logging::write('search\\index' . $e->getMessage() . "\r\n" . $sql);
            return false;
            //             }
        }
    }
    ...
    private function _createXS($pagesize, $appname)
    {
        ...
        $obj_xs = new \XS(core\config::get('fullindexer.appname'));
        ...
        if ($keyword != '') {
            ...
            if (!empty($tag_list) && is_array($tag_list)) {
                foreach ($tag_list as $key => $val) {
                    if (strpos($val['gc_tag_value'], $keyword)) {
                        $data[] = $val['gc_id'];
                    }
                }
            }
        ...
        foreach ($data as $value) {
            ...
            if (!empty($gc_list[$value[0]])) {
                ...
                if (!empty($gc_list[$value[0]]['class2'][$value[1]])) {
                    ...
                    if (!empty($gc_list[$value[0]]['class2'][$value[1]]['class3'][$value[2]])) {
                        // 三级
    ...
    public function indexerSearch($get = array(), $pagesize)
    {
        ...
        if (isset($get['key']) && in_array($get['key'], array('1', '2', '3'))) {
            $order = array(str_replace(array('1', '2', '3'), array('goods_salenum', 'goods_click', 'goods_price'), $get['key']) => $get['order'] == '1' ? true : false);
        }
        //取得商品主键等信息
