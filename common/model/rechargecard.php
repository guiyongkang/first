...
class rechargecard extends core\model
{
    ...
    {
        return $this->where(array('sn' => (string) $sn))->find();
    }
    /**
     * 设置充值卡为已使用
     *
    ...
    {
        return $this->where(array('id' => (string) $id))->update(array('tsused' => time(), 'state' => 1, 'member_id' => $memberId, 'member_name' => $memberName));
    }
    /**
     * 通过ID删除充值卡（自动添加未使用标记）
     *
    ...
    {
        return $this->where(array('id' => array('in', (array) $id), 'state' => 0))->delete();
    }
    /**
    ...
    {
        $array = $this->field('sn')->where(array('sn' => array('in', $sns)))->select();
        $data = array();
