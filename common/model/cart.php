...
class cart extends core\model
{
    ...
    {
        return (int) $this->where(array('buyer_id' => (int) $memberId))->count();
    }
    /**
     * 取得 单条购物车信息
    ...
    private function _addCartCookie($goods_info = array(), $quantity = null)
    {
        ...
        $cart_array[$goods_info['goods_id']] = array('store_id' => $goods_info['store_id'], 'goods_id' => $goods_info['goods_id'], 'goods_name' => $goods_info['goods_name'], 'goods_price' => $goods_info['goods_price'], 'goods_image' => $goods_info['goods_image'], 'goods_num' => $quantity);
    ...
    public function editCart($data, $condition)
    {
        ...
        if ($result) {
            $this->getCartNum('db', array('buyer_id' => isset($condition['buyer_id']) ? $condition['buyer_id'] : 0));
        }
        return $result;
    }
    /**
