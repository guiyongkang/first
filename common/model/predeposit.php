...
class predeposit extends core\model
{
    /**
     * 生成充值编号
     * @return string
    ...
    {
        return mt_rand(10, 99) . sprintf('%010d', time() - 946656000) . sprintf('%03d', (double) microtime() * 1000) . sprintf('%03d', (int) core\session::get('member_id') % 1000);
    }
    public function addRechargeCard($sn, array $session)
    {
        ...
        if ($memberId < 1 || !$memberName) {
            throw_exception("当前登录状态为未登录，不能使用充值卡");
        }
    ...
    public function changeRcb($type, $data = array())
    {
        ...
        // 更新会员
        $updateSuccess = model('member')->editMember(array('member_id' => $data['member_id']), $update);
        if (!$updateSuccess) {
            throw_exception('操作失败');
        }
        // 添加日志
        $log = array('member_id' => $data['member_id'], 'member_name' => $data['member_name'], 'type' => $type, 'add_time' => TIMESTAMP, 'available_amount' => $available, 'freeze_amount' => $freeze, 'description' => $desc);
        $insertSuccess = $this->table('rcb_log')->insert($log);
        if (!$insertSuccess) {
            throw_exception('操作失败');
        }
        $msg = array('code' => 'recharge_card_balance_change', 'member_id' => $data['member_id'], 'param' => array('time' => date('Y-m-d H:i:s', TIMESTAMP), 'url' => urlShop('predeposit', 'rcb_log_list'), 'available_amount' => ncPriceFormat($available), 'freeze_amount' => ncPriceFormat($freeze), 'description' => $desc));
        // 发送买家消息
        lib\queue::push('sendMemberMsg', $msg);
    ...
    public function changePd($change_type, $data = array())
    {
        ...
        switch ($change_type) {
            ...
            case 'order_cancel':
                $data_log['lg_av_amount'] = $data['amount'];
                $data_log['lg_freeze_amount'] = -$data['amount'];
                $data_log['lg_desc'] = '取消订单，解冻见余额，订单号: ' . $data['order_sn'];
                $data_pd['freeze_predeposit'] = array('exp', 'freeze_predeposit-' . $data['amount']);
                $data_pd['available_predeposit'] = array('exp', 'available_predeposit+' . $data['amount']);
                $data_msg['av_amount'] = $data['amount'];
                $data_msg['freeze_amount'] = -$data['amount'];
			...
			case 'commission_come':
				$data_log['lg_av_amount'] = $data['amount'];
                $data_log['lg_desc'] = '提现转入余额，订单号: ' . $data['order_sn'];
                $data_pd['available_predeposit'] = array('exp', 'available_predeposit+' . $data['amount']);
                $data_msg['av_amount'] = $data['amount'];
                $data_msg['freeze_amount'] = 0;
                $data_msg['desc'] = $data_log['lg_desc'];
     ...
     * @param array $data
     */
    public function addPdZz($data)
    {
        return $this->table('pd_zz')->insert($data);
    }
	/**
     * 取得转账列表
     * @param unknown $condition
     ...
     * @param string $order
     */
    public function getPdZzList($condition = array(), $pagesize = '', $fields = '*', $order = '', $limit = '')
    {
        return $this->table('pd_zz')->where($condition)->field($fields)->order($order)->limit($limit)->page($pagesize)->select();
    }
    /**
     * 添加提现记录
     * @param array $data
