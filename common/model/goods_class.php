...
class goods_class extends core\model
{
    /**
     * 缓存数据
     */
    ...
    public function getGoodsClassForCacheModel()
    {
		...
		if($data){
			...
			$p = $data['parent'];
			$c = $data['children'];
			$c2 = isset($data['children2']) ? $data['children2'] : array();
			$r = (array) $r;
			foreach ($r as $k => &$v) {
				if ((string) $p[$k] == '0') {
					$v['depth'] = 1;
					...
					if (isset($data['children2'][$k])) {
						$v['childchild'] = implode(',', $c2[$k]);
					}
				} else {
					if (isset($p[$p[$k]]) && (string) $p[$p[$k]] == '0') {
						$v['depth'] = 2;
						if (isset($data['children'][$k])) {
							$v['child'] = implode(',', $c[$k]);
						}
					} else {
						if (isset($p[$p[$k]]) && isset($p[$p[$p[$k]]]) && (string) $p[$p[$p[$k]]] == '0') {
							$v['depth'] = 3;
						}
					}
				}
        ...
        if ($update_all == 1 || !($gc_list = rkcache('all_categories'))) {
            ...
            if (is_array($class_list) && !empty($class_list)) {
                ...
                foreach ($gc_list as $key => $value) {
                    ...
                    $class3s = isset($class1_deep[$gc_id]) ? $class1_deep[$gc_id] : array();
    ...
    public function getGoodsClassNav($id = 0, $sign = 1)
    {
        if (intval($id) > 0) {
            ...
            if ($sign == 1) {
                ...
                $nav_link[] = array('title' => $data[$id]['gc_name'], 'link' => urlShop('search', 'index', array('cate_id' => $data[$id]['gc_id'])));
            }
            // 最多循环4层
            for ($i = 1; $i < 5; $i++) {
        ...
        // 首页导航
        $nav_link[] = array('title' => Language::get('homepage'), 'link' => APP_URL);
        krsort($nav_link);
        return $nav_link;
    }
    /**
    ...
    public function getGoodsClassLineForTag($id = 0)
    {
        if (intval($id) > 0) {
            ...
            $gc_line['gc_id'] = isset($class['gc_id']) ? $class['gc_id'] : 0;
            $gc_line['type_id'] = isset($class['type_id']) ? $class['type_id'] : 0;
            $gc_line['gc_virtual'] = isset($class['gc_virtual']) ? $class['gc_virtual'] : 0;
            /**
             * 是否是子类
             */
			$gc_line['gc_tag_name'] = $gc_line['gc_tag_value'] = '';
            if (isset($class['gc_parent_id']) && $class['gc_parent_id'] != 0) {
            ...
            if (!isset($gc_line['gc_id_1'])) {
                $gc_line['gc_id_1'] = isset($class['gc_id']) ? $class['gc_id'] : 0;
            } else {
                if (!isset($gc_line['gc_id_2'])) {
                    $gc_line['gc_id_2'] = isset($class['gc_id']) ? $class['gc_id'] : 0;
                } else {
                    $gc_line['gc_id_3'] = isset($class['gc_id']) ? $class['gc_id'] : 0;
                }
            }
