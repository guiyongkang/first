...
class message
{
    ...
    public function listMessage($condition, $page = '')
    {
        ...
        $message_list = db\mysqli::select($param, $page);
        return $message_list;
    }
    /**
     * 卖家站内信列表
    ...
    public function countMessage($condition)
    {
        ...
        $message_list = db\mysqli::select($param);
    ...
    public function saveMessage($param)
    {
        ...
        return db\mysqli::insert('message', $array);
    }
    /**
     * 更新站内信
     */
    public function updateCommonMessage($param, $condition)
    {
        ...
        //得到条件语句
        $condition_str = $this->getCondition($condition);
        db\mysqli::update('message', $param, $condition_str);
    }
    /**
     * 删除发送信息
     */
    public function dropCommonMessage($condition, $drop_type)
    {
        ...
        //查询站内信列表
        $message_list = array();
        $message_list = db\mysqli::select(array('table' => 'message', 'where' => $condition_str, 'field' => 'message_id,from_member_id,to_member_id,message_state,message_open'));
        unset($condition_str);
        ...
        if (!empty($delmessage_id)) {
            ...
            db\mysqli::delete('message', $condition_str);
            unset($condition_str);
        }
        if (!empty($updatemessage_id)) {
            ...
            if ($drop_type == 'msg_private') {
                db\mysqli::update('message', array('message_state' => 1), $condition_str);
            } elseif ($drop_type == 'msg_list') {
                db\mysqli::update('message', array('message_state' => 2), $condition_str);
            }
        }
        return true;
    }
    ...
    public function dropBatchMessage($condition, $to_member_id)
    {
        ...
        //查询站内信列表
        $message_list = array();
        $message_list = db\mysqli::select(array('table' => 'message', 'where' => $condition_str));
        unset($condition_str);
        if (empty($message_list)) {
            return true;
        }
        foreach ($message_list as $k => $v) {
            ...
            if ($tmp_delid_str == $v['to_member_id']) {
                //所有用户已经全部阅读过了可以删除
                db\mysqli::delete('message', " message_id = '{$v['message_id']}' ");
            } else {
                db\mysqli::update('message', array('del_member_id' => $tmp_delid_str), " message_id = '{$v['message_id']}' ");
            }
        }
        return true;
    }
    private function getCondition($condition_array)
    {
        ...
        if (!empty($condition_array['to_member_id'])) {
        ...
        //发信人
        if (!empty($condition_array['from_member_id'])) {
            $condition_sql .= " and message.from_member_id='{$condition_array['from_member_id']}' ";
        }
        if (!empty($condition_array['from_to_member_id'])) {
        ...
        //未删除
        if (!empty($condition_array['no_del_member_id'])) {
            $condition_sql .= " and message.del_member_id not like '%,{$condition_array['no_del_member_id']},%' ";
        }
        //未读
        if (!empty($condition_array['no_read_member_id'])) {
        ...
        if (isset($condition_array['message_id_in'])) {
            if (!empty($condition_array['message_id_in'])) {
