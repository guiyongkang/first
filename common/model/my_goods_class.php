...
class my_goods_class extends core\model
{
    ...
    public function getClassInfo($param, $field = '*')
    {
        ...
        $param['field'] = $field;
        $class_info = db\mysqli::select($param);
        return $class_info[0];
    }
    /**
     * 类别添加
    ...
    public function addGoodsClass($param)
    {
        ...
        $result = db\mysqli::insert('store_goods_class', $class_array);
        if ($result) {
            return true;
        } else {
            return false;
    ...
    public function editGoodsClass($param, $class_id)
    {
        ...
        $class_array = array();
        if (!empty($param['stc_name'])) {
            $class_array['stc_name'] = $param['stc_name'];
        }
        if (!empty($param['stc_parent_id'])) {
            $class_array['stc_parent_id'] = $param['stc_parent_id'];
        }
        if (!empty($param['stc_state'])) {
            $class_array['stc_state'] = $param['stc_state'];
        }
        if (!empty($param['stc_sort'])) {
            $class_array['stc_sort'] = $param['stc_sort'];
        }
        $state = db\mysqli::update('store_goods_class', $class_array, " WHERE stc_id='{$class_id}'");
        if ($state) {
            return true;
        } else {
            return false;
    ...
    public function dropGoodsClass($store_class_id)
    {
        ...
        foreach ($array as $val) {
            ...
            if (!empty($class_info['stc_id'])) {
                db\mysqli::delete('store_goods_class', " WHERE stc_id='{$val}'");
            }
        }
        return true;
    }
        ...
        if (is_array($class_list)) {
            foreach ($class_list as $v) {
                if ($v['stc_parent_id'] == 0) {
                    ...
                    if (isset($d[$v['stc_parent_id']])) {
                        $d[$v['stc_parent_id']]['child'][] = $v;
                    }
                    //防止出现父类不显示时子类被调出
                }
            }
    ...
    public function getChildAndSelfClass($stc_id, $stc_state = '')
    {
        ...
        if (!empty($stc_id_arr) && is_array($stc_id_arr)) {
            $stc_id_arr[] = $stc_id;
            //加上一级分类本身
            return $stc_id_arr;
        } else {
    ...
    {
        $condition_sql = '';
        if (!empty($condition_array['stc_top'])) {
            $condition_sql .= " and stc_parent_id=0";
        }
        if (!empty($condition_array['stc_id'])) {
        ...
        if (!empty($condition_array['store_id'])) {
            $condition_sql .= " and store_id= '{$condition_array['store_id']}'";
        }
        if (!empty($condition_array['stc_parent_id'])) {
