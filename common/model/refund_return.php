...
class refund_return extends core\model
{
    /**
     * 取得退单数量
    ...
    public function addRefundReturn($refund_array, $order = array(), $goods = array())
    {
        ...
        $type = isset($refund_array['order_lock']) && $refund_array['order_lock'] == 2 ? '售前' : '售后';
    ...
    public function addDetail($refund, $order)
    {
        ...
        $detail_array['batch_no'] = date('YmdHis') . $refund['refund_id'];
        ...
        if (!empty($order['trade_no']) && in_array($order['payment_code'], array('wxpay', 'wx_jsapi', 'wx_saoma'))) {
            //微信支付
            ...
            if (!empty($apiclient_cert) && !empty($apiclient_key)) {
                //验证商户证书路径设置
                $detail_array['refund_code'] = $order['payment_code'];
            }
        }
        if (!empty($order['trade_no']) && $order['payment_code'] == 'alipay') {
            //支付宝
    ...
    public function getPayDetailInfo($detail_array)
    {
        ...
        //订单详细
        $order['pay_amount'] = ncPriceFormat($order['order_amount'] - $order['rcb_amount'] - $order['pd_amount']);
        //在线支付金额=订单总价格-充值卡支付金额-预存款支付金额
        $out_amount = $order['pay_amount'] - $order['refund_amount'];
        //可在线退款金额
        $refund_amount = $detail_array['refund_amount'];
        //本次退款总金额
        ...
        if (in_array($order['payment_code'], array('wxpay', 'wx_jsapi'))) {
            //手机客户端微信支付
            if ($order['payment_code'] == 'wx_jsapi') {
                $condition['payment_code'] = 'wxpay_jsapi';
            }
            $model_payment = model('mb_payment');
            $payment_info = $model_payment->getMbPaymentInfo($condition);
            //接口参数
    ...
    public function getGoodsRefundList($order_list = array(), $order_refund = 0)
    {
        ...
        if (!empty($order_list) && is_array($order_list)) {
            foreach ($order_list as $key => $value) {
                $order_id = $key;
                $goods_list = $value['extend_order_goods'];
                //订单商品
                ...
                if ($order_state == $order_paid && $payment_code != 'offline') {
                    ...
                    //已发货后对商品操作
                    $refund = $this->getRefundState($value);
                    //根据订单状态判断是否可以退款退货
                    foreach ($goods_list as $k => $v) {
                        ...
                        if (!empty($refund_goods[$order_id][$goods_id])) {
                            $seller_state = $refund_goods[$order_id][$goods_id]['seller_state'];
                            //卖家处理状态:1为待审核,2为同意,3为不同意
                            if ($seller_state == 3) {
                                $order_list[$order_id]['extend_complain'][$goods_id] = '1';
                                //不同意可以发起退款投诉
                            } else {
                                $v['refund'] = '0';
                                //已经存在处理中或同意的商品不能再操作
        ...
        if (!empty($order) && is_array($order)) {
            ...
            foreach ($goods_list as $k => $v) {
                ...
                if (!empty($v['extend_refund'])) {
                    $v['refund_state'] = $v['extend_refund']['seller_state'];
                    //卖家处理状态为3,不同意时能退款
                }
                if ($v['refund_state'] > 2) {
                    //可退商品
    ...
    public function getRightOrderList($order_condition, $order_goods_id = 0)
    {
        ...
        core\tpl::output('order', $order_info);
        $order_id = $order_info['order_id'];
        $store = $order_info['extend_store'];
        core\tpl::output('store', $store);
        $order_common = $order_info['extend_order_common'];
        core\tpl::output('order_common', $order_common);
        if ($order_common['shipping_express_id'] > 0) {
            $express = rkcache('express', true);
            core\tpl::output('e_code', $express[$order_common['shipping_express_id']]['e_code']);
            core\tpl::output('e_name', $express[$order_common['shipping_express_id']]['e_name']);
        }
        $condition = array();
        $condition['order_id'] = $order_id;
        if ($order_goods_id > 0) {
    ...
    public function getRefundState($order)
    {
        ...
        //30:已发货
        $order_completed = $model_trade->getOrderState('order_completed');
        //40:已收货
        switch ($order_state) {
            case $order_shipped:
            ...
            case $order_completed:
                $order_refund = $model_trade->getMaxDay('order_refund');
                //15:收货完成后可以申请退款退货
    ...
    {
        core\language::read('refund');
        $state_array = array('1' => core\language::get('refund_state_confirm'), '2' => core\language::get('refund_state_yes'), '3' => core\language::get('refund_state_no'));
        //卖家处理状态:1为待审核,2为同意,3为不同意
        core\tpl::output('state_array', $state_array);
        $admin_array = array('1' => '处理中', '2' => '待处理', '3' => '已完成');
        //确认状态:1为买家或卖家处理中,2为待平台管理员处理,3为退款退货已完成
        core\tpl::output('admin_array', $admin_array);
