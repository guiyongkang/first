...
class type extends core\model
{
    ...
    {
        $spec_json = $spec_list = $attr_list = $brand_list = array();
        if ($type_id > 0) {
            ...
            if (!empty($spec_list) && is_array($spec_list)) {
                ...
                foreach ($spec_list as $val) {
                    ...
                    $array[$val['sp_id']]['sp_format'] = isset($val['sp_format']) ? $val['sp_format'] : '';
    ...
    public function addGoodsType($goods_id, $commonid, $param)
    {
        ...
        if (!empty($param['attr']) && is_array($param['attr'])) {
    ...
    public function typeList($param, $page = '', $field = '*')
    {
        ...
        $list_type = db\mysqli::select($array, $page);
        return $list_type;
    }
    /**
     * 添加类型信息
    ...
    {
        return db\mysqli::insert($table, $param);
    }
    /**
     * 添加对应关系信息
     * @param string $table 表名
    ...
    public function typeRelatedAdd($table, $param, $id, $row = '')
    {
        ...
        return db\mysqli::query('insert into `' . DBPRE . $table . '` (' . $row . ') values ' . $insert_str);
    }
    /**
     * 添加商品与规格、属性对应关系信息
     * 
    ...
    public function typeRelatedList($table, $param, $field = '*')
    {
        ...
        $list_type = db\mysqli::select($array);
        return $list_type;
    }
    /**
     * 计算商品类型与品牌对应表数量
    ...
    public function typeUpdate($update, $param, $table)
    {
        ...
        if (is_array($update)) {
            ...
            $result = db\mysqli::update($table, $tmp, $condition_str);
            return $result;
        } else {
            return false;
        }
     ...
     * @return boolean
     */
    public function typeRelatedJoinList($param = array(), $type = '', $field = '*', $order = '')
    {
        $array = array();
        switch ($type) {
            case 'spec':
    ...
    {
        $condition_str = $this->getCondition($param);
        return db\mysqli::delete($table, $condition_str);
    }
    /**
     * 将条件数组组合为SQL语句的条件部分
     * 
    ...
    {
        $condition_str = '';
        if (!empty($condition_array['goods_id'])) {
            $condition_str .= ' and goods_id =\'' . $condition_array['goods_id'] . '\'';
        }
        if (!empty($condition_array['in_goods_id'])) {
