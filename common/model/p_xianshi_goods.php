...
class p_xianshi_goods extends core\model
{
        ...
        if ($result) {
            ...
            if (!empty($xianshi_goods_list)) {
                foreach ($xianshi_goods_list as $val) {
                    ...
                    // 插入对列 更新促销价格
                    lib\queue::push('updateGoodsPromotionPriceByGoodsId', $val['goods_id']);
                }
            }
        }
        return $result;
    ...
    public function getXianshiGoodsInfoByGoodsID($goods_id)
    {
        ...
        if (empty($info)) {
            ...
            $info['info'] = isset($xianshi_goods_list[0]) ? serialize($xianshi_goods_list[0]) : '';
