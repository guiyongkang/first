...
class model
{
    ...
    public function tableInfo($table)
    {
		...
		if (cache::has('_pk')){
            $this->fields = rkcache('_pk');
        } else {
            $_pk_array = self::fetchTablePkArray();
			wkcache('_pk', $_pk_array);
    ...
    {
        $full_table = db\mysqli::showTables();
		
        $_pk_array = array();
        $count = strlen(config::get('tablepre'));
        foreach ($full_table as $v_table) {
            $v = array_values($v_table);
            if (substr($v[0], 0, $count) != config::get('tablepre')) {
                continue;
            }
            $tb = preg_replace('/^' . config::get('tablepre') . '/', '', $v[0]);
            $fields = db\mysqli::showColumns($tb);
    ...
    public function __call($method, $args)
    {
        if (in_array(strtolower($method), array('table', 'order', 'where', 'on', 'limit', 'having', 'group', 'lock', 'master', 'distinct', 'index', 'attr', 'key'), true)) {
            ...
            if (isset($args[0]) && $args[0] == null) {
                return $this;
            } elseif (isset($args[0]) && (!is_numeric($args[0]) || $args[0] <= 0)) {
                $args[0] = 10;
            }
            ...
            pagecmd('setEachNum', $args[0]);
            $this->unoptions = false;
            pagecmd('setTotalNum', $this->get_field('COUNT(*) AS count'));
            return $this;
            ...
            $field = isset($args[0]) ? $args[0] : '*';
            $options['field'] = 'count(' . $field . ') AS count';
    ...
    public function select($options = array())
    {
        ...
        if (isset($options['limit']) && $options['limit'] !== false) {
            if (empty($options['where']) && empty($options['limit'])) {
                //如果无条件，默认检索30条数据
                $options['limit'] = 30;
            } elseif (isset($options['where']) && $options['where'] !== true && empty($options['limit'])) {
    ...
    public function delete($options = array())
    {
        if (is_numeric($options) || is_string($options)) {
            ...
            if (strpos($options, ',')) {
                ...
                $where[$pk] = (isset($this->fields['_pk_type']) && $this->fields['_pk_type'] == 'int') ? intval($options) : $options;
                $pkValue = $options;
            }
            $options = array();
            $options['where'] = $where;
    ...
    {
        db\mysqli::beginTransaction($host);
    }
    /**
     * 提交事务
     *
...
class ModelDb
{
    ...
    public function select($options = array())
    {
        ...
        $result = db\mysqli::getAll($sql, (isset($options['lock']) && $options['lock'] === true) || (isset($options['master']) && $options['master'] === true) || defined('TRANS_MASTER') ? 'master' : 'slave');
    ...
    protected function parseWhereItem($key, $val)
    {
        ...
        if (is_array($val)) {
            if (isset($val[0]) && is_string($val[0])) {
                ...
                $count = count($val);
                if (isset($val[$count - 1]) && in_array(strtoupper(trim($val[$count - 1])), array('AND', 'OR', 'XOR'))) {
                ...
                for ($i = 0; $i < $count; $i++) {
                    if (isset($val[$i]) && is_array($val[$i])) {
                        if (isset($val[$i][1]) && is_array($val[$i][1])) {
                            $data = implode(',', $val[$i][1]);
                        } else {
                            $data = $val[$i][1];
                        }
                    } else {
                        $data = isset($val[$i]) ? $val[$i] : '';
                    }
                    if (isset($val[$i][0]) && 'exp' == strtolower($val[$i][0])) {
                        $whereStr .= '(' . $key . ' ' . $data . ') ' . $rule . ' ';
                    } else {
                        $op = (isset($val[$i]) && is_array($val[$i])) ? $this->comparison[strtolower($val[$i][0])] : '=';
    ...
    {
        $joinStr = '';
        if (empty($options['table']) || false === strpos($options['table'], ',')) {
            return null;
        }
        ...
        for ($i = 0; $i < count($table) - 1; $i++) {
            $joinStr .= ' ' . (isset($join[$i]) ? $join[$i] : 'LEFT JOIN') . ' ' . $table[$i + 1] . ' ON ' . (isset($on[$i]) ? $on[$i] : '');
        }
        return $joinStr;
    }
    public function delete($options = array())
