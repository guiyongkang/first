...
class route {
     ...
     * @var string
     */
    private static $_pathinfo_split = '-';
     ...
     * @var string
     */
    private static $_pathinfo_pattern = '';
     ...
     * @var string
     */
    private static $_rewrite_extname = '.html';

	public static function init($config = array()){
		self::$_config = $config;
        self::$_pathinfo_pattern = self::$_pathinfo_split;
        self::parseRule();
	}
    /**
     * 路由解析
    ...
    public static function parseRule() {
        if (self::$_config['url_model']) {
            self::_parseRuleRewrite();
        } else {
            self::_parseRuleNormal();
        }
    }
     ...
     *
     */
    private static function _parseRuleNormal() {
        //不进行任何处理
    }
     ...
     *
     */
    private static function _parseRuleRewrite() {
        $path_info = $_SERVER['REQUEST_URI'];
        $path_info = substr($path_info,strrpos($path_info,'/')+1);
        if(strpos($path_info, '?')) {
            $path_info = substr($path_info, 0, (int) strpos($path_info, '?'));
        }
        if (!empty($path_info) && $path_info != 'index.php' && strpos($path_info, self::$_rewrite_extname)){
            //去掉伪静态扩展名
            $path_info = substr($path_info,0,-strlen(self::$_rewrite_extname));

            //根据不同APP匹配url规则
            $path_info_function = '_' . APP_ID . 'PathInfo';
            if (!method_exists(__CLASS__, $path_info_function)) {
                return ;
            }
            $path_info = self::$path_info_function($path_info);

            $split_array = preg_split(self::$_pathinfo_pattern, $path_info);
            //act,op强制赋值
     ...
     * @return mixed
     */
    private static function _shopPathInfo($path_info) {
     ...
     * @return mixed
     */
    private static function _cmsPathInfo($path_info) {
        $reg_match_from = array(
            '/^article-(\d+)$/',
            '/^picture-(\d+)$/'
        );
