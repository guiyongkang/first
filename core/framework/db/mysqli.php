...
class mysqli
{
    ...
    {
        if (core\config::get('db.master') == core\config::get('db.slave')) {
            if (isset(self::$link['slave']) && is_object(self::$link['slave'])) {
                self::$link['master'] =& self::$link['slave'];
                return;
            } elseif (isset(self::$link['master']) && is_object(self::$link['master'])) {
    ...
    public static function query($sql, $host = 'master')
    {
        ...
        if ($query === false) {
            ...
            if (core\config::get('debug')) {
                ...
                core\logging::write($error . "\r\n" . $sql);
                return false;
            }
        } else {
            core\logging::record($sql . ' [ RunTime:' . addUpTime('queryStartTime', 'queryEndTime', 6) . 's ]');
            return $query;
        }
    }
    /**
    ...
    public static function select($param, $obj_page = '', $host = 'slave')
    {
        ...
        if (!empty($tmp_table) && count($tmp_table) > 1) {
            ...
            $sql = 'SELECT ' . $param['field'] . ' FROM `' . DBPRE . $tmp_table[0] . '` as `' . $tmp_table[0] . '` ' . $tmp_sql . ' ' . (isset($param['where']) ? $param['where'] : '') . (isset($param['where_group']) ? $param['where_group'] : '') . (isset($param['where_order']) ? $param['where_order'] : '');
            //如果有分页，那么计算信息总数
            $count_sql = 'SELECT ' . $param['count'] . ' as count FROM `' . DBPRE . $tmp_table[0] . '` as `' . $tmp_table[0] . '` ' . $tmp_sql . ' ' . $param['where'] . $param['where_group'];
        } else {
            $sql = 'SELECT ' . (empty($param['field']) ? '*' : $param['field']) . ' FROM `' . DBPRE . $param['table'] . '` as `' . $param['table'] . '` ' . (isset($param['index']) ? $param['index'] : '') . ' ' . (isset($param['where']) ? $param['where'] : '') . (isset($param['where_group']) ? $param['where_group'] : '') . (isset($param['where_order']) ? $param['where_order'] : '');
            $count_sql = 'SELECT ' . (isset($param['count']) ? $param['count'] : 'count(*)') . ' as count FROM `' . DBPRE . $param['table'] . '` as `' . $param['table'] . '` ' . (isset($param['index']) ? $param['index'] : '') . ' ' . (isset($param['where']) ? $param['where'] : '');
        }
        //limit ，如果有分页对象的话，那么优先分页对象
        if (!empty($obj_page) && $obj_page instanceof core\page) {
