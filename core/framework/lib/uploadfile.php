...
class uploadfile
{
    ...
    {
        $this->config['thumb_type'] = core\config::get('thumb.cut_type');
        //加载语言包
    ...
    public function upfile($field)
    {
        ...
        if ($this->upload_file['tmp_name'] == '') {
            $this->setError(core\language::get('cant_find_temporary_files'));
            return false;
        }
        //对上传文件错误码进行验证
        ...
        if (!is_uploaded_file($this->upload_file['tmp_name'])) {
            $this->setError(core\language::get('upload_file_attack'));
            return false;
        }
        //验证文件大小
        if ($this->upload_file['size'] == 0) {
            $error = core\language::get('upload_file_size_none');
            $this->setError($error);
            return false;
        }
		if(empty($this->max_size)){
			$this->max_size = core\config::get('image_max_filesize');
		}
        if ($this->upload_file['size'] > $this->max_size * 1024) {
            $error = core\language::get('upload_file_size_cant_over') . $this->max_size . 'KB';
            $this->setError($error);
            return false;
        }
        //文件后缀名
        ...
        //验证文件格式是否为系统允许
		if(empty($this->allow_type)){
			$this->allow_type = explode(',',core\config::get('image_allow_ext'));
		}
        if (!in_array($this->ext, $this->allow_type)) {
            $error = core\language::get('image_allow_ext_is') . implode(',', $this->allow_type);
            $this->setError($error);
            return false;
        }
        //检查是否为有效图片
		if($this->is_img){
			if (!($image_info = getimagesize($this->upload_file['tmp_name']))) {
				$error = core\language::get('upload_image_is_not_image');
				$this->setError($error);
				return false;
			}
		}
        ...
        if (move_uploaded_file($this->upload_file['tmp_name'], BASE_UPLOAD_PATH . DS . $this->save_path . DS . $this->file_name)) {
            ...
            $this->setError(core\language::get('upload_file_fail'));
            return false;
        }
    ...
    private function fileInputError()
    {
        switch ($this->upload_file['error']) {
            ...
            case 1:
                ...
                $this->setError(core\language::get('upload_file_size_over'));
                return false;
                break;
            case 2:
            ...
            case 3:
                //文件只有部分被上传
                $this->setError(core\language::get('upload_file_is_not_complete'));
                return false;
                break;
            case 4:
                //没有文件被上传
                $this->setError(core\language::get('upload_file_is_not_uploaded'));
                return false;
                break;
            case 6:
                //找不到临时文件夹
                $this->setError(core\language::get('upload_dir_chmod'));
                return false;
                break;
            case 7:
                //文件写入失败
                $this->setError(core\language::get('upload_file_write_fail'));
                return false;
                break;
            default:
                return true;
    ...
    public function setPath()
    {
        ...
        if (!is_writable(BASE_UPLOAD_PATH . DS . $this->default_dir)) {
            $this->setError(core\language::get('upload_file_dir') . $this->default_dir . core\language::get('upload_file_dir_cant_touch_file'));
            return false;
        }
    ...
    {
        switch (core\config::get('image_dir_type')) {
            case '1':
                //按文件类型存放,例如/a.jpg
                $subpath = '';
                break;
