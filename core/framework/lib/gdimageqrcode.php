...
class gdimageqrcode
{
	 ...
	 * 9 = diy
     */
	private $wm_image_top = 0;
	
	private $wm_image_left = 0;
	...
	private $wm_text_top = 0;
	
	private $wm_text_left = 0;
    ...
    public function setWatermark($watermark)
    {
        ...
        if (!empty($watermark['wm_image_name'])) {
            $this->set('wm_image_name', $watermark['wm_image_name']);
        }
        $this->set('wm_image_pos', $watermark['wm_image_pos']);
		
		$this->set('wm_image_left', $watermark['wm_image_left']);
		
		$this->set('wm_image_top', $watermark['wm_image_top']);
		
        $this->set('wm_image_transition', $watermark['wm_image_transition']);
		
		if(!empty($watermark['wm_text'])){
			 $this->set('wm_text', $watermark['wm_text']);
		}
		
		if(!empty($watermark['wm_text_size'])){
			 $this->set('wm_text_size', $watermark['wm_text_size']);
		}
		
		if(isset($watermark['wm_text_angle'])){
			 $this->set('wm_text_angle', $watermark['wm_text_angle']);
		}
		
		if(!empty($watermark['wm_text_pos'])){
			 $this->set('wm_text_pos', $watermark['wm_text_pos']);
		}
		
		if(!empty($watermark['wm_text_font'])){
			 $this->set('wm_text_font', BASE_DATA_PATH . DS . 'resource' . DS . 'font' . DS . $watermark['wm_text_font'] . '.ttf');
		}
		
		if(!empty($watermark['wm_text_left'])){
			 $this->set('wm_text_left', $watermark['wm_text_left']);
		}
		
		if(!empty($watermark['wm_text_top'])){
			 $this->set('wm_text_top', $watermark['wm_text_top']);
		}
		
		if(!empty($watermark['wm_text_color'])){
			 $this->set('wm_text_color', $watermark['wm_text_color']);
		}
		
		if(!empty($watermark['save_file'])){
			 $this->set('save_file', $watermark['save_file']);
		}
    ...
    private function getPos($sourcefile_width, $sourcefile_height, $pos, $wm_image = '')
    {
        ...
        switch ($pos) {
            ...
            case 8:
                ...
                break;
			case 9:
				$dest_x = $this->wm_image_left ? $this->wm_image_left : $this->wm_text_left;
                $dest_y = $this->wm_image_top ? $this->wm_image_top : $this->wm_text_top;
				break;
            default:
                $dest_x = $sourcefile_width - $insertfile_width;
                $dest_y = $sourcefile_height - $insertfile_height;
