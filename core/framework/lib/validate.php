...
class validate
{
     ...
     * @var array
     */
    private $validator = array('email' => '/^([.a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+(\\.[a-zA-Z0-9_-])+/', 'phone' => '/^(([0-9]{2,3})|([0-9]{3}-))?((0[0-9]{2,3})|0[0-9]{2,3}-)?[1-9][0-9]{6,7}(-[0-9]{1,4})?$/', 'mobile' => '/^1[0-9]{10}$/', 'url' => '/^http:(\\/){2}[A-Za-z0-9]+.[A-Za-z0-9]+[\\/=?%-&_~`@\\[\\]\':+!]*([^<>\\"\\"])*$/', 'currency' => '/^[0-9]+(\\.[0-9]+)?$/', 'number' => '/^[0-9]+$/', 'zip' => '/^[0-9][0-9]{5}$/', 'qq' => '/^[1-9][0-9]{4,8}$/', 'integer' => '/^[-+]?[0-9]+$/', 'integerpositive' => '/^[+]?[0-9]+$/', 'double' => '/^[-+]?[0-9]+(\\.[0-9]+)?$/', 'doublepositive' => '/^[+]?[0-9]+(\\.[0-9]+)?$/', 'english' => '/^[A-Za-z]+$/', 'chinese' => '/^[\\x80-\\xff]+$/', 'username' => '/^[\\w]{3,}$/', 'nochinese' => '/^[A-Za-z0-9_-]+$/');
    /**
     * 验证数组中的值
     *
     * <code>
    ...
    public function validate()
    {
        ...
        foreach ($this->validateparam as $k => $v) {
            $v['validator'] = isset($v['validator']) ? strtolower($v['validator']) : '';
            if (empty($v['require'])) {
                $v['require'] = false;
            }
            if (empty($v['input']) && $v['require'] == 'true') {
            ...
            if ($this->validateparam[$k]['result'] && $v['input'] != '') {
                switch ($v['validator']) {
                    ...
                    case 'compare':
                        if ($v['operator'] != '') {
							$result = $v['input'] . $v['operator'] . $v['to'];
                    ...
                    case 'range':
                        ...
                        break;
                    default:
                        $this->validateparam[$k]['result'] = $this->check($v['input'], isset($this->validator[$v['validator']]) ? $this->validator[$v['validator']] : '');
                }
            }
        }
        $error = $this->getError();
