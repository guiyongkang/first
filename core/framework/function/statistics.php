...
function getStatData_LineLabels($stat_arr)
{
    ...
    //子标题如果为字符串则使用默认样式
    (isset($stat_arr['subtitle']) && is_string($stat_arr['subtitle'])) ? $stat_arr['subtitle'] = array('text' => '<b>' . $stat_arr['subtitle']. '</b>', 'x' => -20) : '';
    //Y轴如果为字符串则使用默认样式
...
function getStatData_Column2D($stat_arr)
{
    ...
    foreach ($stat_arr['series'] as $series_k => $series_v) {
        foreach ($series_v['data'] as $data_k => $data_v) {
            $data_v['color'] = isset($color[$data_k]) ? $color[$data_k] : '';
...
function getStatData_Pie($data)
{
    ...
    $stat_arr['plotOptions']['pie'] = array('allowPointSelect' => true, 'cursor' => 'pointer', 'dataLabels' => array('enabled' => $data['label_show'], 'color' => '#000000', 'connectorColor' => '#000000', 'format' => '<b>{point.name}</b>: {point.percentage:.1f} %'));
