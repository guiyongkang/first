...
class web_chat extends control
{
    ...
    {
        parent::__construct();
        core\language::read('member_chat');
    ...
    public function send_msgOp()
    {
        ...
        if ($member_id < 1 || $member_id != $f_id) {
            $this->error(core\language::get('nc_member_chat_login'));
        }
        $member = $model_chat->getMember($t_id);
        if ($t_name != $member['member_name']) {
            $this->error(core\language::get('nc_member_chat_name_error'));
        }
        $msg = array();
        $msg['f_id'] = $f_id;
    ...
    public function get_seller_listOp()
    {
        ...
        if (empty(core\session::get('seller_list'))) {
            $member_list = $model_chat->getSellerList(array('store_id' => $store_id), $n, $member_list);
            core\session::set('seller_list', $member_list);
        } else {
            $member_list = core\session::get('seller_list');
        }
        $add_time = date("Y-m-d");
        $add_time30 = strtotime($add_time) - 60 * 60 * 24 * 30;
        $member_list = $model_chat->getRecentList(array('f_id' => $f_id, 'add_time' => array('egt', $add_time30)), 10, $member_list);
    ...
    {
        if (core\session::get('member_id') < 1) {
            $this->error(core\language::get('nc_member_chat_login'));
        }
        $val = '';
        $member = array();
    ...
    {
        if (strtoupper(CHARSET) == 'GBK') {
            $json = core\language::getUTF8($json);
            //GBKtoUTF-8
        }
