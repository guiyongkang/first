	...
	$(function(){
		if(user['seller_id'] != '') {
			...
			var chat_user_list = '<div class="chat-box"><div class="chat-list"><div class="chat-list-top"><h1><i></i>联系人</h1><span class="minimize-chat-list" onclick="chat_show_list();"></span></div>'+
									...
									'</div></div>';
			var ajaxurl = APP_URL+'/index.php?act=web_chat&op=get_seller_list&n=99&f_id='+user['u_id'];
			$.ajax({
				type: "GET",
				url: ajaxurl,
				dataType:"jsonp",
	...
	function store_msg(msg){
	    ...
	    if( user['seller_is_admin'] == 1 || msg_limits[code] == 1) {
	        ...
	        var text_append = '<a href="'+APP_URL+'/index.php?act=store_msg&op=index">'+sm_content+'</a>';
