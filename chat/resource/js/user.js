//  IM聊天出声音
	...
	var audio_info = '<object width="1" height="1" style="position: absolute; left: -1px;" id="msg_audio" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000">'+
								'<param value="'+APP_URL+'/resource/audio.swf" name="movie"><param value="always" name="allowscriptaccess">'+
								'<embed width="1" height="1" allowscriptaccess="always" src="'+APP_URL+'/resource/audio.swf" name="msg_audio"></object>';
    $(function(){
		...
		if(user['u_id'] != '') {
			...
			var chat_user_list = '<div class="chat-box"><div class="chat-list"><div class="chat-list-top"><h1><i></i>联系人</h1><span class="minimize-chat-list" onclick="chat_show_list();"></span></div>'+
									...
									'</div></div>';
			var ajaxurl = APP_URL+'/index.php?act=web_chat&op=get_user_list&n=99&f_id='+user['u_id'];
			$.ajax({
				type: "GET",
				url: ajaxurl,
				dataType:"jsonp",
			    ...
			    success: function(u_list){
			  	for (var u_id in u_list){
					...
					setInterval( function () {
						$.get(APP_URL+'/index.php?act=web_chat&op=get_session&key=member_id');
					}, time_max*60000);
	        ...
	        if ( act_op == 'goods_index' ) {//异步调用商品信息
				var ajaxurl = APP_URL+'/index.php?act=web_chat&op=get_goods_info&goods_id='+chat_goods_id;
				$.ajax({
					type: "GET",
					url: ajaxurl,
					dataType:"jsonp",
	...
	function send_msg(){//发消息
		...
		$.ajax({
			type: "POST",
			url: APP_URL+'/index.php?act=web_chat&op=send_msg',
			dataType:"jsonp",
			data: msg,
			async: false,
	...
	function get_msg(list){//接收消息
		...
		for (var k in list){
			...
			if ( typeof user_list[u_id]['avatar'] === "undefined" ) {//当没获得会员信息时调用一次
				var ajaxurl = APP_URL+'/index.php?act=web_chat&op=get_info&t=member&u_id='+u_id;
				$.ajax({
					type: "GET",
					url: ajaxurl,
					dataType:"jsonp",
	...
	function get_chat_msg(t){
		var ajaxurl = APP_URL+'/index.php?act=web_chat&op=get_chat_log&page=30&f_id='+user['u_id']+'&t_id='+chat_log['u_id']+'&t='+chat_log['time_from'];
	    ...
	    if ( typeof msg['goods_id'] === "undefined" ) {//没有定义商品编号时调用检测一次
    	    ...
    	    if ( goods_id > 0 ) {
    	        if ( typeof goods_list[goods_id] === "undefined" ) {
    				var ajaxurl = APP_URL+'/index.php?act=web_chat&op=get_goods_info&goods_id='+goods_id;
    				$.ajax({
    					type: "GET",
    					url: ajaxurl,
    					dataType:"jsonp",
	...
	function get_goods_list(s_id){
		...
		var ajaxurl = APP_URL+'/index.php?act=web_chat&op=get_goods_list&s_id='+s_id;
		$.ajax({
			type: "GET",
			url: ajaxurl,
			dataType:"jsonp",
	...
	function show_msg(u_id){//显示会员的消息
		...
		if(user_show != u_id) {
			...
			if (typeof user_info['s_name'] !== "undefined" && user_info['s_name'] !== "") {
			    ...
			    store_html = '<a target="_blank" href="'+APP_URL+'/index.php?act=show_store&op=index&store_id='+
	...
	function play_audio(){//提示声音
	    if ( chat_audio === 1 ) {
	        ...
	        if ( typeof swf.pplay === "function" ) {
	            swf.load(APP_URL+'/resource/msg.mp3');
	            swf.pplay();
			}
	    }
	}
