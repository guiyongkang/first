...
class store_bill extends BaseSellerControl
{
    /**
     * 每次导出多少条记录
     * @var unknown
    ...
    public function indexOp()
    {
        ...
        if (isset($_GET['ob_no']) && preg_match('/^20\\d{5,12}$/', $_GET['ob_no'])) {
        ...
        core\tpl::showpage('store_bill.index');
    }
    /**
     * 查看结算单详细
     *
    ...
    public function show_billOp()
    {
        ...
        if (substr($_GET['ob_no'], 6) != core\session::get('store_id')) {
            error('参数错误');
        }
        $model_bill = model('bill');
		...
		if(isset($_GET['query_start_date'])){
			$if_start_date = preg_match('/^20\\d{2}-\\d{2}-\\d{2}$/', $_GET['query_start_date']);
		}else{
			$if_start_date = '';
		}
        if(isset($_GET['query_end_date'])){
        ...
        if (isset($_GET['type']) && $_GET['type'] == 'refund') {
            if (isset($_GET['query_order_no']) && preg_match('/^\\d{8,20}$/', $_GET['query_order_no'])) {
        ...
        } elseif (isset($_GET['type']) && $_GET['type'] == 'cost') {
            //店铺费用
        ...
        core\tpl::output('sub_tpl_name', $sub_tpl_name);
        core\tpl::output('bill_info', $bill_info);
        core\tpl::showpage('store_bill.show');
    }
    /**
     * 打印结算单
     *
    ...
    public function bill_printOp()
    {
        ...
        core\tpl::output('bill_info', $bill_info);
        core\tpl::showpage('store_bill.print', 'null_layout');
    }
    /**
     * 店铺确认出账单
     *
    ...
    public function confirm_billOp()
    {
        ...
        if ($update) {
            ...
            showDialog(lang('nc_common_op_fail'), 'reload', 'error');
        }
    }
    /**
     * 导出结算订单明细CSV
    ...
    public function export_orderOp()
    {
        ...
        if (isset($_GET['curpage']) && !is_numeric($_GET['curpage'])) {
            ...
            if ($count > self::EXPORT_SIZE) {
                ...
                core\tpl::output('murl', 'index.php?act=store_bill&op=show_bill&ob_no=' . $_GET['ob_no']);
    ...
    public function export_refund_orderOp()
    {
        ...
        if (isset($_GET['curpage']) && !is_numeric($_GET['curpage'])) {
            ...
            if ($count > self::EXPORT_SIZE) {
                ...
                core\tpl::output('list', $array);
                core\tpl::output('murl', 'index.php?act=store_bill&op=show_bill&query_type=refund&ob_no=' . $_GET['ob_no']);
                core\tpl::showpage('store_export.excel');
                exit;
            } else {
                //如果数量小，直接下载
    ...
    private function profile_menu($menu_type, $menu_key = '')
    {
        ...
        switch ($menu_type) {
            case 'list':
                $menu_array = array(1 => array('menu_key' => 'list', 'menu_name' => '实物订单结算', 'menu_url' => 'index.php?act=store_bill&op=index'));
                break;
            case 'show':
                $menu_array = array(array('menu_key' => 'order_list', 'menu_name' => '订单列表', 'menu_url' => 'index.php?act=store_bill&op=show_bill&ob_no=' . $_GET['ob_no']), array('menu_key' => 'refund_list', 'menu_name' => '退款订单', 'menu_url' => 'index.php?act=store_bill&op=show_bill&type=refund&ob_no=' . $_GET['ob_no']), array('menu_key' => 'cost_list', 'menu_name' => '促销费用', 'menu_url' => 'index.php?act=store_bill&op=show_bill&type=cost&ob_no=' . $_GET['ob_no']));
                break;
        }
