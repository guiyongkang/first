...
class store_complain extends BaseSellerControl
{
    //定义投诉状态常量
    const STATE_NEW = 10;
    ...
    {
        parent::__construct();
        core\language::read('member_layout,member_complain');
    }
    /*
     * 被投诉列表
     */
    public function indexOp()
    {
		...
		$_GET['state'] = isset($_GET['state']) ? intval($_GET['state']) : 0;
        switch ($_GET['state']) {
            case 1:
    ...
    public function appeal_saveOp()
    {
        ...
        if (intval($complain_info['complain_state']) !== self::STATE_APPEAL) {
            showDialog(core\language::get('para_error'), 'reload');
        }
        $input = array();
        $input['appeal_message'] = trim($_POST['input_appeal_message']);
        //验证输入的信息
        $obj_validate = new lib\validate();
        $obj_validate->validateparam = array(array("input" => $input['appeal_message'], "require" => "true", "validator" => "Length", "min" => "1", "max" => "255", "message" => core\language::get('appeal_message_error')));
        ...
        showDialog(core\language::get('appeal_submit_success'), 'index.php?act=store_complain', 'succ');
    }
    /*
     * 申请仲裁
     */
    public function apply_handleOp()
    {
        ...
        if ($complain_state < self::STATE_TALK || $complain_state === 99) {
            error(core\language::get('para_error'));
        }
        $update_array = array();
        ...
        success(core\language::get('handle_submit_success'), 'index.php?act=store_complain');
    }
    /*
     * 根据投诉id获取投诉对话
     */
    public function get_complain_talkOp()
    {
        ...
        if (!empty($complain_talk_list)) {
            foreach ($complain_talk_list as $talk) {
                ...
                $talk_list[$i]['talk'] .= '(' . $talk['talk_member_name'] . ')' . core\language::get('complain_text_say') . ':' . $talk['talk_content'];
                $i++;
            }
        }
    ...
    {
        $menu_array = array(array('menu_key' => 'complain_accused_list', 'menu_name' => core\language::get('complain_manage_title'), 'menu_url' => 'index.php?act=store_complain'));
