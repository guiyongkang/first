...
class store_vr_bill extends BaseSellerControl
{
    /**
     * 每次导出多少条记录
     * @var unknown
    ...
    public function indexOp()
    {
        ...
        if (preg_match('/^20\\d{5,12}$/', isset($_GET['ob_no']) ? $_GET['ob_no'] : '')) {
        ...
        core\tpl::showpage('store_vr_bill.index');
    }
    /**
     * 查看结算单详细
     *
    ...
    public function show_billOp()
    {
        ...
        core\tpl::output('sub_tpl_name', 'store_vr_bill.show.code_list');
        core\tpl::output('bill_info', $bill_info);
        $this->profile_menu('show', $_GET['type']);
        core\tpl::showpage('store_vr_bill.show');
    }
    /**
     * 打印结算单
     *
    ...
    public function bill_printOp()
    {
        ...
        core\tpl::output('bill_info', $bill_info);
        core\tpl::showpage('store_vr_bill.print', 'null_layout');
    }
    /**
     * 店铺确认出账单
     *
    ...
    public function export_orderOp()
    {
        ...
        if (!is_numeric($_GET['curpage'])) {
            ...
            if ($count > self::EXPORT_SIZE) {
                ...
                core\tpl::output('murl', 'index.php?act=store_vr_bill&op=show_bill&ob_no=' . $_GET['ob_no']);
    ...
    private function profile_menu($menu_type, $menu_key = '')
    {
        ...
        switch ($menu_type) {
            case 'list':
                $menu_array = array(1 => array('menu_key' => 'list', 'menu_name' => '虚拟订单结算', 'menu_url' => 'index.php?act=store_vr_bill&op=index'));
                break;
            case 'show':
                $menu_array = array(array('menu_key' => '', 'menu_name' => '已使用', 'menu_url' => 'index.php?act=store_vr_bill&op=show_bill&ob_no=' . $_GET['ob_no']), array('menu_key' => 'timeout', 'menu_name' => '已过期', 'menu_url' => 'index.php?act=store_vr_bill&op=show_bill&type=timeout&ob_no=' . $_GET['ob_no']));
                break;
        }
