...
class taobao_import extends BaseSellerControl
{
        ...
        if (!checkPlatformStore()) {
            ...
            if (intval($this->store_grade['sg_goods_limit']) != 0) {
                if ($goods_num >= $this->store_grade['sg_goods_limit']) {
                    error(lang('store_goods_index_goods_limit') . $this->store_grade['sg_goods_limit'] . lang('store_goods_index_goods_limit1'), 'index.php?act=store_goods&op=goods_list');
                }
            }
        }
    }
    ...
    public function indexOp()
    {
        ...
        if (!$_POST) {
            ...
            $gc_list = $gc->getGoodsClass(core\session::get('store_id'));
            core\tpl::output('gc_list', $gc_list);
            /**
             * 获取店铺商品分类
             */
            $model_store_class = model('my_goods_class');
            $store_goods_class = $model_store_class->getClassTree(array('store_id' => core\session::get('store_id'), 'stc_state' => '1'));
            core\tpl::output('store_goods_class', $store_goods_class);
            if (!empty($_GET['step'])) {
                core\tpl::output('step', $_GET['step']);
            } else {
             ...
             */
            if (empty($file['name'])) {
                error($lang['store_goods_import_choose_file']);
            }
            /**
             * 文件来源判定
             */
            if (!is_uploaded_file($file['tmp_name'])) {
                error($lang['store_goods_import_unknown_file']);
            }
            /**
             * 文件类型判定
             */
            ...
            if ($records === false) {
                error($lang['store_goods_import_wrong_column']);
            }
            /**
             * 转码
             */
            if (strtoupper(CHARSET) == 'GBK') {
                $records = core\language::getGBK($records);
            }
            $model_goodsclass = model('goods_class');
            $model_store_goods = model('goods');
            $model_type = model('type');
            // 商品数量
            $goods_num = $model_store_goods->getGoodsCommonCount(array('store_id' => core\session::get('store_id')));
            /**
             * 商品数,空间使用，使用期限判断
             */
            ...
            if (intval($store_grade['sg_goods_limit']) != 0) {
                if ($goods_num >= $store_grade['sg_goods_limit']) {
                    error($lang['store_goods_index_goods_limit'] . $store_grade['sg_goods_limit'] . $lang['store_goods_index_goods_limit1'], 'index.php?act=store_goods&op=goods_list');
                }
            ...
            if (intval($store_info['store_end_time']) != 0) {
                if (time() >= $store_info['store_end_time']) {
                    error($lang['store_goods_index_time_limit'], 'index.php?act=store_goods&op=goods_list');
                }
            }
            /**
             * 循环添加数据
            ...
            if (is_array($records) and count($records) > 0) {
                foreach ($records as $k => $record) {
                    if ($remain_num > 0 and $k >= $remain_num) {
                        error($lang['store_goods_index_goods_limit'] . $store_grade['sg_goods_limit'] . $lang['store_goods_index_goods_limit1'] . $lang['store_goods_import_end'] . (count($records) - $remain_num) . $lang['store_goods_import_products_no_import'], 'index.php?act=store_goods&op=taobao_import&step=4');
                    }
                    ...
                    //导入后直接生成二维码
                    $PhpQRCode->set('date', APP_URL . '/tmpl/product_detail.html?goods_id=' . $goods_id1);
                ...
                if ($goods_id_str != "") {
                    core\tpl::output('goods_id_str', substr($goods_id_str, 1, strlen($goods_id_str)));
                }
            }
        ...
        core\tpl::output('menu_sign', 'taobao_import');
        core\tpl::showpage('store_goods_import');
    }
    ...
    {
        return array('goods_name' => '宝贝名称', 'cid' => '宝贝类目', 'goods_form' => '新旧程度', 'goods_store_price' => '宝贝价格', 'spec_goods_storage' => '宝贝数量', 'goods_indate' => '有效期', 'goods_transfee_charge' => '运费承担', 'py_price' => '平邮', 'es_price' => 'EMS', 'kd_price' => '快递', 'spec' => '销售属性别名', 'goods_commend' => '橱窗推荐', 'goods_body' => '宝贝描述', 'goods_image' => '新图片', 'goods_serial' => '商家编码');
    }
    /**
