...
class store_groupbuy extends BaseSellerControl
{
    public function __construct()
    {
        parent::__construct();
        //读取语言包
        core\language::read('member_groupbuy');
        //检查抢购功能是否开启
        if (intval(core\config::get('groupbuy_allow')) !== 1) {
            error(core\language::get('groupbuy_unavailable'), 'index.php?act=seller_center');
        }
    }
    /**
     * 默认显示抢购列表
    ...
    public function groupbuy_quota_addOp()
    {
        ...
        core\tpl::showpage('store_groupbuy_quota.add');
    }
    /**
     * 抢购套餐购买保存
     **/
    public function groupbuy_quota_add_saveOp()
    {
        ...
        //获取当前价格
        $current_price = intval(core\config::get('groupbuy_price'));
        //获取该用户已有套餐
        ...
        $this->recordSellerLog('购买' . $groupbuy_quota_quantity . '份抢购套餐，单价' . $current_price . lang('nc_yuan'));
        showDialog(core\language::get('groupbuy_quota_add_success'), urlBiz('store_groupbuy', 'groupbuy_list'), 'succ');
    }
    /**
     * 抢购列表
     **/
    public function groupbuy_listOp()
    {
        ...
        if (checkPlatformStore()) {
            ...
            $current_groupbuy_quota = $model_groupbuy_quota->getGroupbuyQuotaCurrent($_SESSION['store_id']);
            core\tpl::output('current_groupbuy_quota', $current_groupbuy_quota);
        }
        $condition = array();
		...
		if(isset($_GET['groupbuy_name'])){
			$condition['groupbuy_name'] = array('like', '%' . $_GET['groupbuy_name'] . '%');
		}
        if (isset($_GET['groupbuy_vr']) && strlen($groupbuy_vr = trim($_GET['groupbuy_vr']))) {
            $condition['is_vr'] = $groupbuy_vr ? 1 : 0;
            core\tpl::output('groupbuy_vr', $groupbuy_vr);
        }
        $groupbuy_list = $model_groupbuy->getGroupbuyExtendList($condition, 10);
        core\tpl::output('group', $groupbuy_list);
        ...
        core\tpl::showpage('store_groupbuy.list');
    }
    /**
     * 添加抢购页面
     **/
    public function groupbuy_addOp()
    {
        ...
        if (checkPlatformStore()) {
            ...
            $current_groupbuy_quota = $model_groupbuy_quota->getGroupbuyQuotaCurrent($_SESSION['store_id']);
            if (empty($current_groupbuy_quota)) {
                error('当前没有可用套餐，请先购买套餐', urlBiz('store_groupbuy', 'groupbuy_quota_add'));
            }
            core\tpl::output('current_groupbuy_quota', $current_groupbuy_quota);
        }
        // 根据后台设置的审核期重新设置抢购开始时间
        core\tpl::output('groupbuy_start_time', TIMESTAMP + intval(core\config::get('groupbuy_review_day')) * 86400);
        core\tpl::output('groupbuy_classes', model('groupbuy')->getGroupbuyClasses());
        self::profile_menu('groupbuy_add');
        core\tpl::showpage('store_groupbuy.add');
    }
    /**
     * 抢购保存
     **/
    public function groupbuy_saveOp()
    {
        ...
        if (empty($goods_id)) {
            showDialog(core\language::get('param_error'));
        }
        $model_groupbuy = model('groupbuy');
        ...
        if (!checkPlatformStore()) {
            ...
            $current_groupbuy_quota = $model_groupbuy_quota->getGroupbuyQuotaCurrent($_SESSION['store_id']);
            if (empty($current_groupbuy_quota)) {
                showDialog('当前没有可用套餐，请先购买套餐', urlBiz('store_groupbuy', 'groupbuy_quota_add'), 'error');
            }
        }
        ...
        if ($result) {
            ...
            showDialog(core\language::get('groupbuy_add_success'), 'index.php?act=store_groupbuy', 'succ');
        } else {
            showDialog(core\language::get('groupbuy_add_fail'), 'index.php?act=store_groupbuy');
        }
    }
    public function groupbuy_goods_infoOp()
    {
        ...
        $data['goods_href'] = urlBiz('goods', 'index', array('goods_id' => $goods_info['goods_id']));
    ...
    public function search_goodsOp()
    {
        ...
        core\tpl::showpage('store_groupbuy.goods', 'null_layout');
    }
    /**
     * 添加虚拟抢购页面
     */
    public function groupbuy_add_vrOp()
    {
        ...
        core\tpl::output('groupbuy_start_time', TIMESTAMP + intval(core\config::get('groupbuy_review_day')) * 86400);
        // 虚拟抢购分类
        // core\tpl::output('groupbuy_vr_classes', model('groupbuy')->getGroupbuyVrClasses());
        ...
        core\tpl::output('classlist', $classlist);
        // 虚拟区域分类
        // core\tpl::output('groupbuy_vr_cities', model('groupbuy')->getGroupbuyVrCities());
        ...
        core\tpl::output('arealist', $arealist);
        self::profile_menu('groupbuy_add_vr');
        core\tpl::showpage('store_groupbuy.add_vr');
    }
    ...
    {
        $menu_array = array(1 => array('menu_key' => 'groupbuy_list', 'menu_name' => lang('nc_member_path_group_list'), 'menu_url' => urlBiz('store_groupbuy', 'groupbuy_list')));
        switch ($menu_key) {
            case 'groupbuy_add':
                $menu_array[] = array('menu_key' => 'groupbuy_add', 'menu_name' => lang('nc_member_path_new_group'), 'menu_url' => 'index.php?act=store_groupbuy&op=groupbuy_add');
                break;
            ...
            case 'groupbuy_quota_add':
                $menu_array[] = array('menu_key' => 'groupbuy_quota_add', 'menu_name' => '购买套餐', 'menu_url' => urlBiz('store_groupbuy', 'groupbuy_quota_add'));
                break;
            case 'groupbuy_edit':
                $menu_array[] = array('menu_key' => 'groupbuy_edit', 'menu_name' => lang('nc_member_path_edit_group'), 'menu_url' => 'index.php?act=store_groupbuy');
                break;
            case 'cancel':
                $menu_array[] = array('menu_key' => 'groupbuy_cancel', 'menu_name' => lang('nc_member_path_cancel_group'));
                break;
        }
