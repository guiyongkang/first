...
class store_return extends BaseSellerControl
{
    ...
    public function editOp()
    {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                $param['param'] = array('refund_url' => urlBiz('member_return', 'view', array('return_id' => $return['refund_id'])), 'refund_sn' => $return['refund_sn']);
        ...
        core\tpl::showpage('store_return_edit');
    }
    /**
     * 收货
     *
    ...
    public function receiveOp()
    {
        ...
        core\tpl::output('return_delay', $return_delay);
        core\tpl::output('return_confirm', $model_trade->getMaxDay('return_confirm'));
        //卖家不处理收货时按同意并弃货处理
        core\tpl::output('delay_time', $delay_time);
        if (chksubmit()) {
            if ($return['seller_state'] != '2' || $return['goods_state'] != '2') {
                //检查状态,防止页面刷新不及时造成数据错误
                showDialog(core\language::get('wrong_argument'), 'reload', 'error', 'CUR_DIALOG.close();');
            }
            $refund_array = array();
            ...
            if ($state) {
                ...
                $param['member_id'] = $return['buyer_id'];
                $param['param'] = array('refund_url' => urlBiz('member_return', 'view', array('return_id' => $return['refund_id'])), 'refund_sn' => $return['refund_sn']);
                lib\queue::push('sendMemberMsg', sendMemberMsg);
                showDialog(core\language::get('nc_common_save_succ'), 'reload', 'succ', 'CUR_DIALOG.close();');
            } else {
                showDialog(core\language::get('nc_common_save_fail'), 'reload', 'error', 'CUR_DIALOG.close();');
            }
        }
        $express_list = rkcache('express', true);
        if ($return['express_id'] > 0 && !empty($return['invoice_no'])) {
            core\tpl::output('e_name', $express_list[$return['express_id']]['e_name']);
            core\tpl::output('e_code', $express_list[$return['express_id']]['e_code']);
        }
        core\tpl::showpage('store_return_receive', 'null_layout');
    }
    /**
     * 退货记录查看页
     *
    ...
    public function viewOp()
    {
        ...
        $condition['order_id'] = $return['order_id'];
        $model_refund->getRightOrderList($condition, $return['order_goods_id']);
        core\tpl::showpage('store_return_view');
    }
    /**
     * 用户中心右边，小导航
     *
    ...
    private function profile_menu($menu_type, $menu_key = '')
    {
        ...
        switch ($menu_type) {
            case 'return':
                $menu_array = array(array('menu_key' => '2', 'menu_name' => '售前退货', 'menu_url' => 'index.php?act=store_return&lock=2'), array('menu_key' => '1', 'menu_name' => '售后退货', 'menu_url' => 'index.php?act=store_return&lock=1'));
                break;
        }
