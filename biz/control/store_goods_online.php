...
class store_goods_online extends BaseSellerControl
{
    ...
    public function goods_listOp()
    {
        ...
        if (isset($_GET['stc_id']) && intval($_GET['stc_id']) > 0) {
        ...
        core\tpl::showpage('store_goods_list.online');
    }
    /**
     * 编辑商品页面
     */
    public function edit_goodsOp()
    {
        ...
        if (empty($goodscommon_info) || $goodscommon_info['store_id'] != core\session::get('store_id') || $goodscommon_info['goods_lock'] == 1) {
            error(lang('wrong_argument'));
        }
        $where = array('goods_commonid' => $common_id, 'store_id' => core\session::get('store_id'));
        ...
        core\tpl::output('goods', $goodscommon_info);
        ...
        // 获取类型相关数据
        $typeinfo = $model_type->getAttr($goods_class['type_id'], core\session::get('store_id'), $goodscommon_info['gc_id']);
        ...
        if (is_array($goods_array) && !empty($goods_array)) {
            ...
            if (!empty($attr_checked_l) && is_array($attr_checked_l)) {
                foreach ($attr_checked_l as $val) {
                    $attr_checked[] = $val['attr_value_id'];
                }
            }
            core\tpl::output('attr_checked', $attr_checked);
            $spec_checked = array();
            ...
            core\tpl::output('spec_checked', $spec_checked);
        }
        core\tpl::output('sp_value', $sp_value);
        // 实例化店铺商品分类模型
        $store_goods_class = model('store_goods_class')->getClassTree(array('store_id' => core\session::get('store_id'), 'stc_state' => '1'));
        core\tpl::output('store_goods_class', $store_goods_class);
        // 处理商品所属分类
        ...
        if (!empty($goods_stcids)) {
            foreach ($goods_stcids as $k => $v) {
                $stc_parent_id = isset($store_goods_class_tmp[$v]['stc_parent_id']) ? $store_goods_class_tmp[$v]['stc_parent_id'] : 0;
        ...
        core\tpl::output('store_class_goods', $goods_stcids_new);
        // 是否能使用编辑器
        ...
        if ($goodscommon_info['is_fcode'] == 1) {
            ...
            core\tpl::output('fcode_array', $fcode_array);
        }
        $menu_promotion = array('lock' => $goodscommon_info['goods_lock'] == 1 ? true : false, 'gift' => $model_goods->checkGoodsIfAllowGift($goodscommon_info), 'combo' => $model_goods->checkGoodsIfAllowCombo($goodscommon_info));
        $this->profile_menu('edit_detail', 'edit_detail', $menu_promotion);
        core\tpl::output('edit_goods_sign', true);
    ...
    public function edit_save_goodsOp()
    {
        ...
        if (!chksubmit() || $common_id <= 0) {
            showDialog(lang('store_goods_index_goods_edit_fail'), urlBiz('store_goods_online', 'index'));
        }
        // 验证表单
        ...
        if ($error != '') {
            showDialog(lang('error') . $error, urlBiz('store_goods_online', 'index'));
        }
        ...
        $update_common['goods_attr'] = isset($_POST['attr']) ? serialize($_POST['attr']) : serialize(null);
        ...
        // 店铺关闭时，商品下架
        $update_common['goods_selltime'] = (isset($_POST['starttime']) ? strtotime($_POST['starttime']) : 0) + (isset($_POST['starttime_H']) ? intval($_POST['starttime_H']) : 0) * 3600 + (isset($_POST['starttime_i']) ? intval($_POST['starttime_i']) : 0) * 60;
        $update_common['goods_verify'] = core\config::get('goods_verify') == 1 ? 10 : 1;
        $update_common['spec_name'] = (isset($_POST['spec']) && is_array($_POST['spec'])) ? serialize($_POST['sp_name']) : serialize(null);
        $update_common['spec_value'] = (isset($_POST['spec']) && is_array($_POST['spec'])) ? serialize($_POST['sp_val']) : serialize(null);
        ...
        $update_common['transport_id'] = (isset($_POST['freight']) && $_POST['freight'] == '0') ? '0' : intval($_POST['transport_id']);
        // 售卖区域
        ...
        $update_common['is_virtual'] = isset($_POST['is_gv']) ? intval($_POST['is_gv']) : 0;
        $update_common['virtual_indate'] = !empty($_POST['g_vindate']) ? strtotime($_POST['g_vindate']) + 24 * 60 * 60 - 1 : 0;
        // 当天的最后一秒结束
        $update_common['virtual_limit'] = empty($_POST['g_vlimit']) || intval($_POST['g_vlimit']) > 10 || intval($_POST['g_vlimit']) < 0 ? 10 : intval($_POST['g_vlimit']);
        $update_common['virtual_invalid_refund'] = isset($_POST['g_vinvalidrefund']) ? intval($_POST['g_vinvalidrefund']) : 0;
        ...
        // 预售商品的发货时间
        $update_common['is_own_shop'] = in_array(core\session::get('store_id'), model('store')->getOwnShopIds()) ? 1 : 0;
        // 开始事务
        ...
        if (!empty($_POST['spec']) && is_array($_POST['spec'])) {
            foreach ($_POST['spec'] as $value) {
                $goods_info = $model_goods->getGoodsInfo(array('goods_id' => $value['goods_id'], 'goods_commonid' => $common_id, 'store_id' => core\session::get('store_id')), 'goods_id');
                if (!empty($goods_info)) {
                    ...
                    $update['store_id'] = core\session::get('store_id');
                    $update['store_name'] = core\session::get('store_name');
                    ...
                    $insert['store_name'] = core\session::get('store_name');
            ...
            $goods_info = $model_goods->getGoodsInfo(array('goods_spec' => serialize(null), 'goods_commonid' => $common_id, 'store_id' => core\session::get('store_id')), 'goods_id');
            if (!empty($goods_info)) {
                $goods_id = $goods_info['goods_id'];
                $update = array();
                $update['goods_commonid'] = $common_id;
        ...
        if (!empty($goodsid_array)) {
            // QueueClient::push('createGoodsQRCode', array('store_id' => core\session::get('store_id'), 'goodsid_array' => $goodsid_array));
            // 生成商品二维码
            $PhpQRCode->set('date', APP_URL . '/tmpl/product_detail.html?goods_id=' . $goods_id);
            $PhpQRCode->set('pngTempName', $goods_id . '.png');
            $PhpQRCode->init();
        ...
        // 清理商品数据
        $model_goods->delGoods(array('goods_id' => array('not in', $goodsid_array), 'goods_commonid' => $common_id, 'store_id' => core\session::get('store_id')));
        // 清理商品图片表
        ...
        if ($update_common['is_virtual'] == 1 || $update_common['is_fcode'] == 1 || $update_common['is_presell'] == 1) {
            ...
            lib\queue::push('clearSpecialGoodsPromotion', array('goods_commonid' => $common_id, 'goodsid_array' => $goodsid_array));
        } else {
            // 更新商品促销价格
            lib\queue::push('updateGoodsPromotionPriceByGoodsCommonId', $common_id);
        }
        // 生成F码
        if ($update_common['is_fcode'] == 1) {
            lib\queue::push('createGoodsFCode', array('goods_commonid' => $common_id, 'fc_count' => intval($_POST['g_fccount']), 'fc_prefix' => $_POST['g_fcprefix']));
        }
        $return = $model_goods->editGoodsCommon($update_common, array('goods_commonid' => $common_id, 'store_id' => core\session::get('store_id')));
        if ($return) {
            // 提交事务
            model()->commit();
            showDialog(lang('nc_common_op_succ'), getReferer(), 'succ');
        } else {
            // 回滚事务
            model()->rollback();
            showDialog(lang('store_goods_index_goods_edit_fail'), urlBiz('store_goods_online', 'index'));
    ...
    public function edit_imageOp()
    {
        ...
        if ($common_list['store_id'] != core\session::get('store_id') || $common_list['goods_lock'] == 1) {
            error(lang('wrong_argument'), urlBiz('index'));
        }
        ...
        $menu_promotion = array('lock' => $common_list['goods_lock'] == 1 ? true : false, 'gift' => $model_goods->checkGoodsIfAllowGift($common_list), 'combo' => $model_goods->checkGoodsIfAllowCombo($common_list));
    ...
    public function edit_save_imageOp()
    {
        if (chksubmit()) {
            ...
            if ($common_id <= 0 || empty($_POST['img'])) {
                showDialog(lang('wrong_argument'), urlBiz('store_goods_online', 'index'));
            }
            $model_goods = model('goods');
            // 删除原有图片信息
            $model_goods->delGoodsImages(array('goods_commonid' => $common_id, 'store_id' => core\session::get('store_id')));
            // 保存
            $insert_array = array();
            foreach ($_POST['img'] as $key => $value) {
                foreach ($value as $v) {
                    ...
                    $update_where['store_id'] = core\session::get('store_id');
            ...
            if ($rs) {
                ...
                showDialog(lang('nc_common_save_fail'), urlBiz('store_goods_online', 'index'));
            }
        }
    }
    /**
    ...
    public function edit_classOp()
    {
        ...
        // 商品分类
        $goods_class = $model_goodsclass->getGoodsClass(core\session::get('store_id'));
        // 常用商品分类
        ...
        core\tpl::output('goods_class', $goods_class);
        core\tpl::output('commonid', $_GET['commonid']);
    ...
    public function goods_unshowOp()
    {
        ...
        if ($return) {
            ...
            showDialog(lang('store_goods_index_goods_unshow_success'), getReferer() ? getReferer() : 'index.php?act=store_goods_online&op=goods_list', 'succ', '', 2);
        } else {
    ...
    public function edit_jingleOp()
    {
        if (chksubmit()) {
            ...
            $commonid_array = explode(',', $common_id);
            $where = array('goods_commonid' => array('in', $commonid_array), 'store_id' => core\session::get('store_id'));
        ...
        $common_id = $this->checkRequestCommonId($_GET['commonid']);
        core\tpl::showpage('store_goods_list.edit_jingle', 'null_layout');
    }
    /**
     * 设置关联版式
     */
    public function edit_plateOp()
    {
        ...
        $plate_list = array_under_reset($plate_list, 'plate_position', 2);
        core\tpl::output('plate_list', $plate_list);
        core\tpl::showpage('store_goods_list.edit_plate', 'null_layout');
    }
    /**
     * 添加赠品
     */
    public function add_giftOp()
    {
        ...
        core\tpl::output('gift_array', $gift_array);
        $menu_promotion = array('lock' => $goodscommon_info['goods_lock'] == 1 ? true : false, 'gift' => $model_goods->checkGoodsIfAllowGift($goods_array[0]), 'combo' => $model_goods->checkGoodsIfAllowCombo($goods_array[0]));
        $this->profile_menu('edit_detail', 'add_gift', $menu_promotion);
        core\tpl::showpage('store_goods_edit.add_gift');
    }
    /**
     * 保存赠品
     */
    public function save_giftOp()
    {
        ...
        $model_gift = model('goods_gift');
        // 验证商品是否存在
        $goods_list = $model_goods->getGoodsListForPromotion(array('goods_commonid' => $commonid, 'store_id' => core\session::get('store_id')), 'goods_id', 0, 'gift');
        if (empty($goods_list)) {
            showDialog(lang('wrong_argument'));
        }
        // 删除该商品原有赠品
        ...
        foreach ($data as $key => $val) {
            ...
            foreach ($val as $k => $v) {
                ...
                $is_general = $model_goods->checkIsGeneral($gift_info);
                // 验证是否为普通商品
                if ($gift_info['store_id'] != core\session::get('store_id') || $is_general == false) {
                    continue;
                }
                $array = array();
        ...
        showDialog(lang('nc_common_save_succ'), getReferer(), 'succ');
    }
    /**
     * 推荐搭配
     */
    public function add_comboOp()
    {
        ...
        core\tpl::output('combo_array', $combo_array);
        $menu_promotion = array('lock' => $goodscommon_info['goods_lock'] == 1 ? true : false, 'gift' => $model_goods->checkGoodsIfAllowGift(isset($goods_array[0]) ? $goods_array[0] : array()), 'combo' => $model_goods->checkGoodsIfAllowCombo(isset($goods_array[0]) ? $goods_array[0] : array()));
        $this->profile_menu('edit_detail', 'add_combo', $menu_promotion);
        core\tpl::showpage('store_goods_edit.add_combo');
    }
    /**
     * 保存赠品
     */
    public function save_comboOp()
    {
        ...
        $data = isset($_POST['combo']) ? $_POST['combo'] : array();
        $commonid = intval($_POST['commonid']);
        if ($commonid <= 0) {
            showDialog(lang('wrong_argument'));
        }
        ...
        // 验证商品是否存在
        $goods_list = $model_goods->getGoodsListForPromotion(array('goods_commonid' => $commonid, 'store_id' => core\session::get('store_id')), 'goods_id', 0, 'combo');
        if (empty($goods_list)) {
            showDialog(lang('wrong_argument'));
        }
        // 删除该商品原有赠品
        ...
        if (!empty($data)) {
            foreach ($data as $key => $val) {
                ...
                foreach ($val as $v) {
                    ...
                    $is_general = $model_goods->checkIsGeneral($combo_info);
                    // 验证是否为普通商品
                    if ($combo_info['store_id'] != core\session::get('store_id') || $is_general == false || $owner_gid == $combo_gid) {
                        continue;
                    }
                    $array = array();
                    $array['goods_id'] = $owner_gid;
    ...
    public function search_goodsOp()
    {
        ...
        core\tpl::output('show_page', $model_goods->showpage(2));
        core\tpl::output('goods_list', $goods_list);
        core\tpl::showpage('store_goods_edit.search_goods', 'null_layout');
    }
    /**
     * 下载F码
     */
    public function download_f_code_excelOp()
    {
        ...
        if (empty($common_info) || $common_info['store_id'] != core\session::get('store_id')) {
            error(lang('wrong_argument'));
        }
    ...
    public function get_goods_list_ajaxOp()
    {
        ...
        if (empty($goodscommon_list) || $goodscommon_list['store_id'] != core\session::get('store_id')) {
            echo 'false';
            exit;
        }
        $goods_list = $model_goods->getGoodsList(array('store_id' => core\session::get('store_id'), 'goods_commonid' => $common_id), 'goods_id,goods_spec,store_id,goods_price,goods_serial,goods_storage_alarm,goods_storage,goods_image');
        if (empty($goods_list)) {
            echo 'false';
            exit;
        }
        ...
        foreach ($goods_list as $key => $val) {
            ...
            $goods_list[$key]['url'] = urlBiz('goods', 'index', array('goods_id' => $val['goods_id']));
        }
        /**
         * 转码
         */
    ...
    private function profile_menu($menu_type, $menu_key, $allow_promotion = array())
    {
        ...
        switch ($menu_type) {
            case 'goods_list':
                $menu_array = array(array('menu_key' => 'goods_list', 'menu_name' => '出售中的商品', 'menu_url' => urlBiz('store_goods_online', 'index')));
                break;
            case 'edit_detail':
                if ($allow_promotion['lock'] === false) {
                    $menu_array = array(array('menu_key' => 'edit_detail', 'menu_name' => '编辑商品', 'menu_url' => urlBiz('store_goods_online', 'edit_goods', array('commonid' => $_GET['commonid'], 'ref_url' => getReferer()))), array('menu_key' => 'edit_image', 'menu_name' => '编辑图片', 'menu_url' => urlBiz('store_goods_online', 'edit_image', array('commonid' => $_GET['commonid'], 'ref_url' => getReferer()))));
                }
                if ($allow_promotion['gift']) {
                    $menu_array[] = array('menu_key' => 'add_gift', 'menu_name' => '赠送赠品', 'menu_url' => urlBiz('store_goods_online', 'add_gift', array('commonid' => $_GET['commonid'], 'ref_url' => getReferer())));
                }
                if ($allow_promotion['combo']) {
                    $menu_array[] = array('menu_key' => 'add_combo', 'menu_name' => '推荐组合', 'menu_url' => urlBiz('store_goods_online', 'add_combo', array('commonid' => $_GET['commonid'], 'ref_url' => getReferer())));
                }
                break;
            case 'edit_class':
                $menu_array = array(array('menu_key' => 'edit_class', 'menu_name' => '选择分类', 'menu_url' => urlBiz('store_goods_online', 'edit_class', array('commonid' => $_GET['commonid'], 'ref_url' => getReferer()))), array('menu_key' => 'edit_detail', 'menu_name' => '编辑商品', 'menu_url' => urlBiz('store_goods_online', 'edit_goods', array('commonid' => $_GET['commonid'], 'ref_url' => getReferer()))), array('menu_key' => 'edit_image', 'menu_name' => '编辑图片', 'menu_url' => urlBiz('store_goods_online', 'edit_image', array('commonid' => $_GET['commonid'], 'ref_url' => getReferer()))));
                break;
        }
    ...
    public function maker_qrcodeOp()
    {
        ...
        foreach ($lst as $k => $v) {
            $goods_id = $v['goods_id'];
            $qrcode_url = APP_URL . '/tmpl/product_detail.html?goods_id=' . $goods_id;
            $PhpQRCode->set('date', $qrcode_url);
            $PhpQRCode->set('pngTempName', $goods_id . '.png');
            $PhpQRCode->init();
        }
        // 生成店铺二维码
        $qrcode_url = APP_URL . '/tmpl/store.html?store_id=' . $store_id;
        $PhpQRCode->set('date', $qrcode_url);
