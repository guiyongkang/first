...
class store_spec extends BaseSellerControl
{
    ...
    public function indexOp()
    {
        ...
        $gc_list = $model_goodsclass->getGoodsClass(core\session::get('store_id'));
        core\tpl::output('gc_list', $gc_list);
        $this->_profile_menu('spec', 'spec');
        core\tpl::showpage('store_spec.index');
    }
    /**
     * 添加规格值
     */
    public function add_specOp()
    {
        ...
        $gc_id = isset($_GET['gcid']) ? intval($_GET['gcid']) : 0;
        // 验证参数
        if ($sp_id <= 0) {
        ...
        core\tpl::output('gc_info', $gc_info);
        // 规格信息
        $model_spec = model('spec');
        $sp_info = $model_spec->getSpecInfo($sp_id, 'sp_id,sp_name');
        core\tpl::output('sp_info', $sp_info);
        // 规格值信息
        $sp_value_list = $model_spec->getSpecValueList(array('store_id' => core\session::get('store_id'), 'sp_id' => $sp_id, 'gc_id' => $gc_id));
        core\tpl::output('sp_value_list', $sp_value_list);
        $this->_profile_menu('spec_add', 'add');
        core\tpl::showpage('store_spec.add', 'null_layout');
    }
    /**
     * 保存规格值
     */
    public function save_specOp()
    {
        ...
        $model_spec = model('spec');
        // 更新原规格值
        if (!empty($_POST['sv']['old']) && is_array($_POST['sv']['old'])) {
            foreach ($_POST['sv']['old'] as $key => $value) {
                if (empty($value['name'])) {
                    continue;
                }
                $where = array('sp_value_id' => $key);
                $update = array('sp_value_name' => $value['name'], 'sp_id' => $sp_id, 'gc_id' => $gc_id, 'store_id' => core\session::get('store_id'), 'sp_value_color' => $value['color'], 'sp_value_sort' => intval($value['sort']));
                $model_spec->editSpecValue($update, $where);
            }
        }
        // 添加新规格值
        if (!empty($_POST['sv']['new']) && is_array($_POST['sv']['new'])) {
            $insert_array = array();
            foreach ($_POST['sv']['new'] as $value) {
                if (empty($value['name'])) {
                    continue;
                }
                $tmp_insert = array('sp_value_name' => $value['name'], 'sp_id' => $sp_id, 'gc_id' => $gc_id, 'store_id' => core\session::get('store_id'), 'sp_value_color' => $value['color'], 'sp_value_sort' => intval($value['sort']));
    ...
    public function ajax_delspecOp()
    {
        ...
        $rs = model('spec')->delSpecValue(array('sp_value_id' => $sp_value_id, 'store_id' => core\session::get('store_id')));
        if ($rs) {
            echo 'true';
            exit;
        } else {
    ...
    public function ajax_classOp()
    {
        ...
        // 读取商品分类
        if ($deep != 4) {
            $gc_list = $model_goodsclass->getGoodsClass(core\session::get('store_id'), $id, $deep);
        }
        // 分类不为空输出分类信息
        if (!empty($gc_list)) {
    ...
    private function _profile_menu($menu_type, $menu_key)
    {
        ...
        switch ($menu_type) {
            case 'spec':
                $menu_array = array(array('menu_key' => 'spec', 'menu_name' => lang('nc_member_path_store_spec'), 'menu_url' => 'index.php?act=store_spec'));
                break;
        }
