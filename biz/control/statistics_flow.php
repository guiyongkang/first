...
class statistics_flow extends BaseSellerControl
{
    private $search_arr;
    //处理后的参数
    ...
    public function storeflowOp()
    {
        ...
        //获取店铺ID的末位数字
        $tablenum = ($t = intval(core\config::get('flowstat_tablenum'))) > 1 ? $t : 1;
        //处理流量统计记录表数量
        ...
        core\tpl::showpage('stat.flow.store');
    }
    /**
     * 商品流量统计
     */
    public function goodsflowOp()
    {
        ...
        core\tpl::showpage('stat.flow.goods');
    }
    /**
     * 用户中心右边，小导航
     *
    ...
    {
        $menu_array = array(1 => array('menu_key' => 'storeflow', 'menu_name' => '店铺总流量', 'menu_url' => 'index.php?act=statistics_flow&op=storeflow'), 2 => array('menu_key' => 'goodsflow', 'menu_name' => '商品流量排名', 'menu_url' => 'index.php?act=statistics_flow&op=goodsflow'));
