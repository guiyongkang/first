...
class store_voucher extends BaseSellerControl
{
    //定义代金券类常量
    ...
    public function __construct()
    {
        parent::__construct();
        //读取语言包
        core\language::read('member_layout,member_voucher');
        //判断系统是否开启代金券功能
        if (core\config::get('voucher_allow') != 1) {
            error(core\language::get('voucher_unavailable'), 'index.php?act=store');
        }
        //申请记录状态
        $this->applystate_arr = array('new' => array(1, core\language::get('voucher_applystate_new')), 'verify' => array(2, core\language::get('voucher_applystate_verify')), 'cancel' => array(3, core\language::get('voucher_applystate_cancel')));
        //套餐状态
        $this->quotastate_arr = array('activity' => array(1, core\language::get('voucher_quotastate_activity')), 'cancel' => array(2, core\language::get('voucher_quotastate_cancel')), 'expire' => array(3, core\language::get('voucher_quotastate_expire')));
        //代金券模板状态
        $this->templatestate_arr = array('usable' => array(1, core\language::get('voucher_templatestate_usable')), 'disabled' => array(2, core\language::get('voucher_templatestate_disabled')));
    ...
    public function templatelistOp()
    {
        ...
        if (checkPlatformStore()) {
            ...
            //查询是否存在可用套餐
            $current_quota = $model_voucher->getCurrentQuota(core\session::get('store_id'));
            core\tpl::output('current_quota', $current_quota);
        }
        //领取方式
        ...
        $param = array();
        //领取方式查询
        $gettype_sel = isset($_GET['gettype_sel']) ? trim($_GET['gettype_sel']) : '';
        if ($gettype_sel) {
            $param['voucher_t_gettype'] = $gettype_arr[$gettype_sel]['sign'];
        }
        $param['voucher_t_store_id'] = core\session::get('store_id');
        if (isset($_GET['txt_keyword']) && trim($_GET['txt_keyword'])) {
            $param['voucher_t_title'] = array('like', '%' . trim($_GET['txt_keyword']) . '%');
        }
        $select_state = isset($_GET['select_state']) ? intval($_GET['select_state']) : 0;
        if ($select_state) {
            $param['voucher_t_state'] = $select_state;
        }
        if (isset($_GET['txt_startdate'])) {
            $param['voucher_t_end_date'] = array('egt', strtotime($_GET['txt_startdate']));
        }
        if (isset($_GET['txt_enddate'])) {
        ...
        //领取方式
        core\tpl::output('gettype_arr', $gettype_arr);
        core\tpl::output('list', $list);
        core\tpl::output('show_page', $model_voucher->showpage(2));
        $this->profile_menu('voucher', 'templatelist');
        core\tpl::showpage('store_voucher_template.index');
    }
    /*
     * 代金券模版列表
     */
    public function voucherlistOp()
    {
        ...
        if ($t_id <= 0) {
            core\tpl::showpage('store_voucher.list', 'null_layout');
        }
        $model_voucher = model('voucher');
        //查询代金券模板
        $where = array();
        $where['voucher_t_id'] = $t_id;
        $where['voucher_t_store_id'] = core\session::get('store_id');
        ...
        core\tpl::output('voucher_list', $voucher_list);
        core\tpl::output('show_page', $model_voucher->showpage(2));
        core\tpl::showpage('store_voucher.list', 'null_layout');
    }
    /**
    ...
    public function voucher_exportOp()
    {
        ...
        if (isset($_GET['curpage']) && !is_numeric($_GET['curpage'])) {
            ...
            if ($count > self::EXPORT_SIZE) {
                ...
                core\tpl::output('murl', 'index.php?act=store_voucher&op=voucher_export');
                core\tpl::showpage('store_export.excel');
            } else {
                //如果数量小，直接下载
    ...
    private function createExcel($data = array(), $t_info)
    {
        ...
        $excel_obj->generateXML($excel_obj->charset('代金券模板(' . $voucher_t_title . ')的代金券及其卡密', CHARSET) . (isset($_GET['curpage']) ? $_GET['curpage'] : 1));
    }
    /**
     * 购买套餐
     */
    public function quotaaddOp()
    {
        if (chksubmit()) {
            ...
            if ($quota_quantity <= 0 || $quota_quantity > 12) {
                showDialog(core\language::get('voucher_apply_num_error'));
            }
            //获取当前价格
            $current_price = intval(core\config::get('promotion_voucher_price'));
            $model = model();
            $model_voucher = model('voucher');
            //获取该用户已有套餐
            $current_quota = $model_voucher->getCurrentQuota(core\session::get('store_id'));
            ...
            if (empty($current_quota)) {
                //生成套餐
                $param = array();
                $param['quota_memberid'] = core\session::get('member_id');
                $param['quota_membername'] = core\session::get('member_name');
                $param['quota_storeid'] = core\session::get('store_id');
                $param['quota_storename'] = core\session::get('store_name');
            ...
            $this->recordSellerLog('购买' . $quota_quantity . '份代金券套餐，单价' . $current_price . lang('nc_yuan'));
            if ($reault) {
                showDialog(core\language::get('voucher_apply_buy_succ'), 'index.php?act=store_voucher&op=quotalist', 'succ');
            } else {
                showDialog(core\language::get('nc_common_op_fail'), 'index.php?act=store_voucher&op=quotalist');
            }
        } else {
            //输出导航
            self::profile_menu('quota_add', 'quotaadd');
            core\tpl::showpage('store_voucher_quota.add');
        }
    }
    /*
     * 代金券模版添加
    ...
    public function templateaddOp()
    {
        ...
        if ($isOwnShop = checkPlatformStore()) {
            ...
            //查询当前可以套餐
            $quotainfo = $model->getCurrentQuota(core\session::get('store_id'));
            if (empty($quotainfo)) {
                error(core\language::get('voucher_template_quotanull'), 'index.php?act=store_voucher&op=quotaadd');
            }
            //查询该套餐下代金券模板列表
            ...
            if ($count >= core\config::get('promotion_voucher_storetimes_limit')) {
                $message = sprintf(core\language::get('voucher_template_noresidual'), core\config::get('promotion_voucher_storetimes_limit'));
                error($message, 'index.php?act=store_voucher&op=templatelist');
            }
        }
        //查询面额列表
        $pricelist = $model->table('voucher_price')->order('voucher_price asc')->select();
        if (empty($pricelist)) {
            error(core\language::get('voucher_template_pricelisterror'), 'index.php?act=store_voucher&op=templatelist');
        }
        //获取领取方式
        ...
        if (chksubmit()) {
            ...
            $obj_validate->validateparam = array(array('input' => $_POST['txt_template_title'], 'require' => 'true', 'validator' => 'Length', 'min' => '1', 'max' => '50', 'message' => core\language::get('voucher_template_title_error')), array('input' => $_POST['txt_template_total'], 'require' => 'true', 'validator' => 'Number', 'min' => '1', 'message' => core\language::get('voucher_template_total_error')), array('input' => $_POST['select_template_price'], 'require' => 'true', 'validator' => 'Number', 'message' => core\language::get('voucher_template_price_error')), array('input' => $_POST['txt_template_limit'], 'require' => 'true', 'validator' => 'Double', 'message' => core\language::get('voucher_template_limit_error')), array('input' => $_POST['txt_template_describe'], 'require' => 'true', 'validator' => 'Length', 'min' => '1', 'max' => '255', 'message' => core\language::get('voucher_template_describe_error')), array('input' => $_POST['gettype_sel'], 'require' => 'true', 'message' => '请选择领取方式'));
            ...
            if (empty($chooseprice)) {
                $error .= core\language::get('voucher_template_pricelisterror');
            }
			
            $limit = floatval($_POST['txt_template_limit']) > 0 ? floatval($_POST['txt_template_limit']) : 0;
            if ($limit > 0 && $price >= $limit) {
                $error .= core\language::get('voucher_template_price_error');
            }
            //验证卡密代金券发放数量
            ...
            if ($error) {
                ...
                $insert_arr['voucher_t_store_id'] = core\session::get('store_id');
                $insert_arr['voucher_t_storename'] = core\session::get('store_name');
                $insert_arr['voucher_t_sc_id'] = intval($_POST['sc_id']);
                $insert_arr['voucher_t_creator_id'] = core\session::get('member_id');
                ...
                $insert_arr['voucher_t_quotaid'] = isset($quotainfo['quota_id']) ? $quotainfo['quota_id'] : 0;
                ...
                if (!empty($_FILES['customimg']['name'])) {
                    ...
                    $upload->set('default_dir', ATTACH_VOUCHER . DS . core\session::get('store_id'));
                ...
                if ($rs) {
                    //生成卡密代金券
                    if ($gettype == 'pwd') {
                        lib\queue::push('build_pwdvoucher', $rs);
                    }
                    showDialog(core\language::get('nc_common_save_succ'), 'index.php?act=store_voucher&op=templatelist', 'succ');
                } else {
                    showDialog(core\language::get('nc_common_save_fail'), 'index.php?act=store_voucher&op=templatelist', 'error');
                }
            }
        } else {
            //店铺分类
            ...
            //查询店铺详情
            $store_info = model('store')->getStoreInfoByID(core\session::get('store_id'));
            core\tpl::output('store_info', $store_info);
            //领取方式
            core\tpl::output('gettype_arr', $gettype_array);
            //会员级别
            core\tpl::output('member_grade', $member_grade);
            core\tpl::output('type', 'add');
            core\tpl::output('quotainfo', isset($quotainfo) ? $quotainfo : array());
            core\tpl::output('pricelist', $pricelist);
            $this->profile_menu('template', 'templateadd');
            core\tpl::showpage('store_voucher_template.add');
        }
    }
    /*
     * 代金券模版编辑
    ...
    public function templateeditOp()
    {
        ...
        if ($t_id <= 0) {
            showMessage(core\language::get('wrong_argument'), 'index.php?act=store_voucher&op=templatelist', 'html', 'error');
        }
        $model = model('voucher');
        //查询模板信息
        $param = array();
        ...
        if ($isOwnShop = checkPlatformStore()) {
            ...
            //查询套餐信息
            $quotainfo = $model->table('voucher_quota')->where(array('quota_id' => $t_info['voucher_t_quotaid'], 'quota_storeid' => core\session::get('store_id')))->find();
            if (empty($quotainfo)) {
                showMessage(core\language::get('voucher_template_quotanull'), 'index.php?act=store_voucher&op=quotaadd', 'html', 'error');
            }
        }
        //查询面额列表
        $pricelist = $model->table('voucher_price')->order('voucher_price asc')->select();
        if (empty($pricelist)) {
            showMessage(core\language::get('voucher_template_pricelisterror'), 'index.php?act=store_voucher&op=templatelist', 'html', 'error');
        }
        //获取领取方式
        $gettype_array = $model->getVoucherGettypeArray();
        //会员级别
        ...
        if (chksubmit()) {
            //验证提交的内容面额不能大于限额
            $obj_validate = new lib\validate();
            $obj_validate->validateparam = array(array('input' => $_POST['txt_template_title'], 'require' => 'true', 'validator' => 'Length', 'min' => '1', 'max' => '50', 'message' => core\language::get('voucher_template_title_error')), array('input' => $_POST['txt_template_total'], 'require' => 'true', 'validator' => 'Number', 'message' => core\language::get('voucher_template_total_error')), array('input' => $_POST['select_template_price'], 'require' => 'true', 'validator' => 'Number', 'message' => core\language::get('voucher_template_price_error')), array('input' => $_POST['txt_template_limit'], 'require' => 'true', 'validator' => 'Double', 'message' => core\language::get('voucher_template_limit_error')), array('input' => $_POST['txt_template_describe'], 'require' => 'true', 'validator' => 'Length', 'min' => '1', 'max' => '255', 'message' => core\language::get('voucher_template_describe_error')), array('input' => $_POST['gettype_sel'], 'require' => 'true', 'message' => '请选择领取方式'));
            $error = $obj_validate->validate();
            //金额验证
            $price = intval($_POST['select_template_price']) > 0 ? intval($_POST['select_template_price']) : 0;
            foreach ($pricelist as $k => $v) {
            ...
            if ($error) {
                ...
                if (!empty($_FILES['customimg']['name'])) {
                    ...
                    if ($result) {
                        ...
                        if (!empty($t_info['voucher_t_customimg'])) {
                            //如果模板存在，则删除原模板图片
                            @unlink(BASE_UPLOAD_PATH . DS . ATTACH_VOUCHER . DS . core\session::get('store_id') . DS . $t_info['voucher_t_customimg']);
                            @unlink(BASE_UPLOAD_PATH . DS . ATTACH_VOUCHER . DS . core\session::get('store_id') . DS . str_ireplace('.', '_small.', $t_info['voucher_t_customimg']));
                        }
                ...
                if ($rs) {
                    //生成卡密代金券
                    if ($gettype == 'pwd') {
                        lib\queue::push('build_pwdvoucher', $t_info['voucher_t_id']);
                    }
                    showDialog(core\language::get('nc_common_op_succ'), 'index.php?act=store_voucher&op=templatelist', 'succ');
                } else {
                    showDialog(core\language::get('nc_common_op_fail'), 'index.php?act=store_voucher&op=templatelist', 'error');
                }
            }
        } else {
            if (!$t_info['voucher_t_customimg'] || !file_exists(BASE_UPLOAD_PATH . DS . ATTACH_VOUCHER . DS . core\session::get('store_id') . DS . $t_info['voucher_t_customimg'])) {
                $t_info['voucher_t_customimg'] = UPLOAD_SITE_URL . DS . defaultGoodsImage(240);
            } else {
                $t_info['voucher_t_customimg'] = UPLOAD_SITE_URL . DS . ATTACH_VOUCHER . DS . core\session::get('store_id') . DS . str_ireplace('.', '_small.', $t_info['voucher_t_customimg']);
            }
            core\tpl::output('type', 'edit');
            core\tpl::output('t_info', $t_info);
            //店铺分类
            $store_class = rkcache('store_class', true);
            core\tpl::output('store_class', $store_class);
    ...
    public function templatedelOp()
    {
        ...
        $t_info = $model->table('voucher_template')->where($param)->find();
        if (empty($t_info)) {
            error(core\language::get('wrong_argument'), 'index.php?act=store_voucher&op=templatelist');
        }
        ...
        if ($rs) {
            ...
            showDialog(core\language::get('nc_common_del_succ'), 'reload', 'succ');
        } else {
    ...
    public function templateinfoOp()
    {
        ...
        core\tpl::showpage('store_voucher_template.info');
    }
    /*
     * 把代金券模版设为失效
     */
    ...
    public function bulidvoucherOp()
    {
        ...
        //生成卡密代金券队列
        lib\queue::push('build_pwdvoucher', $t_id);
    ...
    private function profile_menu($menu_type, $menu_key = '')
    {
        ...
        switch ($menu_type) {
            case 'voucher':
                $menu_array = array(1 => array('menu_key' => 'templatelist', 'menu_name' => core\language::get('nc_member_path_store_voucher'), 'menu_url' => 'index.php?act=store_voucher&op=templatelist'));
                break;
            case 'quota_add':
                $menu_array = array(1 => array('menu_key' => 'templatelist', 'menu_name' => core\language::get('nc_member_path_store_voucher'), 'menu_url' => 'index.php?act=store_voucher&op=templatelist'), 4 => array('menu_key' => 'quotaadd', 'menu_name' => core\language::get('voucher_applyadd'), 'menu_url' => 'index.php?act=store_voucher&op=quotaadd'));
                break;
            case 'template':
                $menu_array = array(1 => array('menu_key' => 'templatelist', 'menu_name' => core\language::get('nc_member_path_store_voucher'), 'menu_url' => 'index.php?act=store_voucher&op=templatelist'), 2 => array('menu_key' => 'templateadd', 'menu_name' => core\language::get('voucher_templateadd'), 'menu_url' => 'index.php?act=store_voucher&op=templateadd'));
                break;
            case 'templateedit':
                $menu_array = array(1 => array('menu_key' => 'templatelist', 'menu_name' => core\language::get('nc_member_path_store_voucher'), 'menu_url' => 'index.php?act=store_voucher&op=templatelist'), 2 => array('menu_key' => 'templateedit', 'menu_name' => core\language::get('voucher_templateedit'), 'menu_url' => ''));
                break;
            case 'templateinfo':
                $menu_array = array(1 => array('menu_key' => 'templatelist', 'menu_name' => core\language::get('nc_member_path_store_voucher'), 'menu_url' => 'index.php?act=store_voucher&op=templatelist'), 2 => array('menu_key' => 'templateinfo', 'menu_name' => core\language::get('voucher_templateinfo'), 'menu_url' => ''));
                break;
        }
