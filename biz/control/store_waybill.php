...
class store_waybill extends BaseSellerControl
{
    ...
    public function waybill_manageOp()
    {
        ...
        $store_extend_info = $model_store_extend->getStoreExtendInfo(array('store_id' => core\session::get('store_id')), 'express');
        $store_express = isset($store_extend_info['express']) ? $store_extend_info['express'] : array();
        $express_list = $model_express->getExpressListByID($store_express);
        $store_waybill_list = $model_store_waybill->getStoreWaybillListWithWaybillInfo(core\session::get('store_id'), $store_express);
        ...
        core\tpl::showpage('store_waybill.manage');
    }
    /**
     * 绑定运单打印模板
     */
    public function waybill_bindOp()
    {
        ...
        $waybill_list = $model_waybill->getWaybillUsableList($express_id, core\session::get('store_id'));
        core\tpl::output('waybill_list', $waybill_list);
        $this->profile_menu('waybill_bind');
        core\tpl::showpage('store_waybill.bind');
    }
    /**
     * 绑定运单打印模板保存
     */
    public function waybill_bind_saveOp()
    {
        ...
        if ($result) {
            success('绑定成功', urlBiz('store_waybill', 'waybill_manage'));
        } else {
            error('绑定失败');
        }
    }
    ...
    public function waybill_settingOp()
    {
        ...
        core\tpl::output('store_waybill_id', $store_waybill_info['store_waybill_id']);
        core\tpl::output('store_waybill_left', $store_waybill_info['store_waybill_left']);
        core\tpl::output('store_waybill_top', $store_waybill_info['store_waybill_top']);
        core\tpl::output('store_waybill_data', $store_waybill_info['store_waybill_data']);
        $this->profile_menu('waybill_setting');
        core\tpl::showpage('store_waybill.setting');
    }
    /**
     * 运单模板设置保存 
     */
    public function waybill_setting_saveOp()
    {
        ...
        if ($result) {
            showDialog(lang('nc_common_save_succ'), urlBiz('store_waybill', 'waybill_manage'), 'succ');
        } else {
            showDialog(lang('nc_common_save_fail'), '', 'error');
        }
    }
    /**
     * 运单打印测试
    ...
    public function waybill_testOp()
    {
        ...
        core\tpl::showpage('store_waybill.test', 'null_layout');
    }
    /**
     * 设置默认打印模板
     */
    public function waybill_set_defaultOp()
    {
        $store_waybill_id = intval($_POST['store_waybill_id']);
        $model_store_waybill = model('store_waybill');
        $result = $model_store_waybill->editStoreWaybillDefault($store_waybill_id, core\session::get('store_id'));
        if ($result) {
            showDialog(lang('nc_common_save_succ'), '');
        } else {
            showDialog(lang('nc_common_save_fail'), '', '', 'error');
        }
    }
    ...
    {
        $model_waybill = model('waybill');
        $waybill_list = $model_waybill->getWaybillSellerList(core\session::get('store_id'));
        core\tpl::output('waybill_list', $waybill_list);
        $this->profile_menu('waybill_list');
        core\tpl::showpage('store_waybill.list');
    }
    /**
     * 添加运单模板
     */
    public function waybill_addOp()
    {
        ...
        core\tpl::showpage('store_waybill.add');
    }
    /**
     * 保存运单模板
     */
    ...
    {
        $model_waybill = model('waybill');
        $result = $model_waybill->saveWaybill($_POST, core\session::get('store_id'));
        if (!isset($result['error'])) {
            showDialog(lang('nc_common_save_succ'), urlBiz('store_waybill', 'waybill_list'), 'succ');
        } else {
            showDialog(lang('nc_common_save_fail'), urlBiz('store_waybill', 'waybill_list'));
        }
    }
    /**
     * 删除运单模板
    ...
    public function waybill_editOp()
    {
        ...
        $model_waybill = model('waybill');
        $waybill_info = $model_waybill->getWaybillInfoByID($_GET['waybill_id']);
        if (!$waybill_info || $waybill_info['store_id'] != core\session::get('store_id')) {
            showDialog('运单模板不存在');
        }
        core\tpl::output('waybill_info', $waybill_info);
    ...
    public function waybill_designOp()
    {
        ...
        core\tpl::showpage('store_waybill.design');
    }
    /**
     * 设计运单模板保存
     */
    ...
    {
        $model_waybill = model('waybill');
        $result = $model_waybill->editWaybillDataByID($_POST['waybill_data'], $_POST['waybill_id'], core\session::get('store_id'));
        if ($result) {
            showDialog(lang('nc_common_save_succ'), urlBiz('store_waybill', 'waybill_list'), 'succ');
        } else {
            showDialog(lang('nc_common_save_fail'), urlBiz('store_waybill', 'waybill_list'));
    ...
    {
        $menu_array = array();
        $menu_array[] = array('menu_key' => 'waybill_manage', 'menu_name' => '模板绑定', 'menu_url' => urlBiz('store_waybill', 'waybill_manage'));
        $menu_array[] = array('menu_key' => 'waybill_list', 'menu_name' => '自建模板', 'menu_url' => urlBiz('store_waybill', 'waybill_list'));
        if ($menu_key == 'waybill_bind') {
            $menu_array[] = array('menu_key' => 'waybill_bind', 'menu_name' => '选择模板', 'menu_url' => urlBiz('store_waybill', 'waybill_bind'));
        }
        if ($menu_key == 'waybill_setting') {
            $menu_array[] = array('menu_key' => 'waybill_setting', 'menu_name' => '模板设置', 'menu_url' => urlBiz('store_waybill', 'waybill_setting'));
        }
        if ($menu_key == 'waybill_add') {
            $menu_array[] = array('menu_key' => 'waybill_add', 'menu_name' => '添加模板', 'menu_url' => urlBiz('store_waybill', 'waybill_add'));
        }
        if ($menu_key == 'waybill_edit') {
            $menu_array[] = array('menu_key' => 'waybill_edit', 'menu_name' => '编辑模板', 'menu_url' => urlBiz('store_waybill', 'waybill_edit'));
        }
        if ($menu_key == 'waybill_design') {
            $menu_array[] = array('menu_key' => 'waybill_design', 'menu_name' => '设计模板', 'menu_url' => urlBiz('store_waybill', 'waybill_design'));
        }
