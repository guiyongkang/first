...
class store_activity extends BaseSellerControl
{
    ...
    public function store_activityOp()
    {
        ...
        core\tpl::showpage('store_activity.list');
    }
    /**
     * 参与活动
     */
    public function activity_applyOp()
    {
        ...
        if ($activity_id <= 0) {
            showMessage(core\language::get('para_error'), 'index.php?act=store_activity&op=store_activity', 'html', 'error');
        }
        ...
        if (empty($activity_info) || $activity_info['activity_type'] != '1' || $activity_info['activity_state'] != 1 || $activity_info['activity_start_date'] > time() || $activity_info['activity_end_date'] < time()) {
            error(core\language::get('store_activity_not_exists'), 'index.php?act=store_activity&op=store_activity');
        }
        core\tpl::output('activity_info', $activity_info);
        $list = array();
        //声明存放活动细节的数组
        //查询商品分类列表
        ...
        $list = $activity_detail_model->getGoodsJoinList(array('activity_id' => $activity_id, 'store_id' => core\session::get('store_id'), 'activity_detail_state_in' => "'0','1','3'", 'group' => 'activity_detail_state asc'));
        ...
        core\tpl::showpage('store_activity.apply');
    }
    /**
     * 活动申请保存
     */
    public function activity_apply_saveOp()
    {
        ...
        if (empty($_POST['item_id'])) {
            showDialog(core\language::get('store_activity_choose_goods'), 'index.php?act=store_activity&op=store_activity');
        }
        $activity_id = intval($_POST['activity_id']);
        if ($activity_id <= 0) {
            showDialog(core\language::get('para_error'), 'index.php?act=store_activity&op=store_activity');
        }
        ...
        if (empty($activity) || $activity['activity_type'] != '1' || $activity['activity_state'] != '1' || $activity['activity_start_date'] > time() || $activity['activity_end_date'] < time()) {
            showDialog(core\language::get('store_activity_not_exists'), 'index.php?act=store_activity&op=store_activity');
        }
        $activity_detail = model('activity_detail');
        $list = $activity_detail->getList(array('store_id' => core\session::get('store_id'), 'activity_id' => $activity_id));
        $ids = array();
        //已经存在的活动内容编号
        $ids_state2 = array();
        //已经存在的被拒绝的活动编号
        ...
        foreach ($_POST['item_id'] as $item_id) {
            ...
            if (!in_array($item_id, $ids)) {
                ...
                if (empty($item) || $item['store_id'] != core\session::get('store_id')) {
                    continue;
                }
        ...
        showDialog(core\language::get('store_activity_submitted'), 'reload', 'succ');
    }
    /**
     * 用户中心右边，小导航
     *
    ...
    private function profile_menu($menu_key = '')
    {
        ...
        $menu_array = array(1 => array('menu_key' => 'store_activity', 'menu_name' => core\language::get('nc_member_path_activity_list'), 'menu_url' => 'index.php?act=store_activity&op=store_activity'), 2 => array('menu_key' => 'activity_apply', 'menu_name' => core\language::get('nc_member_path_join_activity'), 'menu_url' => ''));
