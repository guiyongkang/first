...
class store_deliver extends BaseSellerControl
{
    ...
    public function indexOp()
    {
        ...
        if (empty($_GET['state']) || !in_array($_GET['state'], array('deliverno', 'delivering', 'delivered'))) {
        ...
        foreach ($order_list as $key => $order_info) {
            foreach ($order_info['extend_order_goods'] as $value) {
                ...
                $value['goods_url'] = urlBiz('goods', 'index', array('goods_id' => $value['goods_id']));
        ...
        core\tpl::showpage('store_order.deliver');
    }
    /**
     * 发货
     */
    public function sendOp()
    {
        ...
        if (chksubmit()) {
            ...
            $result = $logic_order->changeOrderSend($order_info, 'seller', core\session::get('member_name'), $_POST);
            if (!$result['state']) {
                error($result['msg']);
            } else {
				
				/*分销相关处理*/
				logic('distributor')->deal_commission_state(array(),$order_info,30);
				...
				$flag = logic('weixin_message')->addorder($access_token, $disconfig ,$weixin_config, $order_info['buyer_name'], $order_info['order_amount'], $order_info['order_id'], 2);
        ...
        if ($order_info['extend_order_common']['daddress_id'] > 0) {
            ...
            //取默认地址
            $daddress_info = $model_daddress->getAddressList(array('store_id' => core\session::get('store_id')), '*', 'is_default desc', 1);
            $daddress_info = isset($daddress_info[0]) ? $daddress_info[0] : array();
            //写入发货地址编号
            $this->_edit_order_daddress(isset($daddress_info['address_id']) ? $daddress_info['address_id'] : 0, $order_id);
        }
        ...
        if (isset($order_info['extend_order_common']['reciver_info']['dlyp'])) {
            ...
            //快递公司
            $my_express_list = model()->table('store_extend')->getfby_store_id(core\session::get('store_id'), 'express');
            if (!empty($my_express_list)) {
                $my_express_list = explode(',', $my_express_list);
            }
        }
        core\tpl::output('my_express_list', $my_express_list);
        core\tpl::output('express_list', $express_list);
        core\tpl::showpage('store_deliver.send');
    }
    /**
     * 编辑收货地址
     * @return boolean
    ...
    public function buyer_address_editOp()
    {
        ...
        core\tpl::output('address_info', $order_common_info);
        core\tpl::showpage('store_deliver.buyer_address.edit', 'null_layout');
    }
    /**
     * 收货地址保存
     */
    ...
    {
        $reciver_info = array('address' => $_POST['reciver_area'] . ' ' . $_POST['reciver_street'], 'phone' => trim($_POST['reciver_mob_phone'] . ',' . $_POST['reciver_tel_phone'], ','), 'area' => $_POST['reciver_area'], 'street' => $_POST['reciver_street'], 'mob_phone' => $_POST['reciver_mob_phone'], 'tel_phone' => $_POST['reciver_tel_phone'], 'dlyp' => $_POST['reciver_dlyp']);
    ...
    {
        core\language::read('deliver');
        $address_list = model('daddress')->getAddressList(array('store_id' => core\session::get('store_id')));
        core\tpl::output('address_list', $address_list);
        core\tpl::output('order_id', $_GET['order_id']);
        core\tpl::showpage('store_deliver.daddress.select', 'null_layout');
    }
    /**
     * 保存发货地址修改
     */
    ...
    public function search_deliverOp()
    {
        ...
        core\tpl::output('e_code', $express[$order_info['extend_order_common']['shipping_express_id']]['e_code']);
        core\tpl::output('e_name', $express[$order_info['extend_order_common']['shipping_express_id']]['e_name']);
        core\tpl::output('e_url', $express[$order_info['extend_order_common']['shipping_express_id']]['e_url']);
        core\tpl::output('shipping_code', $order_info['shipping_code']);
        ...
        core\tpl::output('shipping_express', $get_content);
        self::profile_menu('search', 'search');
        core\tpl::showpage('store_deliver.detail');
    }
    /**
     * 延迟收货
     */
    public function delay_receiveOp()
    {
        ...
        if (chksubmit()) {
            ...
            if (!in_array($delay_date, array(5, 10, 15))) {
                showDialog(core\language::get('wrong_argument'), '', 'error', empty($_GET['inajax']) ? '' : 'CUR_DIALOG.close();');
            }
            ...
            core\tpl::output('order_info', $order_info);
            core\tpl::showpage('store_deliver.delay_receive', 'null_layout');
            exit;
        }
    }
    /**
    ...
    public function get_expressOp()
    {
        ...
        if (strtoupper(CHARSET) == 'GBK') {
            $output = core\language::getUTF8($output);
    ...
    public function waybill_printOp()
    {
        ...
        $store_waybill_info = $this->_getCurrentWaybill($store_waybill_list, isset($_GET['store_waybill_id']) ? $_GET['store_waybill_id'] : 0);
        if (empty($store_waybill_info)) {
            error('请首先绑定打印模板', urlBiz('store_waybill', 'waybill_manage'));
        }
        ...
        core\tpl::output('store_waybill_list', $store_waybill_list);
        core\tpl::showpage('waybill.print', 'null_layout');
    }
    /**
     * 获取当前打印模板
     */
    ...
    private function profile_menu($menu_type, $menu_key = '')
    {
        ...
        switch ($menu_type) {
            case 'deliver':
                $menu_array = array(array('menu_key' => 'deliverno', 'menu_name' => core\language::get('nc_member_path_deliverno'), 'menu_url' => 'index.php?act=store_deliver&op=index&state=deliverno'), array('menu_key' => 'delivering', 'menu_name' => core\language::get('nc_member_path_delivering'), 'menu_url' => 'index.php?act=store_deliver&op=index&state=delivering'), array('menu_key' => 'delivered', 'menu_name' => core\language::get('nc_member_path_delivered'), 'menu_url' => 'index.php?act=store_deliver&op=index&state=delivered'));
                break;
            case 'search':
                $menu_array = array(1 => array('menu_key' => 'nodeliver', 'menu_name' => core\language::get('nc_member_path_deliverno'), 'menu_url' => 'index.php?act=store_deliver&op=index&state=nodeliver'), 2 => array('menu_key' => 'delivering', 'menu_name' => core\language::get('nc_member_path_delivering'), 'menu_url' => 'index.php?act=store_deliver&op=index&state=delivering'), 3 => array('menu_key' => 'delivered', 'menu_name' => core\language::get('nc_member_path_delivered'), 'menu_url' => 'index.php?act=store_deliver&op=index&state=delivered'), 4 => array('menu_key' => 'search', 'menu_name' => core\language::get('nc_member_path_deliver_info'), 'menu_url' => '###'));
                break;
        }
