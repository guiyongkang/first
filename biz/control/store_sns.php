...
class store_sns extends BaseSellerControl
{
    public function __construct()
    {
        parent::__construct();
        core\language::read('store_sns,member_sns');
    }
    ...
    public function addOp()
    {
        ...
        // 热销商品
        // where条件
        $where = array('store_id' => core\session::get('store_id'));
        ...
        core\tpl::output('hotsell_list', $hotsell_list);
        // 新品
        // where条件
        $where = array('store_id' => core\session::get('store_id'));
        $field = 'goods_id,goods_name,goods_image,goods_price,goods_salenum,store_id';
        ...
        core\tpl::output('new_list', $new_list);
        $this->profile_menu('store_sns_add');
        core\tpl::showpage('store_sns_add');
    }
    /**
     * 上传图片
     */
    public function image_uploadOp()
    {
        ...
        // 取得图像大小
        list($width, $height, $type, $attr) = getimagesize(UPLOAD_SITE_URL . '/' . ATTACH_GOODS . '/' . core\session::get('store_id') . DS . $img_path);
        // 存入相册
        ...
        $data = array();
        $data['image'] = cthumb($img_path, 240, core\session::get('store_id'));
        // 整理为json格式
    ...
    public function store_sns_saveOp()
    {
        ...
        $obj_validate->validateparam = array(array('input' => $_POST['content'], 'require' => 'true', 'validator' => 'Length', 'max' => 140, 'min' => 1, 'message' => core\language::get('store_sns_center_error')), array('input' => isset($_POST['goods_url']) ? $_POST['goods_url'] : '', 'require' => 'false', 'validator' => 'url', 'message' => core\language::get('store_goods_index_goods_price_null')));
        ...
        switch ($_POST['type']) {
            ...
            case '10':
                if (is_array($_POST['goods_id'])) {
                    ...
                    showDialog(core\language::get('store_sns_choose_goods'));
                }
                $field = 'goods_id,store_id,goods_name,goods_image,goods_price,goods_freight';
                $where = array('store_id' => core\session::get('store_id'), 'goods_id' => array('in', $goods_id_array));
            ...
            case '3':
                ...
                break;
            default:
                showDialog(core\language::get('para_error'));
        }
        ...
        if ($rs) {
            showDialog(core\language::get('nc_common_op_succ'), 'index.php?act=store_sns', 'succ');
        } else {
    ...
    public function settingOp()
    {
        ...
        $sauto_info = $model_storesnssetting->getStoreSnsSettingInfo(array('sauto_storeid' => core\session::get('store_id')));
        core\tpl::output('sauto_info', $sauto_info);
        $this->profile_menu('store_sns_setting');
        core\tpl::showpage('store_sns_setting');
    }
    /**
     * 用户中心右边，小导航
     *
    ...
    {
        $menu_array = array(1 => array('menu_key' => 'store_sns_add', 'menu_name' => core\language::get('store_sns_add'), 'menu_url' => 'index.php?act=store_sns&op=add'), 2 => array('menu_key' => 'store_sns_setting', 'menu_name' => core\language::get('store_sns_setting'), 'menu_url' => 'index.php?act=store_sns&op=setting'), /*3 => array('menu_key' => 'store_sns_brower', 'menu_name' => core\language::get('store_sns_browse'), 'menu_url' => urlBiz('store_snshome', 'index', array('sid' => core\session::get('store_id'))), 'target' => '_blank')*/);
    ...
    private function getGoodsByUrl($url)
    {
        ...
        if (intval($id) > 0) {
            ...
            if (!empty($result) && is_array($result)) {
                ...
                showDialog(core\language::get('store_sns_goods_url_error'));
            }
        } else {
            showDialog(core\language::get('store_sns_goods_url_error'));
        }
