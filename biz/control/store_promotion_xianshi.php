...
class store_promotion_xianshi extends BaseSellerControl
{
    ...
    public function __construct()
    {
        parent::__construct();
        //读取语言包
        core\language::read('member_layout,promotion_xianshi');
        //检查限时折扣是否开启
        if (intval(core\config::get('promotion_allow')) !== 1) {
            error(core\language::get('promotion_unavailable'), 'index.php?act=store');
        }
    }
    ...
    public function xianshi_listOp()
    {
		...
		core\tpl::output('isOwnShop', $isOwnShop);
        if (!$isOwnShop) {
            $current_xianshi_quota = $model_xianshi_quota->getXianshiQuotaCurrent(core\session::get('store_id'));
            core\tpl::output('current_xianshi_quota', $current_xianshi_quota);
        }
        $condition = array();
        ...
        core\tpl::showpage('store_promotion_xianshi.list');
    }
    /**
     * 添加限时折扣活动
     **/
    public function xianshi_addOp()
    {
        ...
        if(!$isOwnShop) {
            ...
            if (empty($current_xianshi_quota)) {
                error(core\language::get('xianshi_quota_current_error1'));
            }
            core\tpl::output('current_xianshi_quota', $current_xianshi_quota);
        }
        //输出导航
        self::profile_menu('xianshi_add');
        core\tpl::showpage('store_promotion_xianshi.add');
    }
    /**
     * 保存添加的限时折扣活动
     **/
    public function xianshi_saveOp()
    {
        ...
        if (empty($xianshi_name)) {
            showDialog(core\language::get('xianshi_name_error'));
        }
        ...
        if (!checkPlatformStore()) {
            ...
            if ($start_time < $quota_start_time) {
                showDialog(sprintf(core\language::get('xianshi_add_start_time_explain'), date('Y-m-d', $current_xianshi_quota['start_time'])));
            }
            if ($end_time > $quota_end_time) {
                showDialog(sprintf(core\language::get('xianshi_add_end_time_explain'), date('Y-m-d', $current_xianshi_quota['end_time'])));
            }
        }
        //生成活动
        $model_xianshi = model('p_xianshi');
        ...
        $param['quota_id'] = !empty($current_xianshi_quota['quota_id']) ? $current_xianshi_quota['quota_id'] : 0;
        ...
        if ($result) {
            ...
            showDialog(core\language::get('xianshi_add_success'), self::LINK_XIANSHI_MANAGE . $result, 'succ', '', 3);
        } else {
            showDialog(core\language::get('xianshi_add_fail'));
        }
    }
    /**
     * 编辑限时折扣活动
    ...
    public function xianshi_edit_saveOp()
    {
        ...
        $xianshi_info = $model_xianshi->getXianshiInfoByID($xianshi_id, core\session::get('store_id'));
        if (empty($xianshi_info) || !$xianshi_info['editable']) {
            error(lang('param_error'));
        }
        //验证输入
        ...
        if ($result && $result) {
            ...
            showDialog(core\language::get('nc_common_op_succ'), self::LINK_XIANSHI_LIST, 'succ', '', 3);
        } else {
    ...
    public function xianshi_delOp()
    {
        ...
        if ($result) {
            ...
            showDialog(lang('nc_common_op_succ'), urlBiz('store_promotion_xianshi', 'xianshi_list'), 'succ');
        } else {
    ...
    public function xianshi_manageOp()
    {
        ...
        core\tpl::output('xianshi_goods_list', $xianshi_goods_list);
        //输出导航
        self::profile_menu('xianshi_manage');
        core\tpl::showpage('store_promotion_xianshi.manage');
    }
    /**
     * 限时折扣套餐购买
     **/
    public function xianshi_quota_addOp()
    {
        ...
        core\tpl::showpage('store_promotion_xianshi_quota.add');
    }
    /**
     * 限时折扣套餐购买保存
     **/
    public function xianshi_quota_add_saveOp()
    {
        ...
        if ($xianshi_quota_quantity <= 0 || $xianshi_quota_quantity > 12) {
            showDialog(core\language::get('xianshi_quota_quantity_error'));
        }
        //获取当前价格
        $current_price = intval(core\config::get('promotion_xianshi_price'));
        //获取该用户已有套餐
        $model_xianshi_quota = model('p_xianshi_quota');
        $current_xianshi_quota = $model_xianshi_quota->getXianshiQuotaCurrent(core\session::get('store_id'));
        ...
        showDialog(core\language::get('xianshi_quota_add_success'), self::LINK_XIANSHI_LIST, 'succ');
    }
    /**
     * 选择活动商品
     **/
    public function goods_selectOp()
    {
        ...
        core\tpl::showpage('store_promotion_xianshi.goods', 'null_layout');
    }
    /**
     * 限时折扣商品添加
     **/
    ...
    public function xianshi_goods_price_editOp()
    {
        ...
        $data['result'] = true;
        $model_xianshi_goods = model('p_xianshi_goods');
        $xianshi_goods_info = $model_xianshi_goods->getXianshiGoodsInfoByID($xianshi_goods_id, core\session::get('store_id'));
        if (!$xianshi_goods_info) {
            $data['result'] = false;
            $data['message'] = lang('param_error');
            echo json_encode($data);
        ...
        if ($result) {
            ...
            // 添加对列修改商品促销价格
            lib\queue::push('updateGoodsPromotionPriceByGoodsId', $xianshi_goods_info['goods_id']);
            $this->recordSellerLog('限时折扣价格修改为：' . $xianshi_goods_info['xianshi_price'] . '，商品名称：' . $xianshi_goods_info['goods_name']);
        } else {
            $data['result'] = false;
            $data['message'] = lang('nc_common_op_succ');
        }
        echo json_encode($data);
        die;
    }
    ...
    public function xianshi_goods_deleteOp()
    {
        ...
        $xianshi_info = $model_xianshi->getXianshiInfoByID($xianshi_goods_info['xianshi_id'], core\session::get('store_id'));
        if (!$xianshi_info) {
            $data['result'] = false;
            $data['message'] = lang('param_error');
            echo json_encode($data);
    ...
    {
        $menu_array = array(1 => array('menu_key' => 'xianshi_list', 'menu_name' => core\language::get('promotion_active_list'), 'menu_url' => 'index.php?act=store_promotion_xianshi&op=xianshi_list'));
        switch ($menu_key) {
            case 'xianshi_add':
                $menu_array[] = array('menu_key' => 'xianshi_add', 'menu_name' => core\language::get('promotion_join_active'), 'menu_url' => 'index.php?act=store_promotion_xianshi&op=xianshi_add');
                break;
            case 'xianshi_edit':
                $menu_array[] = array('menu_key' => 'xianshi_edit', 'menu_name' => '编辑活动', 'menu_url' => 'javascript:;');
                break;
            case 'xianshi_quota_add':
                $menu_array[] = array('menu_key' => 'xianshi_quota_add', 'menu_name' => core\language::get('promotion_buy_product'), 'menu_url' => 'index.php?act=store_promotion_xianshi&op=xianshi_quota_add');
                break;
            case 'xianshi_manage':
                $menu_array[] = array('menu_key' => 'xianshi_manage', 'menu_name' => core\language::get('promotion_goods_manage'), 'menu_url' => 'index.php?act=store_promotion_xianshi&op=xianshi_manage&xianshi_id=' . $_GET['xianshi_id']);
                break;
        }
