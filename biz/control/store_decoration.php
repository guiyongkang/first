...
class store_decoration extends BaseSellerControl
{
    ...
    public function decoration_settingOp()
    {
        ...
        $store_decoration_info = $model_store_decoration->getStoreDecorationInfo(array('store_id' => core\session::get('store_id')));
        ...
        core\tpl::output('store_decoration_switch', $this->store_info['store_decoration_switch']);
        core\tpl::output('store_decoration_only', $this->store_info['store_decoration_only']);
        core\tpl::output('decoration_id', $decoration_id);
        $this->profile_menu('decoration_setting');
        core\tpl::showpage('store_decoration.setting');
    }
    /**
     * 店铺装修设置保存
     */
    public function decoration_setting_saveOp()
    {
        ...
        $result = $model_store->editStore($update, array('store_id' => core\session::get('store_id')));
        if ($result) {
    ...
    public function decoration_albumOp()
    {
        ...
        core\tpl::output('image_list', $image_list);
        core\tpl::output('show_page', $model_store_decoration_album->showpage());
        $this->profile_menu('decoration_album');
        core\tpl::showpage('store_decoration.album');
    }
    /**
     * 图片上传
     */
    public function decoration_album_uploadOp()
    {
        ...
        if ($result) {
            //装修相册计数加1
            model('store')->editStore(array('store_decoration_image_count' => array('exp', 'store_decoration_image_count+1')), array('store_id' => core\session::get('store_id')));
    ...
    public function decoration_editOp()
    {
        ...
        $model_store_decoration = model('store_decoration');
        $decoration_info = $model_store_decoration->getStoreDecorationInfoDetail($decoration_id, core\session::get('store_id'));
        if ($decoration_info) {
        ...
        //设定模板为完成宽度
        core\tpl::output('seller_layout_no_menu', true);
        core\tpl::showpage('store_decoration.edit');
    }
    /**
     * 输出装修设置
     */
    private function _output_decoration_info($decoration_info)
    {
        ...
        core\tpl::output('decoration_background_style', $decoration_background_style);
        core\tpl::output('decoration_nav', $decoration_info['decoration_nav']);
        core\tpl::output('decoration_banner', $decoration_info['decoration_banner']);
        core\tpl::output('decoration_setting', $decoration_info['decoration_setting']);
        core\tpl::output('block_list', $decoration_info['block_list']);
    }
    /**
     * 保存店铺装修背景设置
     */
    ...
    private function _validate_background_setting_input($decoration_id, $setting)
    {
        ...
        if (!empty($setting['background_image'])) {
            $setting['background_image_url'] = getStoreDecorationImageUrl($setting['background_image'], core\session::get('store_id'));
    ...
    public function decoration_banner_saveOp()
    {
        ...
        if ($result) {
            $data['message'] = '保存成功';
            $data['image_url'] = getStoreDecorationImageUrl($banner['image'], core\session::get('store_id'));
        } else {
            $data['error'] = '保存失败';
        }
        echo json_encode($data);
    ...
    public function decoration_block_addOp()
    {
        ...
        $condition = array();
        $condition['decoration_id'] = $decoration_id;
        $decoration_info = $model_store_decoration->getStoreDecorationInfo($condition, core\session::get('store_id'));
        if (!$decoration_info) {
            $data['error'] = L('param_error');
            echo json_encode($data);
        }
        ...
        if ($block_id) {
            ob_start();
            core\tpl::output('block', array('block_id' => $block_id));
            core\tpl::showpage('store_decoration_block', 'null_layout');
    ...
    {
        ob_start();
        core\tpl::output('block_content', $content);
        core\tpl::showpage('store_decoration_module.' . $module_type, 'null_layout');
        $temp = ob_get_contents();
        ob_end_clean();
        return $temp;
    }
    ...
    public function goods_searchOp()
    {
        ...
        core\tpl::showpage('store_decoration_module.goods', 'null_layout');
    }
    /**
     * 更新商品模块的商品价格
     */
        ...
        if (!empty($block_list_goods) && is_array($block_list_goods)) {
            foreach ($block_list_goods as $block) {
                ...
                foreach ($goods_array as $goods_key => $goods_value) {
                    ...
                    //抢购
                    if (core\config::get('groupbuy_allow')) {
                    ...
                    //限时折扣
                    if (core\config::get('promotion_allow') && empty($groupbuy_info)) {
    ...
    public function decoration_previewOp()
    {
        ...
        //店铺信息
        $model_store = model('store');
        $store_info = $model_store->getStoreOnlineInfoByID(core\session::get('store_id'));
        core\tpl::output('store_info', $store_info);
        core\tpl::output('store_theme', 'default');
        core\tpl::setLayout('store_layout');
        core\tpl::showpage('store_decoration.preview');
    }
    /**
     * 装修静态文件生成
     */
    public function decoration_buildOp()
    {
        ...
        $decoration_id = intval($_GET['decoration_id']);
        //更新商品数据
        $this->_update_module_goods_info($decoration_id, core\session::get('store_id'));
        $model_store_decoration = model('store_decoration');
        $decoration_info = $model_store_decoration->getStoreDecorationInfoDetail($decoration_id, core\session::get('store_id'));
        if ($decoration_info) {
            $this->_output_decoration_info($decoration_info);
        ...
        $file_name = md5(core\session::get('store_id'));
        ob_start();
        core\tpl::showpage('store_decoration.preview', 'null_layout');
    ...
    {
        $menu_array = array(1 => array('menu_key' => 'decoration_setting', 'menu_name' => '店铺装修', 'menu_url' => urlBiz('store_decoration', 'decoration_setting')), 2 => array('menu_key' => 'decoration_album', 'menu_name' => '装修图库', 'menu_url' => urlBiz('store_decoration', 'decoration_album')));
