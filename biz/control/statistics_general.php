...
class statistics_general extends BaseSellerControl
{
    ...
    public function generalOp()
    {
        ...
        //商品总数、收藏量
        $goods_list = $model->statByGoods(array('store_id' => core\session::get('store_id'), 'is_virtual' => 0), 'COUNT(*) as goodsnum, SUM(goods_collect) as gcollectnum');
        $statnew_arr['goodsnum'] = ($t = $goods_list[0]['goodsnum']) > 0 ? $t : 0;
        $statnew_arr['gcollectnum'] = ($t = $goods_list[0]['gcollectnum']) > 0 ? $t : 0;
        //店铺收藏量
        $store_list = $model->getoneByStore(array('store_id' => core\session::get('store_id')), 'store_collect');
        ...
        //计入统计的有效订单
        $where['order_add_time'] = array('between', array($stime, $etime));
        $where['store_id'] = array('neq', core\session::get('store_id'));
        if (!checkPlatformStore()) {
            //如果不是平台店铺，则查询店铺经营类目的同行数据
            //查询店铺经营类目
            $store_bindclass = model('store_bind_class')->getStoreBindClassList(array('store_id' => core\session::get('store_id')));
        ...
        core\tpl::output('goodstop30_arr', $goodstop30_arr);
        core\tpl::output('othergoodstop30_arr', $othergoodstop30_arr);
        core\tpl::output('stattoday_json', $stattoday_json);
        core\tpl::output('statnew_arr', $statnew_arr);
        core\tpl::output('stat_time', $stat_time);
    ...
    public function pricesettingOp()
    {
		...
		$condition = array('store_id' => core\session::get('store_id'));
		$flag = $model->getStoreExtendInfo($condition, 'pricerange');
        if (chksubmit()) {
            $update_array = array();
            if ($_POST['pricerange']) {
                ...
                $update_array['pricerange'] = serialize(null);
            }
			if($flag){
				$result = $model->where($condition)->update($update_array);
			}else{
				$update_array['store_id'] = core\session::get('store_id');
				$result = $model->insert($update_array);
			}
            if ($result) {
        ...
        $pricerange = $model->getfby_store_id(core\session::get('store_id'), 'pricerange');
        $pricerange = $pricerange ? unserialize($pricerange) : array();
        core\tpl::output('pricerange', $pricerange);
        self::profile_menu('setting', 'pricesetting');
        core\tpl::showpage('stat.pricesetting');
    }
    /**
     * 订单价格区间设置
     */
    public function orderprangeOp()
    {
        $model = model('store_extend');
		$condition = array('store_id' => core\session::get('store_id'));
		$flag = $model->getStoreExtendInfo($condition, 'orderpricerange');
        if (chksubmit()) {
            $update_array = array();
            if ($_POST['pricerange']) {
                foreach ((array) $_POST['pricerange'] as $k => $v) {
                ...
                $update_array['orderpricerange'] = serialize(null);
            }
			if($flag){
				$result = $model->where($condition)->update($update_array);
			}else{
        ...
        $pricerange = $model->getfby_store_id(core\session::get('store_id'), 'orderpricerange');
        $pricerange = $pricerange ? unserialize($pricerange) : array();
        core\tpl::output('pricerange', $pricerange);
        self::profile_menu('setting', 'orderprange');
        core\tpl::showpage('stat.orderpricerange');
    }
    /**
     * 用户中心右边，小导航
     *
    ...
    private function profile_menu($menu_type, $menu_key = '')
    {
        ...
        switch ($menu_type) {
            case 'setting':
                $menu_array = array(1 => array('menu_key' => 'pricesetting', 'menu_name' => '商品价格区间', 'menu_url' => 'index.php?act=statistics_general&op=pricesetting'), 2 => array('menu_key' => 'orderprange', 'menu_name' => '订单金额区间', 'menu_url' => 'index.php?act=statistics_general&op=orderprange'));
                break;
        }
        core\tpl::output('member_menu', $menu_array);
        core\tpl::output('menu_key', $menu_key);
