...
class store_order extends BaseSellerControl
{
    ...
    public function indexOp()
    {
        ...
        $order_list = $model_order->getOrderList($condition, 20, '*', 'order_id desc', 0, array('order_goods', 'order_common', 'member'));
		$totalnum = $model_order->getOrderCount($condition);
		pagecmd('setTotalNum', $totalnum);
        ...
        core\tpl::showpage('store_order.index');
    }
    /**
     * 卖家订单详情
     *
    ...
    public function show_orderOp()
    {
        ...
        core\tpl::showpage('store_order.show');
    }
    /**
     * 卖家订单状态操作
     *
    ...
    private function _order_cancel($order_info, $post)
    {
        ...
        if (!chksubmit()) {
            ...
            core\tpl::showpage('store_order.cancel', 'null_layout');
            exit;
        } else {
    ...
    private function _order_ship_price($order_info, $post)
    {
        ...
        if (!chksubmit()) {
            core\tpl::output('order_info', $order_info);
            core\tpl::output('order_id', $order_info['order_id']);
            core\tpl::showpage('store_order.edit_price', 'null_layout');
            exit;
        } else {
    ...
    private function _order_spay_price($order_info, $post)
    {
        ...
        if (!chksubmit()) {
            core\tpl::output('order_info', $order_info);
            core\tpl::output('order_id', $order_info['order_id']);
            core\tpl::showpage('store_order.edit_spay_price', 'null_layout');
            exit;
        } else {
    ...
    private function profile_menu($menu_type = '', $menu_key = '')
    {
        ...
        switch ($menu_type) {
            case 'list':
                $menu_array = array(array('menu_key' => 'store_order', 'menu_name' => core\language::get('nc_member_path_all_order'), 'menu_url' => 'index.php?act=store_order'), array('menu_key' => 'state_new', 'menu_name' => core\language::get('nc_member_path_wait_pay'), 'menu_url' => 'index.php?act=store_order&op=index&state_type=state_new'), array('menu_key' => 'state_pay', 'menu_name' => core\language::get('nc_member_path_wait_send'), 'menu_url' => 'index.php?act=store_order&op=index&state_type=state_pay'), array('menu_key' => 'state_send', 'menu_name' => core\language::get('nc_member_path_sent'), 'menu_url' => 'index.php?act=store_order&op=index&state_type=state_send'), array('menu_key' => 'state_success', 'menu_name' => core\language::get('nc_member_path_finished'), 'menu_url' => 'index.php?act=store_order&op=index&state_type=state_success'), array('menu_key' => 'state_cancel', 'menu_name' => core\language::get('nc_member_path_canceled'), 'menu_url' => 'index.php?act=store_order&op=index&state_type=state_cancel'));
                break;
        }
