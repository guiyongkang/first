...
class statistics_sale extends BaseSellerControl
{
    private $search_arr;
    //处理后的参数
    private $gc_arr;
    ...
    public function saleOp()
    {
        ...
        if ($this->search_arr['search_type'] == 'week') {
            ...
            foreach ((array) $orderlist as $k => $v) {
                if ($up_week == $v['weekval']) {
                    ...
                    $uplist_arr[$v['dayofweekval']]['val'] = isset($v['search_type']) ? intval($v['search_type']) : 0;
                }
                if ($curr_week == $v['weekval']) {
                    ...
                    $currlist_arr[$v['dayofweekval']]['val'] = isset($v['search_type']) ? intval($v['search_type']) : 0;
                }
            }
        ...
        if ($this->search_arr['search_type'] == 'month') {
            ...
            foreach ($orderlist as $k => $v) {
                ...
                if ($curr_month == $v['monthval']) {
                    ...
                    $currlist_arr[$v['dayval']]['val'] = isset($v['search_type']) ? intval($v['search_type']) : 0;
                }
            }
        ...
        core\tpl::output('stattype', isset($search_type) ? $search_type : '');
        //总数统计
        ...
        core\tpl::output('searchtime', implode('|', array($curr_stime, $etime)));
        core\tpl::output('show_page', $model->showpage(2));
        self::profile_menu('sale');
        core\tpl::showpage('stat.sale.index');
    }
    /**
     * 订单列表
     */
    public function salelistOp()
    {
        ...
        core\tpl::output('show_page', $model->showpage(2));
        core\tpl::output('actionurl', 'index.php?act=' . $this->search_arr['act'] . '&op=' . $this->search_arr['op']. '&order_type=' . $this->search_arr['order_type'] . '&t=' . $this->search_arr['t']);
    ...
    public function areaOp()
    {
        ...
        core\tpl::output('stat_json_map', $stat_json_map);
        core\tpl::output('stat_json_bar', $stat_json_bar);
        self::profile_menu('area');
        core\tpl::showpage('stat.sale.area');
    }
    /**
     * 购买分析
     */
    public function buyingOp()
    {
        ...
        $where['order_add_time'] = array('between', $searchtime_arr);
        $field = '1';
        $pricerange = model('store_extend')->getfby_store_id(core\session::get('store_id'), 'orderpricerange');
        ...
        core\tpl::showpage('stat.sale.buying');
    }
    /**
     * 用户中心右边，小导航
     *
    ...
    {
        $menu_array = array(1 => array('menu_key' => 'sale', 'menu_name' => '销售统计', 'menu_url' => 'index.php?act=statistics_sale&op=sale'), 2 => array('menu_key' => 'area', 'menu_name' => '区域分布', 'menu_url' => 'index.php?act=statistics_sale&op=area'), 3 => array('menu_key' => 'buying', 'menu_name' => '购买分析', 'menu_url' => 'index.php?act=statistics_sale&op=buying'));
