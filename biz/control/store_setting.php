...
class store_setting extends BaseSellerControl
{
    ...
    public function store_settingOp()
    {
         ...
         * 获取设置
         */
        $config_subdomain_edit = core\config::get('subdomain_edit');
        $config_subdomain_times = core\config::get('subdomain_times');
        $config_subdomain_length = core\config::get('subdomain_length');
        $config_subdomain_reserved = core\config::get('subdomain_reserved');
        $config_enabled_subdomain = core\config::get('enabled_subdomain');
        ...
        core\tpl::output('subdomain_length', $subdomain_length);
        /**
         * 保存店铺设置
         */
        if (chksubmit()) {
            $_POST['store_domain'] = isset($_POST['store_domain']) ? trim($_POST['store_domain']) : '';
            ...
            if (!empty($store_domain) && $store_domain != $store_info['store_domain']) {
                ...
                if ($store_domain_count < $subdomain_length[0] || $store_domain_count > $subdomain_length[1]) {
                    showDialog(core\language::get('store_setting_wrong_uri') . ': ' . $config_subdomain_length, 'reload', 'error');
                }
                if (!preg_match('/^[\\w-]+$/i', $store_domain)) {
                    //判断域名是否正确
                    showDialog(core\language::get('store_setting_lack_uri'));
                }
                $store = $model_class->getStoreInfo(array('store_domain' => $store_domain));
                //二级域名存在,则提示错误
                if (!empty($store) && $store_id != $store['store_id']) {
                    showDialog(core\language::get('store_setting_exists_uri'), 'reload', 'error');
                }
                //判断二级域名是否为系统禁止
                ...
                if (!empty($subdomain_reserved) && is_array($subdomain_reserved)) {
                    if (in_array($store_domain, $subdomain_reserved)) {
                        showDialog(core\language::get('store_setting_invalid_uri'));
                    }
                }
            ...
            if (!empty($_POST['store_banner']) && !empty($store_info['store_banner'])) {
				if(file_exists(BASE_UPLOAD_PATH . DS . ATTACH_STORE . DS . $store_info['store_banner'])){
             ...
             * 更新入库
             */
            $param = array('store_label' => empty($_POST['store_label']) ? $store_info['store_label'] : $_POST['store_label'], 'store_banner' => empty($_POST['store_banner']) ? $store_info['store_banner'] : $_POST['store_banner'], 'store_avatar' => empty($_POST['store_avatar']) ? $store_info['store_avatar'] : $_POST['store_avatar'], 'store_qq' => $_POST['store_qq'], 'store_ww' => $_POST['store_ww'], 'store_phone' => $_POST['store_phone'], 'store_zy' => $_POST['store_zy'], 'store_keywords' => isset($_POST['seo_keywords']) ? $_POST['seo_keywords'] : '', 'store_description' => isset($_POST['seo_description']) ? $_POST['seo_description'] : '');
			...
			$param['area_info'] = isset($_POST['company_address']) ? $_POST['company_address'] : '';
            //店铺名称修改处理
            ...
            $PhpQRCode->init();
            $model_class->editStore($param, array('store_id' => $store_id));
            showDialog(core\language::get('nc_common_save_succ'), 'index.php?act=store_setting&op=store_setting', 'succ');
        }
        /**
         * 实例化店铺等级模型
         */
        ...
        if ($subdomain_edit == 1 && $subdomain_times > $store_domain_times) {
            //可继续修改二级域名
            core\tpl::output('subdomain_edit', $subdomain_edit);
        }
        /**
         * 输出店铺信息
         */
        ...
        core\tpl::output('store_grade', $store_grade);
        core\tpl::output('subdomain', $config_enabled_subdomain);
        core\tpl::output('subdomain_times', $config_subdomain_times);
        /**
         * 页面输出
         */
        core\tpl::showpage('store_setting_form');
    }
    /**
     * 店铺幻灯片
     */
    public function store_slideOp()
    {
        ...
        if (chksubmit()) {
            ...
            $model_store->editStore($update, array('store_id' => core\session::get('store_id')));
            // 删除upload表中数据
            $model_upload->delByWhere(array('upload_type' => 3, 'item_id' => core\session::get('store_id')));
            showDialog(core\language::get('nc_common_save_succ'), 'index.php?act=store_setting&op=store_slide', 'succ');
        }
        // 删除upload中的无用数据
        $upload_info = $model_upload->getUploadList(array('upload_type' => 3, 'item_id' => core\session::get('store_id')), 'file_name');
        if (is_array($upload_info) && !empty($upload_info)) {
            foreach ($upload_info as $val) {
				if(file_exists(BASE_UPLOAD_PATH . DS . ATTACH_SLIDE . DS . $val['file_name'])){
					unlink(BASE_UPLOAD_PATH . DS . ATTACH_SLIDE . DS . $val['file_name']);
				}
            }
        }
        $model_upload->delByWhere(array('upload_type' => 3, 'item_id' => core\session::get('store_id')));
        $store_info = $model_store->getStoreInfoByID(core\session::get('store_id'));
        if ($store_info['store_slide'] != '' && $store_info['store_slide'] != ',,,,') {
            core\tpl::output('store_slide', explode(',', $store_info['store_slide']));
            core\tpl::output('store_slide_url', explode(',', $store_info['store_slide_url']));
        }
        self::profile_menu('store_slide');
        /**
         * 页面输出
         */
        core\tpl::showpage('store_slide_form');
    }
    /**
     * 店铺幻灯片ajax上传
     */
    public function silde_image_uploadOp()
    {
        ...
        if (intval($_POST['file_id']) > 0) {
			...
			if(file_exists(BASE_UPLOAD_PATH . DS . ATTACH_SLIDE . DS . $file_info['file_name'])){
            ...
            $insert_array['item_id'] = core\session::get('store_id');
            $insert_array['upload_time'] = time();
            $result = $model_upload->add($insert_array);
            if (!$result) {
				if(file_exists(BASE_UPLOAD_PATH . DS . ATTACH_SLIDE . DS . $img_path)){
					unlink(BASE_UPLOAD_PATH . DS . ATTACH_SLIDE . DS . $img_path);
				}
                $output['error'] = core\language::get('store_slide_upload_fail', 'UTF-8');
                echo json_encode($output);
                die;
            }
    ...
    public function dorp_imgOp()
    {
        ...
        echo json_encode(array('succeed' => core\language::get('nc_common_save_succ', 'UTF-8')));
        die;
    }
    /**
     * 卖家店铺主题设置
    ...
    public function themeOp()
    {
        ...
        $store_info = $store_class->getStoreInfoByID(core\session::get('store_id'));
        /**
         * 主题配置信息
         */
        $style_data = array();
        $style_configurl = BASE_ROOT_PATH . DS . DIR_PC . '/templates/' . TPL_SHOP_NAME . DS . 'store' . DS . 'style' . DS . "styleconfig.php";
         ...
         */
        if (strtoupper(CHARSET) == 'GBK') {
            $style_data = core\language::getGBK($style_data);
        }
        /**
         * 当前店铺主题
         */
        ...
        core\tpl::output('store_info', $store_info);
        core\tpl::output('curr_theme', $curr_theme);
        core\tpl::output('theme_list', $theme_list);
        core\tpl::showpage('store_theme');
    }
    /**
     * 卖家店铺主题设置
     *
    ...
    public function set_themeOp()
    {
        ...
        if (!empty($style) && file_exists(BASE_TPL_PATH . DS . '/store/style/' . $style . '/images/preview.jpg')) {
            $store_class = model('store');
            $rs = $store_class->editStore(array('store_theme' => $style), array('store_id' => core\session::get('store_id')));
    ...
    protected function getStoreMbSliders()
    {
        ...
        if (!$mbSliders) {
            $mbSliders = array_fill(1, self::MAX_MB_SLIDERS, array('img' => '', 'type' => 1, 'link' => ''));
        }
        return $mbSliders;
    }
    protected function setStoreMbSliders(array $mbSliders)
    {
        return model('store')->editStore(array('mb_sliders' => serialize($mbSliders)), array('store_id' => core\session::get('store_id')));
    }
    public function store_mb_slidersOp()
    {
        try {
            ...
            echo json_encode(array('uploadedUrl' => UPLOAD_SITE_URL . DS . ATTACH_STORE . DS . $newImg));
        } catch (\Exception $ex) {
    ...
    {
        core\tpl::output('max_mb_sliders', self::MAX_MB_SLIDERS);
        $store_info = model('store')->getStoreInfoByID(core\session::get('store_id'));
        // 页头背景图
        ...
        if (chksubmit()) {
            ...
            if ($skuToValid) {
                $validSkus = (array) model()->table('goods')->field('goods_id')->where(array('goods_id' => array('in', $skuToValid), 'store_id' => core\session::get('store_id')))->key('goods_id')->select();
            ...
            model('store')->editStore($update_array, array('store_id' => core\session::get('store_id')));
        ...
        core\tpl::output('mb_title_img', $mb_title_img);
        core\tpl::output('mbSliders', $mbSliders);
        core\tpl::output('mbSliderUrls', $mbSliderUrls);
        $this->profile_menu('store_mobile');
        core\tpl::showPage('store_setting.store_mobile');
    }
    /**
     * 用户中心右边，小导航
     *
    ...
    private function profile_menu($menu_key = '')
    {
        ...
        $menu_array = array(1 => array('menu_key' => 'store_setting', 'menu_name' => core\language::get('nc_member_path_store_config'), 'menu_url' => 'index.php?act=store_setting&op=store_setting'), /*4 => array('menu_key' => 'store_slide', 'menu_name' => core\language::get('nc_member_path_store_slide'), 'menu_url' => 'index.php?act=store_setting&op=store_slide'),*/ /*5 => array('menu_key' => 'store_theme', 'menu_name' => '店铺主题', 'menu_url' => 'index.php?act=store_setting&op=theme'),*/ 7 => array('menu_key' => 'store_mobile', 'menu_name' => '其他设置', 'menu_url' => 'index.php?act=store_setting&op=store_mobile'));
