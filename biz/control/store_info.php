...
class store_info extends BaseSellerControl
{
    ...
    public function indexOp()
    {
		...
		if($store_class_info){
			core\tpl::output('store_class_name', $store_class_info['sc_name']);
		}else{
			core\tpl::output('store_class_name', '');
		}
        // 店铺等级信息
        $store_grade_info = $model_store_grade->getOneGrade($store_info['grade_id']);
        core\tpl::output('store_grade_name', $store_grade_info['sg_name']);
        ...
        $store_bind_class_list = $model_store_bind_class->getStoreBindClassList(array('store_id' => core\session::get('store_id'), 'state' => array('in', array(1, 2))), null);
    ...
    {
        $model_store_bind_class = model('store_bind_class');
        $store_bind_class_list = $model_store_bind_class->getStoreBindClassList(array('store_id' => core\session::get('store_id')), null);
        $goods_class = model('goods_class')->getGoodsClassIndexedListAll();
        for ($i = 0, $j = count($store_bind_class_list); $i < $j; $i++) {
            $store_bind_class_list[$i]['class_1_name'] = $goods_class[$store_bind_class_list[$i]['class_1']]['gc_name'];
            $store_bind_class_list[$i]['class_2_name'] = isset($goods_class[$store_bind_class_list[$i]['class_2']]['gc_name']) ? $goods_class[$store_bind_class_list[$i]['class_2']]['gc_name'] : '';
        ...
        core\tpl::showpage('store_bind_class.index');
    }
    /**
     * 申请新的经营类目
     */
    public function bind_class_addOp()
    {
        ...
        self::profile_menu('index', 'bind_class');
        core\tpl::showpage('store_bind_class.add', 'null_layout');
    }
    /**
     * 申请新经营类目保存
     */
    ...
    public function reopenOp()
    {
        ...
        $reopen_list = $model_store_reopen->getStoreReopenList(array('re_store_id' => core\session::get('store_id')));
        ...
        //默认选中当前级别
        core\tpl::output('current_grade_id', core\session::get('grade_id'));
        //如果存在有未上传凭证或审核中的信息，则不能再申请续签
        $condition = array();
        $condition['re_state'] = array('in', array(0, 1));
        $condition['re_store_id'] = core\session::get('store_id');
        $reopen_info = $model_store_reopen->getStoreReopenInfo($condition);
        if ($reopen_info) {
            if ($reopen_info['re_state'] == '0') {
                core\tpl::output('upload_cert', true);
                core\tpl::output('reopen_info', $reopen_info);
            }
        } else {
            core\tpl::output('applay_reopen', (isset($store_info['reopen']) && $store_info['reopen']) ? true : false);
        }
        self::profile_menu('index', 'reopen');
        core\tpl::showpage('store_reopen.index');
    }
    /**
     * 申请续签
     */
    public function reopen_addOp()
    {
        ...
        $data['re_store_id'] = core\session::get('store_id');
        $data['re_store_name'] = core\session::get('store_name');
        ...
        if ($insert) {
            if ($data['re_pay_amount'] == 0) {
                // 	            $model_store->editStore(array('store_end_time'=>$data['re_end_time']),array('store_id'=>core\session::get('store_id')));
                showDialog('您的申请已经提交，请等待管理员审核', 'reload', 'succ', '', 5);
            } else {
                showDialog(core\language::get('nc_common_save_succ') . '，需付款金额' . ncPriceFormat($data['re_pay_amount']) . '元，请尽快完成付款，付款完成后请上传付款凭证', 'reload', 'succ', '', 5);
            }
        } else {
            showDialog(core\language::get('nc_common_del_fail'));
        }
