...
class store_promotion_bundling extends BaseSellerControl
{
    public function __construct()
    {
         ...
         * 读取语言包
         */
        core\language::read('member_layout,member_store_promotion_bundling');
        //检查是否开启
        if (intval(core\config::get('promotion_allow')) !== 1) {
            error(core\language::get('promotion_unavailable'), urlBiz('index', 'index'));
        }
    }
    ...
    public function bundling_listOp()
    {
        ...
        if (checkPlatformStore()) {
            ...
            core\tpl::output('bundling_quota', $bundling_quota);
            if (!empty($bundling_quota)) {
                // 计算已经发布活动、剩余活动数量
                $bundling_published = $model_bundling->getBundlingCount(array('store_id' => core\session::get('store_id')));
                $bundling_surplus = intval(core\config::get('promotion_bundling_sum')) - intval($bundling_published);
                core\tpl::output('bundling_published', $bundling_published);
                core\tpl::output('bundling_surplus', $bundling_surplus);
                $hasList = true;
            }
        }
        if ($hasList) {
            ...
            core\tpl::output('show_page', $model_bundling->showpage(2));
            if (!empty($bundling_list)) {
                ...
                foreach ($bundling_list as $key => $val) {
                    $bundling_list[$key]['goods_id'] = isset($bgoods_array[$val['bl_id']]['goods_id']) ? $bgoods_array[$val['bl_id']]['goods_id'] : 0;
                    $bundling_list[$key]['count'] = isset($bgoods_array[$val['bl_id']]['count']) ? $bgoods_array[$val['bl_id']]['count'] : 0;
                    $bundling_list[$key]['img'] = thumb((isset($bgoods_array[$val['bl_id']]['goods_id']) && isset($goods_array[$bgoods_array[$val['bl_id']]['goods_id']]) ? $goods_array[$bgoods_array[$val['bl_id']]['goods_id']] : 0), 60);
                }
            }
            core\tpl::output('list', $bundling_list);
            // 状态数组
            $state_array = array(0 => core\language::get('bundling_status_0'), 1 => core\language::get('bundling_status_1'));
            core\tpl::output('state_array', $state_array);
        }
        $this->profile_menu('bundling_list', 'bundling_list');
        core\tpl::showpage('store_promotion_bundling.list');
    }
    /**
     * 套餐购买
     */
    public function bundling_quota_addOp()
    {
        if (chksubmit()) {
            ...
            // 购买数量（月）
            $price_quantity = $quantity * intval(core\config::get('promotion_bundling_price'));
            // 扣款数
            if ($quantity <= 0 || $quantity > 12) {
                showDialog(core\language::get('bundling_quota_price_fail'), urlBiz('store_promotion_bundling', 'bundling_quota_add'), '', 'error');
            }
            // 实例化模型
            $model_bundling = model('p_bundling');
            $data = array();
            $data['store_id'] = core\session::get('store_id');
            $data['store_name'] = core\session::get('store_name');
            $data['member_id'] = core\session::get('member_id');
            $data['member_name'] = core\session::get('member_name');
            ...
            if ($return) {
                ...
                $this->addcron(array('exetime' => $end_time, 'exeid' => core\session::get('store_id'), 'type' => 3), true);
                $this->recordSellerLog('购买' . $quantity . '套优惠套装，单位元');
                showDialog(lang('bundling_quota_price_succ'), urlBiz('store_promotion_bundling', 'bundling_list'), 'succ');
            } else {
                showDialog(lang('bundling_quota_price_fail'), urlBiz('store_promotion_bundling', 'bundling_quota_add'));
            }
        }
        // 输出导航
        self::profile_menu('bundling_quota_add', 'bundling_quota_add');
        core\tpl::showpage('store_promotion_bundling.quota_add');
    }
    /**
     * 套餐续费
     */
    public function bundling_renewOp()
    {
        if (chksubmit()) {
            ...
            if ($return) {
                ...
                $this->recordStoreCost($price_quantity, '购买优惠套装');
                // 添加任务队列
                $this->addcron(array('exetime' => $update['bl_quota_endtime'], 'exeid' => core\session::get('store_id'), 'type' => 3), true);
    ...
    public function bundling_addOp()
    {
        ...
        $model_bundling = model('p_bundling');
        // 验证套餐数量
        if (intval(core\config::get('promotion_bundling_sum')) != 0 && !isset($_REQUEST['bundling_id'])) {
            $count = $model_bundling->getBundlingCount(array('store_id' => core\session::get('store_id')));
            if (intval(core\config::get('promotion_bundling_sum')) <= intval($count)) {
                error(lang('bundling_add_fail_quantity_beyond'));
            }
        }
        if (chksubmit()) {
            // 插入套餐
            ...
            if (!$return) {
                showDialog(lang('nc_common_op_fail'), '', '', 'error');
            }
            // 插入套餐商品
            $model_goods = model('goods');
            $data_goods = array();
            $appoint_goodsid = false;
            $model_bundling->delBundlingGoods(array('bl_id' => isset($_POST['bundling_id']) ? intval($_POST['bundling_id']) : 0));
            if (!empty($_POST['goods']) && is_array($_POST['goods'])) {
                foreach ($_POST['goods'] as $key => $val) {
                    ...
                    if (empty($goods_info) || $goods_info['store_id'] != core\session::get('store_id')) {
                        continue;
                    }
                    $data = array();
            ...
            $this->recordSellerLog('添加优惠套装，名称：' . (isset($data['bl_name']) ? $data['bl_name'] : '') . ' id：' . $return);
            showDialog(lang('nc_common_op_succ'), urlBiz('store_promotion_bundling', 'bundling_list'), 'succ');
        }
        // 是否能使用编辑器
        if (checkPlatformStore()) {
            // 平台店铺可以使用编辑器
        ...
        if (isset($_GET['bundling_id']) && intval($_GET['bundling_id']) > 0) {
            $bundling_info = $model_bundling->getBundlingInfo(array('bl_id' => intval($_GET['bundling_id']), 'store_id' => core\session::get('store_id')));
            core\tpl::output('bundling_info', $bundling_info);
            // 验证是否属于自己的组合套餐
            if (empty($bundling_info['store_id'])) {
                error(lang('wrong_argument'), urlBiz('store_promotion_bundling', 'bundling_list'));
            }
            ...
            if (!empty($b_goods_list)) {
                ...
                core\tpl::output('goods_list', array_under_reset($goods_list, 'goods_id'));
            }
            core\tpl::output('b_goods_list', $b_goods_list);
            // 输出导航
        ...
        core\tpl::showpage('store_promotion_bundling.add');
    }
    /**
     * 套餐活动添加商品
     */
    public function bundling_add_goodsOp()
    {
        ...
        core\tpl::showpage('store_promotion_bundling.add_goods', 'null_layout');
    }
    /**
     * 删除优惠套装活动
     */
    public function drop_bundlingOp()
    {
        ...
        $return = model('p_bundling')->delBundling($blids, core\session::get('store_id'));
        if ($return) {
    ...
    private function profile_menu($menu_type, $menu_key = '')
    {
        ...
        switch ($menu_type) {
            ...
            case 'bundling_quota_list':
                $menu_array = array(1 => array('menu_key' => 'bundling_list', 'menu_name' => core\language::get('bundling_list'), 'menu_url' => urlBiz('store_promotion_bundling', 'bundling_list')));
                break;
            case 'bundling_quota_add':
                $menu_array = array(1 => array('menu_key' => 'bundling_list', 'menu_name' => core\language::get('bundling_list'), 'menu_url' => urlBiz('store_promotion_bundling', 'bundling_list')), 2 => array('menu_key' => 'bundling_quota_add', 'menu_name' => core\language::get('bundling_quota_add'), 'menu_url' => urlBiz('store_promotion_bundling', 'bundling_quota_add')));
                break;
            case 'bundling_renew':
                $menu_array = array(1 => array('menu_key' => 'bundling_list', 'menu_name' => core\language::get('bundling_list'), 'menu_url' => urlBiz('store_promotion_bundling', 'bundling_list')), 2 => array('menu_key' => 'bundling_renew', 'menu_name' => '套餐续费', 'menu_url' => urlBiz('store_promotion_bundling', 'bundling_renew')));
                break;
            case 'bundling_add':
                $menu_array = array(1 => array('menu_key' => 'bundling_list', 'menu_name' => core\language::get('bundling_list'), 'menu_url' => urlBiz('store_promotion_bundling', 'bundling_list')), 2 => array('menu_key' => 'bundling_add', 'menu_name' => core\language::get('bundling_add'), 'menu_url' => urlBiz('store_promotion_bundling', 'bundling_add')));
                break;
            case 'bundling_edit':
                $menu_array = array(1 => array('menu_key' => 'bundling_list', 'menu_name' => core\language::get('bundling_list'), 'menu_url' => urlBiz('store_promotion_bundling', 'bundling_list')), 2 => array('menu_key' => 'bundling_edit', 'menu_name' => core\language::get('bundling_edit'), 'menu_url' => urlBiz('store_promotion_bundling', 'bundling_edit')));
                break;
        }
