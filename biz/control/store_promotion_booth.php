...
class store_promotion_booth extends BaseSellerControl
{
    ...
    public function booth_goods_listOp()
    {
        ...
        if (checkPlatformStore()) {
            ...
            core\tpl::output('booth_quota', $booth_quota);
        ...
        if ($hasList) {
            // 查询已选择商品
            $boothgoods_list = $model_booth->getBoothGoodsList(array('store_id' => core\session::get('store_id')), 'goods_id');
            if (!empty($boothgoods_list)) {
                ...
                if (!empty($goods_list)) {
                    ...
                    core\tpl::output('goodsclass_list', $goodsclass_list);
                }
            }
        }
        $this->profile_menu('booth_goods_list', 'booth_goods_list');
        core\tpl::showpage('store_promotion_booth.goods_list');
    }
    /**
     * 选择商品
     */
    public function booth_select_goodsOp()
    {
        ...
        core\tpl::showpage('store_promotion_booth.select_goods', 'null_layout');
    }
    /**
     * 购买套餐
     */
    ...
    {
        if (chksubmit()) {
            $quantity = intval($_POST['booth_quota_quantity']);
            // 购买数量（月）
            $price_quantity = $quantity * intval(core\config::get('promotion_booth_price'));
            // 扣款数
            if ($quantity <= 0 || $quantity > 12) {
                showDialog('参数错误，购买失败。', urlBiz('store_promotion_booth', 'booth_quota_add'), '', 'error');
            }
            // 实例化模型
            $model_booth = model('p_booth');
            $data = array();
            ...
            if ($return) {
                ...
                $this->addcron(array('exetime' => $end_time, 'exeid' => core\session::get('store_id'), 'type' => 4), true);
                $this->recordSellerLog('购买' . $quantity . '套推荐展位，单位元');
                showDialog('购买成功', urlBiz('store_promotion_booth', 'booth_goods_list'), 'succ');
            } else {
                showDialog('购买失败', urlBiz('store_promotion_booth', 'booth_quota_add'));
            }
        }
        // 输出导航
        self::profile_menu('booth_quota_add', 'booth_quota_add');
        core\tpl::showpage('store_promotion_booth.quota_add');
    }
    /**
     * 套餐续费
     */
    public function booth_renewOp()
    {
        if (chksubmit()) {
            ...
            if ($return) {
                ...
                showDialog('购买成功', urlBiz('store_promotion_booth', 'index'), 'succ');
            } else {
                showDialog('购买失败', urlBiz('store_promotion_booth', 'booth_quota_add'));
            }
        }
    ...
    public function choosed_goodsOp()
    {
        ...
        if (!checkPlatformStore()) {
            // 验证套餐时候过期
            $booth_info = $model_booth->getBoothQuotaInfo(array('store_id' => core\session::get('store_id'), 'booth_quota_endtime' => array('gt', TIMESTAMP)), 'booth_quota_id');
            if (empty($booth_info)) {
        ...
        $bootgoods_info = $model_booth->getBoothGoodsList(array('store_id' => core\session::get('store_id')), 'goods_id');
        // 已添加商品总数
        if (count($bootgoods_info) >= core\config::get('promotion_booth_goods_sum')) {
            $data = array('result' => 'false', 'msg' => '只能添加' . core\config::get('promotion_booth_goods_sum') . '个商品');
            $this->_echoJson($data);
        }
        // 商品是否已经被添加
        ...
        $goods_info['url'] = urlBiz('goods', 'index', array('goods_id' => $goods_info['goods_id']));
    ...
    public function del_choosed_goodsOp()
    {
        ...
        $result = model('p_booth')->delBoothGoods(array('goods_id' => $gid, 'store_id' => core\session::get('store_id')));
        if ($result) {
    ...
    private function _echoJson($data)
    {
        if (strtoupper(CHARSET) == 'GBK') {
            $data = core\language::getUTF8($data);
    ...
    private function profile_menu($menu_type, $menu_key = '')
    {
        ...
        switch ($menu_type) {
            case 'booth_goods_list':
                $menu_array = array(1 => array('menu_key' => 'booth_goods_list', 'menu_name' => '商品列表', 'menu_url' => urlBiz('store_promotion_booth', 'booth_goods_list')));
                break;
            case 'booth_quota_add':
                $menu_array = array(1 => array('menu_key' => 'booth_goods_list', 'menu_name' => '商品列表', 'menu_url' => urlBiz('store_promotion_booth', 'booth_goods_list')), 2 => array('menu_key' => 'booth_quota_add', 'menu_name' => '购买套餐', 'menu_url' => urlBiz('store_promotion_booth', 'booth_quota_add')));
                break;
            case 'booth_renew':
                $menu_array = array(1 => array('menu_key' => 'booth_goods_list', 'menu_name' => '商品列表', 'menu_url' => urlBiz('store_promotion_booth', 'booth_goods_list')), 2 => array('menu_key' => 'booth_renew', 'menu_name' => '套餐续费', 'menu_url' => urlBiz('store_promotion_booth', 'booth_renew')));
                break;
        }
