...
<div class="tabmenu">
	<ul id="listpj" class="tab">
		<li class="active"><a href="<?php echo urlBiz('store_evaluate', 'list');?>"><?php echo $lang['member_evaluation_frombuyer'];?></a></li>
	</ul>
</div>
<form method="get">
	<table class="search-form">
		...
		<tr>
			...
			<td class="w160"><input type="text" class="text w150" name="goods_name" value="<?php echo isset($_GET['goods_name']) ? $_GET['goods_name'] : '';?>"/></td>
			<th class="w110">评价人</th>
			<td class="w160"><input type="text" class="text w150" name="member_name" value="<?php echo isset($_GET['member_name']) ? $_GET['member_name'] : '';?>"/></td>
...
<table class="ncsc-default-table">
	...
	<tbody>
		...
		<tr>
			<th></th>
			<th> <span class="goods-name"><?php echo $v['geval_goodsname']?></span> <span>商品评价：<em class="raty" data-score="<?php echo $v['geval_scores'];?>"></em></span> <span>评价人：<?php echo $v['geval_frommembername'];?>
		...
		<tr>
			<td rowspan="2"></td>
			<td class="tl"><strong>评价内容：</strong> <span><?php echo $v['geval_content'];?></span></td>
			<td rowspan="2" class="nscs-table-handle vt"><span><a nctype="btn_show_explain_dialog" data-geval-id="<?php echo $v['geval_id'];?>" data-geval-content="<?php echo $v['geval_content'];?>" href="javascript:;" class="btn-acidblue"> <i class="icon-comments-alt "></i>
...
<div id="dialog_explain" style="display:none;">
	<div class="eject_con">
		...
		<form id="explain_form">
			...
			<div class="bottom"> <a href="javascript:void(0);" id="btn_explain_submit" class="submit">确定</a> </div>
		</form>
	</div>
</div>
...
$(document).ready(function(){
    ...
    $('#btn_explain_submit').on('click', function() {
        if($('#explain_form').valid()){
            ...
            $.post("<?php echo urlBiz('store_evaluate', 'explain_save');?>",{
                geval_id: geval_id,
