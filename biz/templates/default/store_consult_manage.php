	...
	<table class="search-form">
		<tr>
			...
			<td class="w160"><select name="ctid" class="w150">
					...
					<option <?php if (isset($_GET['ctid']) && intval($_GET['ctid']) == $val['ct_id']) {?>selected="selected"<?php }?> value="<?php echo $val['ct_id']?>"><?php echo $val['ct_name'];?></option>
					<?php }?>
					<?php }?>
				</select></td>
	...
	<tbody>
		...
		<tr>
			...
			<td rowspan="2" class="nscs-table-handle vt"><?php if($consult['consult_reply'] == ''){?>
				<span><a href="javascript:void(0);" class="btn-acidblue" nc_type="dialog" dialog_id="my_qa_reply" dialog_title="<?php echo $lang['store_consult_list_reply_consult'];?>" dialog_width="460" uri="index.php?act=store_consult&op=reply_consult&id=<?php echo $consult['consult_id'];?>"><i class="icon-comments-alt "></i>
				<p><?php echo $lang['store_consult_list_reply'];?></p>
				</a></span>
				<?php }else{?>
				<span><a href="javascript:void(0);" nc_type="dialog" dialog_id="my_qa_edit_reply" dialog_title="<?php echo $lang['store_consult_list_reply_consult'];?>" dialog_width="480" uri="index.php?act=store_consult&op=reply_consult&id=<?php echo $consult['consult_id'];?>" class="btn-blue"><i class="icon-edit"></i>
				<p><?php echo $lang['nc_edit'];?></p>
				</a></span>
				<?php }?>
				<span><a href="javascript:void(0)" onclick="ajax_get_confirm('<?php echo $lang['nc_ensure_del'];?>', 'index.php?act=store_consult&op=drop_consult&id=<?php echo $consult['consult_id'];?>');" class="btn-red"><i class="icon-trash"></i>
