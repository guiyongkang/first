...
<body>
<div class="waybill_back"> <img src="<?php echo $output['waybill_info']['waybill_image_url'];?>"
			alt=""> </div>
<div class="waybill_design">
	...
	<?php if(!empty($value['check']) && !empty($value['show'])) {?>
...
<div class="control"> <a id="btn_print" href="javascript:;">打印</a> 其它模板：
	<select
			id="waybill_list">
		<?php if(!empty($output['store_waybill_list']) && is_array($output['store_waybill_list'])) {?>
		<?php foreach($output['store_waybill_list'] as $value) {?>
		<option value="<?php echo $value['store_waybill_id'];?>"
				<?php if(!empty($_GET['store_waybill_id']) && $value['store_waybill_id'] == $_GET['store_waybill_id']) { echo 'selected'; } ?>><?php echo $value['waybill_name'];?></option>
		<?php } ?>
		<?php } ?>
	</select>
	<a id="btn_change" href="javascript:;">更换模板</a> </div>
<script type="text/javascript"
		src="<?php echo RESOURCE_SITE_URL;?>/js/jquery.js"></script> 
<script>
