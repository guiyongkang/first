	...
	<table class="search-form">
		...
		<tr>
			...
			<td class="w80"><select name="state">
					...
					<option value="1" <?php if(isset($_GET['state']) && $_GET['state'] == '1'){?>selected<?php }?>><?php echo $lang['complain_state_inprogress']; ?></option>
					<option value="2" <?php if(isset($_GET['state']) && $_GET['state'] == '2'){?>selected<?php }?>><?php echo $lang['complain_state_finish']; ?></option>
				</select></td>
			<th class="w120"><select name="type">
					<option value="accuser_name" <?php if(isset($_GET['type']) && $_GET['type'] == 'accuser_name'){?>selected<?php }?>><?php echo $lang['complain_accuser']; ?></option>
					<option value="complain_subject" <?php if(isset($_GET['type']) && $_GET['type'] == 'complain_subject'){?>selected<?php }?>><?php echo $lang['complain_subject_content']; ?></option>
					<option value="complain_id" <?php if(isset($_GET['type']) && $_GET['type'] == 'complain_id'){?>selected<?php }?>>投诉号</option>
				</select></th>
...
<table class="ncsc-default-table">
	...
	<tbody>
		...
		<tr class="bd-line">
			...
			<td class="w50"><div class="pic-thumb"><a target="_blank" href="<?php echo urlBiz('goods','index',array('goods_id'=> $goods['goods_id'])); ?>"> <img src="<?php echo thumb($goods,60); ?>" onMouseOver="toolTip('<img src=<?php echo thumb($goods,240); ?>>')" onMouseOut="toolTip()" /></a></div></td>
			<td class="tl"><dl class="goods-name">
					<dt><a target="_blank" href="<?php echo urlBiz('goods','index',array('goods_id'=> $goods['goods_id'])); ?>"><?php echo $goods['goods_name']; ?></a></dt>
