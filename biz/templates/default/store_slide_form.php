...
<div class="ncsc-form-default"> <div class="alert">
      <ul>
        ...
        <li>2. <?php printf($lang['store_slide_description_two'],intval($output['setting_config']['image_max_filesize'])/1024);?></li>
    ...
    <ul class="ncsc-store-slider" id="goods_images">
      ...
      <li nc_type="handle_pic" id="thumbnail_<?php echo $i;?>">
        <div class="picture" nctype="file_<?php echo $i;?>">
          ...
          <?php }?>
          <input type="hidden" name="image_path[]" nctype="file_<?php echo $i;?>" value="<?php echo isset($output['store_slide'][$i]) ? $output['store_slide'][$i] : '';?>" /><a href="javascript:void(0)" nctype="del" class="del" title="移除">X</a></div>
        
        <div class="url">
          <label><?php echo $lang['store_slide_image_url'];?></label>
          <input type="text" class="text w150" name="image_url[]" value="<?php if(empty($output['store_slide_url'][$i])){  echo 'http://';}else{echo $output['store_slide_url'][$i];}?>" />
        </div>
...
<script src="<?php echo APP_RESOURCE_SITE_URL;?>/js/store_slide.js" charset="utf-8"></script>
<!-- 引入幻灯片JS --> 
...
var APP_TEMPLATES_URL = '<?php echo APP_TEMPLATES_URL;?>';
