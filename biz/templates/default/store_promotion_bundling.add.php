...
<div class="wrap">
	...
	<div class="alert"> <strong><?php echo $lang['nc_explain'].$lang['nc_colon'];?></strong>
		<ul>
			<?php if(intval($output['setting_config']['promotion_bundling_sum']) != 0){?>
			<li><?php printf($lang['bundling_add_explain1'], $output['setting_config']['promotion_bundling_sum'], $output['setting_config']['promotion_bundling_goods_sum']);?></li>
			<?php }else{?>
			<li><?php printf($lang['bundling_add_explain2'], $output['setting_config']['promotion_bundling_goods_sum']);?></li>
			<?php }?>
	...
	<div class="ncsc-form-default"> 
		...
		<form id="add_form" method="post" action="index.php?act=store_promotion_bundling&op=bundling_add">
			...
			<dl>
				...
				<dd>
					<p>
						<input id="bundling_name" name="bundling_name" type="text" maxlength="25" class="w400 text" value="<?php echo isset($output['bundling_info']['bl_name']) ? $output['bundling_info']['bl_name'] : '';?>" />
						<span></span> </p>
				</dd>
			</dl>
			<dl>
				...
				<dd>
					<input id="discount_price" name="discount_price" type="text" readonly style="background:#E7E7E7 none;" class="text w60 mr5" value="<?php echo isset($output['bundling_info']['bl_discount_price']) ? $output['bundling_info']['bl_discount_price'] : '';?>"/>
					<?php echo $lang['currency_zh'];?> <span></span>
					<p class="hint mt10"><?php echo $lang['bundling_cost_price'];?><span nctype="cost_price" class="price mr5 ml5"><?php echo isset($output['bundling_info']['bl_cost_price']) ? $output['bundling_info']['bl_cost_price'] : '';?></span><?php echo $lang['currency_zh'].$lang['bundling_cost_price_note'];?></p>
				</dd>
			</dl>
			<dl>
						...
						<tbody nctype="bundling_data"  class="bd-line tip" title="<?php echo $lang['bundling_add_goods_explain'];?>">
							...
							<tr id="bundling_tr_<?php echo $val['goods_id']?>" class="off-shelf">
								...
								<td class="w50"><div class="shelf-state">
										<div class="pic-thumb"><img src="<?php echo cthumb($output['goods_list'][$val['goods_id']]['goods_image'], 60, $_SESSION['store_id']);?>" ncname="<?php echo $output['goods_list'][$val['goods_id']]['goods_image'];?>" nctype="bundling_data_img"> </div>
									</div></td>
								...
								<td class="w90"><?php echo isset($val['goods_store_price']) ? $val['goods_store_price'] : '';?>
					...
					<ul class="ncsc-form-radio-list">
						<li>
							<label for="whops_seller">
								<input id="whops_seller" type="radio" name="bundling_freight_choose" <?php if(!isset($output['bundling_info']) || $output['bundling_info']['bl_freight_choose'] == '1'){ ?>checked="checked"<?php }?> value="1" />
								<?php echo $lang['bundling_add_freight_method_seller'];?></label>
						</li>
						<li>
							<label for="whops_buyer">
								<input id="whops_buyer" type="radio" name="bundling_freight_choose" <?php if(isset($output['bundling_info']) && $output['bundling_info']['bl_freight_choose'] == '0'){ ?>checked="checked"<?php }?> value="0" />
								<?php echo $lang['bundling_add_freight_method_buyer'];?></label>
							<div id="whops_buyer_box" class="transport_tpl" style="<?php if(!isset($output['bundling_info']) || $output['bundling_info']['bl_freight_choose'] == '1'){ ?>display:none;<?php }?>">
								<input class="w50 text" type="text" name="bundling_freight" value="<?php echo isset($output['bundling_info']['bl_freight']) ? $output['bundling_info']['bl_freight'] : '';?>" />
...
<script src="<?php echo APP_RESOURCE_SITE_URL;?>/js/store_bundling.js"></script> 
