	...
	<form id="add_form" method="post" enctype="multipart/form-data" action="index.php?act=store_voucher&op=<?php echo $output['type']=='add'?'templateadd':'templateedit'; ?>">
		...
		<dl>
			...
			<dd>
				<select name="sc_id">
					...
					<option value="<?php echo $v['sc_id'];?>" <?php if (isset($output['t_info']['voucher_t_sc_id']) && ($output['t_info']['voucher_t_sc_id']==$v['sc_id'])){ echo 'selected';}?>><?php echo $v['sc_name'];?></option>
					<?php }?>
				</select>
				<span></span> </dd>
		</dl>
		...
		<dl>
			...
			<dd>
				...
				<span></span>
				<p class="hint">“积分兑换”时会员可以在积分中心用积分进行兑换；“卡密兑换”时会员需要在“我的商城——我的代金券”中输入卡密获得代金券；<br>
					“免费领取”时会员可以点击店铺的代金券推广广告领取代金券。</p>
			</dd>
		</dl>
		<dl>
			<dt><em class="pngFix"></em><?php echo $lang['voucher_template_enddate'].$lang['nc_colon']; ?></dt>
			<dd>
				<input type="text" class="text w70" id="txt_template_enddate" name="txt_template_enddate" value="" readonly>
		...
		<dl>
			...
			<dd>
				...
				<em class="add-on"><i class="icon-renminbi"></i></em> <span></span> </dd>
		</dl>
		<dl>
		...
		<dl id="eachlimit_dl">
			...
			<dd>
				<select name="eachlimit" class="w80">
					...
					<?php for($i=1;$i<=intval($output['setting_config']['promotion_voucher_buyertimes_limit']);$i++){?>
					<option value="<?php echo $i;?>" <?php echo isset($output['t_info']['voucher_t_eachlimit']) && ($output['t_info']['voucher_t_eachlimit'] == $i)?'selected':'';?>><?php echo $i;?><?php echo $lang['voucher_template_eachlimit_unit'];?></option>
					<?php }?>
				</select>
			</dd>
		</dl>
		...
		<dl id="mgrade_dl">
			...
			<dd>
				<select name="mgrade_limit" class="w80">
					...
					<option value="<?php echo $v['level'];?>" <?php echo isset($output['t_info']['voucher_t_mgradelimit']) && ($output['t_info']['voucher_t_mgradelimit'] == $v['level'])?'selected':'';?>>V<?php echo $v['level'];?></option>
					<?php }?>
					<?php }?>
				</select>
		...
		<dl>
			...
			<dd>
				<div id="customimg_preview" class="ncsc-upload-thumb voucher-pic">
					<p>
		...
		<dl>
			...
			<dd>
				<input type="radio" value="<?php echo $output['templatestate_arr']['usable'][0];?>" name="tstate" <?php echo !empty($output['t_info']['voucher_t_state']) && ($output['t_info']['voucher_t_state'] == $output['templatestate_arr']['usable'][0])?'checked':'';?>>
				<?php echo $output['templatestate_arr']['usable'][1];?>
				<input type="radio" value="<?php echo $output['templatestate_arr']['disabled'][0];?>" name="tstate" <?php echo !empty($output['t_info']['voucher_t_state']) && ($output['t_info']['voucher_t_state'] == $output['templatestate_arr']['disabled'][0])?'checked':'';?>>
				<?php echo $output['templatestate_arr']['disabled'][1];?> </dd>
		</dl>
		<?php }?>
		<div class="bottom">
			<label class="submit-border"> <a id='btn_add' class="submit" href="javascript:void(0);"><?php echo $lang['nc_submit'];?></a> </label>
		</div>
	</form>
</div>
...
<?php }?>
var year = <?php echo date('Y',isset($output['quotainfo']['quota_endtime']) ? $output['quotainfo']['quota_endtime'] : 0);?>;
var month = <?php echo intval(date('m',isset($output['quotainfo']['quota_endtime']) ? $output['quotainfo']['quota_endtime'] : 0));?>;
var day = <?php echo intval(date('d',isset($output['quotainfo']['quota_endtime']) ? $output['quotainfo']['quota_endtime'] : 0));?>;
...
<?php } ?>
    ...
    $('#txt_template_enddate').val("<?php echo !empty($output['t_info']['voucher_t_end_date'])?date('Y-m-d',$output['t_info']['voucher_t_end_date']):'';?>");
