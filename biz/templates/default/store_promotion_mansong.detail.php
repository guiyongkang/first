...
<table class="ncsc-default-table">
	...
	<tbody>
		<tr class="bd-line">
			...
			<td><p><?php echo date('Y-m-d H:i',$output['mansong_info']['start_time']);?></p>
				<p>至</p>
				<p><?php echo date('Y-m-d H:i',$output['mansong_info']['end_time']);?></p></td>
			<td><ul class="ncsc-mansong-rule-list">
					<?php if(!empty($output['list']) && is_array($output['list'])){?>
					<?php foreach($output['list'] as $key=>$val){?>
					<li> <?php echo $lang['level_price'];?><strong><?php echo $val['price'];?></strong>积分，&nbsp;
						<?php if(!empty($val['discount'])) { ?>
						<?php echo $lang['level_discount'];?><strong><?php echo $val['discount'];?></strong>积分，&nbsp;
						<?php } ?>
