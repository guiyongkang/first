	...
	<form id="add_form" action="index.php?act=store_groupbuy&op=groupbuy_save&vr=1" method="post" enctype="multipart/form-data">
		...
		<dl>
			...
			<dd>
				...
				<em class="add-on"><i class="icon-calendar"></i></em><span></span>
				<p class="hint"> 抢购结束时间不能大于虚拟商品的过期时间 <span id="vr-expire-time"></span>
					<?php if (!$output['isOwnShop']) { ?>
					和抢购套餐的过期时间 <span>（<?php echo date('Y-m-d H:i', $output['current_groupbuy_quota']['end_time']); ?>）</span>
					<?php } ?>
				</p>
			</dd>
		</dl>
				...
				<div id="div_search_goods" class="div-goods-select mt10" style="display: none;">
					<table class="search-form">
						<tr>
							...
							<td class="w10"></td>
							<td><p class="hint">不输入名称直接搜索将显示店内所有虚拟商品。</p></td>
						</tr>
					</table>
...
$("select[name=area]").change(function(){
    ...
    $('#btn_search_goods').on('click', function() {
        var url = "<?php echo urlBiz('store_groupbuy', 'search_vr_goods'); ?>";
