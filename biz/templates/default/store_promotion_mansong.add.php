	...
	<form id="add_form" action="index.php?act=store_promotion_mansong&op=mansong_save" method="post">
		...
		<dl>
			...
			<dd>
				...
				<em class="add-on"><i class="icon-calendar"></i></em> <span class="error-message"></span>
		...
		<dl>
			...
			<dd>
				...
				<div id="div_add_rule" style="display:none;">
					...
					<div class="ncsc-mansong-rule"> <span>单笔订单满&nbsp;
						<input id="mansong_price" type="text" class="text w50">
						<em class="add-on"><i class="icon-renminbi"></i></em>，</span> <span>立减现金&nbsp;
						<input id="mansong_discount" type="text" class="text w50">
						<em class="add-on"><i class="icon-renminbi"></i></em>，</span> <span>送礼品&nbsp;<a href="javascript:void(0);" id="btn_show_search_goods" class="ncsc-btn"><i class="icon-gift"></i>选择礼品</a></span>
						...
						<div id="div_search_goods" class="div-goods-select mt10" style="display: none;">
							<table class="search-form">
								<tr>
									...
									<td class="w70 tc"><a href="javascript:void(0);" id="btn_search_goods" class="ncsc-btn"/><i class="icon-search"></i><?php echo $lang['nc_search'];?></a></td>
									<td class="w10"></td>
									<td><p class="hint">不输入名称直接搜索将显示店内所有出售中的商品</p></td>
								</tr>
							</table>
					...
					<div class="mt10"> <a href="javascript:void(0);" id="btn_save_rule" class="ncsc-btn ncsc-btn-acidblue"><i class="icon-ok-circle"></i>确定规则设置</a> <a href="javascript:void(0);" id="btn_cancel_add_rule" class="ncsc-btn ncsc-btn-orange"><i class="icon-ban-circle"></i>取消</a></div>
				</div>
				<span class="error-message"></span>
				<p class="hint">设置当单笔订单满足金额时（必填选项），减免金额（选填）或赠送的礼品（选填）；留空为不做减免金额或赠送礼品处理。<br/>
					系统最多支持设置三组等级规则。</p>
			</dd>
		</dl>
		<dl>
...
<span>单笔订单满<strong><%=price%></strong>积分， </span>
<span>立减现金<strong><%=discount%></strong>积分， </span>
<%if(goods_id>0){%>
<span>送礼品 <%==goods%></span>
<%}%>
...
<?php } ?>
        ...
        messages : {
            ...
            start_time : {
                required : '<i class="icon-exclamation-sign"></i><?php echo sprintf($lang['mansong_add_start_time_explain'],date('Y-m-d H:i',isset($output['start_time']) ? $output['start_time'] : 0));?>',
                greaterThanDate : '<i class="icon-exclamation-sign"></i><?php echo sprintf($lang['mansong_add_start_time_explain'],date('Y-m-d H:i',isset($output['start_time']) ? $output['start_time'] : 0));?>'
            },
            end_time : {
                required : '<i class="icon-exclamation-sign"></i><?php echo sprintf($lang['mansong_add_end_time_explain'],date('Y-m-d H:i',(isset($output['end_time']) ? $output['end_time'] : 0)));?>',
<?php if (!$output['isOwnShop']) { ?>
                lessThanDate : '<i class="icon-exclamation-sign"></i><?php echo sprintf($lang['mansong_add_end_time_explain'],date('Y-m-d H:i',(isset($output['end_time']) ? $output['end_time'] : 0)));?>',
<?php } ?>
    ...
    $('#btn_search_goods').on('click', function() {
        var url = "<?php echo urlBiz('store_promotion_mansong', 'search_goods');?>";
