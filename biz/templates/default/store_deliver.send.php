...
<span class="fr mr5"> <a href="<?php echo urlBiz('store_deliver', 'waybill_print', array('order_id' => $output['order_info']['order_id']));?>" class="ncsc-btn-mini" target="_blank" title="打印运单"/><i class="icon-print"></i>打印运单</a></span>
<div class="wrap">
	...
	<form name="deliver_form" method="POST" id="deliver_form" action="index.php?act=store_deliver&op=send&order_id=<?php echo $_GET['order_id'];?>" onsubmit="ajaxpost('deliver_form', '', '', 'onerror');return false;">
		...
		<input type="hidden" id="shipping_express_id" value="<?php echo isset($output['order_info']['shipping_express_id'])? $output['order_info']['shipping_express_id'] : 0;?>" name="shipping_express_id">
		...
		<input type="hidden" value="<?php echo isset($output['order_info']['extend_order_common']['reciver_info']['dlyp']) ? $output['order_info']['extend_order_common']['reciver_info']['dlyp'] : '';?>" name="reciver_dlyp" id="reciver_dlyp">
		<table class="ncsc-default-table order deliver">
			<tbody>
				...
				<?php }?>
				<tr>
					<td colspan="20" class="tl bdl bdr" style="padding:8px" id="address"><strong class="fl"><?php echo $lang['store_deliver_buyer_adress'].$lang['nc_colon'];?></strong><span id="buyer_address_span"><?php echo $output['order_info']['extend_order_common']['reciver_name'];?>&nbsp;<?php echo $output['order_info']['extend_order_common']['reciver_info']['phone'];?>&nbsp;<?php echo $output['order_info']['extend_order_common']['reciver_info']['address'];?></span><?php echo !empty($output['order_info']['extend_order_common']['reciver_info']['dlyp']) ? '[自提服务站]' : '';?> <a href="javascript:void(0)" nc_type="dialog" dialog_title="<?php echo $lang['store_deliver_buyer_adress'];?>" dialog_id="edit_buyer_address" uri="index.php?act=store_deliver&op=buyer_address_edit&order_id=<?php echo $output['order_info']['order_id'];?>" dialog_width="550" class="ncsc-btn-mini fr"><i class="icon-edit"></i><?php echo $lang['nc_edit'];?></a></td>
				</tr>
				<?php } else { ?>
				<tr>
		...
		<div class="deliver-sell-info"><strong class="fl"><?php echo $lang['store_deliver_my_daddress'].$lang['nc_colon'];?></strong> <a href="javascript:void(0);" onclick="ajax_form('modfiy_daddress', '<?php echo $lang['store_deliver_select_daddress'];?>', 'index.php?act=store_deliver&op=send_address_select&order_id=<?php echo $output['order_info']['order_id'];?>', 640,0);" class="ncsc-btn-mini fr"><i class="icon-edit"></i><?php echo $lang['nc_edit'];?></a> <span id="seller_address_span">
		...
		<input type="hidden" name="daddress_id" id="daddress_id" value="<?php echo isset($output['daddress_info']['address_id']) ? $output['daddress_info']['address_id'] : 0;?>">
