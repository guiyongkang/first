<div class="eject_con">
	...
	<form method="post" target="_parent" action="index.php?act=store_brand&op=<?php if (!empty($output['brand_array']['brand_id'])) echo 'brand_edit'; else echo 'brand_save'; ?>"enctype="multipart/form-data" id="brand_apply_form">
		<input type="hidden" name="form_submit" value="ok" />
		<input type="hidden" name="brand_id" value="<?php echo !empty($output['brand_array']) ? $output['brand_array']['brand_id'] : ''; ?>" />
		<dl>
			<dt><i class="required">*</i><?php echo $lang['store_goods_brand_name'].$lang['nc_colon'];?></dt>
			<dd>
				<input type="text" class="text" name="brand_name" value="<?php echo !empty($output['brand_array']) ? $output['brand_array']['brand_name'] : ''; ?>" id="brand_name" />
			</dd>
		</dl>
		<dl>
			<dt><i class="required">*</i>名称首字母<?php echo $lang['nc_colon'];?></dt>
			<dd>
				<input type="text" class="text" name="brand_initial" value="<?php echo !empty($output['brand_array']) ? $output['brand_array']['brand_initial'] : '';?>" id="brand_initial" />
			</dd>
		</dl>
		<dl>
			<dt><?php echo $lang['store_goods_brand_class'].$lang['nc_colon'];?></dt>
			<dd id="gcategory">
				<input type="hidden" value="<?php echo !empty($output['brand_array']) ? $output['brand_array']['class_id'] : 0?>" name="class_id" class="mls_id">
				<input type="hidden" value="<?php echo !empty($output['brand_array']) ? $output['brand_array']['brand_class'] : ''?>" name="brand_class" class="mls_name">
				<?php if(!empty($output['brand_array']['brand_id'])){?>
				<span><?php echo $output['brand_array']['brand_class']?></span>
				<input class="edit_gcategory" type="button" value="<?php echo $lang['nc_edit'];?>">
				<?php }?>
				<select <?php if(!empty($output['brand_array']['brand_id'])){?>style="display:none;"<?php }?>>
		...
		<dl>
			...
			<dd>
				<div class=""><span class="sign"><img src="<?php echo brandImage(!empty($output['brand_array']) ? $output['brand_array']['brand_pic'] : '');?>" onload="javascript:DrawImage(this,150,50)" nc_type="logo1"/></span></div>
