...
<div class="ncsc-index">
		...
		<div class="basic-info">
			<dl class="ncsc-seller-info">
				<dt class="seller-name">
					<h3><?php echo $output['session']['seller_name']; ?></h3>
					<h5>(用户名：<?php echo $output['session']['member_name']; ?>)</h5>
				</dt>
				<dd class="store-logo">
					<p><img src="<?php echo getStoreLogo($output['store_info']['store_label'],'store_logo');?>"/></p>
					<a href="<?php echo urlBiz('store_setting', 'store_setting');?>"><i class="icon-edit"></i>编辑店铺设置</a> </dd>
				<dd class="seller-permission">管理权限：<strong><?php echo isset($output['session']['seller_group_name']) ? $output['session']['seller_group_name'] : '';?></strong></dd>
				<dd class="seller-last-login">最后登录：<strong><?php echo isset($output['session']['seller_last_login_time']) ? $output['session']['seller_last_login_time'] : '';?></strong> </dd>
				...
				<dd class="store-validity"><?php echo $lang['store_validity'].$lang['nc_colon'];?><strong><?php echo $output['store_info']['store_end_time_text'];?>
					<?php if (!empty($output['store_info']['reopen_tip'])) {?>
			...
			<div class="detail-rate">
				...
				<ul>
					...
					<li> <?php echo $value['text'];?><span class="credit"><?php echo $value['credit'];?> 分</span> <span class="<?php echo isset($value['percent_class']) ? $value['percent_class'] : '';?>"><i></i><?php echo isset($value['percent_text']) ? $value['percent_text'] : '';?><em><?php echo isset($value['percent']) ? $value['percent'] : '';?></em></span> </li>
					<?php } ?>
				</ul>
			</div>
			<?php } ?>
	...
	<div class="seller-cont">
		<div class="container type-a">
			...
			<div class="content">
				...
				<ul>
					...
					<?php if ($output['setting_config']['goods_verify']) {?>
					...
					<li style="display:none"><a href="index.php?act=store_consult&op=consult_list&type=to_reply"><?php echo $lang['store_to_consult'];?> <strong id="nc_consult"></strong></a></li>
				</ul>
			</div>
		</div>
			...
			<div class="content">
				<!--<ul>
					<?php
						...
						?>
								<li><a target="_blank" href="<?php echo urlShop('show_help', 'index', array('t_id' => $val['type_id']));?>" title="<?php echo $val['type_name']; ?>"> <?php echo $val['type_name'];?></a></li>
								<?php
							}
						}
				    ?>
				...
				<dl>
				    ...
				    ?>
					<dd><?php echo $lang['store_site_email'].$lang['nc_colon'];?><?php echo $output['setting_config']['site_email'];?></dd>
				</dl>
			</div>
		</div>
		<div class="container type-a">
			...
			<div class="content">
				<dl class="focus">
					...
					<dt style="display:none;"><?php echo $lang['store_complain_info'].$lang['nc_colon'];?></dt>
					<dd style="display:none;"><a href="index.php?act=store_complain&select_complain_state=1"><?php echo $lang['store_complain'];?> <strong id="nc_complain"></strong></a></dd>
				</dl>
				<ul>
				...
				<table class="ncsc-default-table">
					...
					<tbody>
						...
						<tr class="bd-line">
							...
							<td class="w60"><div class="pic-thumb"><img alt="<?php echo $val['goods_name'];?>" src="<?php echo thumb($val, '60');?>"></div></td>
		...
		<div class="container type-d h500">
			...
			<div class="content">
				<?php if ($output['setting_config']['groupbuy_allow'] == 1){ ?>
				<dl class="tghd">
					<dt class="p-name"> <a href="index.php?act=store_groupbuy"><?php echo $lang['store_groupbuy'];?></a></dt>
					<dd class="p-ico"></dd>
					<dd class="p-hint">
						<?php if (!empty($output['isOwnShop']) || !empty($output['groupquota_info'])) {?>
				...
				<?php } ?>
				<?php if (intval($output['setting_config']['promotion_allow']) == 1){ ?>
				<dl class="xszk">
					...
					<dd class="p-hint"><span>
						<?php if (!empty($output['isOwnShop']) || !empty($output['xianshiquota_info'])) {?>
						<i class="icon-ok-sign"></i>已开通
						<?php } else {?>
						<i class="icon-minus-sign"></i>未开通
						<?php }?>
				...
				<dl class="mjs">
					...
					<dd class="p-ico"></dd>
					<dd class="p-hint"><span>
						<?php if (!empty($output['isOwnShop']) || !empty($output['mansongquota_info'])) {?>
						<i class="icon-ok-sign"></i>已开通
						<?php } else {?>
						<i class="icon-minus-sign"></i>未开通
						<?php }?>
				...
				<dl class="tjzw" style="display:none;">
					<dt class="p-name"><a href="index.php?act=store_promotion_booth&op=index">推荐展位</a></dt>
					<dd class="p-ico"></dd>
					<dd class="p-hint"><span>
						<?php if (!empty($output['isOwnShop']) || !empty($output['boothquota_info'])) {?>
						<i class="icon-ok-sign"></i>已开通
						<?php } else {?>
						<i class="icon-minus-sign"></i>未开通
						<?php }?>
				...
				<?php } ?>
				<?php if ($output['setting_config']['voucher_allow'] == 1){?>
				<dl class="djq">
					<dt class="p-name"><a href="index.php?act=store_voucher"><?php echo $lang['store_voucher'];?></a></span></dt>
					<dd class="p-ico"></dd>
					<dd class="p-hint"><span>
						<?php if (!empty($output['isOwnShop']) || !empty($output['voucherquota_info'])) {?>
						<i class="icon-ok-sign"></i>已开通
						<?php } else {?>
						<i class="icon-minus-sign"></i>未开通
						<?php }?>
...
$(function(){
    ...
    $.getJSON('index.php?act=index&op=statistics&rand='+timestamp, null, function(data){
