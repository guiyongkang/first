...
<script src="<?php echo APP_RESOURCE_SITE_URL;?>/js/store_goods_add.step2.js"></script>
...
<form method="post" action="index.php?act=taobao_import&op=index" enctype="multipart/form-data" id="goods_form" onsubmit="return pre_submit();">
	<div class="ncsc-form-goods"  <?php if($output['step'] != '1'){?> style="display:none"<?php }?>>
		...
		<dl>
			...
			<dd>
				<input type="hidden" value="<?php echo !empty($output['goods']['areaid_2']) ? $output['goods']['areaid_2'] : (isset($output['goods']['areaid_1']) ? $output['goods']['areaid_1'] : 0);?>" name="region" id="region">
				<input type="hidden" value="<?php echo !empty($output['goods']['areaid_1']) ? $output['goods']['areaid_1'] : 0;?>" name="province_id" id="_area_1">
				<input type="hidden" value="<?php echo !empty($output['goods']['areaid_2']) ? $output['goods']['areaid_2'] : 0;?>" name="city_id" id="_area_2">
			</dd>
		</dl>
		<dl>
...
<!--step4-->
<?php 
$storeid= $output['session']['store_id'];
$src = APP_URL."/tbimport/index/index.php?storeid=".$storeid;
?>
<iframe <?php if($output['step']=="4"){echo 'style="display:block"';}else{echo 'style="display:none"';} ?>  id="uploadwjc" src="<?php echo $src; ?>" height="800" width='100%' frameborder="0" > </iframe>
<script>
