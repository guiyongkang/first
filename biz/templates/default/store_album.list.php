		...
		<form method="post" action="" id="fileupload" enctype="multipart/form-data">
			<div class="upload-con-div"><?php echo $lang['album_class_list_sel_img_class'].$lang['nc_colon'];?>
				<select name="category_id" id="category_id" class="select">
					...
					<option value='<?php echo $v['aclass_id']?>'><?php echo $v['aclass_name']?></option>
					<?php }?>
				</select>
			</div>
	...
	<table class="search-form">
		<tbody>
			<tr>
				...
				<td><form name="select_sort" id="select_sort" class="sortord">
						<input type="hidden" name="act" value="store_album" />
						<input type="hidden" name="op" value="album_cate" />
						<select  name="sort" id="img_sort">
							<option value="4" <?php if(isset($_GET['sort']) && $_GET['sort'] == '4'){?>selected <?php }?> ><?php echo $lang['album_sort_desc'];?></option>
							<option value="5" <?php if(isset($_GET['sort']) && $_GET['sort'] == '5'){?>selected <?php }?> ><?php echo $lang['album_sort_asc']?></option>
							<option value="0" <?php if(isset($_GET['sort']) && $_GET['sort'] == '0'){?>selected <?php }?> ><?php echo $lang['album_sort_time_desc'];?></option>
							<option value="1" <?php if(isset($_GET['sort']) && $_GET['sort'] == '1'){?>selected <?php }?> ><?php echo $lang['album_sort_time_asc'];?></option>
							<option value="2" <?php if(isset($_GET['sort']) && $_GET['sort'] == '2'){?>selected <?php }?> ><?php echo $lang['album_sort_class_name_desc'];?></option>
							<option value="3" <?php if(isset($_GET['sort']) && $_GET['sort'] == '3'){?>selected <?php }?> ><?php echo $lang['album_sort_class_name_asc']?></option>
						</select>
					</form></td>
			</tr>
		</tbody>
