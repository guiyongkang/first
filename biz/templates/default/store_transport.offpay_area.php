...
<form id="area_form" method="post">
	...
	<table class="ncsc-default-table order" id="table_area_box">
		<tbody>
			...
			<tr>
				<td class="w120"><label>
						...
						<strong><?php echo $areas['name'][$provinceId]; ?></strong> </label></td>
				<td><?php foreach ($areas['children'][$provinceId] as $cityId) { ?>
					<div class="area-list">
						<label>
							<input type="checkbox" nc_province="<?php echo $provinceId; ?>"<?php if (!empty($output['city_checked_array'][$cityId])) echo ' checked="checked"';?> value="<?php echo $cityId; ?>" name="city[]" />
							<?php echo $areas['name'][$cityId]; ?> </label>
						(<span city_id="<?php echo $cityId; ?>" title="已选下级地区"><?php echo !empty($output['city_checked_child_array'][$cityId]) ? count($output['city_checked_child_array'][$cityId]) : 0; ?></span>) <a city_id="<?php echo $cityId; ?>" nc_title="<?php echo $areas['name'][$cityId]; ?>" province_id="<?php echo $provinceId; ?>" nc_type="edit" href="javascript:void(0);" title="选择下级地区"><i class="icon-pencil"></i></a> </div>
					<?php } ?></td>
			</tr>
			<?php } ?>
		</tbody>
		<tfoot>
			<tr class="tfoot">
				<td colspan="15"><div class="tc mt10 mb10"> <a href="javascript:void(0);" class="ncsc-btn ncsc-btn-green" id="submitBtn"><span>提交保存</span></a> </div></td>
			</tr>
		</tfoot>
	</table>
</form>
