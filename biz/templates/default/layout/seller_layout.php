...
<link href="<?php echo APP_TEMPLATES_URL?>/css/base.css" rel="stylesheet" type="text/css">
<link href="<?php echo APP_TEMPLATES_URL?>/css/seller_center.css" rel="stylesheet" type="text/css">
<link href="<?php echo APP_RESOURCE_SITE_URL;?>/font/font-awesome/css/font-awesome.min.css" rel="stylesheet" />
<!--[if IE 7]>
  <link rel="stylesheet" href="<?php echo APP_RESOURCE_SITE_URL;?>/font/font-awesome/css/font-awesome-ie7.min.css">
<![endif]-->
<script>
var COOKIE_PRE = '<?php echo COOKIE_PRE;?>';var _CHARSET = '<?php echo strtolower(CHARSET);?>';var SITEURL = '<?php echo APP_URL;?>';var RESOURCE_SITE_URL = '<?php echo RESOURCE_SITE_URL;?>';var APP_RESOURCE_SITE_URL = '<?php echo APP_RESOURCE_SITE_URL;?>';var APP_TEMPLATES_URL = '<?php echo APP_TEMPLATES_URL;?>';</script>
<script type="text/javascript" src="<?php echo RESOURCE_SITE_URL;?>/js/jquery.js"></script>
<script type="text/javascript" src="<?php echo APP_RESOURCE_SITE_URL;?>/js/seller.js"></script>
	...
	<div class="wrapper">
		<div class="ncsc-admin">
			<dl class="ncsc-admin-info">
				<dt class="admin-avatar"><img src="<?php echo getMemberAvatarForID($output['session']['member_id']);?>" width="32" class="pngFix" alt=""/></dt>
				<dd class="admin-permission">当前用户</dd>
				<dd class="admin-name"><?php echo $output['session']['seller_name'];?></dd>
			</dl>
			<div class="ncsc-admin-function"><a href="<?php echo urlBiz('index', 'index');?>" title="首页" ><i class="icon-home"></i></a><a href="<?php echo urlBiz('store_setting', 'store_setting');?>" title="店铺设置"><i class="icon-wrench"></i></a><a href="<?php echo urlBiz('seller_logout', 'logout');?>" title="安全退出"><i class="icon-signout"></i></a></div>
		</div>
		<div class="center-logo"> <a href="<?php echo APP_URL;?>" target="_blank"><img src="<?php echo UPLOAD_SITE_URL.'/'.ATTACH_COMMON.DS.$output['setting_config']['seller_center_logo'];?>" class="pngFix" alt=""/></a>
			<h1>商家中心</h1>
		</div>
		...
		<nav class="ncsc-nav">
			<dl class="<?php echo $output['current_menu']['model'] == 'index'?'current':'';?>">
				<dt><a href="index.php?act=index&op=index">首页</a></dt>
				<dd class="arrow"></dd>
			</dl>
			<?php if(!empty($output['menu']) && is_array($output['menu'])) {?>
...
<?php if(empty($output['seller_layout_no_menu'])) { ?>
<div class="ncsc-layout wrapper">
		...
		<div id="sidebar" class="sidebar">
			...
			<div class="column-menu">
				<ul id="seller_center_left_menu">
					...
					<?php if(is_array($submenu_value) && isset($submenu_value['act'])){?>
					<li <?php echo $_GET['act'] == 'index'?"id='quicklink_".$submenu_value['act']."'":'';?>class="<?php echo $submenu_value['act'] == $_GET['act']?'current':'';?>"> <a href="index.php?act=<?php echo $submenu_value['act'];?>&op=<?php echo $submenu_value['op'];?>"> <?php echo $submenu_value['name'];?> </a> </li>
					<?php } ?>
					<?php } ?>
					<?php } else { ?>
	...
	<div id="layoutRight" class="ncsc-layout-right">
		<div class="ncsc-path"><i class="icon-desktop"></i>商家管理中心<i class="icon-angle-right"></i><?php echo $output['current_menu']['model_name'];?><i class="icon-angle-right"></i><?php echo isset($output['current_menu']['name']) ? $output['current_menu']['name'] : '';?></div>
...
$(document).ready(function(){
    ...
    $('[nctype="btn_add_quicklink"]').on('click', function() {
        ...
        if($quicklink_item.hasClass('selected')) {
            $.post("<?php echo urlBiz('index', 'quicklink_del');?>", { item: item }, function(data) {
            ...
            if(count >= 8) {
                ...
                $.post("<?php echo urlBiz('index', 'quicklink_add');?>", { item: item }, function(data) {
...
<div id="tbox">
	<div class="btn" id="msg"><a href="<?php echo urlBiz('store_msg', 'index');?>"><i class="msg">
		<?php if ($output['store_msg_num'] > 0) { ?>
		<em><?php echo $output['store_msg_num'];?></em>
		<?php } ?>
		</i>站内消息</a></div>
