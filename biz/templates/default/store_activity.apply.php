...
<form method="GET">
	...
	<table class="ncsc-default-table" >
		...
		<tbody>
			...
			<tr class="bd-line">
				<td><div class="pic-thumb"><a href="index.php?act=goods&goods_id=<?php echo $v['goods_id']; ?>" target="_blank"><img src="<?php echo cthumb($v['goods_image'], 60,$output['session']['store_id']);?>"></a></div></td>
				<td class="tl"><dl class="goods-name">
						<dt><a target="_blank" href="<?php echo urlBiz('goods', 'index', array('goods_id' => $v['goods_id']));?>"><?php echo $v['goods_name'];?></a></dt>
						<dd><?php echo isset($v['gc_name']) ? $v['gc_name'] : '';?></dd>
					</dl></td>
	...
	<form method="GET">
		...
		<table class="search-form">
			<tr>
				...
				<td class="w160"><input type="text" class="text w150" name="name" value="<?php echo isset($output['search']['name']) ? $output['search']['name'] : '';?>" placeholder="搜索商品名称"/></td>
	...
	<form method="POST" id="apply_form" onsubmit="ajaxpost('apply_form','','','onerror');" action="index.php?act=store_activity&op=activity_apply_save">
		...
		<div class="search-result">
			<ul class="goods-list">
				...
				<li>
					<div class="goods-thumb"><img alt="<?php echo $goods['goods_name'];?>" title="<?php echo $goods['goods_name'];?>" src="<?php echo cthumb($goods['goods_image'], 240, $output['session']['store_id']);?>"/></div>
					<dl class="goods-info">
						<dt>
							<input type="checkbox" value="<?php echo $goods['goods_id'];?>" class="vm" name="item_id[]"/>
							<label><?php echo $goods['goods_name'];?></label>
						</dt>
