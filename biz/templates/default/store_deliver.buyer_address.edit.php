...
<div class="eject_con">
	<div class="adds">
		...
		<form method="post" action="index.php?act=store_deliver&op=buyer_address&order_id=<?php echo $_GET['order_id'];?>" id="address_form" target="_parent">
			...
			<div class="bottom">
				<label class="submit-border"><a href="javascript:void(0);" id="submit" class="submit"><?php echo $lang['nc_common_button_save'];?></a></label>
			</div>
		</form>
	</div>
</div>
...
$(document).ready(function(){
	...
	$('#submit').on('click',function(){
		if ($('#address_form').valid()) {
            ...
            $.post(
            "<?php echo urlBiz('store_deliver', 'buyer_address_save');?>", 
            {
                ...
                reciver_dlyp:'<?php echo isset($output['address_info']['reciver_info']['dlyp']) ? $output['address_info']['reciver_info']['dlyp'] : '';?>'
            })
            .done(function(data) {
                if(data == 'true') {
