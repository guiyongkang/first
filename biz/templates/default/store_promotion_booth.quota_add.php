...
<div class="ncsc-form-default">
	...
	<form id="add_form" action="<?php echo urlBiz('store_promotion_booth', 'booth_quota_add');?>" method="post">
	<?php } else {?>
	<form id="add_form" action="<?php echo urlBiz('store_promotion_booth', 'booth_renew');?>" method="post">
		<?php }?>
		<input type="hidden" name="form_submit" value="ok" />
		<dl>
			<dt><i class="required">*</i>套餐购买数量：</dt>
			<dd>
				<input id="booth_quota_quantity" name="booth_quota_quantity" type="text" class="text w50"/>
				<em class="add-on">月</em><span></span>
				<p class="hint">购买单位为月(30天)，一次最多购买12个月，购买后立即生效，即可添加推荐商品。</p>
				<p class="hint"><?php printf('每月您需要支付%d积分。', intval($output['setting_config']['promotion_booth_price']));?></p>
...
$(document).ready(function(){
	...
	$("#add_form").validate({
		...
		submitHandler:function(form){
			var unit_price = parseInt(<?php echo $output['setting_config']['promotion_booth_price'];?>);
			var quantity = parseInt($("#booth_quota_quantity").val());
			var price = unit_price * quantity;
			showDialog('确认购买?您总共需要支付'+price+'积分', 'confirm', '', function(){ajaxpost('add_form', '', '', 'onerror');});
		},
		rules : {
