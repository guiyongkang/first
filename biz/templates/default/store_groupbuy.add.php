		...
		<dl>
			...
			<dd>
				...
				<p class="hint">
					...
					<?php echo '抢购开始时间不能大于'.date('Y-m-d H:i', $output['current_groupbuy_quota']['end_time']);?> </p>
				<?php } ?>
			</dd>
		</dl>
		<dl>
			...
			<dd>
				<div nctype="groupbuy_goods_info" class="selected-group-goods " style="display:none;">
					...
					<div class="goods-name"> <a nctype="groupbuy_goods_href" id="groupbuy_goods_name" href="" target="_blank"></a> </div>
				...
				<div id="div_search_goods" class="div-goods-select mt10" style="display: none;">
					...
					<a id="btn_hide_search_goods" class="close" href="javascript:void(0);">X</a> </div>
				<p class="hint"><?php echo $lang['groupbuy_goods_explain'];?></br>
					<span class="red">抢购生效后该商品的所有规格SKU都将执行统一的抢购价格</span></p>
			</dd>
		</dl>
		<dl nctype="groupbuy_goods_info" style="display:none;">
			<dt><?php echo $lang['groupbuy_index_store_price'].$lang['nc_colon'];?></dt>
			<dd> <?php echo $lang['currency'];?><span nctype="groupbuy_goods_price"></span>
				<input id="input_groupbuy_goods_price" type="hidden">
			</dd>
		</dl>
		<dl>
			<dt><i class="required">*</i><?php echo $lang['groupbuy_price'].$lang['nc_colon'];?></dt>
			<dd>
				<input class="w70 text" id="groupbuy_price" name="groupbuy_price" type="text" value=""/>
...
$(document).ready(function(){
    ...
    $('#btn_search_goods').on('click', function() {
        var url = "<?php echo urlBiz('store_groupbuy', 'search_goods');?>";
    ...
    $('#div_goods_search_result').on('click', '[nctype="btn_add_groupbuy_goods"]', function() {
        ...
        $.get('<?php echo urlBiz('store_groupbuy', 'groupbuy_goods_info');?>', {goods_commonid: goods_commonid}, function(data) {
            if(data.result) {
    ...
    $('[nctype="btn_upload_image"]').fileupload({
        dataType: 'json',
            url: "<?php echo urlBiz('store_groupbuy', 'image_upload');?>",
            add: function(e, data) {
                ...
                $img.attr('src', "<?php echo APP_TEMPLATES_URL.'/images/loading.gif';?>");
                data.submit();
            },
    ...
    jQuery.validator.methods.checkGroupbuyGoods = function(value, element) {
        ...
        $.ajax({
            type:"GET",
            url:'<?php echo urlBiz('store_groupbuy', 'check_groupbuy_goods');?>',
            async:false,
...
<?php } ?>
        ...
        messages : {
            ...
            start_time : {
                ...
                greaterThanDate : '<i class="icon-exclamation-sign"></i><?php echo sprintf('抢购开始时间必须大于{0}',date('Y-m-d H:i',isset($output['current_groupbuy_quota']['start_time']) ? $output['current_groupbuy_quota']['start_time'] : 0));?>'
            },
            end_time : {
                required : '<i class="icon-exclamation-sign"></i>抢购结束时间不能为空',
<?php if (!$output['isOwnShop']) { ?>
                lessThanDate : '<i class="icon-exclamation-sign"></i><?php echo sprintf('抢购结束时间必须小于{0}',date('Y-m-d H:i',isset($output['current_groupbuy_quota']['end_time']) ? $output['current_groupbuy_quota']['end_time'] : 0));?>',
<?php } ?>
    ...
    $('.des_demo').ajaxContent({
        event:'click', //mouseover
            loaderType:"img",
            loadingMsg:"<?php echo APP_TEMPLATES_URL;?>/images/loading.gif",
            target:'#des_demo'
    });
});
