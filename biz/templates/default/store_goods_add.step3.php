...
<?php }?>
<form method="post" id="goods_image" action="<?php if ($output['edit_goods_sign']) { echo urlBiz('store_goods_online', 'edit_save_image'); } else { echo urlBiz('store_goods_add', 'save_image');}?>">
	...
	<div class="ncsc-form-goods-pic">
		<div class="container">
			...
			<div class="ncsc-goodspic-list">
				<div class="title">
					<h3>属性：
						<?php if (isset($output['value'][$value['sp_value_id']])) { echo $output['value'][$value['sp_value_id']];} else {echo $value['sp_value_name'];}?>
					</h3>
				</div>
				<ul nctype="ul<?php echo $value['sp_value_id'];?>">
					<?php for ($i = 0; $i < 5; $i++) {?>
					<li class="ncsc-goodspic-upload">
						<div class="upload-thumb"><img src="<?php echo cthumb(isset($output['img'][$value['sp_value_id']][$i]['goods_image']) ? $output['img'][$value['sp_value_id']][$i]['goods_image'] : '', 240);?>" nctype="file_<?php echo $value['sp_value_id'] . $i;?>">
				...
				<div class="ncsc-select-album"> <a class="ncsc-btn" href="index.php?act=store_album&op=pic_list&item=goods_image&color_id=<?php echo $value['sp_value_id'];?>" nctype="select-<?php echo $value['sp_value_id'];?>"><i class="icon-picture"></i>从图片空间选择</a> <a href="javascript:void(0);" nctype="close_album" class="ncsc-btn ml5" style="display: none;"><i class=" icon-circle-arrow-up"></i>关闭相册</a> </div>
		...
		<div class="sidebar">
			<div class="alert alert-info alert-block" id="uploadHelp">
				...
				<h4>上传要求：</h4>
				<ul>
					<li>1. 请使用jpg\jpeg\png等格式、单张大小不超过<?php echo intval($output['setting_config']['image_max_filesize'])/1024;?>M的正方形图片。</li>
					<li>2. 上传图片最大尺寸将被保留为1280像素。</li>
					<li>3. 每种属性最多可上传5张图片或从图片空间中选择已有的图片，上传后的图片也将被保存在店铺图片空间中以便其它使用。</li>
	...
	<div class="bottom tc hr32">
		<label class="submit-border">
			<input type="submit" class="submit" value="<?php if ($output['edit_goods_sign']) { echo '提交'; } else { ?><?php echo $lang['store_goods_add_next'];?>，确认商品发布<?php }?>" />
		</label>
	</div>
</form>
...
<script type="text/javascript" src="<?php echo APP_RESOURCE_SITE_URL;?>/js/store_goods_add.step3.js" charset="utf-8"></script> 
<script>
...
$(function(){
    ...
    $('a[nctype="select-<?php echo $value['sp_value_id'];?>"]').ajaxContent({
        ...
        loaderType:"img",
        loadingMsg:APP_TEMPLATES_URL+"/images/loading.gif",
