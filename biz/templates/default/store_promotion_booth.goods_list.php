...
<div class="tabmenu">
	...
	<a class="ncsc-btn ncsc-btn-acidblue" href="<?php echo urlBiz('store_promotion_booth', 'booth_quota_add');?>" title="购买套餐"><i class="icon-money"></i>购买套餐</a>
	<?php } else { ?>
	...
	<?php } ?>
	<a class="ncsc-btn ncsc-btn ncsc-btn-acidblue" href="<?php echo urlBiz('store_promotion_booth', 'booth_renew');?>"><i class="icon-money"></i>套餐续费</a>
	<?php } ?>
	<?php } ?>
</div>
<?php if ($output['isOwnShop']) { ?>
<div class="alert alert-block mt10">
	<ul>
		...
		<?php if ($output['setting_config']['promotion_booth_goods_sum'] != 0) {?>
		<li>2、您最多可以推荐<?php echo $output['setting_config']['promotion_booth_goods_sum'];?>个商品。</li>
		<?php }?>
	</ul>
</div>
<?php } else { ?>
...
<div class="alert alert-block mt10">
	...
	<ul>
		...
		<?php if ($output['setting_config']['promotion_booth_goods_sum'] != 0) {?>
		<li>4、您最多可以推荐<?php echo $output['setting_config']['promotion_booth_goods_sum'];?>个商品。</li>
		<?php }?>
	</ul>
</div>
<?php } ?>
...
<div nvtype="div_goods_select" class="div-goods-select" style="display: none;">
	<table class="search-form">
		<tr>
			...
			<td class="w160"><input nctype="search_goods_name" type="text w150" class="text" name="goods_name" value=""/></td>
			<td class="w70 tc"><label class="submit-border">
					<input nctype="btn_search_goods" type="button" value="<?php echo $lang['nc_search'];?>" class="submit"/>
				</label></td>
			<td class="w10"></td>
...
<table class="ncsc-default-table">
	...
	<tbody nctype="choose_goods_list">
		...
		<tr class="bd-line">
			<td></td>
			<td><div class="pic-thumb"><img src="<?php echo $val['goods_image'];?>"/></div></td>
			...
			<td class="nscs-table-handle"><span><a class="btn-red" href='javascript:void(0);' nctype="del_choosed" data-param="{gid:<?php echo $val['goods_id'];?>}"><i class="icon-trash"></i>
...
<table class="ncsc-default-table ncsc-promotion-buy">
	<tbody>
		<tr>
			<td colspan="20" class="norecord"><div class="no-promotion"><i class="zw"></i><span>您还没有购买套餐，或该促销活动已经关闭。<br />
					请先购买套餐，再查看活动列表。</span></div></td>
		</tr>
	</tbody>
</table>
<?php }?>
...
$(function(){
    ...
    $('input[nctype="btn_search_goods"]').click(function(){
        _url = '<?php echo urlBiz('store_promotion_booth', 'booth_select_goods');?>';
    ...
    $('div[nvtype="div_goods_select"]').on('click', 'a[nctype="a_choose_goods"]', function(){
        _url = '<?php echo urlBiz('store_promotion_booth', 'choosed_goods');?>';
    ...
    $('tbody[nctype="choose_goods_list"]').on('click','a[nctype="del_choosed"]', function(){
        $this = $(this);
        _url = '<?php echo urlBiz('store_promotion_booth', 'del_choosed_goods');?>';
        eval('var data_str = ' + $(this).attr('data-param'));
        $.getJSON(_url, {gid : data_str.gid}, function(data){
            if (data.result == 'true') {
