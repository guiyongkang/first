	...
	<form id="add_form" action="index.php?act=store_promotion_xianshi&op=xianshi_edit_save" method="post">
		...
		<dl>
			...
			<dd>
				...
				<p class="hint">
					<?php if (empty($output['isOwnShop']) && $output['current_xianshi_quota']['start_time'] > 1) { ?>
		...
		<dl>
			...
			<dd>
				<input id="lower_limit" name="lower_limit" type="text" class="text w130" value="<?php echo empty($output['xianshi_info'])?'1':$output['xianshi_info']['lower_limit'];?>"/>
				<span></span>
...
$(document).ready(function(){
    ...
    $("#add_form").validate({
        ...
        rules : {
            ...
            start_time : {
                required : true,
                greaterThanDate : '<?php echo date('Y-m-d H:i',isset($output['current_xianshi_quota']['start_time']) ? $output['current_xianshi_quota']['start_time'] : 0);?>'
            },
            end_time : {
                required : true,
                <?php if (empty($output['isOwnShop'])) { ?>
                lessThanDate : '<?php echo date('Y-m-d H:i',isset($output['current_xianshi_quota']['end_time']) ? $output['current_xianshi_quota']['end_time'] : 0);?>',
                <?php } ?>
        ...
        messages : {
            ...
            start_time : {
            required : '<i class="icon-exclamation-sign"></i><?php echo sprintf($lang['xianshi_add_start_time_explain'],date('Y-m-d H:i',isset($output['current_xianshi_quota']['start_time']) ? $output['current_xianshi_quota']['start_time'] : 0));?>',
                greaterThanDate : '<i class="icon-exclamation-sign"></i><?php echo sprintf($lang['xianshi_add_start_time_explain'],date('Y-m-d H:i',isset($output['current_xianshi_quota']['start_time']) ? $output['current_xianshi_quota']['start_time'] : 0));?>'
            },
            end_time : {
            required : '<i class="icon-exclamation-sign"></i><?php echo sprintf($lang['xianshi_add_end_time_explain'],date('Y-m-d H:i',isset($output['current_xianshi_quota']['end_time']) ? $output['current_xianshi_quota']['end_time'] : 0));?>',
            <?php if (empty($output['isOwnShop'])) { ?>
                lessThanDate : '<i class="icon-exclamation-sign"></i><?php echo sprintf($lang['xianshi_add_end_time_explain'],date('Y-m-d H:i',isset($output['current_xianshi_quota']['end_time']) ? $output['current_xianshi_quota']['end_time'] : 0));?>',
            <?php } ?>
