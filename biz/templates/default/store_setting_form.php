...
<div class="ncsc-form-default">
	<form method="post"  action="index.php?act=store_setting&op=store_setting" enctype="multipart/form-data" id="my_store_form">
		...
		<dl>
			<dt>公司所在地：</dt>
			<dd>
				<input id="company_address" name="company_address" type="hidden" value="<?php echo $output['store_info']['area_info'];?>"/>
			</dd>
		</dl>
		<dl>
		...
		<dl>
			<dt>店铺二维码：</dt>
			<dd>
				<p><img src="<?php echo storeQRCode($output['store_info']['store_id']);?>" /></p>
		...
		<dl class="setup store-logo">
			...
			<dd>
			    <div class="ncsc-upload-thumb" style="width:100px;">
					<p>
						<?php if(empty($output['store_info']['store_avatar'])){?>
						<i class="icon-picture"></i>
						<?php }else{?>
						<img src="<?php echo UPLOAD_SITE_URL.'/'.ATTACH_STORE.'/'.$output['store_info']['store_avatar'];?>" nc_type="store_avatar" />
						<?php }?>
					</p>
				</div>
			    <div class="ncsc-upload-btn"> <a href="javascript:void(0);"><span>
				    <input type="hidden" value="" name="store_avatar" id="store_avatar">
					<input type="file" hidefocus="true" size="1" class="input-file" name="_pic" id="_pic"/>
					</span>
					<p><i class="icon-upload-alt"></i>图片上传</p>
					</a> 
				</div>
		...
		<?php if($output['setting_config']['pc_isuse']){?>
		<dl>
