<div class="goods-gallery add-step2"> <a class="sample_demo" id="select_s" href="index.php?act=store_album&op=pic_list&item=des" style="display:none;"><?php echo $lang['nc_submit'];?></a>
	<div class="nav"><span class="l"><?php echo $lang['store_goods_album_users'];?> >
		...
		<select name="jumpMenu" id="jump_menu" style="width:100px;">
			...
			<option value="<?php echo $val['aclass_id']; ?>" <?php if($val['aclass_id']==isset($_GET['id']) ? $_GET['id'] : 0){echo 'selected';}?>><?php echo $val['aclass_name']; ?></option>
			<?php } ?>
		</select>
		</span></div>
	...
	<ul class="list">
		...
		<li onclick="insert_editor('<?php echo cthumb($v['apic_cover'], 360, $output['session']['store_id']);?>');"><a href="JavaScript:void(0);"><img src="<?php echo cthumb($v['apic_cover'], 240, $output['session']['store_id']);?>" title='<?php echo $v['apic_name']?>'/></span></a></li>
		<?php }?>
	</ul>
	<?php }else{?>
