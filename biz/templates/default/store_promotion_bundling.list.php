...
<div class="tabmenu">
	...
	<?php } else if((isset($output['bundling_surplus']) && intval($output['bundling_surplus']) != 0 ) || $output['setting_config']['promotion_bundling_sum'] == 0) { ?>
...
<?php if ($output['isOwnShop']) { ?>
<?php if ($output['setting_config']['promotion_bundling_sum'] != 0) {?>
<div class="alert alert-block mt10">
	<ul>
		<li>1、您最多可以发布<?php echo $output['setting_config']['promotion_bundling_sum'];?>个优惠套装。</li>
	</ul>
</div>
<?php }?>
<?php } else { ?>
...
<div class="alert alert-block mt10">
	...
	<ul>
		...
		<?php if ($output['setting_config']['promotion_bundling_sum'] != 0) {?>
		<li>3、您最多可以发布<?php echo $output['setting_config']['promotion_bundling_sum'];?>个优惠套装。</li>
		<?php }?>
	</ul>
</div>
<?php } ?>
<form method="get">
	...
	<table class="search-form">
		<tr>
			...
			<td class="w160"><input type="text" class="text w150" name="bundling_name" value="<?php echo isset($_GET['bundling_name']) ? $_GET['bundling_name'] : '';?>"/></td>
...
<table class="ncsc-default-table">
	...
	<tbody>
		<tr class="bd-line">
			...
			<td><div class="pic-thumb"><img src="<?php echo $val['img'];?>"/></div></td>
			<td class="tl"><dl class="goods-name">
					<dt><?php echo $val['bl_name'];?></dt>
				</dl></td>
			...
			<td class="nscs-table-handle"><span><a href="index.php?act=store_promotion_bundling&op=bundling_add&bundling_id=<?php echo $val['bl_id'];?>" class="btn-blue"><i class="icon-cog"></i>
				...
				</a></span> <span><a class="btn-red" href='javascript:void(0);' onclick="ajax_get_confirm('<?php echo $lang['nc_ensure_del'];?>', '<?php echo urlBiz('store_promotion_bundling', 'drop_bundling', array('bundling_id' => $val['bl_id']));?>');"><i class="icon-trash"></i>
