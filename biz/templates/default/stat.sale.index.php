			...
			<td class="tr"><div class="fr">
				...
				<div class="fr">&nbsp;
					<select name="order_type" id="order_type" class="querySelect">
						<option value="" <?php echo (isset($_REQUEST['order_type']) && $_REQUEST['order_type'] == '') ? 'selected' : ''; ?>>请选择</option>
						<option value="<?php echo ORDER_STATE_NEW; ?>" <?php echo !empty($_REQUEST['order_type']) && $_REQUEST['order_type']==ORDER_STATE_NEW?'selected':''; ?>>待付款</option>
						<option value="<?php echo ORDER_STATE_PAY; ?>" <?php echo !empty($_REQUEST['order_type']) && $_REQUEST['order_type']==ORDER_STATE_PAY?'selected':''; ?>>待发货</option>
						<option value="<?php echo ORDER_STATE_SEND; ?>" <?php echo !empty($_REQUEST['order_type']) && $_REQUEST['order_type']==ORDER_STATE_SEND?'selected':''; ?>>待收货</option>
						<option value="<?php echo ORDER_STATE_SUCCESS; ?>" <?php echo !empty($_REQUEST['order_type']) && $_REQUEST['order_type']==ORDER_STATE_SUCCESS?'selected':''; ?>>交易完成</option>
						<option value="<?php echo ORDER_STATE_CANCEL; ?>" <?php echo isset($_REQUEST['order_type']) && $_REQUEST['order_type']==ORDER_STATE_CANCEL?'selected':''; ?>>已取消</option>
					</select>
				</div>
				<div class="fr">
					<div class="fl" style="margin-right:3px;">
						<select name="search_type" id="search_type" class="querySelect">
							<option value="day" <?php echo isset($output['search_arr']['search_type']) && $output['search_arr']['search_type']=='day'?'selected':''; ?>>按照天统计</option>
							<option value="week" <?php echo isset($output['search_arr']['search_type']) && $output['search_arr']['search_type']=='week'?'selected':''; ?>>按照周统计</option>
							<option value="month" <?php echo isset($output['search_arr']['search_type']) && $output['search_arr']['search_type']=='month'?'selected':''; ?>>按照月统计</option>
						</select>
					</div>
					<div id="searchtype_day" style="display:none;" class="fl">
						<input type="text" class="text w70" name="search_time" id="search_time" value="<?php echo date('Y-m-d',$output['search_arr']['day']['search_time']);?>" />
...
<div class="alert alert-info mt10" style="clear:both;">
	<ul class="mt5">
		<li> <span class="w210 fl h30" style="display:block;"> <i title="店铺符合搜索条件的订单总金额" class="tip icon-question-sign"></i> 总下单金额：<strong><?php echo $output['statcount_arr']['orderamount'].$lang['currency_zh'];?></strong> </span> <span class="w210 fl h30" style="display:block;"> <i title="店铺符合搜索条件的订单数量" class="tip icon-question-sign"></i> 总下单量：<strong><?php echo $output['statcount_arr']['ordernum'];?></strong> </span> </li>
	</ul>
...
$(function(){
	...
	$('#statlist').load('index.php?act=statistics_sale&op=salelist&t=<?php echo $output['searchtime'];?>&order_type=<?php echo isset($_REQUEST['order_type']) ? $_REQUEST['order_type'] : '';?>');
});
</script>