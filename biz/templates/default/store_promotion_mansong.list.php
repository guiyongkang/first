...
<div class="tabmenu">
	...
	<a class="ncsc-btn ncsc-btn-green" href="<?php echo urlBiz('store_promotion_mansong', 'mansong_add');?>"><i class="icon-plus-sign"></i><?php echo $lang['mansong_add'];?></a>
	<?php } else { ?>
	<?php if(!empty($output['current_mansong_quota'])) { ?>
	<a class="ncsc-btn ncsc-btn-green" style="right:100px" href="<?php echo urlBiz('store_promotion_mansong', 'mansong_add');?>"><i class="icon-plus-sign"></i><?php echo $lang['mansong_add'];?></a> <a class="ncsc-btn ncsc-btn-acidblue" href="<?php echo urlBiz('store_promotion_mansong', 'mansong_quota_add');?>" title=""><i class="icon-money"></i>套餐续费</a>
	<?php } else { ?>
	<a class="ncsc-btn ncsc-btn-acidblue" href="<?php echo urlBiz('store_promotion_mansong', 'mansong_quota_add');?>" title=""><i class="icon-money"></i>购买套餐</a>
	<?php } ?>
	<?php } ?>
</div>
<?php if ($output['isOwnShop']) { ?>
...
<form method="get">
	<table class="search-form">
		...
		<tr>
			...
			<td class="w160"><input type="text" class="text w150" name="mansong_name" value="<?php echo isset($_GET['mansong_name']) ? $_GET['mansong_name'] : '';?>"/></td>
    ...
    $(document).ready(function(){
        $('[nctype="btn_mansong_del"]').on('click', function() {
            if(confirm('<?php echo $lang['nc_ensure_cancel'];?>')) {
                var action = "<?php echo urlBiz('store_promotion_mansong', 'mansong_del');?>";
