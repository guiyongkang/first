		...
		<tbody>
			<tr>
				...
				<td class="w160"><select name="stc_id" class="w150">
						...
						<option value="<?php echo $val['stc_id']; ?>" <?php if($val['stc_id'] == (isset($_GET['stc_id']) ? intval($_GET['stc_id']) : 0)) echo 'selected="selected"';?>><?php echo $val['stc_name']; ?></option>
						<?php if (is_array($val['child']) && count($val['child'])>0){?>
						<?php foreach ($val['child'] as $child_val){?>
						<option value="<?php echo $child_val['stc_id']; ?>" <?php if($child_val['stc_id'] == (isset($_GET['stc_id']) ? intval($_GET['stc_id']) : 0)) echo 'selected="selected"';?>>&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $child_val['stc_name']; ?></option>
						<?php }}}}?>
					</select></td>
				<th><?php echo $lang['bundling_goods_name'];?></th>
				<td class="w160"><input type="text" name="b_search_keyword" class="text" value="<?php echo isset($_GET['keyword']) ? $_GET['keyword'] : '';?>" /></td>
	...
	<div class="search-result" style="width:739px;">
		...
		<ul class="goods-list" nctype="bundling_goods_add_tbody" style=" width:760px;">
			...
			<li nctype="<?php echo $val['goods_id'];?>">
				<div class="goods-thumb"><img src="<?php echo cthumb($val['goods_image'], 240, $output['session']['store_id']);?>" nctype="<?php echo $val['goods_image'];?>" /></div>
				<dl class="goods-info">
				...
				<div data-param="{gid:<?php echo $val['goods_id'];?>,image:'<?php echo $val['goods_image'];?>',src:'<?php echo cthumb($val['goods_image'], 60, $output['session']['store_id']);?>',gname:'<?php echo $val['goods_name'];?>',gprice:'<?php echo $val['goods_price'];?>',gstorang:'<?php echo $val['goods_storage'];?>'}"><a href="JavaScript:void(0);" class="ncsc-btn-mini ncsc-btn-green" onclick="bundling_goods_add($(this))"><i class="icon-plus"></i><?php echo $lang['bundling_goods_add_bundling'];?></a></div>
			</li>
			<?php }?>
		</ul>
		<?php }else{?>
...
function bundling_goods_add(o){
	...
	if(typeof(_bundlingtr) != 'undefined'){
		if(_bundlingtr.length == <?php echo $output['setting_config']['promotion_bundling_goods_sum'];?>){
			alert('<?php printf($lang['bundling_goods_add_enough_prompt'], $output['setting_config']['promotion_bundling_goods_sum']);?>');
			return false;
		}
	}
