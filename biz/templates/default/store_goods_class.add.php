<div class="eject_con">
	...
	<form id="category_form" method="post" target="_parent" action="index.php?act=store_goods_class&op=goods_class_save">
		<?php if (isset($output['class_info']['stc_id']) && $output['class_info']['stc_id'] != 0) { ?>
		...
		<dl>
			...
			<dd>
				<input class="text w200" type="text" name="stc_name" id="stc_name" value="<?php echo !empty($output['class_info']['stc_name']) ? $output['class_info']['stc_name'] : ''; ?>" />
			</dd>
		</dl>
		<?php //if (isset($output['class_info']['stc_id']) && $output['class_info']['stc_id']==0) { ?>
		<dl>
			...
			<dd>
				<select name="stc_parent_id" id="stc_parent_id">
					...
					<option value="<?php echo $val['stc_id']; ?>" <?php if ($val['stc_id'] == (isset($output['class_info']['stc_parent_id']) ? $output['class_info']['stc_parent_id'] : 0)) { ?>selected="selected"<?php } ?>><?php echo $val['stc_name']; ?></option>
					<?php } ?>
					<?php } ?>
				</select>
			</dd>
		...
		<dl>
			...
			<dd>
				<input class="text w60" type="text" name="stc_sort" value="<?php echo isset($output['class_info']['stc_sort']) ? intval($output['class_info']['stc_sort']) : ''; ?>"  />
			</dd>
		</dl>
		<dl>
			...
			<dd>
				<label>
					<input type="radio" name="stc_state" value="1" <?php if (empty($_GET['class_id']) or (isset($output['class_info']['stc_state']) && $output['class_info']['stc_state']==1)) echo 'checked="checked"'; ?> />
					<?php echo $lang['store_create_yes'];?></label>
				<label>
					<input type="radio" name="stc_state" value="0" <?php if ((isset($output['class_info']['stc_state']) && $output['class_info']['stc_state']==0) and !empty($_GET['class_id'])) echo 'checked="checked"'; ?> />
