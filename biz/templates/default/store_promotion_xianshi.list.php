...
<div class="tabmenu">
	...
	<a class="ncsc-btn ncsc-btn-green" href="<?php echo urlBiz('store_promotion_xianshi', 'xianshi_add');?>"><i class="icon-plus-sign"></i><?php echo $lang['xianshi_add'];?></a>
	<?php } else { ?>
	<?php if(!empty($output['current_xianshi_quota'])) { ?>
	<a class="ncsc-btn ncsc-btn-green" style="right:100px" href="<?php echo urlBiz('store_promotion_xianshi', 'xianshi_add');?>"><i class="icon-plus-sign"></i><?php echo $lang['xianshi_add'];?></a> <a class="ncsc-btn ncsc-btn-acidblue" href="<?php echo urlBiz('store_promotion_xianshi', 'xianshi_quota_add');?>" title=""><i class="icon-money"></i>套餐续费</a>
	<?php } else { ?>
	<a class="ncsc-btn ncsc-btn-acidblue" href="<?php echo urlBiz('store_promotion_xianshi', 'xianshi_quota_add');?>" title=""><i class="icon-money"></i><?php echo $lang['xianshi_quota_add'];?></a>
	<?php } ?>
	<?php } ?>
</div>
<?php if ($output['isOwnShop']) { ?>
	...
	<table class="search-form">
		...
		<tr>
			...
			<td class="w100"><select name="state">
					...
					<option value="<?php echo $key;?>" <?php if(isset($_GET['state']) && intval($key) === intval($_GET['state'])) echo 'selected';?>><?php echo $val;?></option>
					<?php } ?>
					<?php } ?>
				</select></td>
			<th class="w110"><?php echo $lang['xianshi_name'];?></th>
			<td class="w160"><input type="text" class="text w150" name="xianshi_name" value="<?php echo isset($_GET['xianshi_name']) ? $_GET['xianshi_name'] : '';?>"/></td>
	...
	<tbody id="xianshi_list">
		<tr class="bd-line">
			...
			<td class="nscs-table-handle tr"><?php if($val['editable']) { ?>
				<span> <a href="index.php?act=store_promotion_xianshi&op=xianshi_edit&xianshi_id=<?php echo $val['xianshi_id'];?>" class="btn-blue"> <i class="icon-edit"></i>
				<p><?php echo $lang['nc_edit'];?></p>
				</a> </span>
				<?php } ?>
				<span> <a href="index.php?act=store_promotion_xianshi&op=xianshi_manage&xianshi_id=<?php echo $val['xianshi_id'];?>" class="btn-green"> <i class="icon-cog"></i>
				<p><?php echo $lang['nc_manage'];?></p>
				</a> </span> <span> <a href="javascript:;" nctype="btn_del_xianshi" data-xianshi-id=<?php echo $val['xianshi_id'];?> class="btn-red"> <i class="icon-trash"></i>
    ...
    $(document).ready(function(){
        $('[nctype="btn_del_xianshi"]').on('click', function() {
            if(confirm('<?php echo $lang['nc_ensure_del'];?>')) {
                var action = "<?php echo urlBiz('store_promotion_xianshi', 'xianshi_del');?>";
