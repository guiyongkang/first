...
<div class="wrapper_search">
			...
			<div class="sort_title"><?php echo $lang['store_goods_step1_choose_common_category'];?>
				<div class="text" id="commSelect">
					...
					<div class="select_list" id="commListArea">
						<ul>
							<?php if(!empty($output['staple_array']) && is_array($output['staple_array'])) {?>
							<?php foreach ($output['staple_array'] as $val) {?>
							<li
								data-param="{stapleid:<?=$val['staple_id']?>}"><span
								nctype="staple_name"><?=$val['staple_name']?></span><a
								href="JavaScript:void(0);" nctype="del-comm-cate"
								title="<?php echo $lang['nc_delete'];?>">X</a></li>
							<?php }?>
							<?php }?>
							<li id="select_list_no"
								<?php if (!empty($output['staple_array'])) {?>
								style="display: none;" <?php }?>><span class="title"><?php echo $lang['store_goods_step1_no_common_category'];?></span>
							</li>
						</ul>
					</div>
				</div>
				...
				<div class="wp_category_list">
					<div id="class_div_1" class="category_list">
						<ul>
							...
							<li class="" nctype="selClass"
								data-param="{gcid:<?php echo $val['gc_id'];?>,deep:1,tid:<?php echo $val['type_id'];?>}"> <a class="" href="javascript:void(0)"><i
									class="icon-double-angle-right"></i><?php echo $val['gc_name'];?></a> </li>
							<?php }?>
							<?php }?>
						</ul>
					</div>
	...
	<div class="wp_confirm">
		<form method="get">
			...
			<input
				type="hidden" name="op" value="edit_goods" />
			<input type="hidden"
				name="commonid" value="<?php echo $output['commonid'];?>" />
			<input
				type="hidden" name="ref_url" value="<?php echo $_GET['ref_url'];?>" />
			<?php } else {?>
			<input type="hidden" name="act" value="store_goods_add" />
			<input
				type="hidden" name="op" value="add_step_two" />
			<?php }?>
			<input type="hidden" name="class_id" id="class_id" value="0" />
			<input
				type="hidden" name="t_id" id="t_id" value="0" />
			<div class="bottom tc">
				<label class="submit-border">
					<input disabled="disabled"
					nctype="buttonNextStep"
					value="<?php echo $lang['store_goods_add_next'];?>，填写商品信息"
					type="submit" class="submit" style="width: 200px;" />
				</label>
			</div>
		</form>
	</div>
...
<script
	src="<?=APP_RESOURCE_SITE_URL?>/js/store_goods_add.step1.js"></script> 
<script>
SEARCHKEY = '<?=$lang['store_goods_step1_search_input_text']?>';
