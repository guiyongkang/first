...
$(function(){
    ...
    $('a[nctype="del"]').unbind().click(
        function() {
            ...
            obj.find('img:first').attr('src', APP_TEMPLATES_URL + "/images/loading.gif");
            ...
            $.getScript(APP_RESOURCE_SITE_URL + "/js/store_slide.js");
        });
});
...
function ajaxFileUpload(url, id, file_id)
{
        $('div[nctype="'+id+'"]').find('i').remove().end().find('img').remove()
            .end().prepend('<img nctype="'+id+'" scr="'+APP_TEMPLATES_URL+'/images/loading.gif">');
	$('img[nctype="'+id+'"]').attr('src',APP_TEMPLATES_URL+"/images/loading.gif");
	...
	(
		{
			...
			success: function (data, status)
			{
				...
				$.getScript(APP_RESOURCE_SITE_URL+"/js/jquery.flexslider-min.js");
				$.getScript(APP_RESOURCE_SITE_URL+"/js/store_slide.js");
			},
