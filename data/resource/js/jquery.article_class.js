				...
				success: function(data){
					...
					for(var i = 0; i < data.length; i++){
						...
						if(data[i].have_child == 1){
							src += "<img fieldid='"+data[i].ac_id+"' status='open' nc_type='flex' src='"+APP_TEMPLATES_URL+"/images/tv-expandable.gif' />";
						}else{
							src += "<img fieldid='"+data[i].ac_id+"' status='none' nc_type='flex' src='"+APP_TEMPLATES_URL+"/images/tv-item.gif' />";
						}
						//图片
						...
						if(data[i].have_child == 1){
							src += " <img fieldid='"+data[i].ac_id+"' status='open' nc_type='flex' src='"+APP_TEMPLATES_URL+"/images/tv-item1.gif' />";
						}else{
							src += " <img fieldid='"+data[i].ac_id+"' status='none' nc_type='flex' src='"+APP_TEMPLATES_URL+"/images/tv-expandable1.gif' />";
						}
